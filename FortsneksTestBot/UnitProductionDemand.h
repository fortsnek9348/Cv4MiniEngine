...
namespace mybot
{
	...
	//	ivec2 target{};
	//	// We MUST have the new unit at the target, or the building/project built, within this number of turns.
	//	uint16_t turns{};
	//	uint16_t count{};
	//	EProductionUrgency urgency{};
	//};

	enum class EUnitProductionUrgency
	{
		None,
		MilitaryPolice, // Just make something cheap for hereditary rule happiness.
		NonCombat, // We settling or need to improve some plots. Don't take time away from important production.
	...
	struct UnitProductionDemand
	{
		...
		// Anything that can move has a target, even scouts and workers.
		ivec2 target{};
		// We MUST have the new unit at the target within this number of turns.
		int turns{};
		int count = 1;
