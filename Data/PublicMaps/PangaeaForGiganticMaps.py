...
def generatePlotTypes():
	...
	if userInputLandmass == 3: # Solid
		...
		if terrainRoll > 6:
			...
			pangaea_type = 2
			print "Heck Pangaea Random pangaea_type=2"
			return fractal_world.generatePlotsByRegion(pangaea_type)
		elif terrainRoll == 4:
			pangaea_type = 1
			return fractal_world.generatePlotsByRegion(pangaea_type)
...
def findStartingPlot(argsList):
	...
	# fortsnek: pull this out of isValid as it can loop over 900+ areas on a super huge map.
	global isValidArrayCache
	map = CyMap()
	iW = map.getGridWidth()
	
	if isValidArrayCache is None:
		iH = map.getGridHeight()
		isValidArrayCache = [False] * (iW * iH)
		biggestAreaId = map.findBiggestArea(False).getID()
		
		def isValid(x, y):
			global isTeamGame
			pPlot = map.plot(x, y)
		
			if pPlot.getArea() != biggestAreaId:
				return False
		...
		for y in range(iH):
			for x in range(iW):
				isValidArrayCache[x + y * iW] = isValid(x, y)
	
	return CvMapGeneratorUtil.findStartingPlot(playerID, lambda playerID, x, y: isValidArrayCache[x + y * iW])
