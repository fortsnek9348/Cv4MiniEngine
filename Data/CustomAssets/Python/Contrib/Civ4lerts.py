# fortsnek: Edited to stop engine assert on isHasMet.
...
class RefusesToTalk(AbstractStatefulAlert):
	...
	def checkIfIsAnyOrHasMetAllTeams(self, *eTeams):
		...
		for eTeam in eTeams:
			# fortsnek: Ignore NO_TEAM
			if eTeam >= 0 and eActiveTeam != eTeam and not activeTeam.isHasMet(eTeam):
				return
		self.check()
		...
		for eTeam, eNewEnemy in newEnemies.iteritems():
			if activeTeam.isHasMet(eTeam):
				...
				if eOldEnemy != -1 and not gc.getTeam(eOldEnemy).isAlive():
					...
					enemies[eTeam] = -1
				# fortsnek: Edited to stop engine assert on isHasMet.
				if eActiveTeam != eNewEnemy and eNewEnemy >= 0 and not activeTeam.isHasMet(eNewEnemy):
					eNewEnemy = -1
