...
void CvEventReporter::getPlayerStatistics(PlayerTypes ePlayer, std::vector<CvStatBase*>& aStats)
{
	...
	if (pRecord != nullptr)
	{
		...
		for (int j = 0; j < GC.getNumUnitInfos(); ++j)
		{
			...
			aStats.push_back(new CvStatInt(strKey.c_str(), pRecord->getNumUnitsBuilt(j)));

			strKey.format("unit_%d_killed", j);
			aStats.push_back(new CvStatInt(strKey.c_str(), pRecord->getNumUnitsKilled(j)));

			strKey.format("unit_%d_lost", j);
			aStats.push_back(new CvStatInt(strKey.c_str(), pRecord->getNumUnitsWasKilled(j)));
		}
		...
		for (int j = 0; j < GC.getNumBuildingInfos(); ++j)
		{
			...
			aStats.push_back(new CvStatInt(strKey.c_str(), pRecord->getNumBuildingsBuilt((BuildingTypes)j)));
		}
		...
		for (int j = 0; j < GC.getNumReligionInfos(); ++j)
		{
			...
			aStats.push_back(new CvStatInt(strKey.c_str(), pRecord->getReligionFounded((ReligionTypes)j)));
		}
	}
}
