...
inline void FVariable::CopyFrom(const FVariable& varSrc)
{
	if (varSrc.m_eType == FVARTYPE_STRING && varSrc.m_szValue)
	{
		...
		// fortsnek: Use strcpy_s.
		const size_t bufSize = strlen(varSrc.m_szValue)+1;
		m_szValue = new char[bufSize];
		strcpy_s(m_szValue, bufSize, varSrc.m_szValue);
	}
	else
	if (varSrc.m_eType == FVARTYPE_WSTRING && varSrc.m_wszValue)
	{
		// copy string to new allocation
		// fortsnek: Use wcscpy_s.
		const size_t bufSize = wcslen(varSrc.m_wszValue)+1;
		m_wszValue = new wchar_t[bufSize];
		wcscpy_s(m_wszValue, bufSize, varSrc.m_wszValue);
	}
	else
	{
...
//---------------------------------------------------------------------------------------
// inline unsigned int FVariableSystem::GetSize() const
//---------------------------------------------------------------------------------------
...
inline unsigned int FVariableSystem::GetSize() const
{
	return (unsigned int)m_mapVariableMap.size();
}
...
inline void FVariableSystem::Write(FDataStreamBase *pStream) const
{
	...
	// write vars/keys
	[[maybe_unused]] int iNumWritten=0;
	VSIteratorC iIterator = m_mapVariableMap.begin();
	FVariable * pkVariable;
	while ( iIterator != m_mapVariableMap.end())
	{
...
//---------------------------------------------------------------------------------------
// inline bool FVariableSystem::GetValue( const char * szVariable, unsigned int & uiValue ) const
//---------------------------------------------------------------------------------------
//! \brief Gets the value of the given variable
//! \param szVariable The name of the variable containing the value to query
...
//---------------------------------------------------------------------------------------
// inline void FVariableSystem::SetValue( const char * szVariable, unsigned int uiValue )
//---------------------------------------------------------------------------------------
//! \brief Creates (or modifies) a variable with the given name and sets it value.
//! \param szVariable The name of the variable to create
...
inline void FVariableSystem::SetValue( const char * szVariable, const char * szValue )
{
	...
	// fortsnek: Use strcpy_s.
	const size_t bufSize = strlen(szValue) + 1;
	char* const buf = new char[bufSize];
	strcpy_s(buf, bufSize, szValue);
	pkVariable->m_szValue = buf;
	m_mapVariableMap[szVariable] = pkVariable;
	m_iVariableIterator = m_mapVariableMap.begin();
}
...
inline void FVariableSystem::SetValue( const char * szVariable, const wchar_t * wszValue )
{
	...
	// fortsnek: Use wcscpy_s.
	const size_t bufSize = wcslen(wszValue) + 1;
	wchar_t* const buf = new wchar_t[bufSize];
	wcscpy_s(buf, bufSize, wszValue);
	pkVariable->m_wszValue = buf;
	m_mapVariableMap[szVariable] = pkVariable;
	m_iVariableIterator = m_mapVariableMap.begin();
}
