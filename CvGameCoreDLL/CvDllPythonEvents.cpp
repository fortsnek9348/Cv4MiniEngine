...
bool CvDllPythonEvents::reportMouseEvent(int evt, int iCursorX, int iCursorY, bool bInterfaceConsumed)
{
	if (preEvent())
	{
		...
		eventData.add(screens.size() ? &screens[0] : nullptr, screens.size());
...
void CvDllPythonEvents::reportSelectionGroupPushMission(CvSelectionGroup* pSelectionGroup, MissionTypes eMission)
{
	...
	if (preEvent())
	{
		...
		// fortsnek: This is an array, use delete[]!
		delete[] aiUnitIds;
	}
}
...
void CvDllPythonEvents::reportGenericEvent(const char* szEventName, void *pyArgs)
{
	if (preEvent())
	{
		...
		// fortsnek: Not sure what's going on here. There needs to be an extra nesting.
		eventData.add(pybind11::make_tuple(pybind11::reinterpret_steal<pybind11::object>(static_cast<PyObject*>(pyArgs))).release().ptr());		// generic args tuple
		postEvent(eventData);
	}
}
