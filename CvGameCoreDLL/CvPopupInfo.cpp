...
const CvPopupInfo& CvPopupInfo::operator=(const CvPopupInfo& other)
{
	...
	setText(other.getText().c_str());
	setPendingDelete(other.getPendingDelete());
	setOnFocusPythonCallback(other.getOnFocusPythonCallback().c_str());
	setOnClickedPythonCallback(other.getOnClickedPythonCallback().c_str());
	setPythonModule(other.getPythonModule().c_str());
...
void CvPopupInfo::write(FDataStreamBase& stream) const
{
	...
	unsigned int iSize = static_cast<unsigned int>(m_aPythonButtons.size());
	stream.Write(iSize);
	for (unsigned int i = 0; i < iSize; i++)
	{
