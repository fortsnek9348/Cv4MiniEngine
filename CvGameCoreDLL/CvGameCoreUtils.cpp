...
{
	if ((city.eOwner >= 0) && std::to_underlying(city.eOwner) < MAX_PLAYERS)
	{
...
{
	if ((unit.eOwner >= 0) && std::to_underlying(unit.eOwner) < MAX_PLAYERS)
	{
...
int getCombatOdds(CvUnit* pAttacker, CvUnit* pDefender)
{
	...
	//Added ST
	iAttackerStrength = pAttacker->currCombatStr(nullptr, nullptr);
	iAttackerFirepower = pAttacker->currFirepower(nullptr, nullptr);
		...
		for (iJ = iDefenderLowFS; iJ < iDefenderHighFS + 1; iJ++)
		{
			...
			if (iI >= iJ)
			{
				...
				for (iI3 = 0; iI3 < (iFirstStrikes + 1); iI3++)
				{
					...
					fOddsEvent = static_cast<float>(((float)getBinomialCoefficient(iFirstStrikes, iI3)) * pow((((float)iAttackerOdds) / GC.getDefineINT("COMBAT_DIE_SIDES")), iI3) * pow((1.0f - (((float)iAttackerOdds) / GC.getDefineINT("COMBAT_DIE_SIDES"))), (iFirstStrikes - iI3)));
					...
					else
					{
						...
						for (iI4 = (iNeededRoundsAttacker - iI3); iI4 < (iMaxRounds - iI3 + 1); iI4++)
						{
							...
							fOddsAfterEvent += static_cast<float>(((float)getBinomialCoefficient((iMaxRounds - iI3), iI4)) * pow((((float)iAttackerOdds) / GC.getDefineINT("COMBAT_DIE_SIDES")), iI4) * pow((1.0f - (((float)iAttackerOdds) / GC.getDefineINT("COMBAT_DIE_SIDES"))), ((iMaxRounds - iI3) - iI4)));
						}
					}
			...
			else // (iI < iJ)
			{
				...
				for (iI3 = 0; iI3 < (iFirstStrikes + 1); iI3++)
				{
					...
					if (iI3 < iNeededRoundsDefender)
					{
						...
						fOddsEvent = static_cast<float>(((float)getBinomialCoefficient(iFirstStrikes, iI3)) * pow((((float)iDefenderOdds) / GC.getDefineINT("COMBAT_DIE_SIDES")), iI3) * pow((1.0f - (((float)iDefenderOdds) / GC.getDefineINT("COMBAT_DIE_SIDES"))), (iFirstStrikes - iI3)));
...
void ::setListHelp(wchar_t* szBuffer, std::wstring_view szStart, std::wstring_view szItem, std::wstring_view szSeparator, bool bFirst)
{
	if (bFirst)
	{
		wcsncat(szBuffer, szStart.data(), szStart.size());
	}
	else
	{
		wcsncat(szBuffer, szSeparator.data(), szSeparator.size());
	}
...
bool PUF_isGroupHead(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return (pUnit->isGroupHead());
}

bool PUF_isPlayer(const CvUnit* pUnit, int iData1, [[maybe_unused]] int iData2)
{
	FAssertMsg(iData1 != -1, "Invalid data argument, should be >= 0");
	return (pUnit->getOwnerINLINE() == iData1);
}

bool PUF_isTeam(const CvUnit* pUnit, int iData1, [[maybe_unused]] int iData2)
{
	FAssertMsg(iData1 != -1, "Invalid data argument, should be >= 0");
...
bool PUF_isOtherPlayer(const CvUnit* pUnit, int iData1, [[maybe_unused]] int iData2)
{
	FAssertMsg(iData1 != -1, "Invalid data argument, should be >= 0");
	return (pUnit->getOwnerINLINE() != iData1);
}

bool PUF_isOtherTeam(const CvUnit* pUnit, int iData1, [[maybe_unused]] int iData2)
{
	FAssertMsg(iData1 != -1, "Invalid data argument, should be >= 0");
...
bool PUF_isVisible(const CvUnit* pUnit, int iData1, [[maybe_unused]] int iData2)
{
	FAssertMsg(iData1 != -1, "Invalid data argument, should be >= 0");
	return !(pUnit->isInvisible(GET_PLAYER((PlayerTypes)iData1).getTeam(), false));
}

bool PUF_isVisibleDebug(const CvUnit* pUnit, int iData1, [[maybe_unused]] int iData2)
{
	FAssertMsg(iData1 != -1, "Invalid data argument, should be >= 0");
	return !(pUnit->isInvisible(GET_PLAYER((PlayerTypes)iData1).getTeam(), true));
}

bool PUF_canSiege(const CvUnit* pUnit, int iData1, [[maybe_unused]] int iData2)
{
	FAssertMsg(iData1 != -1, "Invalid data argument, should be >= 0");
...
bool PUF_canDefend(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return pUnit->canDefend();
}

bool PUF_cannotDefend(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return !(pUnit->canDefend());
}
...
bool PUF_canAirAttack(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return pUnit->canAirAttack();
}

bool PUF_canAirDefend(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return pUnit->canAirDefend();
}

bool PUF_isFighting(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return pUnit->isFighting();
}

bool PUF_isAnimal( const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return pUnit->isAnimal();
}

bool PUF_isMilitaryHappiness(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return pUnit->isMilitaryHappiness();
}

bool PUF_isInvestigate(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return pUnit->isInvestigate();
}

bool PUF_isCounterSpy(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return pUnit->isCounterSpy();
}

bool PUF_isSpy(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return pUnit->isSpy();
}

bool PUF_isDomainType(const CvUnit* pUnit, int iData1, [[maybe_unused]] int iData2)
{
	FAssertMsg(iData1 != -1, "Invalid data argument, should be >= 0");
	return (pUnit->getDomainType() == iData1);
}

bool PUF_isUnitType(const CvUnit* pUnit, int iData1, [[maybe_unused]] int iData2)
{
	FAssertMsg(iData1 != -1, "Invalid data argument, should be >= 0");
	return (pUnit->getUnitType() == iData1);
}

bool PUF_isUnitAIType(const CvUnit* pUnit, int iData1, [[maybe_unused]] int iData2)
{
	FAssertMsg(iData1 != -1, "Invalid data argument, should be >= 0");
	return (pUnit->AI_getUnitAIType() == iData1);
}

bool PUF_isCityAIType(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
...
bool PUF_isSelected(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return pUnit->IsSelected();
}

bool PUF_makeInfoBarDirty(CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	pUnit->setInfoBarDirty(true);
	return true;
}

bool PUF_isNoMission(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return (pUnit->getGroup()->getActivityType() != ACTIVITY_MISSION);
}

bool PUF_isFiniteRange(const CvUnit* pUnit, [[maybe_unused]] int iData1, [[maybe_unused]] int iData2)
{
	return ((pUnit->getDomainType() != DOMAIN_AIR) || (pUnit->getUnitInfo().getAirRange() > 0));
}

int potentialIrrigation(FAStarNode* parent, FAStarNode* node, [[maybe_unused]] int data, [[maybe_unused]] const void* pointer, [[maybe_unused]] FAStar* finder)
{
	if (parent == nullptr)
	{
		return true;
...
int checkFreshWater([[maybe_unused]] FAStarNode* parent, FAStarNode* node, int data, const void* pointer, [[maybe_unused]] FAStar* finder)
{
...
int changeIrrigated([[maybe_unused]] FAStarNode* parent, FAStarNode* node, int data, const void* pointer, [[maybe_unused]] FAStar* finder)
{
	if (data == ASNL_ADDCLOSED)
	{
...
//				if (gGlobals.enhancedDLLGameContext->hasPotentialPlotDanger(pSelectionGroup->getTeam(), *pToPlot))
//#endif
...
int pathCost(FAStarNode* parent, FAStarNode* node, [[maybe_unused]] int data, const void* pointer, FAStar* finder)
{
	...
	//constexpr bool loggingCost = false;// node->m_iX == 292 && node->m_iY == 386;
	...
	while (pUnitNode != nullptr)
	{
		...
		if (iMovesLeft <= iWorstMovesLeft)
		{
			if ((iMovesLeft < iWorstMovesLeft) || (iMax <= iWorstMax))
			{
				...
				//if (loggingCost)
				//	std::clog << __func__ << ": " << iCost << std::endl;
				...
				if (pLoopUnit->canFight())
				{
					if (iMovesLeft == 0)
					{
						...
						//if (loggingCost)
						//	std::clog << __func__ << ": Stop defence. " << iCost << std::endl;
					}
						...
						if (pLoopUnit->canAttack())
						{
							if (gDLL->getFAStarIFace()->IsPathDest(finder, pToPlot->getX_INLINE(), pToPlot->getY_INLINE()))
							{
								if (pToPlot->isVisibleEnemyDefender(pLoopUnit))
								{
									...
									//if (loggingCost)
									//	std::clog << __func__ << ": Attack defence. " << iCost << std::endl;

									if (!(pFromPlot->isCity()))
									{
										iCost += PATH_CITY_WEIGHT;
										//if (loggingCost)
										//	std::clog << __func__ << ": City weight. " << iCost << std::endl;
									}
									...
									if (pFromPlot->isRiverCrossing(directionXY(pFromPlot, pToPlot)))
									{
										if (!(pLoopUnit->isRiver()))
										{
											...
											//if (loggingCost)
											//	std::clog << __func__ << ": River weight. " << iCost << std::endl;
										}
									}
								}
							}
	...
	//if (loggingCost)
	//	std::clog << __func__ << ": Final weight. " << iWorstCost << std::endl;
...
int pathValid(FAStarNode* parent, FAStarNode* node, [[maybe_unused]] int data, const void* pointer, FAStar* finder)
{
	CvSelectionGroup* pSelectionGroup;
	CvPlot* pFromPlot;
	CvPlot* pToPlot;
...
//#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
//		gGlobals.enhancedDLLGameContext->updatePotentialPlotDanger();
//#endif
	}
	else
	{
...
int stepDestValid(int iToX, int iToY, [[maybe_unused]] const void* pointer, FAStar* finder)
{
	PROFILE_FUNC();
...
int stepCost([[maybe_unused]] FAStarNode* parent, [[maybe_unused]] FAStarNode* node, [[maybe_unused]] int data, [[maybe_unused]] const void* pointer, [[maybe_unused]] FAStar* finder)
{
	return 1;
}


int stepValid(FAStarNode* parent, FAStarNode* node, [[maybe_unused]] int data, [[maybe_unused]] const void* pointer, [[maybe_unused]] FAStar* finder)
{
	CvPlot* pNewPlot;
...
int stepAdd(FAStarNode* parent, FAStarNode* node, int data, [[maybe_unused]] const void* pointer, [[maybe_unused]] FAStar* finder)
{
	if (data == ASNC_INITIALADD)
	{
		node->m_iData1 = 0;
...
int routeValid(FAStarNode* parent, FAStarNode* node, [[maybe_unused]] int data, [[maybe_unused]] const void* pointer, FAStar* finder)
{
	CvPlot* pNewPlot;
	PlayerTypes ePlayer;
...
int borderValid(FAStarNode* parent, FAStarNode* node, [[maybe_unused]] int data, [[maybe_unused]] const void* pointer, FAStar* finder)
{
	CvPlot* pNewPlot;
	PlayerTypes ePlayer;
...
int areaValid(FAStarNode* parent, FAStarNode* node, [[maybe_unused]] int data, [[maybe_unused]] const void* pointer, [[maybe_unused]] FAStar* finder)
{
	if (parent == nullptr)
	{
		return true;
...
int joinArea([[maybe_unused]] FAStarNode* parent, FAStarNode* node, int data, [[maybe_unused]] const void* pointer, FAStar* finder)
{
	if (data == ASNL_ADDCLOSED)
	{
...
int plotGroupValid(FAStarNode* parent, FAStarNode* node, [[maybe_unused]] int data, [[maybe_unused]] const void* pointer, FAStar* finder)
{
	CvPlot* pOldPlot;
	CvPlot* pNewPlot;
	PlayerTypes ePlayer;
...
#else
	if (pOldPlot->isSamePlotGroup(ePlayer, pNewPlot))
#endif
	{
...
int countPlotGroup([[maybe_unused]] FAStarNode* parent, [[maybe_unused]] FAStarNode* node, int data, const void* pointer, [[maybe_unused]] FAStar* finder)
{
	if (data == ASNL_ADDCLOSED)
	{
		(*((int*)pointer))++;
...
bool isPickableName(const TCHAR* szName)
{
	...
	{
#ifdef _WIN32
		int iLen = static_cast<int>(_tcslen(szName));
		
		if (!_tcsicmp(&szName[iLen-6], "NOPICK"))
#else
		int iLen = static_cast<int>(strlen(szName));

		if (!strcasecmp(&szName[iLen-6], "NOPICK"))
#endif
		{
			return false;
		}
...
void shuffleArray(int* piShuffle, int iNum, CvRandom& rand)
{
	...
	for (iI = 0; iI < iNum; iI++)
	{
		iJ = (rand.get(static_cast<unsigned short>(iNum - iI), nullptr) + iI);
