...
//m_pGameTextMgr(m_pGameTextMgr) // fortsnek: Well, I guess it would work most of the time when there's hardly any member variables.
...
{
	CvWStringBuffer szBuffer;
	GAMETEXT.setUnitHelp(szBuffer, (UnitTypes)iUnit, bCivilopediaText, bStrategyText, bTechChooserText, ((pCity != nullptr) ? pCity->getCity() : nullptr));
	return szBuffer.getCString();
}
...
{
	CvWStringBuffer szBuffer;
	GAMETEXT.setBuildingHelp(szBuffer, (BuildingTypes)iBuilding, bCivilopediaText, bStrategyText, bTechChooserText, ((pCity != nullptr) ? pCity->getCity() : nullptr));
	return szBuffer.getCString();
}
...
{
	CvWStringBuffer szBuffer;
	GAMETEXT.setProjectHelp(szBuffer, (ProjectTypes)iProject, bCivilopediaText, ((pCity != nullptr) ? pCity->getCity() : nullptr));
	return szBuffer.getCString();
}
...
{
	CvWStringBuffer szBuffer;
	GAMETEXT.setReligionHelpCity(szBuffer, (ReligionTypes)iReligion, ((pCity != nullptr) ? pCity->getCity() : nullptr), bCityScreen, bForceReligion, bForceState, bNoStateReligion);
	return szBuffer.getCString();
}
...
{
	CvWStringBuffer szBuffer;
	GAMETEXT.setCorporationHelpCity(szBuffer, (CorporationTypes)iCorporation, ((pCity != nullptr) ? pCity->getCity() : nullptr), bCityScreen, bForceCorporation);
	return szBuffer.getCString();
}
...
{
	CvWStringBuffer szBuffer;
	GAMETEXT.parseSpecialistHelp(szBuffer, (SpecialistTypes) iSpecialist, nullptr, bCivilopediaText);
	return szBuffer.getCString();
}
