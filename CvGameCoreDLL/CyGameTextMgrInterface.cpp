...
void CyGameTextMgrInterface(pybind11::module m)
{
	...
	pybind11::class_<CyGameTextMgr>(m, "CyGameTextMgr")
		.def(pybind11::init())
		...
		// fortsnek: Cv4MiniEngine extension. This is called by the Firaxis engine, but here, we'll call it from CvMainInterface.
		.def("getTurnTimerText", [](CyGameTextMgr*) -> std::wstring {
		CvWString str;
		CvGameTextMgr::GetInstance().getTurnTimerText(str);
		return str;
			})
	;
}
