...
{
	if (m_pPlayer && pPlot != nullptr && !pPlot->isNone())
	{
...
{
	return m_pPlayer ? new CyCity(m_pPlayer->initCity(x, y, true, true)) : nullptr;
}
...
{
	return m_pPlayer ? new CyUnit(m_pPlayer->initUnit((UnitTypes) iIndex, iX, iY, eUnitAI, eFacingDirection)) : nullptr;
}

void CyPlayer::disbandUnit(bool bAnnounce)
{
	return m_pPlayer ? m_pPlayer->disbandUnit(bAnnounce) : (void)nullptr;
}
...
{
	return m_pPlayer ? m_pPlayer->getName((unsigned int)iForm) : std::wstring();
}
...
{
	return m_pPlayer ? m_pPlayer->getCivilizationDescription((unsigned int)iForm) : std::wstring();
}
...
{
	return m_pPlayer ? m_pPlayer->getCivilizationShortDescription((unsigned int)iForm) : std::wstring();
}
...
{
	return m_pPlayer ? m_pPlayer->getCivilizationAdjective((unsigned int)iForm) : std::wstring();
}
...
{
	return m_pPlayer ? (std::wstring)m_pPlayer->getBestAttackUnitName((unsigned int)iForm) : std::wstring();
}
...
{
	return m_pPlayer ? (int) m_pPlayer->getBestRoute(nullptr != pPlot ? pPlot->getPlot() : nullptr) : -1;
}
...
bool CyPlayer::canRevolution(int /*CivicTypes**/ paeNewCivics)
{
	if (paeNewCivics)
		std::abort();
	return m_pPlayer ? m_pPlayer->canRevolution(nullptr) : false;
}
...
void CyPlayer::revolution(int /*CivicTypes**/ paeNewCivics, bool bForce)
{
	if (paeNewCivics)
		std::abort();
	if (m_pPlayer)
		m_pPlayer->revolution(nullptr, bForce);
}
...
int CyPlayer::getCivicAnarchyLength(pybind11::list& /*CivicTypes**/ paeNewCivics)
{
	int* pCivics = nullptr;
...
void CyPlayer::setStartingPlot(CyPlot* pPlot, bool bUpdateStartDist)
{
	...
	m_pPlayer->setStartingPlot(nullptr != pPlot ? pPlot->getPlot() : nullptr, bUpdateStartDist);
}
...
{
	return m_pPlayer ? m_pPlayer->getAdvancedStartUnitCost((UnitTypes) eUnit, bAdd, nullptr != pPlot ? pPlot->getPlot() : nullptr) : -1;
}

int CyPlayer::getAdvancedStartCityCost(bool bAdd, CyPlot* pPlot)
{
	return m_pPlayer ? m_pPlayer->getAdvancedStartCityCost(bAdd, nullptr != pPlot ? pPlot->getPlot() : nullptr) : -1;
}
...
{
	return m_pPlayer ? m_pPlayer->getAdvancedStartImprovementCost((ImprovementTypes) eImprovement, bAdd, nullptr != pPlot ? pPlot->getPlot() : nullptr) : -1;
}

int CyPlayer::getAdvancedStartRouteCost(int /*RouteTypes*/ eRoute, bool bAdd, CyPlot* pPlot)
{
	return m_pPlayer ? m_pPlayer->getAdvancedStartRouteCost((RouteTypes) eRoute, bAdd, nullptr != pPlot ? pPlot->getPlot() : nullptr) : -1;
}
...
{
	return m_pPlayer ? m_pPlayer->getAdvancedStartVisibilityCost(bAdd, nullptr != pPlot ? pPlot->getPlot() : nullptr) : -1;
}
...
{
	return m_pPlayer ? m_pPlayer->canDoEspionageMission((EspionageMissionTypes) eMission, (PlayerTypes) eTargetPlayer, nullptr != pPlot ? pPlot->getPlot() : nullptr, iExtraData, nullptr) : false;
}
...
{
	return m_pPlayer ? m_pPlayer->getEspionageMissionCost((EspionageMissionTypes) eMission, (PlayerTypes) eTargetPlayer, nullptr != pPlot ? pPlot->getPlot() : nullptr, iExtraData) : -1;
}
...
{
	if (m_pPlayer)
		m_pPlayer->doEspionageMission((EspionageMissionTypes) eMission, (PlayerTypes) eTargetPlayer, nullptr != pPlot ? pPlot->getPlot() : nullptr, iExtraData, pUnit->getUnit());
}
...
int CyPlayer::getCivicUpkeep(pybind11::list& /*CivicTypes*/ paiCivics, bool bIgnoreAnarchy)
{
	int* pCivics = nullptr;
...
pybind11::tuple CyPlayer::firstCity(bool bRev)
{
	int iterIn = 0;
	CvCity* pvObj = m_pPlayer ? m_pPlayer->firstCity(&iterIn, bRev) : nullptr;
	//CyCity* pyObj = pvObj ? new CyCity(pvObj) : nullptr;
	pybind11::tuple tup=pybind11::make_tuple(pvObj ? pybind11::cast(CyCity(pvObj)) : pybind11::none(), iterIn);
	//delete pyObj;
	return tup;
}

// returns tuple of (CyCity, iterOut)
pybind11::tuple CyPlayer::nextCity(int iterIn, bool bRev)
{
	CvCity* pvObj = m_pPlayer ? m_pPlayer->nextCity(&iterIn, bRev) : nullptr;
	//CyCity* pyObj = pvObj ? new CyCity(pvObj) : nullptr;
	if (pvObj)
		return pybind11::make_tuple(CyCity(pvObj), iterIn);
	else
		return pybind11::make_tuple(pybind11::none(), iterIn);
}
...
pybind11::tuple CyPlayer::firstUnit(bool bRev)
{
	int iterIn = 0;
	CvUnit* pvUnit = m_pPlayer ? m_pPlayer->firstUnit(&iterIn, bRev) : nullptr;
	//CyUnit* pyUnit = pvUnit ? new CyUnit(pvUnit) : nullptr;
	pybind11::tuple tup=pybind11::make_tuple(pvUnit ? pybind11::cast(CyUnit(pvUnit)) : pybind11::none(), iterIn);
	//delete pyUnit;
	return tup;
}

// returns tuple of (CyUnit, iterOut)
pybind11::tuple CyPlayer::nextUnit(int iterIn, bool bRev)
{
	CvUnit* pvObj = m_pPlayer ? m_pPlayer->nextUnit(&iterIn, bRev) : nullptr;
	//CyUnit* pyObj = pvObj ? new CyUnit(pvObj) : nullptr;
	pybind11::tuple tup=pybind11::make_tuple(pvObj ? pybind11::cast(CyUnit(pvObj)) : pybind11::none(), iterIn);
	//delete pyObj;
	return tup;
...
pybind11::tuple CyPlayer::firstSelectionGroup(bool bRev)
{
	int iterIn = 0;
	...
	pybind11::tuple tup=pybind11::make_tuple(pvObj ? pybind11::cast(CySelectionGroup(pvObj)) : pybind11::none(), iterIn);
	//delete pyObj;
	return tup;
}

// returns tuple of (CySelectionGroup, iterOut)
pybind11::tuple CyPlayer::nextSelectionGroup(int iterIn, bool bRev)
{
...
{
	return m_pPlayer ? m_pPlayer->initTriggeredData((EventTriggerTypes)eEventTrigger, bFire, iCityId, iPlotX, iPlotY, (PlayerTypes)eOtherPlayer, iOtherPlayerCityId, (ReligionTypes)eReligion, (CorporationTypes)eCorporation, iUnitId, (BuildingTypes)eBuilding) : nullptr;
}

int CyPlayer::getEventTriggerWeight(int /*EventTriggerTypes*/ eTrigger)
{
	return m_pPlayer ? m_pPlayer->getEventTriggerWeight((EventTriggerTypes)eTrigger) : 0;
}
...
{
	return (nullptr != m_pPlayer ? m_pPlayer->getScoreHistory(iTurn) : 0);
}

int CyPlayer::getEconomyHistory(int iTurn) const
{
	return (nullptr != m_pPlayer ? m_pPlayer->getEconomyHistory(iTurn) : 0);
}

int CyPlayer::getIndustryHistory(int iTurn) const
{
	return (nullptr != m_pPlayer ? m_pPlayer->getIndustryHistory(iTurn) : 0);
}

int CyPlayer::getAgricultureHistory(int iTurn) const
{
	return (nullptr != m_pPlayer ? m_pPlayer->getAgricultureHistory(iTurn) : 0);
}

int CyPlayer::getPowerHistory(int iTurn) const
{
	return (nullptr != m_pPlayer ? m_pPlayer->getPowerHistory(iTurn) : 0);
}

int CyPlayer::getCultureHistory(int iTurn) const
{
	return (nullptr != m_pPlayer ? m_pPlayer->getCultureHistory(iTurn) : 0);
}

int CyPlayer::getEspionageHistory(int iTurn) const
{
	return (nullptr != m_pPlayer ? m_pPlayer->getEspionageHistory(iTurn) : 0);
}
