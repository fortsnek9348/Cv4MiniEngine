...
public:
	...
	{
		return (int)m_aiSelectedRadioButton.size();
	}
	...
	{
//		return ((unsigned int)iGroup<m_aszEditBoxString.size()) ? m_aszEditBoxString[iGroup] : nullptr;
		if ((unsigned int)iGroup<m_aszEditBoxString.size()) 
			return m_aszEditBoxString[iGroup].c_str();
		return nullptr;
	}
	...
	{
		return (int)m_aszEditBoxString.size();
	}
	...
	{
		return ((unsigned int)iGroup<m_aiSpinBoxID.size()) ? m_aiSpinBoxID[iGroup] : -1;
	}

	//	Will get the spinner widget size
	int getSpinnerWidsize( void ) const
	{
		return (int)m_aiSpinnerWidgetValues.size();
	}
	...
	{
		return (int)m_aiPulldownID.size();
	}
	...
	{
		return ((unsigned int)iGroup<m_aiListBoxID.size()) ? m_aiListBoxID[iGroup] : -1;
	}

	//	Will get the listbox size
	int getListBoxSize( void ) const
	{
		return (int)m_aiListBoxID.size();
	}
	...
	{
		return ((unsigned int)iIndex<m_aiSpinBoxID.size()) ? m_aiSpinBoxID[iIndex] : -1;
	}

	//	Will get the spinbox size
	int getSpinBoxSize( void ) const
	{
		return (int)m_aiSpinBoxID.size();
	}
