...
CvUnit* CvSelectionGroupAI::AI_getBestGroupSacrifice(const CvPlot* pPlot, [[maybe_unused]] bool bPotentialEnemy, bool bForce, bool bNoBlitz) const
{
	int iBestValue = 0;
...
CvUnit* CvSelectionGroupAI::AI_ejectBestDefender(CvPlot* pDefendPlot)
{
	...
	if (nullptr != pBestUnit && getNumUnits() > 1)
	{
	...
	{
		// This is called from AI_guardCity and other places. We need to invalidate pathing state as the group has changed.
		resetPath();
	}
#endif
