...
{
	m_piNumUnitsBuilt = nullptr;
	m_piNumUnitsKilled = nullptr;
	m_piNumUnitsWasKilled = nullptr;
	m_piNumBuildingsBuilt = nullptr;
	m_pbReligionFounded = nullptr;
...
void CvStatistics::cityRazed([[maybe_unused]] CvCity * pCity, PlayerTypes ePlayer )
{
...
CvPlayerRecord *CvStatistics::getPlayerRecord(int iIndex)
{
	...
	if ( iIndex >= (int)m_PlayerRecords.size() || m_PlayerRecords[iIndex] == nullptr )
	{
		CvPlayerRecord *pRecord = new CvPlayerRecord;

		pRecord->init();
		pRecord->setPlayerID( iIndex );
		m_PlayerRecords.resize(iIndex + 1, nullptr);
