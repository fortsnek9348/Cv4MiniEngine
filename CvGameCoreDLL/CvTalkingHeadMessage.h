...
{
public:
	DllExport CvTalkingHeadMessage(int iMessageTurn = 0, int iLen = 0, const wchar_t* pszDesc = nullptr, const TCHAR* pszSound = nullptr, InterfaceMessageTypes eType = MESSAGE_TYPE_INFO, const TCHAR* icon = nullptr, ColorTypes eColor = NO_COLOR, int iX = -1, int iY = -1, bool bShowOffScreenArrows = false, bool bShowOnScreenArrows = false);
	...
	void setSound(const TCHAR* pszSound);
	DllExport const CvString& getIcon() const;
	void setIcon(const TCHAR* pszIcon);
