...
public:
	...
	DllExport bool ReadGlobalDefines(const TCHAR* szXMLFileName);
...
// for progress bars
	...
	DllExport void RegisterProgressCB(ProgressCB cbFxn);
	...
	DllExport bool GetChildXmlValByName(std::string& pszVal, const TCHAR* szName, char* pszDefault = nullptr);
	DllExport bool GetChildXmlValByName(std::wstring& pszVal, const TCHAR* szName, wchar_t* pszDefault = nullptr);
	// overloaded function that gets the child value of the tag with szName if there is only one child
	// value of that name
	DllExport bool GetChildXmlValByName(char* pszVal, const TCHAR* szName, char* pszDefault = nullptr);
	DllExport bool GetChildXmlValByName(wchar_t* pszVal, const TCHAR* szName, wchar_t* pszDefault = nullptr);
	// overloaded function that gets the child value of the tag with szName if there is only one child
	// value of that name
	...
	bool GetXmlVal(std::wstring& pszVal, wchar_t* pszDefault = nullptr);
	bool GetXmlVal(std::string& pszVal, char* pszDefault = nullptr);
	// overloaded function that gets either the current xml node's or the next non-comment xml node's string value
	// depending on if the current node is a non-comment node or not
	bool GetXmlVal(wchar_t* pszVal, wchar_t* pszDefault = nullptr);
	...
	bool GetNextXmlVal(std::wstring& pszVal, wchar_t* pszDefault = nullptr);
	// overloaded function that sets the current xml node to it's next sibling and then
	//	gets the next non-comment xml node's string value
	bool GetNextXmlVal(char* pszVal, char* pszDefault = nullptr);
	bool GetNextXmlVal(wchar_t* pszVal, wchar_t* pszDefault = nullptr);
	// overloaded function that sets the current xml node to it's next sibling and then
	...
	bool GetChildXmlVal(std::wstring& pszVal, wchar_t* pszDefault = nullptr);
	// overloaded function that sets the current xml node to it's first non-comment child node
	//	and then gets that node's string value
	bool GetChildXmlVal(char* pszVal, char* pszDefault = nullptr);
	bool GetChildXmlVal(wchar_t* pszVal, wchar_t* pszDefault = nullptr);
	// overloaded function that sets the current xml node to it's first non-comment child node
...
private:
	...
	template <class T>
	void LoadGlobalClassInfo(std::vector<T*>& aInfos, const char* szFileRoot, const char* szFileDirectory, const char* szXmlPath, bool bTwoPass, CvCacheObject* (CvDLLUtilityIFaceBase::*pArgFunction) (const TCHAR*) = nullptr);
#endif
