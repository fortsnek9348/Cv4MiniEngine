...
void CvDLLWidgetData::parseHelp(CvWStringBuffer &szBuffer, CvWidgetDataStruct &widgetDataStruct)
{
	...
	case WIDGET_CITY_TAB:
		{
			CvWString szTemp;
            szTemp.Format(L"%ls", GC.getCityTabInfo((CityTabTypes)widgetDataStruct.m_iData1).getDescription());
	...
	case WIDGET_PEDIA_JUMP_TO_SPECIALIST:
		...
		{
			CvWString szTemp;
			szTemp.Format(L"%ls", GC.getSpecialistInfo((SpecialistTypes)widgetDataStruct.m_iData1).getDescription());
			szBuffer.assign(szTemp);
		}
		break;
...
bool CvDLLWidgetData::executeAction( CvWidgetDataStruct &widgetDataStruct )
{
	...
	case WIDGET_MENU_ICON:
		doMenu();
		// fortsnek: Yes, let's put a break here. That would be nice.
		break;
	...
	case WIDGET_DEAL_KILL:
		{
			...
			bLink = (nullptr != pDeal && pDeal->isCancelable(GC.getGameINLINE().getActivePlayer()));
		}
		break;
	case WIDGET_CONVERT:
...
void CvDLLWidgetData::doResearch(CvWidgetDataStruct &widgetDataStruct)
{
	...
	// fortsnek: What is this? Shouldn't be needed, right?
	//if (!bShift)
	//{
...
void CvDLLWidgetData::doGotoTurnEvent(CvWidgetDataStruct &widgetDataStruct)
{
	...
	if (nullptr != pPlot && !gDLL->getEngineIFace()->isCameraLocked())
	{
...
void CvDLLWidgetData::parseLiberateCityHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity;
...
void CvDLLWidgetData::parseCityNameHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity;
	CvWString szTempBuffer;
		...
		if (!(GET_PLAYER(pHeadSelectedCity->getOwnerINLINE()).canHurry((HurryTypes)(widgetDataStruct.m_iData1))))
		{
			...
			for (iI = 0; iI < GC.getNumCivicInfos(); iI++)
			{
				if (GC.getCivicInfo((CivicTypes)iI).isHurry(widgetDataStruct.m_iData1))
				{
					...
					setListHelp(szBuffer, szTempBuffer.c_str(), GC.getCivicInfo((CivicTypes)iI).getDescription(), gDLL->getText("TXT_KEY_OR").c_str(), bFirst);
					bFirst = false;
				}
			}		
...
void CvDLLWidgetData::parseConscriptHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity;
	CvWString szTempBuffer;
	...
	if (pHeadSelectedCity != nullptr)
	{
		...
		{
			CvWString szTemp;
			szTemp.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR("COLOR_UNIT_TEXT"), GC.getUnitInfo(pHeadSelectedCity->getConscriptUnit()).getDescription());
			szBuffer.assign(szTemp);
...
void CvDLLWidgetData::parseActionHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	...
	CvWString szTemp;
	szTemp.Format(SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getActionInfo(widgetDataStruct.m_iData1).getHotKeyDescription().c_str());
	szBuffer.assign(szTemp);
	...
	if (pHeadSelectedUnit != nullptr)
	{
		if (GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType() != NO_MISSION)
		{
			...
			else if (GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType() == MISSION_SABOTAGE)
			{
				...
				while (pSelectedUnitNode != nullptr)
				{
					...
					if (pSelectedUnit->canSabotage(pMissionPlot, true)) // XXX if queuing up this action, use the current plot along the goto...
					{
						...
						if (iPrice > 0)
						{
							szTempBuffer.Format(L"%d %lc", iPrice, GC.getCommerceInfo(COMMERCE_GOLD).getChar());
							szBuffer.append(NEWLINE);
			...
			else if (GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType() == MISSION_SPREAD_CORPORATION)
			{
				...
				if (pMissionCity != nullptr)
				{
					...
					szTempBuffer.Format(L"%ls%d %lc", NEWLINE, pHeadSelectedUnit->spreadCorporationCost(eCorporation, pMissionCity), GC.getCommerceInfo(COMMERCE_GOLD).getChar());
					szBuffer.append(szTempBuffer);
					...
					else
					{
						...
						while (pSelectedUnitNode != nullptr)
						{
							...
							if (pSelectedUnit->canHurry(pMissionPlot, true))
							{
								...
								if (nullptr != pcKey && pSelectedUnit->getHurryProduction(pMissionPlot) >= pMissionCity->productionLeft())
								{
									szBuffer.append(NEWLINE);
				...
				if (pMissionCity != nullptr)
				{
					...
					else
					{
						...
						while (pSelectedUnitNode != nullptr)
						{
							...
							{
								szTempBuffer.Format(L"%ls+%d%lc", NEWLINE, pSelectedUnit->getTradeGold(pMissionPlot), GC.getCommerceInfo(COMMERCE_GOLD).getChar());
								szBuffer.append(szTempBuffer);
								break;
							}
			...
			else if (GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType() == MISSION_GREAT_WORK)
			{
				...
				while (pSelectedUnitNode != nullptr)
				{
					...
					{
						szTempBuffer.Format(L"%ls+%d%lc", NEWLINE, pSelectedUnit->getGreatWorkCulture(pMissionPlot), GC.getCommerceInfo(COMMERCE_CULTURE).getChar());
						szBuffer.append(szTempBuffer);
						break;
					}
				...
				if (pMissionCity != nullptr)
				{
					...
					else
					{
						...
						while (pSelectedUnitNode != nullptr)
						{
							...
							{
								szTempBuffer.Format(L"%ls+%d%lc", NEWLINE, pSelectedUnit->getEspionagePoints(pMissionPlot), GC.getCommerceInfo(COMMERCE_ESPIONAGE).getChar());
								szBuffer.append(szTempBuffer);
								break;
							}
			...
			else if (GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType() == MISSION_BUILD)
			{
				...
				for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
				{
					...
					if (iYield != 0)
					{
						szTempBuffer.Format(L", %ls%d%lc", (iYield > 0) ? L"+" : L"", iYield, GC.getYieldInfo((YieldTypes) iI).getChar());
						szBuffer.append(szTempBuffer);
					}
				}
				...
				if (NO_IMPROVEMENT != eImprovement)
				{
					...
					if (iHappy != 0)
					{
						szTempBuffer.Format(L", +%d%lc", abs(iHappy), (iHappy > 0 ? gDLL->getSymbolID(HAPPY_CHAR) : gDLL->getSymbolID(UNHAPPY_CHAR)));
						szBuffer.append(szTempBuffer);
					}
				}
					...
					if (eRoute != NO_ROUTE)
					{
						...
						if (!bFoundValid)
						{
							...
							for (std::vector<BonusTypes>::iterator it = aeOrBonuses.begin(); it != aeOrBonuses.end(); ++it)
							{
								...
								szTempBuffer.Format( SETCOLR L"<link=literal>%ls</link>" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getBonusInfo(*it).getDescription());
								setListHelp(szBuffer, szFirstBuffer.GetCString(), szTempBuffer.c_str(), gDLL->getText("TXT_KEY_OR").c_str(), bFirst);
								bFirst = false;
							}
						}
					}
					...
					else
					{
						...
						for (iI = 0; iI < GC.getNumBonusInfos(); iI++)
						{
							if (GET_TEAM(pHeadSelectedUnit->getTeam()).isHasTech((TechTypes)(GC.getBonusInfo((BonusTypes) iI).getTechReveal())))
							{
								...
								{
									szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_ACTION_CHANCE_DISCOVER").c_str());
									szTempBuffer.Format(L"%lc", GC.getBonusInfo((BonusTypes) iI).getChar());
									setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", (GC.getImprovementInfo(eImprovement).getImprovementBonusDiscoverRand(iI) != iLast));
				...
				{
					szBuffer.append(CvWString::format(L"%ls%ls", NEWLINE, GC.getBuildInfo(eBuild).getHelp()).c_str());
				}
			}

			if (!CvWString(GC.getMissionInfo((MissionTypes)(GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType())).getHelp()).empty())
			{
				szBuffer.append(CvWString::format(L"%ls%ls", NEWLINE, GC.getMissionInfo((MissionTypes)(GC.getActionInfo(widgetDataStruct.m_iData1).getMissionType())).getHelp()).c_str());
			}
		}
		...
		if (GC.getActionInfo(widgetDataStruct.m_iData1).getCommandType() != NO_COMMAND)
		{
			...
			else if (GC.getActionInfo(widgetDataStruct.m_iData1).getCommandType() == COMMAND_UPGRADE)
			{
				...
				szTempBuffer.Format(L"%ls%d %lc", NEWLINE, iPrice, GC.getCommerceInfo(COMMERCE_GOLD).getChar());
				szBuffer.append(szTempBuffer);
			}
			else if (GC.getActionInfo(widgetDataStruct.m_iData1).getCommandType() == COMMAND_GIFT)
			{
				...
				if (eGiftPlayer != NO_PLAYER)
				{
					...
					szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, GET_PLAYER(eGiftPlayer).getPlayerTextColorR(), GET_PLAYER(eGiftPlayer).getPlayerTextColorG(), GET_PLAYER(eGiftPlayer).getPlayerTextColorB(), GET_PLAYER(eGiftPlayer).getPlayerTextColorA(), GET_PLAYER(eGiftPlayer).getCivilizationShortDescription());
					szBuffer.append(szTempBuffer);
			...
			{
				szBuffer.append(CvWString::format(L"%ls%ls", NEWLINE, GC.getCommandInfo((CommandTypes)(GC.getActionInfo(widgetDataStruct.m_iData1).getCommandType())).getHelp()).c_str());
			}
		}
		...
		if (GC.getActionInfo(widgetDataStruct.m_iData1).getAutomateType() != NO_AUTOMATE)
		{
			...
			{
				szBuffer.append(CvWString::format(L"%ls%ls", NEWLINE, GC.getAutomateInfo((ControlTypes)(GC.getActionInfo(widgetDataStruct.m_iData1).getAutomateType())).getHelp()).c_str());
			}
		}
	}
	...
	if (GC.getActionInfo(widgetDataStruct.m_iData1).getControlType() != NO_CONTROL)
	{
		...
		{
			szBuffer.append(CvWString::format(L"%ls%ls", NEWLINE, GC.getControlInfo((ControlTypes)(GC.getActionInfo(widgetDataStruct.m_iData1).getControlType())).getHelp()).c_str());
		}
	}
	...
	if (GC.getActionInfo(widgetDataStruct.m_iData1).getInterfaceModeType() != NO_INTERFACEMODE)
	{
		...
		{
			szBuffer.append(CvWString::format(L"%ls%ls", NEWLINE, GC.getInterfaceModeInfo((InterfaceModeTypes)(GC.getActionInfo(widgetDataStruct.m_iData1).getInterfaceModeType())).getHelp()).c_str());
		}
	}
}
					...
					if (eLoopBuilding != NO_BUILDING)
					{
						if (GC.getBuildingInfo(eLoopBuilding).getSpecialistCount(widgetDataStruct.m_iData1) > 0)
						{
							if ((pHeadSelectedCity->getNumBuilding(eLoopBuilding) <= 0) && !isLimitedWonderClass((BuildingClassTypes)iI))
							{
								...
								{
									szTempBuffer = NEWLINE + gDLL->getText("TXT_KEY_REQUIRES");
									setListHelp(szBuffer, szTempBuffer.c_str(), GC.getBuildingInfo(eLoopBuilding).getDescription(), gDLL->getText("TXT_KEY_OR").c_str(), bFirst);
									bFirst = false;
								}
							}
						}
...
void CvDLLWidgetData::parseAngryCitizenHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity;
...
void CvDLLWidgetData::parseConvertHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	if (widgetDataStruct.m_iData2 != 0)
	{
		...
		{
//			szBuffer.Format(L"No State %lc", gDLL->getSymbolID(RELIGION_CHAR));
			szBuffer.assign(gDLL->getText("TXT_KEY_MISC_NO_STATE_REL"));
		}
		else
		{
//			szBuffer.Format(L"Convert to %ls", GC.getReligionInfo((ReligionTypes) widgetDataStruct.m_iData1).getDescription());
...
void CvDLLWidgetData::parseAutomateCitizensHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity;
...
void CvDLLWidgetData::parseAutomateProductionHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity;
...
void CvDLLWidgetData::parseTradeItem(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	...
	if ((eWhoFrom != NO_PLAYER) && (eWhoTo != NO_PLAYER))
	{
		...
		{
			szTempBuffer.Format(L"%ls: " SETCOLR L"%ls" ENDCOLR, GET_PLAYER(eWhoDenies).getName(), TEXT_COLOR("COLOR_WARNING_TEXT"), GC.getDenialInfo(eDenial).getDescription());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
	}
...
void CvDLLWidgetData::parseUnitModelHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvUnit* pHeadSelectedUnit;
...
void CvDLLWidgetData::parseFlagHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvWString szTempBuffer;

	szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getCivilizationInfo(GC.getGameINLINE().getActiveCivilizationType()).getDescription());
	szBuffer.append(szTempBuffer);
	szBuffer.append(NEWLINE);
...
void CvDLLWidgetData::parseMaintenanceHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity;
	...
	if (pHeadSelectedCity != nullptr)
	{
		...
		else
		{
			...
			{
				//			swprintf(szTempBuffer, "\n%ls%d%lc: %ls", ((iMaintenanceValue > 0) ?  "+" : ""), iMaintenanceValue, GC.getCommerceInfo(COMMERCE_GOLD).getChar(), ((GET_PLAYER(pHeadSelectedCity->getOwnerINLINE()).getNumGovernmentCenters() > 0) ? "Distance from Palace" : "No Palace Penalty"));
			...
			if (iMaintenanceValue != 0)
			{
				//			swprintf(szTempBuffer, "\n%ls%d%lc: Number of Cities", ((iMaintenanceValue > 0) ? "+" : ""), iMaintenanceValue, GC.getCommerceInfo(COMMERCE_GOLD).getChar());
				CvWString szMaint = CvWString::format(L"%d.%02d", iMaintenanceValue/100, iMaintenanceValue%100);
				szBuffer.append(NEWLINE);
			...
			//		swprintf(szTempBuffer, "\n%d%lc Total Maintenance", pHeadSelectedCity->getMaintenance(), GC.getCommerceInfo(COMMERCE_GOLD).getChar());
			...
			{
#ifdef _WIN32
				swprintf(szTempBuffer, L" (%ls%d%%)", ((iMaintenanceValue > 0) ? L"+" : L""), iMaintenanceValue);
#else
				swprintf(szTempBuffer, std::size(szTempBuffer) - 1, L" (%ls%d%%)", ((iMaintenanceValue > 0) ? L"+" : L""), iMaintenanceValue);
#endif
				szBuffer.append(szTempBuffer);
			}
		}
...
void CvDLLWidgetData::parseHealthHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity = gDLL->getInterfaceIFace()->getHeadSelectedCity();
	if (nullptr != pHeadSelectedCity)
	{
...
void CvDLLWidgetData::parseNationalityHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	wchar_t szTempBuffer[1024];
	CvCity* pHeadSelectedCity;
	...
	if (pHeadSelectedCity != nullptr)
	{
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (GET_PLAYER((PlayerTypes)iI).isAlive())
			{
				...
				{
#ifdef _WIN32
					swprintf(szTempBuffer, L"\n%d%% " SETCOLR L"%ls" ENDCOLR, iCulturePercent, GET_PLAYER((PlayerTypes)iI).getPlayerTextColorR(), GET_PLAYER((PlayerTypes)iI).getPlayerTextColorG(), GET_PLAYER((PlayerTypes)iI).getPlayerTextColorB(), GET_PLAYER((PlayerTypes)iI).getPlayerTextColorA(), GET_PLAYER((PlayerTypes)iI).getCivilizationAdjective());
#else
					swprintf(szTempBuffer, std::size(szTempBuffer) - 1, L"\n%d%% " SETCOLR L"%ls" ENDCOLR, iCulturePercent, GET_PLAYER((PlayerTypes)iI).getPlayerTextColorR(), GET_PLAYER((PlayerTypes)iI).getPlayerTextColorG(), GET_PLAYER((PlayerTypes)iI).getPlayerTextColorB(), GET_PLAYER((PlayerTypes)iI).getPlayerTextColorA(), GET_PLAYER((PlayerTypes)iI).getCivilizationAdjective());
#endif
					szBuffer.append(szTempBuffer);
				}
			}
...
#else
					swprintf(szTempBuffer, std::size(szTempBuffer) - 1, L"%.2f", std::max(0.0f, (1.0f - (((float)iGarrison) / ((float)iCityStrength)))) * ((float)(std::min(100.0f, ((float)pHeadSelectedCity->getRevoltTestProbability())))));
#endif
					szBuffer.append(NEWLINE);
...
void CvDLLWidgetData::parseHappinessHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity;
...
void CvDLLWidgetData::parsePopulationHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity;
...
void CvDLLWidgetData::parseProductionHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity;
	...
	if (pHeadSelectedCity != nullptr)
	{
		if (pHeadSelectedCity->getProductionNeeded() != INT_MAX)
		{
			CvWString szTemp;
			szTemp.Format(L"%ls: %d/%d %lc", pHeadSelectedCity->getProductionName(), pHeadSelectedCity->getProduction(), pHeadSelectedCity->getProductionNeeded(), GC.getYieldInfo(YIELD_PRODUCTION).getChar());
			szBuffer.assign(szTemp);
		}
	}
}
...
void CvDLLWidgetData::parseCultureHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity;
...
void CvDLLWidgetData::parseGreatPeopleHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	CvCity* pHeadSelectedCity = gDLL->getInterfaceIFace()->getHeadSelectedCity();
	if (nullptr != pHeadSelectedCity)
	{
...
void CvDLLWidgetData::parseGreatGeneralHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
...
{
	CvCity* pHeadSelectedCity;
	[[maybe_unused]] CvUnit* pHeadSelectedUnit;
	OrderData* pOrder;
...
{
//	szBuffer.Format(L"%lcThis technology requires the knowledge of %ls", gDLL->getSymbolID(BULLET_CHAR), GC.getTechInfo((TechTypes) widgetDataStruct.m_iData1).getDescription());
...
void CvDLLWidgetData::parseFinanceNumUnits([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
...
void CvDLLWidgetData::parseFinanceGrossIncome([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
//	szBuffer = "Your gross income";
	szBuffer.assign(gDLL->getText("TXT_KEY_ECON_GROSS_INCOME"));
}

void CvDLLWidgetData::parseFinanceNetGold([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
//	szBuffer = "Net Gold per turn";
	szBuffer.assign(gDLL->getText("TXT_KEY_ECON_NET_GOLD"));
}

void CvDLLWidgetData::parseFinanceGoldReserve([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
...
void CvDLLWidgetData::parsePediaBack([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
//	szBuffer = "Back";
	szBuffer.assign(gDLL->getText("TXT_KEY_MISC_PEDIA_BACK"));
}

void CvDLLWidgetData::parsePediaForward([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
//	szBuffer = "Forward";
...
void CvDLLWidgetData::doDealKill(CvWidgetDataStruct &widgetDataStruct)
{
	...
	if (pDeal != nullptr)
	{
		if (!pDeal->isCancelable(GC.getGameINLINE().getActivePlayer()))
		{
			...
			if (nullptr != pInfo)
			{
				pInfo->setText(gDLL->getText("TXT_KEY_POPUP_CANNOT_CANCEL_DEAL").c_str());
				gDLL->getInterfaceIFace()->addPopup(pInfo, GC.getGameINLINE().getActivePlayer(), true);
			}
		}
		else
...
void CvDLLWidgetData::parseProductionModHelp([[maybe_unused]] CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
