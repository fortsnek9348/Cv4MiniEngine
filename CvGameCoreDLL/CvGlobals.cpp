...
static bool gDisplayedSizeCheckMessage = false;

template <class T>
bool readInfoArray(FDataStreamBase* pStream, std::vector<T*>& array, [[maybe_unused]] const char* szClassName)
{
	GC.addToInfosVectors(&array);

	int iSize;
	pStream->Read(&iSize);
	// fortsnek, Because this is a template, this message gets displayed for every instantiation, so guard it with a non-template variable.
	if (iSize!=sizeof(T) && !gDisplayedSizeCheckMessage)
	{
		gDisplayedSizeCheckMessage = true;
...
m_pt3CameraDir{}, // fortsnek: init this too
m_iNewPlayers(0),
...
m_setupData(nullptr),
m_iniInitCore(nullptr), // fortsnek: init this too
m_loadedInitCore(nullptr), // fortsnek: init this too
m_initCore(nullptr),
...
m_interface(nullptr),
m_pArtFileMgr(nullptr), // fortsnek: init this too
m_map(nullptr),
...
//m_aGameOptionsInfo(nullptr),
//m_aPlayerOptionsInfo(nullptr),
m_VarSystem(nullptr),
...
void CvGlobals::setEnhancedDLLConfig([[maybe_unused]] EnhancedDLLConfig config)
{
#if ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (config[EEnhancedDLLConfigOption::CvCity_doTurnParallel] && !config[EEnhancedDLLConfigOption::Faster_CvCityAI_AI_updateRouteToCity])
		throw std::runtime_error("Bad enhanced DLL config: CvCity_doTurnParallel requires Faster_CvCityAI_AI_updateRouteToCity.");

	// Need thread-safe calculatePathDistance.
	if (config[EEnhancedDLLConfigOption::Parallel_CvPlayerAI_AI_findTargetCity] && !config[EEnhancedDLLConfigOption::Fast_CvMap_calculatePathDistance])
		throw std::runtime_error("Bad enhanced DLL config: Parallel_CvPlayerAI_AI_findTargetCity requires Fast_CvMap_calculatePathDistance.");

	enhancedDLLConfig = config;
#endif
}

void CvGlobals::clearGiganticMapsOptimisationsLibContext()
{
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	enhancedDLLGameContext = nullptr;
#endif
}
...
CvString& CvGlobals::getDirectionTypes(AutomateTypes e)
{
	...
	FAssert((int)e < (int)NUM_DIRECTION_TYPES);
...
int CvGlobals::getFootstepAudioTypeByTag(CvString strTag)
{
	...
	{
		if ( strTag.CompareNoCase(m_paszFootstepAudioTypes[i].c_str()) == 0 )
		{
			iIndex = i;
			break;
		}
...
int CvGlobals::getInfoTypeForString(const char* szType, bool hideAssert) const
	...
	if(!hideAssert)
	{
		...
		gDLL->logMsg("xml.log", szError.c_str());
	}

	return -1;
}

int CvGlobals::getInfoTypeForString(const CvString& szType, bool hideAssert) const
{
	return getInfoTypeForString(szType.c_str(), hideAssert);
}
