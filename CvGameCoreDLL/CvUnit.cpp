...
CvUnit::CvUnit()
{
	...
	m_pabHasPromotion = nullptr;

	m_paiTerrainDoubleMoveCount = nullptr;
	m_paiFeatureDoubleMoveCount = nullptr;
	m_paiExtraTerrainAttackPercent = nullptr;
	m_paiExtraTerrainDefensePercent = nullptr;
	m_paiExtraFeatureAttackPercent = nullptr;
	m_paiExtraFeatureDefensePercent = nullptr;
	m_paiExtraUnitCombatModifier = nullptr;
...
{
	[[maybe_unused]] CvPlot* pPlot = plot();
...
void CvUnit::kill(bool bDelay, PlayerTypes ePlayer)
{
	...
	CvPlot* pPlot;
	CvWString szBuffer;
	[[maybe_unused]] PlayerTypes eOwner;
	...
	for (unsigned int i = 0; i < oldUnits.size(); i++)
	{
	...
	setReconPlot(nullptr);
	setBlockading(false);

	FAssertMsg(getAttackPlot() == nullptr, "The current unit instance's attack plot is expected to be nullptr");
	FAssertMsg(getCombatUnit() == nullptr, "The current unit instance's combat unit is expected to be nullptr");
...
void CvUnit::doTurn()
{
	...
	FAssertMsg(getGroup() != nullptr, "getGroup() is not expected to be equal with nullptr");
...
void CvUnit::updateAirStrike(CvPlot* pPlot, bool bQuick, bool bFinish)
{
	...
	if (!bFinish)
	{
		...
		if (!bQuick)
		{
			bVisible = isCombatVisible(nullptr);
		}
		...
		if (bVisible)
		{
			...
			kAirMission.setUnit(BATTLE_UNIT_DEFENDER, nullptr);
...
void CvUnit::resolveAirCombat(CvUnit* pInterceptor, CvPlot* pPlot, CvAirMissionDefinition& kBattle)
{
	...
	int iTheirStrength = (DOMAIN_AIR == pInterceptor->getDomainType() ? pInterceptor->airCurrCombatStr(this) : pInterceptor->currCombatStr(nullptr, nullptr));
	int iOurStrength = (DOMAIN_AIR == getDomainType() ? airCurrCombatStr(pInterceptor) : currCombatStr(nullptr, nullptr));
...
void CvUnit::updateAirCombat(bool bQuick)
{
	...
	if (!bFinish)
	{
		...
		FAssertMsg(pInterceptor != nullptr, "Defender is not assigned a valid value");
	...
	if (bFinish)
	{
		setAttackPlot(nullptr, false);
		setCombatUnit(nullptr);
		pInterceptor->setCombatUnit(nullptr);
...
void CvUnit::updateCombat(bool bQuick)
{
	...
	if (bFinish)
	{
		...
		if (isDead())
		{
			...
			gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getEraInfo(GC.getGameINLINE().getCurrentEra()).getAudioUnitDefeatScript(), MESSAGE_TYPE_INFO, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
			...
			gDLL->getInterfaceIFace()->addMessage(pDefender->getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getEraInfo(GC.getGameINLINE().getCurrentEra()).getAudioUnitVictoryScript(), MESSAGE_TYPE_INFO, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
		...
		else if (pDefender->isDead())
		{
			...
			gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getEraInfo(GC.getGameINLINE().getCurrentEra()).getAudioUnitVictoryScript(), MESSAGE_TYPE_INFO, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
		...
		else
		{
			...
			gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_OUR_WITHDRAWL", MESSAGE_TYPE_INFO, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
			szBuffer = gDLL->getText("TXT_KEY_MISC_ENEMY_UNIT_WITHDRAW", getNameKey(), pDefender->getNameKey());
			gDLL->getInterfaceIFace()->addMessage(pDefender->getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_THEIR_WITHDRAWL", MESSAGE_TYPE_INFO, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
...
bool CvUnit::generatePath(const CvPlot* pToPlot, PathingArg args, bool bReuse, int* piPathTurns) const
{
	return getGroup()->generatePath(plot(), pToPlot, args, bReuse, piPathTurns);
}
...
void CvUnit::move(CvPlot* pPlot, bool bShow)
{
	...
	if(featureType != NO_FEATURE)
	{
		...
		{
			FeatureTypes newFeatureType = (FeatureTypes) GC.getInfoTypeForString(featureString.c_str());
...
bool CvUnit::canGift(bool bTestVisible, bool bTestTransport)
{
	...
	if (!pPlot->isValidDomainForLocation(*this) && nullptr == pTransport)
	{
		return false;
	}
...
void CvUnit::unloadAll()
{
	...
	for (unsigned int i = 0; i < aCargoUnits.size(); ++i)
	{
		...
		if (pCargo->canUnload())
		{
			pCargo->setTransportUnit(nullptr);
		}
		else
		{
...
bool CvUnit::canHold([[maybe_unused]] const CvPlot* pPlot) const
{
	return true;
}


bool CvUnit::canSleep([[maybe_unused]] const CvPlot* pPlot) const
{
	if (isFortifyable())
	{
		return false;
...
bool CvUnit::canFortify([[maybe_unused]] const CvPlot* pPlot) const
{
	if (!isFortifyable())
	{
		return false;
...
void CvUnit::airCircle(bool bStart)
{
	...
	if (bStart)
	{
		...
		kDefinition.setUnit(BATTLE_UNIT_DEFENDER, nullptr);
...
bool CvUnit::airlift(int iX, int iY)
{
	...
	pTargetPlot = GC.getMapINLINE().plotINLINE(iX, iY);
	FAssert(pTargetPlot != nullptr);
	pTargetCity = pTargetPlot->getPlotCity();
	FAssert(pTargetCity != nullptr);
	...
	for (iDX = -(nukeRange()); iDX <= nukeRange(); iDX++)
	{
		for (iDY = -(nukeRange()); iDY <= nukeRange(); iDY++)
		{
			...
			if (pLoopPlot != nullptr)
			{
				...
				if (pLoopPlot->plotCheck(PUF_isCombatTeam, eTeam, getTeam()) != nullptr)
				{
					return true;
				}
			}
...
bool CvUnit::canNuke([[maybe_unused]] const CvPlot* pPlot) const
{
	if (nukeRange() == -1)
	{
		return false;
...
bool CvUnit::nuke(int iX, int iY)
{
	...
	if (GC.getGameINLINE().getSorenRandNum(100, "Nuke") < iBestInterception)
	{
		...
		if (pPlot->isActiveVisible(false))
		{
			...
			// Add the intercepted mission (defender is not nullptr)
	...
	if (pPlot->isActiveVisible(false))
	{
		...
		// Add the non-intercepted mission (defender is nullptr)
		gDLL->getEntityIFace()->AddMission(&kDefiniton);
	}
...
bool CvUnit::canRecon([[maybe_unused]] const CvPlot* pPlot) const
{
	if (getDomainType() != DOMAIN_AIR)
	{
		return false;
...
bool CvUnit::canParadropAt(const CvPlot* pPlot, int iX, int iY) const
{
	...
	if (nullptr == pTargetPlot || pTargetPlot == pPlot)
	{
		return false;
	}
...
bool CvUnit::canAirBomb([[maybe_unused]] const CvPlot* pPlot) const
{
	if (getDomainType() != DOMAIN_AIR)
	{
		return false;
...
bool CvUnit::airBomb(int iX, int iY)
{
	...
	if (pCity != nullptr)
	{
		...
		gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_BOMBARD", MESSAGE_TYPE_INFO, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pCity->getX_INLINE(), pCity->getY_INLINE());
	}
	else
	{
		if (pPlot->getImprovementType() != NO_IMPROVEMENT)
		{
			...
			// Rand = 586486727 on 1509 (Evasion Rand)
			// Rand = 1632255476 on 1509 (Air Bomb - Defense)
			// Rand = -365631203 on 1509 (Air Bomb - Offense)
			// Rand = -2130950254 on 1535 (AI Unit Birthmark)
			const int def = GC.getGameINLINE().getSorenRandNum(GC.getImprovementInfo(pPlot->getImprovementType()).getAirBombDefense(), "Air Bomb - Defense");
			const int off = GC.getGameINLINE().getSorenRandNum(airBombCurrRate(), "Air Bomb - Offense");
			if (off >= def)
			{
...
bool CvUnit::canBombard(const CvPlot* pPlot) const
{
	...
	if (bombardTarget(pPlot) == nullptr)
	{
		return false;
	}
		...
		if (isEnemy((TeamTypes)iTeam))
		{
			for (int i = -iBlockadeRange; i <= iBlockadeRange; ++i)
			{
				for (int j = -iBlockadeRange; j <= iBlockadeRange; ++j)
				{
					...
					if (nullptr != pLoopPlot && pLoopPlot->isWater() && pLoopPlot->area() == area())
					{
						if (!bChanged)
						{
...
int CvUnit::sabotageCost([[maybe_unused]] const CvPlot* pPlot) const
{
...
bool CvUnit::canDiscover([[maybe_unused]] const CvPlot* pPlot) const
{
	TechTypes eTech;
...
int CvUnit::getTradeGold(const CvPlot* pPlot) const
{
	...
	iGold = (m_pUnitInfo->getBaseTrade() + (m_pUnitInfo->getTradeMultiplier() * ((pCapitalCity != nullptr) ? pCity->calculateTradeProfit(pCapitalCity) : 0)));
...
int CvUnit::getGreatWorkCulture([[maybe_unused]] const CvPlot* pPlot) const
{
	int iCulture;
...
int CvUnit::getEspionagePoints([[maybe_unused]] const CvPlot* pPlot) const
{
...
bool CvUnit::canInfiltrate(const CvPlot* pPlot, bool bTestVisible) const
{
	...
	if (getEspionagePoints(nullptr) == 0)
	{
		return false;
	}

	CvCity* pCity = pPlot->getPlotCity();
	if (pCity == nullptr || pCity->isBarbarian())
	{
		return false;
	}
	...
	if (!bTestVisible)
	{
		if (nullptr != pCity && pCity->getTeam() == getTeam())
		{
			return false;
		}
	}
...
bool CvUnit::infiltrate()
{
	...
	int iPoints = getEspionagePoints(nullptr);
...
bool CvUnit::canGoldenAge([[maybe_unused]] const CvPlot* pPlot, bool bTestVisible) const
{
	if (!isGoldenAge())
	{
		return false;
...
bool CvUnit::lead(int iUnitId)
{
	...
	else
	{
		...
		pUnit->joinGroup(nullptr, true, true);
...
BuildTypes CvUnit::getBuildType() const
{
	...
	if (getGroup()->headMissionQueueNode() != nullptr)
	{
...
int CvUnit::maxCombatStr(const CvPlot* pPlot, const CvUnit* pAttacker, CombatDetails* pCombatDetails) const
{
	...
	if (pPlot != nullptr)
	{
		if (!noDefensiveBonus())
		{
			iExtraModifier = pPlot->defenseModifier(getTeam(), (pAttacker != nullptr) ? pAttacker->ignoreBuildingDefense() : true);
			iModifier += iExtraModifier;
			if (pCombatDetails != nullptr)
			{
							...
							if (!pLoopUnit->isInvisible(getTeam(), false))
							{
								if (isEnemy(pLoopUnit->getTeam()))
								{
									if (DOMAIN_SEA == pLoopUnit->getDomainType())
									{
										...
										{
											if (nullptr == pBestUnit || pLoopUnit->isBetterDefenderThan(pBestUnit, this))
											{
...
void CvUnit::joinGroup(CvSelectionGroup* pSelectionGroup, bool bRemoveSelected, bool bRejoin)
{
	...
	if ((pSelectionGroup != pOldSelectionGroup) || (pOldSelectionGroup == nullptr))
	{
		...
		if ((pNewSelectionGroup == nullptr) || canJoinGroup(pPlot, pNewSelectionGroup))
		{
			...
			if ((pNewSelectionGroup != nullptr) && pNewSelectionGroup->addUnit(this, false))
			{
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	const int oldPlotCoordX = m_iX;
	const int oldPlotCoordY = m_iY;
	//if (plot())
	//	gGlobals.enhancedDLLGameContext->onUnitChange(GiganticMapsOptimisationsLib::EGameUnitChangeEvent::MoveFrom, *this);
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (const auto& ctx = gGlobals.enhancedDLLGameContext) // This is sometimes called during init
		ctx->onUnitMoved(*this, { oldPlotCoordX, oldPlotCoordY }, { m_iX, m_iY }); // coord.x must be negative if INVALID_PLOT_COORD
#endif
}
...
void CvUnit::getCargoUnits(std::vector<CvUnit*>& aUnits) const
{
	...
	FAssert(static_cast<size_t>(getCargo()) == aUnits.size());
}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	gGlobals.enhancedDLLGameContext->onUnitChange(GiganticMapsOptimisationsLib::EGameUnitChangeEvent::IsMadeAttack, *this);
#endif
}
...
void CvUnit::collectBlockadeGold()
{
	...
	for (int i = -iBlockadeRange; i <= iBlockadeRange; ++i)
	{
		for (int j = -iBlockadeRange; j <= iBlockadeRange; ++j)
		{
			...
			if (nullptr != pLoopPlot && pLoopPlot->isRevealed(getTeam(), false))
			{
				CvCity* pCity = pLoopPlot->getPlotCity();

				if (nullptr != pCity && !pCity->isPlundered() && isEnemy(pCity->getTeam()) && !atWar(pCity->getTeam(), getTeam()))
				{
...
const CvWString CvUnit::getName(unsigned int uiForm) const
{
	CvWString szBuffer;
	...
	szBuffer.Format(L"%ls (%ls)", m_szName.GetCString(), m_pUnitInfo->getDescription(uiForm));
...
void CvUnit::flankingStrikeCombat(const CvPlot* pPlot, int iAttackerStrength, int iAttackerFirepower, int iDefenderOdds, int iDefenderDamage, CvUnit* pSkipUnit)
{
	...
	{
		int iIndexHit = GC.getGameINLINE().getSorenRandNum(static_cast<int>(listFlankedUnits.size()), "Pick Flanked Unit");
		...
		if (pUnit->isDead())
		{
			...
			gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getEraInfo(GC.getGameINLINE().getCurrentEra()).getAudioUnitVictoryScript(), MESSAGE_TYPE_INFO, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
			...
			gDLL->getInterfaceIFace()->addMessage(pUnit->getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getEraInfo(GC.getGameINLINE().getCurrentEra()).getAudioUnitDefeatScript(), MESSAGE_TYPE_INFO, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
	...
	if (iNumUnitsHit > 0)
	{
		...
		if (nullptr != pSkipUnit)
		{
			szBuffer = gDLL->getText("TXT_KEY_MISC_YOUR_UNITS_DAMAGED_BY_FLANKING", getNameKey(), iNumUnitsHit);
			gDLL->getInterfaceIFace()->addMessage(pSkipUnit->getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getEraInfo(GC.getGameINLINE().getCurrentEra()).getAudioUnitDefeatScript(), MESSAGE_TYPE_INFO, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
		}
	}
}
...
bool CvUnit::canAirStrike(const CvPlot* pPlot) const
{
	...
	if (airStrikeTarget(pPlot) == nullptr)
	{
		return false;
	}
...
#define BATTLE_TURN_RECHECK 4
	...
	[[maybe_unused]] CvBattleRoundVector::iterator	iIterator;
	bool							bIsLoser;
	int								iRoundIndex;
	int								iTotalRounds = 0;
...
bool CvUnit::isCombatVisible(const CvUnit* pDefender) const
{
	...
	{
		if (nullptr == pDefender || !pDefender->getUnitInfo().isQuickCombat())
		{
			if (isHuman())
			{
			...
			else if (nullptr != pDefender && pDefender->isHuman())
			{
...
void CvUnit::cheat(bool bCtrl, [[maybe_unused]] bool bAlt, [[maybe_unused]] bool bShift)
{
	if (gDLL->getChtLvl() > 0)
	{
		if (bCtrl)
