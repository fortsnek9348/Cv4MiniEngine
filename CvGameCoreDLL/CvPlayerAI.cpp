...
//#define ENABLE_ORDER_INDEPENDENT_RNG 1 // for testing
#include "GiganticMapsOptimisationsLib/GameContext.h"
#include "GiganticMapsOptimisationsLib/Util.h"
...
CvPlayerAI::CvPlayerAI()
{
	...
	m_aiBonusValue = nullptr;
	m_aiUnitClassWeights = nullptr;
	m_aiUnitCombatWeights = nullptr;
...
void CvPlayerAI::AI_uninit()
{
	...
	m_playerTradeNetworkSystem = nullptr;
#endif
}
...
void CvPlayerAI::AI_reset(bool bConstructor)
{
	...
	FAssert(m_aiBonusValue == nullptr);
	...
	FAssert(m_aiUnitClassWeights == nullptr);
	...
	FAssert(m_aiUnitCombatWeights == nullptr);
...
void CvPlayerAI::AI_doTurnUnitsPre()
{
	...
	//std::clog << "TODO: AI_doTurnUnitsPre, AI_updateAreaTargets" << std::endl;
...
void CvPlayerAI::AI_doTurnUnitsPost()
{
	...
	CvPlot* pLastUpgradePlot = nullptr;
			...
			if (bValid)
			{
				...
				if (!bNoDisband)
				{
					if (pLoopUnit->canFight())
					{
						...
						if (pPlotCity != nullptr && pPlotCity->getOwnerINLINE() == getID())
						{
							int iCityExp = 0;
...
void CvPlayerAI::AI_updateFoundValues(bool bStartingLoc) const
{
	...
	CvArea* pLoopArea;
	int iLoop;
	[[maybe_unused]] bool bCitySiteCalculations = (GC.getGame().getGameTurn() > GC.getGame().getStartTurn());
...
//#else
		//pLoopArea->setBestFoundValueIsPositive(getID(), false);
//#endif
	}
	...
	{
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::Parallel_CvPlayerAI_AI_updateFoundValues_for_bStartingLoc])
		{
			// Instead of just clearing, pre-compute all found values, in parallel. This speeds up gigantic map generation.
			if (GC.getUSE_GET_CITY_FOUND_VALUE_CALLBACK())
				std::abort();

			CvMap& map = GC.getMapINLINE();

			// Ensure all these are initialised.
			// Also done in CvGame::createBarbarianCities().
			for (size_t i = 0, n = GC.getNumBonusInfos(); i < n; ++i)
				(void)AI_baseBonusVal(BonusTypes(i));

			const int myStartingPlotRange = startingPlotRange();

			// Useful to know for gigantic maps.
			gDLL->logMemState(("Starting plot range for player " + std::to_string(getID()) + ": " + std::to_string(myStartingPlotRange)).c_str());

			std::vector<std::atomic_int> bestAreaFoundValues(map.getIndexAfterLastArea());
			heck::parallelForEachN(map.numPlotsINLINE(), [
				playerId = getID(), &player = GET_PLAYER(getID()), myStartingPlotRange, &map, &bestAreaFoundValues](int iI) {
					CvPlot& plot = *map.plotByIndexINLINE(iI);
					const int value = player.AI_foundValue(plot.getX_INLINE(), plot.getY_INLINE(), -1, true, myStartingPlotRange);
					plot.setFoundValue(playerId, value);
					heck::atomicMaxRelaxed(bestAreaFoundValues[plot.getArea() & FLTA_INDEX_MASK], value);
				});

				for (int i = 0, n = static_cast<int>(bestAreaFoundValues.size()); i < n; ++i)
					if (map.getArea(i))
						map.getArea(i)->setBestFoundValue(getID(), bestAreaFoundValues[i].load(std::memory_order_relaxed));
		}
		else
#endif
		{
	...
	else
	{
		...
		{
			// By Tiamat, if you're using python for this on a 1M-plot map...
			if (GC.getUSE_GET_CITY_FOUND_VALUE_CALLBACK())
				std::abort();

			// Old trivial parallel update code, before major switch over to the found value cache system.
#if 0
			// Ensure all these are initialised.
			// Also done in CvGame::createBarbarianCities().
			for (size_t i = 0, n = GC.getNumBonusInfos(); i < n; ++i)
			...
			CvMap& map = GC.getMapINLINE();
			std::vector<std::atomic_int> bestAreaFoundValues(map.getIndexAfterLastArea());
			for (size_t i = 0; i < bestAreaFoundValues.size(); ++i)
				if (map.getArea(i))
					bestAreaFoundValues[i] = map.getArea(i)->getBestFoundValue(getID());


			// And let's hope there's no race conditions!
			std::for_each_n(std::execution::par, std::views::iota(0).begin(), map.numPlotsINLINE(), [&map, &bestAreaFoundValues, teamI = getTeam(), playerI = getID(), this](int plotI) {
				CvPlot* const pLoopPlot = map.plotByIndexINLINE(plotI);

				if (pLoopPlot->isRevealed(teamI, false))
				{
					const int iValue = AI_foundValue(pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE());

					pLoopPlot->setFoundValue(playerI, iValue);

					//if (iValue > pLoopPlot->area()->getBestFoundValue(playerI))
					//{
					//	pLoopPlot->area()->setBestFoundValue(playerI, iValue);
					//}

					heck::atomicMaxRelaxed(bestAreaFoundValues[pLoopPlot->area()->getID() & FLTA_INDEX_MASK], iValue);
				}
				});
			...
			for (size_t i = 0; i < bestAreaFoundValues.size(); ++i)
				if (map.getArea(i))
					map.getArea(i)->setBestFoundValue(getID(), bestAreaFoundValues[i].load(std::memory_order_relaxed));
#else
			/// New code. V2 found value system (cached vectorised computation).

			auto& ctx = *gGlobals.enhancedDLLGameContext;
			ctx.prepareForFoundValueQuerying(getID());

			CvMap& map = GC.getMapINLINE();
			std::vector<std::atomic_int> bestAreaFoundValues(map.getIndexAfterLastArea());
			heck::parallelForEachN(map.getGridHeightINLINE(), [&ctx, &map, &bestAreaFoundValues, playerId = getID(), width = map.getGridWidthINLINE()](int y) {
				//std::for_each_n(heck::range(0).begin(), map.getGridHeightINLINE(), [&ctx, &map, &bestAreaFoundValues, playerId = getID(), width = map.getGridWidthINLINE()](int y) {
				const std::vector<int> row = ctx.computeRevealedFoundValuesRow(playerId, y);
				for (const int x : heck::range(width))
				{
					CvPlot& plot = *map.plotINLINE(x, y);
					const int value = row[x];
					const int areaI = plot.getArea() & FLTA_INDEX_MASK;
					plot.setFoundValue(playerId, value);
					heck::atomicMaxRelaxed(bestAreaFoundValues[areaI], value);
				}
				});
			for (int i = 0; i < static_cast<int>(bestAreaFoundValues.size()); ++i)
				if (map.getArea(i))
					map.getArea(i)->setBestFoundValue(getID(), bestAreaFoundValues[i].load(std::memory_order_relaxed));
#endif
		}
...
void CvPlayerAI::AI_unitUpdate()
{
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::ParallelBarbarianUnitUpdate] && !isHuman())
	{
		gGlobals.enhancedDLLGameContext->doAIParallelUnitUpdate(*this);
		return;
	}
	...
	if (!hasBusyUnit())
	{
		...
		else
		{
			...
			while (tempGroupCycle.getLength() > 0)
			{
				...
				{
					pLoopSelectionGroup = getSelectionGroup(pCurrUnitNode->m_data);
					FAssertMsg(pLoopSelectionGroup != nullptr, "selection group node with nullptr selection group");
			...
			while (pCurrUnitNode != nullptr)
			{
				...
				if (nullptr != pLoopSelectionGroup)  // group might have been killed by a previous group update
				{
					if (pLoopSelectionGroup->AI_update())
					{
						break; // pointers could become invalid...
	...
	//std::clog << "Updating units for player " << (int)getID() << " took " << std::chrono::duration<double, std::milli>(t1 - t0) << std::endl;
#endif
}
...
bool CvPlayerAI::AI_captureUnit([[maybe_unused]] UnitTypes eUnit, CvPlot* pPlot) const
{
	CvCity* pNearestCity;
...
) const
{
	...
	if (!canFound(iX, iY))// fortsnek: AI_foundValue_dependency
	{
		return 0;
	}
	...
	if (!bStartingLoc)
	{
		if (!AI_isPlotCitySite(pPlot)) // fortsnek: AI_foundValue_dependency
		{
			for (iI = 0; iI < NUM_CITY_PLOTS; iI++)
			{
				...
				if (pLoopPlot != nullptr)
				{
					...
					{
						CvPlot* pCitySitePlot = AI_getCitySite(iJ); // fortsnek: AI_foundValue_dependency
	...
	if (iMinRivalRange != -1)
	{
		for (iDX = -(iMinRivalRange); iDX <= iMinRivalRange; iDX++)
		{
			for (iDY = -(iMinRivalRange); iDY <= iMinRivalRange; iDY++)
			{
				...
				if (pLoopPlot != nullptr)
				{
					if (pLoopPlot->plotCheck(PUF_isOtherTeam, getID()) != nullptr)// fortsnek: AI_foundValue_dependency
					{
						return 0;
					}
				}
	...
	for (iI = 0; iI < NUM_CITY_PLOTS; iI++)
	{
        ...
        {
            if (pLoopPlot->getTeam() != getTeam()) // fortsnek: AI_foundValue_dependency
            {
                iOwnedTiles++;
            }
        }
	...
	[[maybe_unused]] int iYieldLostHere = 0;
	...
	if (bStartingLoc)
	{
		...
		if (!pPlot->isStartingPlot())
		{
			int iMinDistanceFactor = INT_MAX;
#if ENABLE_GAMECOREDLL_ENHANCEMENTS
			int iMinRange = startingPlotRangeOverride >= 0 ? startingPlotRangeOverride : startingPlotRange();
#else
	...
	// fortsnek: AI_foundValue_dependency global
	// fortsnek: AI_foundValue Stage Two start.
	...
	if (pNearestCity != nullptr)
	{
		...
		else
		{
			...
			else if (getCapitalCity() != nullptr)
			{
				...
				[[maybe_unused]] int iDistanceToCapital = plotDistance(iCapitalX, iCapitalY, iX, iY);
	...
	// fortsnek: AI_foundValue_dependency global boolean
	...
	if (bStartingLoc)
	{
		...
		for (iI = 0; iI < NUM_CITY_PLOTS; iI++)
		{
			...
			if ((pLoopPlot == nullptr) || !(pLoopPlot->isWater() || pLoopPlot->area() == pArea))
			{
				iDifferentAreaTile++;
			}
		}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
int CvPlayerAI::AI_targetCityValue(const CvCity* pCity, bool bRandomize, bool bIgnoreAttackers, bool bOverestimate) const
#else
...
CvCity* CvPlayerAI::AI_findTargetCity(CvArea* pArea) const
{
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::Parallel_CvPlayerAI_AI_findTargetCity])
	{
		std::vector<CvCity*> cityList;
		cityList.reserve(100);
		for (const PlayerTypes playerI : heck::range<PlayerTypes>(MAX_CIV_PLAYERS))
		{
			const CvPlayer& player = GET_PLAYER(playerI);
			if (player.isAlive() && isPotentialEnemy(getTeam(), player.getTeam()))
			{
				int iLoop{};
				for (CvCity* city = player.firstCity(&iLoop); city; city = player.nextCity(&iLoop))
					if (city->area() == pArea)
		...
		struct Choice
		{
			...
			{
				// Order by increasing value and prefer smaller indices for same value.
				return std::pair(value, -index) < std::pair(b.value, -b.index);
			}
		};
		...
		GiganticMapsOptimisationsLib::DeterministicParallelPlotHashRNG rng(GC.getGameINLINE().getSorenRand(), "AI Target City Value");
		heck::parallelForEachN(cityList.size(), [&](size_t i) {
			// Don't "randomise", do the RNG ourselves.
			const CvCity* const city = cityList[i];
			int iValue = AI_targetCityValue(city, false);
			iValue += rng.get(static_cast<unsigned short>(((city->getPopulation() / 2) + 1)),
				map.plotNumINLINE(city->plot()), "AI Target City Value");
			heck::atomicMaxRelaxed(bestChoice, Choice{ iValue, static_cast<int>(i) });
			});

		const Choice choosen = bestChoice.load(std::memory_order_relaxed);
		if (choosen.index >= 0)
			pBestCity = cityList[choosen.index];
	}
	else
#endif
	{
#if ENABLE_ORDER_INDEPENDENT_RNG
		const GiganticMapsOptimisationsLib::DeterministicParallelPlotHashRNG rng(gGlobals.getGameINLINE().getSorenRand(), "AI Target City Value");
		const CvMap& map = gGlobals.getMapINLINE();
#endif
		CvCity* pLoopCity;
		int iValue;
			...
			if (GET_PLAYER((PlayerTypes)iI).isAlive())
			{
				if (isPotentialEnemy(getTeam(), GET_PLAYER((PlayerTypes)iI).getTeam()))
				{
					for (pLoopCity = GET_PLAYER((PlayerTypes)iI).firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = GET_PLAYER((PlayerTypes)iI).nextCity(&iLoop))
					{
						...
						{
#if ENABLE_ORDER_INDEPENDENT_RNG
							iValue = AI_targetCityValue(pLoopCity, false);
							iValue += rng.get(static_cast<uint16_t>((pLoopCity->getPopulation() / 2) + 1), map.plotNumINLINE(pLoopCity->plot()), "AI Target City Value");
#else
	...
	//std::wclog << L"Best city = " << (pBestCity ? pBestCity->getName() : CvWString("None")) << std::endl;
	return pBestCity;
...
int CvPlayerAI::AI_getUnitDanger(CvUnit* pUnit, int iRange, bool bTestMoves, [[maybe_unused]] bool bAnyDanger) const
{
	PROFILE_FUNC();
...
int CvPlayerAI::AI_getWaterDanger(CvPlot* pPlot, int iRange, [[maybe_unused]] bool bTestMoves) const
{
	PROFILE_FUNC();
	...
	[[maybe_unused]] CvArea* pWaterArea = pPlot->waterArea();
					...
					if (!(kTeam.isHasTech((TechTypes)iI)))
					{
						if (GC.getTechInfo((TechTypes)iI).getEra() <= (getCurrentEra() + 1))
						{
							...
							if (iPathLength <= iMaxPathLength)
							{
								...
								[[maybe_unused]] int iRandomMax = 2000;
											...
											for (iK = 0; iK < GC.getNumBonusInfos(); iK++)
											{
												...
												if (iBonusValue > 0)
												{
													for (iL = 0; iL < NUM_YIELD_TYPES; iL++)
													{
														...
														if (bAdvancedStart && std::to_underlying(getCurrentEra()) < 2)
														{
										...
										if (isTechRequiredForUnit(((TechTypes)iI), eLoopUnit))
										{
											...
											if (kLoopUnit.getPrereqAndTech() == iI)
											{
												...
												if (kLoopUnit.getUnitAIType(UNITAI_ASSAULT_SEA))
												{
													...
													if (AI_bestAreaUnitAIValue(UNITAI_ASSAULT_SEA, nullptr, &eExistingUnit) == 0)
													{
														iAssaultValue += 1000;
													}
													else
									...
									if (eLoopBuilding != NO_BUILDING)
									{
										...
										if (isTechRequiredForBuilding(((TechTypes)iI), eLoopBuilding))
										{
											...
											if (kLoopBuilding.getMaintenanceModifier() < 0)
											{
												...
												CvCity* pLoopCity;
												iTempValue = 0;
												for (pLoopCity = firstCity(&iLoop, true); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop, true))
												{
										...
										for (iJ = 0; iJ < GC.getNumReligionInfos(); iJ++)
										{
											...
											if (eReligionTech == iI)
											{
												if (!(GC.getGameINLINE().isReligionSlotTaken((ReligionTypes)iJ)))
												{
													...
													iReligionValue += 1 + ((bAsync) ? GC.getASyncRand().get(static_cast<unsigned short>(iRoll), "AI Research Religion ASYNC") : GC.getGameINLINE().getSorenRandNum(iRoll, "AI Research Religion"));
													iRandomMax += iRoll;
...
int CvPlayerAI::AI_calculateStolenCityRadiusPlots(PlayerTypes ePlayer) const
{
	...
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::Fast_CvPlayerAI_AI_calculateStolenCityRadiusPlots])
	{
		...
		//std::for_each_n(std::execution::par, std::views::iota(0).begin(), GC.getMapINLINE().numPlotsINLINE(), [&iCount, ePlayer, me = getID()](int iI) {
		//	CvPlot* pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);
		//
		//	if (pLoopPlot->getOwnerINLINE() == ePlayer)
		//	{
		//		if (pLoopPlot->isPlayerCityRadius(me))
		//		{
		...
		// Faster still!
		return gGlobals.enhancedDLLGameContext->getStolenCityRadiusPlots(getID(), ePlayer);
	}
	else
#endif
	{
...
{
	if (m_aiCloseBordersAttitudeCache[ePlayer] == INT_MAX)
	{
	int iPercent;
...
bool CvPlayerAI::AI_counterPropose(PlayerTypes ePlayer, const CLinkList<TradeData>* pTheirList, const CLinkList<TradeData>* pOurList, CLinkList<TradeData>* pTheirInventory, CLinkList<TradeData>* pOurInventory, CLinkList<TradeData>* pTheirCounter, CLinkList<TradeData>* pOurCounter) const
{
	...
	else if (iHumanDealWeight > iAIDealWeight)
	{
		...
		int iGoldWeight = iHumanDealWeight - iAIDealWeight;
...
int CvPlayerAI::AI_baseBonusVal(BonusTypes eBonus) const
{
	...
	if(m_aiBonusValue[eBonus] == -1)
	{
		...
		if (!GET_TEAM(getTeam()).isBonusObsolete(eBonus))
		{
			...
			CvCity* pCoastalCity = nullptr;
			CvCity* pUnconnectedCoastalCity = nullptr;
			if (iCoastalCityCount > 0)
			{
					...
					for (CvCity* pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
				{
					if (pLoopCity->isCoastal(GC.getMIN_WATER_SIZE_FOR_OCEAN()))
					{
						...
						else if (pUnconnectedCoastalCity == nullptr)
						{
			...
			if (pCoastalCity == nullptr && pUnconnectedCoastalCity != nullptr)
			{
				...
				if (eLoopUnit != NO_UNIT)
				{
					...
					if (iTempValue > 0)
					{
						...
						if (canTrain(eLoopUnit))
						{
							...
							if ((bIsWater && (pCoastalCity == nullptr || (pCoastalCity->allUpgradesAvailable(eLoopUnit) != NO_UNIT))) ||
								// or our capital cannot build because its obsolete (we can already build all its upgrades)
								(pCapital != nullptr && pCapital->allUpgradesAvailable(eLoopUnit) != NO_UNIT))
							{
								// its worthless
								iTempValue = 2;
							}
...
int CvPlayerAI::AI_corporationBonusVal(BonusTypes eBonus) const
{
	...
	for (int iCorporation = 0; iCorporation < GC.getNumCorporationInfos(); ++iCorporation)
	{
		...
		if (iCorpCount > 0)
		{
			[[maybe_unused]] int iNumCorpBonuses = 0;
...
//			iValue += ((iCombatValue * iCombatModifier) / 100);
	...
	case UNITAI_ICBM:
		if (GC.getUnitInfo(eUnit).getNukeRange() != -1)
		{
			int iTempValue = 40 + (GC.getUnitInfo(eUnit).getNukeRange() * 40);
...
int CvPlayerAI::AI_neededExplorers(CvArea* pArea) const
{
	...
	if (0 == iNeeded)
	{
		if ((GC.getGameINLINE().countCivTeamsAlive() - 1) > GET_TEAM(getTeam()).getHasMetCivCount(true))
		{
			...
			{
			    if (getCapitalCity() != nullptr && pArea->getID() == getCapitalCity()->getArea())
			    {
...
int CvPlayerAI::AI_executiveValue(CvArea* pArea, CorporationTypes eCorporation, PlayerTypes* peBestPlayer) const
{
	CvTeam& kTeam = GET_TEAM(getTeam());
	CvGame& kGame = GC.getGame();
	[[maybe_unused]] CvCorporationInfo& kCorp = GC.getCorporationInfo(eCorporation);
	...
	{
		iSpreadInternalValue = 0;
		if (iSpreadExternalValue = 0, iSpreadExternalValue) // fortsnek: using result of an assignment!
		{
			return 0;
		}
	}
...
int CvPlayerAI::AI_plotTargetMissionAIs(const CvPlot* pPlot, MissionAITypes eMissionAI, const CvSelectionGroup* pSkipSelectionGroup, int iRange) const
{
	int iClosestTargetRange;
	return AI_plotTargetMissionAIs(pPlot, &eMissionAI, 1, iClosestTargetRange, pSkipSelectionGroup, iRange);
}

int CvPlayerAI::AI_plotTargetMissionAIs(const CvPlot* pPlot, MissionAITypes eMissionAI, int& iClosestTargetRange, const CvSelectionGroup* pSkipSelectionGroup, int iRange) const
{
	return AI_plotTargetMissionAIs(pPlot, &eMissionAI, 1, iClosestTargetRange, pSkipSelectionGroup, iRange);
}

int CvPlayerAI::AI_plotTargetMissionAIs(const CvPlot* pPlot, const MissionAITypes* aeMissionAI, int iMissionAICount, int& iClosestTargetRange, const CvSelectionGroup* pSkipSelectionGroup, int iRange) const
{
	PROFILE_FUNC();

	int iCount = 0;
	iClosestTargetRange = INT_MAX;
...
int CvPlayerAI::AI_unitTargetMissionAIs(const CvUnit* pUnit, const MissionAITypes* aeMissionAI, int iMissionAICount, const CvSelectionGroup* pSkipSelectionGroup) const
{
	PROFILE_FUNC();
...
int CvPlayerAI::AI_enemyTargetMissionAIs(MissionAITypes* aeMissionAI, int iMissionAICount, const CvSelectionGroup* pSkipSelectionGroup) const
{
	...
	for(CvSelectionGroup* pLoopSelectionGroup = firstSelectionGroup(&iLoop); pLoopSelectionGroup; pLoopSelectionGroup = nextSelectionGroup(&iLoop))
	{
		if (pLoopSelectionGroup != pSkipSelectionGroup)
		{
			...
			if (nullptr != pMissionPlot && pMissionPlot->isOwned())
			{
			MissionAITypes eGroupMissionAI = pLoopSelectionGroup->AI_getMissionAIType();
			for (int iMissionAIIndex = 0; iMissionAIIndex < iMissionAICount; iMissionAIIndex++)
			{
...
int CvPlayerAI::AI_civicValue(CivicTypes eCivic) const
{
	...
	if (kCivic.getWarWearinessModifier() != 0)
	{
		...
		iTempValue = (-kCivic.getWarWearinessModifier() * iAngerPercent * iPopulation) / (GC.getPERCENT_ANGER_DIVISOR() * 100);
		if (iTempValue != 0)
		{
				...
				if (AI_getAttitudeWeight(pSpyPlot->getOwner()) < (GC.getGameINLINE().isOption(GAMEOPTION_AGGRESSIVE_AI) ? 50 : 1))
				{
					...
					for (int iMission = 0; iMission < GC.getNumEspionageMissionInfos(); ++iMission)
					{
						...
						if (kMissionInfo.getDestroyProjectCostFactor() > 0)
						{
							...
							{
								[[maybe_unused]] ProjectTypes eProject = (ProjectTypes)iProject;
...
{
	[[maybe_unused]] TeamTypes eTargetTeam = GET_PLAYER(eTargetPlayer).getTeam();
	...
	if (!canDoEspionageMission(eMission, eTargetPlayer, pPlot, iData, nullptr))
	{
		return 0;
	}
		...
		if (nullptr != pPlot)
		{
			if (pPlot->getOwnerINLINE() == eTargetPlayer)
			{
				...
				if (eImprovement != NO_IMPROVEMENT)
				{
					...
					{
						int iTempValue = 0;
						if (nullptr != pPlot->getWorkingCity())
						{
...
int CvPlayerAI::AI_getContactTimer(PlayerTypes eIndex1, ContactTypes eIndex2) const
{
	...
	FAssertMsg(std::to_underlying(eIndex1) < MAX_PLAYERS, "eIndex1 is expected to be within maximum bounds (invalid Index)");
...
void CvPlayerAI::AI_doMilitary()
{
	...
	// Rand = -294174508 on 199 (AI Research)
	// Rand = -696294787 on 199 (AI Attack Odds Change #2)
	// Rand = 233649522 on 199 (AI Attack Odds Change #1)
	// Rand = 2088021443 on 199 (Wonder Construction Rand)
	// The only difference though is the log text.
	...
	const int odds2 = GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getAttackOddsChangeRand(), "AI Attack Odds Change #2");
	const int odds1 = GC.getGameINLINE().getSorenRandNum(GC.getLeaderHeadInfo(getPersonalityType()).getAttackOddsChangeRand(), "AI Attack Odds Change #1");
	AI_setAttackOddsChange(GC.getLeaderHeadInfo(getPersonalityType()).getBaseAttackOddsChange() + odds1 + odds2);
}
...
void CvPlayerAI::AI_doCommerce()
{
	...
	if (isCommerceFlexible(COMMERCE_ESPIONAGE) && !bFirstTech)
	{
		...
		else
		{
			...
			[[maybe_unused]] int iInitialResearchPercent = getCommercePercent(COMMERCE_RESEARCH);
												...
												if (((pLoopCity->getGameTurnAcquired() + 4) % 20) == (GC.getGameINLINE().getGameTurn() % 20))
												{
													...
													if (iCount >= (iPossibleCount / 2))
													{
														...
														if (canTradeItem(((PlayerTypes)iI), item, true))
														{
															...
															{
																GC.getGameINLINE().implementDeal(getID(), ((PlayerTypes)iI), &ourList, nullptr);
															}
														}
													}
												}
									...
									if (GET_PLAYER((PlayerTypes)iI).isHuman() && (GET_TEAM(getTeam()).getLeaderID() == getID()) && !GET_TEAM(getTeam()).isVassal(GET_PLAYER((PlayerTypes)iI).getTeam()))
									{
										...
										if (eFavoriteCivic != NO_CIVIC)
										{
											if (isCivic(eFavoriteCivic))
											{
												...
												{
													if (GET_PLAYER((PlayerTypes)iI).canRevolution(nullptr))
													{
									...
									if (!(GET_PLAYER((PlayerTypes)iI).isHuman()) || !(abContacted[GET_PLAYER((PlayerTypes)iI).getTeam()]))
									{
										...
										if (GC.getGameINLINE().getSorenRandNum(iDeclareWarTradeRand, "AI Diplo Declare War Trade") == 0)
										{
											...
											if (eBestTeam != NO_TEAM)
											{
												...
												[[maybe_unused]] int iBestValue2 = 0;
...
void CvPlayerAI::write(FDataStreamBase* pStream)
{
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_aiAICitySites.size());
		pStream->Write(iSize);
...
{
	[[maybe_unused]] CvEventTriggerInfo& kTrigger = GC.getEventTriggerInfo(kTriggeredData.m_eTrigger);
	...
	{
		CvUnit* pUnit2 = getUnit(kTriggeredData.m_iUnitId);
		if (nullptr != pUnit2)
		{
			int iUnitValue = pUnit2->getUnitInfo().getProductionCost();
			if (iUnitValue > 0)
			{
				iUnitValue *= 2;
			}
	...
	for (int iTech = 0; iTech < GC.getNumTechInfos(); ++iTech)
	{
		if (canResearch((TechTypes)iTech))
		{
			if (NO_PLAYER == kTriggeredData.m_eOtherPlayer || GET_TEAM(GET_PLAYER(kTriggeredData.m_eOtherPlayer).getTeam()).isHasTech((TechTypes)iTech))
			{
				...
				{
					iValue2 += kEvent.getTechFlavorValue(i) * GC.getTechInfo((TechTypes)iTech).getFlavorValue(i);
				}

				if (iValue2 > iBestValue)
				{
	...
	if (NO_PLAYER != kTriggeredData.m_eOtherPlayer)
	{
		...
		{
			int iBonusValue2 = -AI_bonusVal((BonusTypes)kEvent.getBonusGift(), -1);
			iBonusValue2 += (iOtherPlayerAttitudeWeight - 40) * kOtherPlayer.AI_bonusVal((BonusTypes)kEvent.getBonusGift(), +1);
			//Positive for friends, negative for enemies.
			iDiploValue += (iBonusValue2 * GC.getDefineINT("PEACE_TREATY_LENGTH")) / 60;
		}
		...
		if (kEvent.isDeclareWar())
		{
			[[maybe_unused]] int iWarValue = (GET_TEAM(getTeam()).getDefensivePower() - GET_TEAM(GET_PLAYER(kTriggeredData.m_eOtherPlayer).getTeam()).getPower(true));// / std::max(1, GET_TEAM(getTeam()).getDefensivePower());
			iWarValue -= 30 * AI_getAttitudeVal(kTriggeredData.m_eOtherPlayer);
		}
			
		if (kEvent.getMaxPillage() > 0)
		{
			[[maybe_unused]] int iPillageValue = (40 * (kEvent.getMinPillage() + kEvent.getMaxPillage())) / 2;
...
EventTypes CvPlayerAI::AI_chooseEvent(int iTriggeredId) const
{
	...
	for (int i = 0; i < kTrigger.getNumEvents(); i++)
	{
		...
		{
			[[maybe_unused]] CvEventInfo& kEvent = GC.getEventInfo((EventTypes)kTrigger.getEvent(i));
...
void CvPlayerAI::AI_doSplit()
{
	...
	for (CvArea* pLoopArea = GC.getMapINLINE().firstArea(&iLoop); pLoopArea != nullptr; pLoopArea = GC.getMapINLINE().nextArea(&iLoop))
	{
...
bool CvPlayerAI::AI_disbandUnit(int iExpThreshold, [[maybe_unused]] bool bObsolete)
{
	CvUnit* pLoopUnit;
	CvUnit* pBestUnit;
	int iValue;
...
int CvPlayerAI::AI_getCultureVictoryStage() const
{
    ...
    if (std::to_underlying(GC.getGame().getStartEra()) > 1)
    {
    	return 0;
    }
	...
	// fortsnek: Being extra careful are we?
	//FAssert(false);
}
...
int CvPlayerAI::AI_getStrategyHash() const
{
	...
	[[maybe_unused]] const FlavorTypes AI_FLAVOR_GOLD = (FlavorTypes)3;
	[[maybe_unused]] const FlavorTypes AI_FLAVOR_SCIENCE = (FlavorTypes)4;
	...
	
	{
		...
		int iTheirVictoryCountdown = INT_MAX;
		...
		if (INT_MAX == iTheirVictoryCountdown)
		{
	...
	if (isCurrentResearchRepeat())
	{
		[[maybe_unused]] int iTotalVictories = 0;
...
void CvPlayerAI::AI_nowHasTech([[maybe_unused]] TechTypes eTech)
{
...
int CvPlayerAI::AI_countDeadlockedBonuses(const CvPlot* pPlot) const
{
    ...
    int iMinRange = GC.getMIN_CITY_RANGE(); // fortsnek: 2
	...
	for (int iPlayer = 0; iPlayer < MAX_PLAYERS; iPlayer++)
	{
		...
		if (kPlayer.isAlive())
		{
			if (kPlayer.getTeam() == getTeam())
			{
				...
				{
					FAssertMsg(false, "StartingPlot for a live player is nullptr!");
				}
			}
		}
	}
...
bool CvPlayerAI::AI_advancedStartPlaceCity(CvPlot* pPlot)
{
	...
	if (std::to_underlying(pCity->getCultureLevel()) <= 1)
	{
...
//			{
//				aiPlotGroups.clear();
//				if (pLoopPlot->getPlotGroup(getID()) != nullptr)
//				{
...
//					if ((pLoopPlot2 != nullptr) && (pLoopPlot2->getRouteType() != NO_ROUTE))
//					{
		...
		if (!pLoopCity->isCapital() && !pLoopCity->isConnectedToCapital())
		{
			...
			for (int iX = -iRange; iX <= iRange; iX++)
			{
				for (int iY = -iRange; iY <= iRange; iY++)
				{
					...
					if ((pLoopPlot != nullptr) && (pLoopPlot->getOwner() == getID()))
					{
...
void CvPlayerAI::AI_doAdvancedStart(bool bNoExit)
{
	...
	{
		for (int iPass = 0; iPass < 2 && nullptr == getCapitalCity(); ++iPass)
		{
	...
	[[maybe_unused]] int iArea = -1; //getStartingPlot()->getArea();
	...
	
	{
		...
		if (std::to_underlying(getCurrentEra()) > 2)
		{
			//Sea
...
void CvPlayerAI::AI_recalculateFoundValues(int iX, int iY, int iInnerRadius, int iOuterRadius) const
{
	...
	for (iLoopX = -iOuterRadius; iLoopX <= iOuterRadius; iLoopX++)
	{
		for (iLoopY = -iOuterRadius; iLoopY <= iOuterRadius; iLoopY++)
		{
			...
			if ((nullptr != pLoopPlot) && !AI_isPlotCitySite(pLoopPlot))
			{
				...
				{
					if ((pLoopPlot != nullptr) && (pLoopPlot->isRevealed(getTeam(), false)))
					{
						long lResult=-1;
						if(GC.getUSE_GET_CITY_FOUND_VALUE_CALLBACK())
						{
...
{
	[[maybe_unused]] std::vector<int>::iterator it;

	int iPass = 0;
	while (iPass < iMaxSites)
	{
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::Parallel_CvPlayerAI_AI_updateCitySites])
		{
#if 1
			// Original trivial parallel loop.
			struct Best
			{
				int value = 0;
				int plotI = -1;

				bool operator<(Best b) const
				{
					// Resolving tie-breaks by plot index.
					return std::pair(value, -plotI) < std::pair(b.value, -plotI);
				}
			};
			std::atomic<Best> best{};
			heck::parallelForEachN(GC.getMapINLINE().numPlotsINLINE(), [iMinFoundValueThreshold, teamI = getTeam(), playerI = getID(), &best, this](int iI) {
				CvPlot* pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);
				if (pLoopPlot->isRevealed(teamI, false))
				{
					int iValue = pLoopPlot->getFoundValue(playerI);
					if (iValue > iMinFoundValueThreshold)
					{
						if (!AI_isPlotCitySite(pLoopPlot))
						{
							...
							heck::atomicMaxRelaxed(best, Best{ iValue, iI });
						}
					}
				}
				});
			const Best localBest = best.load(std::memory_order_relaxed);
			const CvPlot* const pBestFoundPlot = localBest.plotI < 0 ? nullptr : GC.getMapINLINE().plotByIndexINLINE(localBest.plotI);
#else
			// Complicated V1 found value system
			auto& ctx = *gGlobals.enhancedDLLGameContext;
			const CvPlot* pBestFoundPlot = nullptr;
			ctx.enumerateHighestCityPlanningValues(iMinFoundValueThreshold + 1, getID(), true,
				[](const CvArea& area) { return std::min(NUM_CITY_PLOTS * 2, area.getNumUnownedTiles()); },
				[&pBestFoundPlot, this](GiganticMapsOptimisationsLib::FoundValueSearchResult result) {
					CvPlot* const pLoopPlot = GC.getMapINLINE().plot(result.coord.x, result.coord.y);
					if (!AI_isPlotCitySite(pLoopPlot))
					{
						// Keep the found values for planned city sites updated.
						pLoopPlot->setFoundValue(getID(), result.foundValue);
						pBestFoundPlot = pLoopPlot;
						return false;
					}
					return true;
			...
			{
				// Perform lite verification.
				gGlobals.enhancedDLLGameContext->notifyFoundValueUsed(getID(), { pBestFoundPlot->getX_INLINE(), pBestFoundPlot->getY_INLINE() },
					-1, localBest.value / std::min(NUM_CITY_PLOTS * 2, pBestFoundPlot->area()->getNumUnownedTiles()));

				m_aiAICitySites.push_back(GC.getMapINLINE().plotNum(pBestFoundPlot->getX_INLINE(), pBestFoundPlot->getY_INLINE()));
				gGlobals.enhancedDLLGameContext->onIsAICitySiteChanged(*pBestFoundPlot, getID());
...
void CvPlayerAI::AI_invalidateCitySites([[maybe_unused]] int iMinFoundValueThreshold) const
{
#ifndef ENABLE_GAMECOREDLL_ENHANCEMENTS
	m_aiAICitySites.clear();
#else
	// Clear list first, then invalidate cache.
	const auto oldCitySites = std::move(m_aiAICitySites);
	m_aiAICitySites.clear();

	for (const int plotI : oldCitySites)
		gGlobals.enhancedDLLGameContext->onIsAICitySiteChanged(*GC.getMapINLINE().plotByIndexINLINE(plotI), getID());
#endif
}

int CvPlayerAI::AI_getNumCitySites() const
{
	return static_cast<int>(m_aiAICitySites.size());
...
int CvPlayerAI::AI_bestCityUnitAIValue(UnitAITypes eUnitAI, CvCity* pCity, UnitTypes* peBestUnitType) const
{
	...
	for (iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
	{
		...
		if (eLoopUnit != NO_UNIT)
		{
			if (!isHuman() || (GC.getUnitInfo(eLoopUnit).getDefaultUnitAIType() == eUnitAI))
			{
				if (nullptr == pCity ? canTrain(eLoopUnit) : pCity->canTrain(eLoopUnit))
				{
...
int CvPlayerAI::AI_calculateUnitAIViability(UnitAITypes eUnitAI, DomainTypes eDomain) const
{
	...
	for (int iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
	{
		[[maybe_unused]] UnitTypes eLoopUnit = (UnitTypes)GC.getUnitClassInfo((UnitClassTypes)iI).getDefaultUnitIndex();
...
ReligionTypes CvPlayerAI::AI_chooseReligion()
{
	...
	{
		return aeReligions[GC.getGameINLINE().getSorenRandNum(static_cast<int>(aeReligions.size()), "AI pick religion")];
	}
...
int CvPlayerAI::AI_getPlotAirbaseValue(const CvPlot* pPlot) const
{
	...
	int iMinOtherCityDistance = INT_MAX;
	[[maybe_unused]] CvPlot* iMinOtherCityPlot = nullptr;
	
	int iMinFriendlyCityDistance = INT_MAX;
	[[maybe_unused]] CvPlot* iMinFriendlyCityPlot = nullptr;
	...
	for (int iX = -iRange; iX <= iRange; iX++)
	{
		...
		{
			CvPlot* pLoopPlot = plotXY(pPlot->getX_INLINE(), pPlot->getY_INLINE(), iX, iY);
			if ((pLoopPlot != nullptr) && (pPlot != pLoopPlot))
			{
...
//	if (iMinFriendlyCityPlot != nullptr)
//	{
//		FAssert(iMinOtherCityPlot != nullptr);
...
{
	[[maybe_unused]] int iWorstHappy = 0;
	[[maybe_unused]] int iBestHappy = 0;
	[[maybe_unused]] int iTotalUnhappy = 0;
	[[maybe_unused]] int iTotalHappy = 0;
	int iLoop;
	CvCity* pLoopCity;
	int iCount = 0;
...
{
	[[maybe_unused]] int iWorstHealth = 0;
	[[maybe_unused]] int iBestHealth = 0;
	[[maybe_unused]] int iTotalUnhappy = 0;
	[[maybe_unused]] int iTotalHealth = 0;
	int iLoop;
	CvCity* pLoopCity;
	int iCount = 0;
...
void CvPlayerAI::AI_invalidateCloseBordersAttitudeCache()
{
	for (int i = 0; i < MAX_PLAYERS; ++i)
	{
		m_aiCloseBordersAttitudeCache[i] = INT_MAX;
	}
}
	...
	for (int iDX = -iRange; iDX <= iRange; iDX++)
	{
		for (int iDY = -iRange; iDY <= iRange; iDY++)
		{
			...
			if (pLoopPlot != nullptr)
			{
				if (pLoopPlot->area() == pPlotArea)
				{
					for (CLLNode<IDInfo>* pUnitNode = pLoopPlot->headUnitNode(); pUnitNode != nullptr; pUnitNode = pLoopPlot->nextUnitNode(pUnitNode))
					{
