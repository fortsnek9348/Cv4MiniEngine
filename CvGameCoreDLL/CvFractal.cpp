...
void CvFractal::fracInitHinted(int iNewXs, int iNewYs, int iGrain, CvRandom& random, const uint8_t* pbyHints, int iHintsLength, int iFlags, CvFractal* pRifts, int iFracXExp/*=7*/, int iFracYExp/*=6*/)
{
...
void CvFractal::fracInitInternal(int iNewXs, int iNewYs, int iGrain, CvRandom& random, const uint8_t* pbyHints, [[maybe_unused]] int iHintsLength, int iFlags, CvFractal* pRifts, int iFracXExp, int iFracYExp)
{
				...
				else  // Interpolate
				{
					...
					else
					{
						if ((iY << iPass) & iScreen)  // (vertical)
						{
							...
							iSum -= 1 << (7 - iSmooth + iPass);
							iSum = range (iSum, 0, 255);
							m_aaiFrac[iX << iPass][iY << iPass] = (uint8_t) iSum;
						}
						else
						{
