...
public:
	...
	// Called in parallel with other cities by player.
	void doTurnParallel();
#endif
	...
	DllExportForInterface bool canTrain(UnitTypes eUnit, bool bContinue = false, bool bTestVisible = false, bool bIgnoreCost = false, bool bIgnoreUpgrades = false) const;					// Exposed to Python 
	DllExportForInterface bool canTrain(UnitCombatTypes eUnitCombat) const;
	DllExportForInterface bool canConstruct(BuildingTypes eBuilding, bool bContinue = false, bool bTestVisible = false, bool bIgnoreCost = false) const;	// Exposed to Python  
	DllExportForInterface bool canCreate(ProjectTypes eProject, bool bContinue = false, bool bTestVisible = false) const;		// Exposed to Python 
	DllExportForInterface bool canMaintain(ProcessTypes eProcess, bool bContinue = false) const;														// Exposed to Python  
	bool canJoin() const;																													// Exposed to Python

	// fortsnek: Normally retrieved through CvGameTextMgr.
	DllExportForInterface int getFoodTurnsLeft() const;																				// Exposed to Python
	...
	// fortsnek: Normally retrieved through CvGameTextMgr.
	DllExportForInterface const wchar_t* getProductionName() const;															// Exposed to Python
	...
	// fortsnek: Normally retrieved through CvGameTextMgr.
	DllExportForInterface int getProductionTurnsLeft() const;																	// Exposed to Python 
	...
	DllExportForInterface CivilizationTypes getCivilizationType() const;								// Exposed to Python
	...
	DllExportForInterface bool isCapital() const;																				// Exposed to Python
	...
	DllExportForInterface int angryPopulation(int iExtra = 0) const;										// Exposed to Python
	...
	// fortsnek: Normally retrieved through CvGameTextMgr.
	DllExportForInterface int getPopulation() const;														// Exposed to Python
	...
	DllExportForInterface bool isGovernmentCenter() const;														// Exposed to Python
	...
	DllExportForInterface bool isPower() const;																									// Exposed to Python
	...
	DllExportForInterface int getOccupationTimer() const;															// Exposed to Python
	DllExportForInterface bool isOccupation() const;																	// Exposed to Python 
	...
	int totalTradeModifier(CvCity* pOtherCity = nullptr) const;																							// Exposed to Python
	...
	DllExportForInterface int getCommerceRateTimes100(CommerceTypes eIndex) const;									// Exposed to Python
	...
	DllExport const CvWString getName(unsigned int uiForm = 0) const;								// Exposed to Python
	...
	// fortsnek: City bonus tracking is entirely for bonuses that come from plot groups. So we should be able to easily avoid using `changeNumBonuses`, and just set directly.
	void setNumBonuses(BonusTypes eIndex, int iNewValue);
#endif
	...
	void startHeadOrder();
	void stopHeadOrder();
	DllExportForInterface int getOrderQueueLength();																		// Exposed to Python
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	virtual void AI_doTurnParallel() = 0;
#endif
	...
	virtual UnitTypes AI_bestUnit(bool bAsync = false, AdvisorTypes eIgnoreAdvisor = NO_ADVISOR, UnitAITypes* peBestUnitAI = nullptr) = 0;
	virtual UnitTypes AI_bestUnitAI(UnitAITypes eUnitAI, bool bAsync = false, AdvisorTypes eIgnoreAdvisor = NO_ADVISOR) = 0;
	virtual BuildingTypes AI_bestBuilding(int iFocusFlags = 0, int iMaxTurns = INT_MAX, bool bAsync = false, AdvisorTypes eIgnoreAdvisor = NO_ADVISOR) = 0;
...
protected:
	...
	virtual bool AI_addBestCitizen(bool bWorkers, bool bSpecialists, int* piBestPlot = nullptr, SpecialistTypes* peBestSpecialist = nullptr) = 0;
