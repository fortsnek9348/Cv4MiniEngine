...
void VoteSelectionData::write(FDataStreamBase* pStream)
{
	...
	pStream->Write((int)aVoteOptions.size());
...
void checkBattleUnitType([[maybe_unused]] BattleUnitTypes unitType)
{
...
{
	return static_cast<int>(m_aBattleRounds.size());
}
...
void CvBattleDefinition::checkBattleTimeType([[maybe_unused]] BattleTimeTypes timeType) const
{
	FAssertMsg((timeType >= 0) && (timeType < BATTLE_TIME_COUNT), "[Jason] Invalid battle time type.");
}

void CvBattleDefinition::checkBattleRound([[maybe_unused]] int index) const
{
...
{
	checkBattleUnitType(unitType);
	FAssertMsg(getUnit(unitType) != nullptr, "[Jason] Invalid battle unit type.");
...
CvPlotIndicatorData::CvPlotIndicatorData() : m_eVisibility(PLOT_INDICATOR_VISIBLE_ALWAYS), m_bFlashing(false), m_pUnit(nullptr), m_bTestEnemyVisibility(false), m_bVisibleOnlyIfSelected(false), m_bPersistentRotation(false)
{
}

CvGlobeLayerData::CvGlobeLayerData(GlobeLayerTypes eType) : m_eType(eType), m_bGlobeViewRequired(true), m_bShouldCitiesZoom(false), m_iNumOptions(0)
{
}

CvFlyoutMenuData::CvFlyoutMenuData(FlyoutTypes eType, int iId, int iX, int iY, const wchar_t* strTitle) : m_eFlyout(eType), m_iID(iId), m_iX(iX), m_iY(iY), m_strTitle(strTitle)
{
}