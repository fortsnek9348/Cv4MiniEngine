...
public:
	...
	struct InputValue
	{
		...
		XYCoords m_point;		// used for mouse coords
		InputValue(int iState=UNINIT, int px=-1, int py=-1) : m_iState(iState) {m_point.iX=px; m_point.iY=py;  }
		bool operator==(const InputValue& in) const		{ return in.m_iState==m_iState && m_point == in.m_point; }
	};

	// holds the input type and data
	struct InputEvent
	{
		unsigned int m_uiModifiers;		// CTRL/ALT/SHIFT
		InputValue m_val;		// state and location
		InputType m_type;		// event type
		InputEvent() : m_uiModifiers(0),m_type(NONE) {}
		InputEvent(InputType it, InputValue iv, unsigned int uiMod=0) : m_uiModifiers(uiMod), m_val(iv), m_type(it) {}
