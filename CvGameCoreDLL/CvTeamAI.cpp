...
CvTeamAI& CvTeamAI::getTeamNonInl(TeamTypes eTeam)
{
...
int CvTeamAI::AI_calculateAdjacentLandPlots(TeamTypes eTeam) const
{
	...
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::Fast_CvTeamAI_AI_calculateAdjacentLandPlots])
	{
		...
		//std::for_each_n(std::execution::par, std::views::iota(0).begin(), GC.getMapINLINE().numPlotsINLINE(), [&iCount, teamI = getID(), otherTeamI = eTeam](int iI) {
		//	CvPlot* pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);
		//
		//	if (!(pLoopPlot->isWater()))
		//	{
		//		if ((pLoopPlot->getTeam() == otherTeamI) && pLoopPlot->isAdjacentTeam(teamI, true))
		//		{
		...
		return gGlobals.enhancedDLLGameContext->getAdjacentLandPlotsCount(eTeam, getID());
	}
	else
#endif
	{
...
int CvTeamAI::AI_chooseElection(const VoteSelectionData& kVoteSelectionData) const
{
	...
	for (int iI = 0; iI < (int)kVoteSelectionData.aVoteOptions.size(); ++iI)
	{
		...
		[[maybe_unused]] CvVoteInfo& kVoteInfo = GC.getVoteInfo(eVote);
		...
		if (!GC.getGameINLINE().isTeamVote(eVote))
		{
			for (int iJ = 0; iJ < MAX_PLAYERS; iJ++)
			{
				if (GET_PLAYER((PlayerTypes)iJ).isAlive())
				{
					if (GET_PLAYER((PlayerTypes)iJ).getTeam() == getID())
					{
						PlayerVoteTypes eVote2 = GET_PLAYER((PlayerTypes)iJ).AI_diploVote(kVoteSelectionData.aVoteOptions[iI], eVoteSource, true);

						if (eVote2 != PLAYER_VOTE_YES || eVote2 == GC.getGameINLINE().getVoteOutcome((VoteTypes)iI))
						{
							bValid = false;
							break;
						}
	...
	if (GC.getGameINLINE().isOption(GAMEOPTION_AGGRESSIVE_AI))
	{
		...
		for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; iPlayer++)
		{
			...
			if (kPlayer.getTeam() == eTeam)
			{
				...
				for (pTheirLoopCity = kPlayer.firstCity(&iLoop); pTheirLoopCity != nullptr; pTheirLoopCity = kPlayer.nextCity(&iLoop))
				{
			...
			if (kPlayer.getTeam() == getID())
			{
				...
				for (pOurLoopCity = kPlayer.firstCity(&iLoop); pOurLoopCity != nullptr; pOurLoopCity = kPlayer.nextCity(&iLoop))
				{
...
int CvTeamAI::AI_techTradeVal(TechTypes eTech, [[maybe_unused]] TeamTypes eTeam) const
{
	FAssert(eTeam != getID());
	int iKnownCount;
...
int CvTeamAI::AI_surrenderTradeVal([[maybe_unused]] TeamTypes eTeam) const
{
	FAssertMsg(eTeam != getID(), "shouldn't call this function on ourselves");
...
{
	FAssert(eIndex >= 0);
	FAssert(std::to_underlying(eIndex) < MAX_TEAMS);
	...
	if (AI_getWarPlan(eIndex) != eNewValue)
	{
		if (bWar || !isAtWar(eIndex))
		{
			...
			if (const auto& ctx = gGlobals.enhancedDLLGameContext) // Not initialised yet during new game init
				gGlobals.enhancedDLLGameContext->onTeamChange(GiganticMapsOptimisationsLib::EGameTeamChangeEvent::AI_setWarPlan, getID(), eIndex);
#endif
		}
	}
}
...
void CvTeamAI::AI_doWar()
{
	...
	[[maybe_unused]] int iLowUnitSpendingPercent = 0;
...
{
	int iBestVictoryCountdown = INT_MAX;
	...
	if (INT_MAX == iBestVictoryCountdown)
	{
