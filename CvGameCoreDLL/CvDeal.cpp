...
void CvDeal::kill(bool bKillTeam)
{
	if ((getLengthFirstTrades() > 0) || (getLengthSecondTrades() > 0))
	{
		...
		if (GET_TEAM(GET_PLAYER(getFirstPlayer()).getTeam()).isHasMet(GET_PLAYER(getSecondPlayer()).getTeam()))
		{
			...
			szString.Format(L"%ls: %ls", szCancelString.GetCString(), szDealString.getCString());
...
void CvDeal::verify()
{
	...
	for (CLLNode<TradeData>* pNode = headFirstTradesNode(); (pNode != nullptr); pNode = nextFirstTradesNode(pNode))
	{
	...
	for (CLLNode<TradeData>* pNode = headSecondTradesNode(); (pNode != nullptr); pNode = nextSecondTradesNode(pNode))
	{
		if (pNode->m_data.m_eItemType == TRADE_RESOURCES)
		{
			// XXX embargoes?
...
bool CvDeal::isVassalTrade(const CLinkList<TradeData>* pList)
{
	if (pList)
	{
		for (CLLNode<TradeData>* pNode = pList->head(); pNode != nullptr; pNode = pList->next(pNode))
		{
...
int CvDeal::turnsToCancel([[maybe_unused]] PlayerTypes eByPlayer)
{
