...
CvMap::CvMap()
{
	...
	m_paiNumBonus = nullptr;
	m_paiNumBonusOnLand = nullptr;
...
void CvMap::init(CvMapInitData* pInitInfo/*=nullptr*/)
{
	...
	PROFILE("CvMap::init");
	gDLL->logMemState( CvString::format("CvMap::init begin - world size=%ls, climate=%ls, sealevel=%ls, num custom options=%i", 
...
void CvMap::reset(CvMapInitData* pInitInfo)
{
	...
	else
	{
		...
		m_iGridWidth *= GC.getInitCore().getWorldSizeMultiplier();
		m_iGridHeight *= GC.getInitCore().getWorldSizeMultiplier();
	}
	...
	else
	{
		...
		bool okX = gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "getTopLatitude", nullptr, &resultTop);
		bool overrideX = !gDLL->getPythonIFace()->pythonUsingDefaultImpl();
		bool okY = gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "getBottomLatitude", nullptr, &resultBottom);
	...
	else
	{
		...
		bool okX = gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "getWrapX", nullptr, &resultX);
		bool overrideX = !gDLL->getPythonIFace()->pythonUsingDefaultImpl();
		bool okY = gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "getWrapY", nullptr, &resultY);
		bool overrideY = !gDLL->getPythonIFace()->pythonUsingDefaultImpl();
	...
	if (GC.getNumBonusInfos())
	{
		...
		FAssertMsg(m_paiNumBonus==nullptr, "mem leak m_paiNumBonus");
		m_paiNumBonus = new int[GC.getNumBonusInfos()];
		FAssertMsg(m_paiNumBonusOnLand==nullptr, "mem leak m_paiNumBonusOnLand");
...
void CvMap::setup()
{
	...
	gDLL->getFAStarIFace()->Initialize(&GC.getPathFinder(), getGridWidthINLINE(), getGridHeightINLINE(), isWrapXINLINE(), isWrapYINLINE(), pathDestValid, pathHeuristic, pathCost, pathValid, pathAdd, nullptr, nullptr);
	gDLL->getFAStarIFace()->Initialize(&GC.getInterfacePathFinder(), getGridWidthINLINE(), getGridHeightINLINE(), isWrapXINLINE(), isWrapYINLINE(), pathDestValid, pathHeuristic, pathCost, pathValid, pathAdd, nullptr, nullptr);
	gDLL->getFAStarIFace()->Initialize(&GC.getStepFinder(), getGridWidthINLINE(), getGridHeightINLINE(), isWrapXINLINE(), isWrapYINLINE(), stepDestValid, stepHeuristic, stepCost, stepValid, stepAdd, nullptr, nullptr);
	gDLL->getFAStarIFace()->Initialize(&GC.getRouteFinder(), getGridWidthINLINE(), getGridHeightINLINE(), isWrapXINLINE(), isWrapYINLINE(), nullptr, nullptr, nullptr, routeValid, nullptr, nullptr, nullptr);
	gDLL->getFAStarIFace()->Initialize(&GC.getBorderFinder(), getGridWidthINLINE(), getGridHeightINLINE(), isWrapXINLINE(), isWrapYINLINE(), nullptr, nullptr, nullptr, borderValid, nullptr, nullptr, nullptr);
	gDLL->getFAStarIFace()->Initialize(&GC.getAreaFinder(), getGridWidthINLINE(), getGridHeightINLINE(), isWrapXINLINE(), isWrapYINLINE(), nullptr, nullptr, nullptr, areaValid, nullptr, joinArea, nullptr);
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (!gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::BlockwisePlotGroupSystem])
#endif
		gDLL->getFAStarIFace()->Initialize(&GC.getPlotGroupFinder(), getGridWidthINLINE(), getGridHeightINLINE(), isWrapXINLINE(), isWrapYINLINE(), nullptr, nullptr, nullptr, plotGroupValid, nullptr, countPlotGroup, nullptr);
}
...
void CvMap::combinePlotGroups([[maybe_unused]] PlayerTypes ePlayer, CvPlotGroup* pPlotGroup1, CvPlotGroup* pPlotGroup2)
{
	CvPlotGroup* pNewPlotGroup;
	CvPlotGroup* pOldPlotGroup;

	FAssertMsg(pPlotGroup1 != nullptr, "pPlotGroup is not assigned to a valid value");
	FAssertMsg(pPlotGroup2 != nullptr, "pPlotGroup is not assigned to a valid value");
...
CvPlot* CvMap::syncRandPlot(int iFlags, int iArea, int iMinUnitDistance, int iTimeout)
{
	...
	while (iCount < iTimeout)
	{
		...
		// Rand = 769093474 on 95 (Feature Growth)
		// Rand = 813070579 on 95 (Feature Growth)
		// Rand = -96618064 on 95 (Rand Plot Height)
		// Rand = -1459888087 on 95 (Rand Plot Width)
		// Rand = 2051088302 on 95 (Rand Plot Height)
		// Rand = -1812614833 on 95 (Rand Plot Width)
		// Rand = 230473948 on 95 (Rand Plot Height)
		// Rand = 1648973285 on 95 (Rand Plot Width)
		// Rand = -981235526 on 95 (Animal Unit Selection)
		const int randY = GC.getGameINLINE().getSorenRandNum(getGridHeightINLINE(), "Rand Plot Height");
		const int randX = GC.getGameINLINE().getSorenRandNum(getGridWidthINLINE(), "Rand Plot Width");
		pTestPlot = plotSorenINLINE(randX, randY);
...
CvSelectionGroup* CvMap::findSelectionGroup(int iX, int iY, PlayerTypes eOwner, bool bReadyToSelect, bool bWorkers)
{
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if ((eOwner == NO_PLAYER) || (iI == eOwner))
			{
				for(pLoopSelectionGroup = GET_PLAYER((PlayerTypes)iI).firstSelectionGroup(&iLoop); pLoopSelectionGroup != nullptr; pLoopSelectionGroup = GET_PLAYER((PlayerTypes)iI).nextSelectionGroup(&iLoop))
				{
...
CvArea* CvMap::findBiggestArea(bool bWater)
{
	...
	for(pLoopArea = firstArea(&iLoop); pLoopArea != nullptr; pLoopArea = nextArea(&iLoop))
	{
...
int CvMap::calculatePathDistance(CvPlot *pSource, CvPlot *pDest)
{
	...
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::Fast_CvMap_calculatePathDistance])
		...
		{
			const int dist = ctx->findPathStepDistance(*pSource, *pDest).value_or(-1);
			...
			//	if (pNode != nullptr)
			//	{
			//		expected = pNode->m_iData1;
			//	}
			//}
			//gDLL->getFAStarIFace()->disableVerificationFixes();
			//
			//if (dist != expected)
			//	std::abort();

			// Okay, because FAStar is buggy, findPathStepDistance returned different values from FAStar, so, Fast_CvMap_calculatePathDistance is no longer game state preserving.
...
void CvMap::read(FDataStreamBase* pStream)
{
	...
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::Fast_CvPlot_isCoastalLand])
		heck::parallelForEachN(numPlotsINLINE(), [this](int iI) { m_pMapPlots[iI].updateAfterMapRead(); });
#endif
}
...
//
void CvMap::rebuild(int iGridW, int iGridH, int iTopLatitude, int iBottomLatitude, bool bWrapX, bool bWrapY, WorldSizeTypes eWorldSize, ClimateTypes eClimate, SeaLevelTypes eSeaLevel, int iNumCustomMapOptions, const CustomMapOptionTypes * aeCustomMapOptions)
{
...
void CvMap::calculateAreas()
{
	...
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::Fast_CvPlot_isCoastalLand])
	{
		// m_bIsCoastalLand depends on areas
		for (iI = 0; iI < numPlotsINLINE(); iI++)
			plotByIndexINLINE(iI)->updateAfterMapRead();
	}
#endif
}
