...
const TCHAR* CvInfoBase::getButton() const
{
	...
	return m_szButton.c_str();
}

const wchar_t* CvInfoBase::getTextKeyWide() const
{
	return m_szTextKey.c_str();
}

const wchar_t* CvInfoBase::getDescription(unsigned int uiForm) const
{
	while(m_aCachedDescriptions.size() <= uiForm)
	{
		m_aCachedDescriptions.push_back(gDLL->getObjectText(m_szTextKey, static_cast<int>(m_aCachedDescriptions.size())));
	}
	
	return m_aCachedDescriptions[uiForm].c_str();
}
...
const wchar_t* CvInfoBase::getCivilopedia() const
{
	...
	return m_szCachedCivilopedia.c_str();
}
...
const wchar_t* CvInfoBase::getStrategy() const
{
	...
	return m_szCachedStrategy.c_str();
}
...
bool CvInfoBase::isMatchForLink(std::wstring szLink, bool bKeysOnly) const
{
	...
	if (!bKeysOnly)
	{
		unsigned int iNumForms = gDLL->getNumForms(getTextKeyWide());
		for (unsigned int i = 0; i < iNumForms; i++)
		{
...
bool CvInfoBase::readPass2([[maybe_unused]] CvXMLLoadUtility* pXML) { FAssertMsg(false, "Override this"); return false; }
bool CvInfoBase::readPass3() { FAssertMsg(false, "Override this"); return false; }

//======================================================================================================
//					CvScalableInfo
//======================================================================================================

CvScalableInfo::CvScalableInfo() : m_fScale(1.0f), m_fInterfaceScale(1.0f)
{
}
...
bool CvHotkeyInfo::read(CvXMLLoadUtility* pXML)
{
	...
	{
		setHotKey(szTextVal.c_str());
	}
	else
	{
		setHotKey("");
	}
	iVal = pXML->GetHotKeyInt(szTextVal.c_str());
	setHotKeyVal(iVal);
  if (pXML->GetChildXmlValByName(&iVal, "iHotKeyPriority"))
	...
	setHotKeyDescription(getTextKeyWide(), nullptr, pXML->CreateHotKeyFromDescription(getHotKey(), m_bShiftDown, m_bAltDown, m_bCtrlDown).c_str());
...
std::wstring CvHotkeyInfo::getHotKeyDescription() const
{
	...
	{
		szTemptext.Format(L"%ls (%ls)", gDLL->getObjectText(m_szHotKeyAltDescriptionKey, 0).GetCString(), gDLL->getObjectText(m_szHotKeyDescriptionKey, 0).GetCString());
	}
	else
	{
...
m_pbCivilizationTypes(nullptr), 
m_pbLeaderHeadTypes(nullptr), 
m_pbAttitudeTypes(nullptr), 
m_pbDiplomacyPowerTypes(nullptr), 
...
{
	return m_paszDiplomacyText[i].c_str();
}
...
m_iExperience(0),
m_bVisible(false), 
m_piYieldChange(nullptr),
m_piCommerceChange(nullptr),
m_piFlavorValue(nullptr)
{
}
...
m_bWaterWork(false),
m_bRiverTrade(false),
m_piDomainExtraMoves(nullptr), 
m_piFlavorValue(nullptr), 
m_piPrereqOrTechs(nullptr),
m_piPrereqAndTechs(nullptr), 
m_pbCommerceFlexible(nullptr), 
m_pbTerrainTrade(nullptr)
{
}
...
bool CvTechInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iAdvisorType = pXML->FindInInfoClass(szTextVal.c_str());
	...
	m_iEra = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "FirstFreeUnitClass");
	m_iFirstFreeUnitClass = pXML->FindInInfoClass(szTextVal.c_str());
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					...
					{
						m_piPrereqOrTechs[j] = GC.getInfoTypeForString(szTextVal.c_str());
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					...
					for (int j = 0; j < iNumSibs; ++j)
					{
						m_piPrereqAndTechs[j] = GC.getInfoTypeForString(szTextVal.c_str());
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
...
m_piTerrainAttackPercent(nullptr),
m_piTerrainDefensePercent(nullptr),
m_piFeatureAttackPercent(nullptr),
m_piFeatureDefensePercent(nullptr),
m_piUnitCombatModifierPercent(nullptr),
m_piDomainModifierPercent(nullptr),
m_pbTerrainDoubleMove(nullptr),
m_pbFeatureDoubleMove(nullptr),
m_pbUnitCombat(nullptr)
{
}
...
bool CvPromotionInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iLayerAnimationPath = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "TechPrereq");
	m_iTechPrereq = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "StateReligionPrereq");
	m_iStateReligionPrereq = pXML->FindInInfoClass(szTextVal.c_str());
...
bool CvPromotionInfo::readPass2(CvXMLLoadUtility* pXML)
{
	...
	m_iPrereqPromotion = GC.getInfoTypeForString(szTextVal.c_str());
	pXML->GetChildXmlValByName(szTextVal, "PromotionPrereqOr1");
	m_iPrereqOrPromotion1 = GC.getInfoTypeForString(szTextVal.c_str());
	pXML->GetChildXmlValByName(szTextVal, "PromotionPrereqOr2");
	m_iPrereqOrPromotion2 = GC.getInfoTypeForString(szTextVal.c_str());
...
bool CvMissionInfo::read(CvXMLLoadUtility* pXML)
{
	...
	{
		m_eEntityEvent = (EntityEventTypes)pXML->FindInInfoClass(szTmp.c_str());
	}
	else
	{
		m_eEntityEvent = ENTITY_EVENT_NONE;
...
bool CvCommandInfo::read(CvXMLLoadUtility* pXML)
{
	...
	{		
		setAutomate(GC.getTypesEnum(szTextVal.c_str()));
	}
...
bool CvAutomateInfo::read(CvXMLLoadUtility* pXML)
{
	...
	setCommand(pXML->FindInInfoClass(szTextVal.c_str()));
...
m_fUnitPadTime(0.0f),
m_piPrereqAndTechs(nullptr),
m_piPrereqOrBonuses(nullptr),
m_piProductionTraits(nullptr),
m_piFlavorValue(nullptr),
m_piTerrainAttackModifier(nullptr),
m_piTerrainDefenseModifier(nullptr),
m_piFeatureAttackModifier(nullptr),
m_piFeatureDefenseModifier(nullptr),
m_piUnitClassAttackModifier(nullptr),
m_piUnitClassDefenseModifier(nullptr),
m_piUnitCombatModifier(nullptr),
m_piUnitCombatCollateralImmune(nullptr),
m_piDomainModifier(nullptr),
m_piBonusProductionModifier(nullptr),
m_piUnitGroupRequired(nullptr),
m_piReligionSpreads(nullptr),
m_piCorporationSpreads(nullptr),
m_piTerrainPassableTech(nullptr),
m_piFeaturePassableTech(nullptr),
m_piFlankingStrikeUnitClass(nullptr),
m_pbUpgradeUnitClass(nullptr),
m_pbTargetUnitClass(nullptr),
m_pbTargetUnitCombat(nullptr),
m_pbDefenderUnitClass(nullptr),
m_pbDefenderUnitCombat(nullptr),
m_pbUnitAIType(nullptr),
m_pbNotUnitAIType(nullptr),
m_pbBuilds(nullptr),
m_pbGreatPeoples(nullptr),
m_pbBuildings(nullptr),
m_pbForceBuildings(nullptr),
m_pbTerrainNative(nullptr),
m_pbFeatureNative(nullptr),
m_pbTerrainImpassable(nullptr),
m_pbFeatureImpassable(nullptr),
m_pbFreePromotions(nullptr),
m_paszEarlyArtDefineTags(nullptr),
m_paszLateArtDefineTags(nullptr),
m_paszMiddleArtDefineTags(nullptr),
...
int CvUnitInfo::getUnitGroupRequired(int i) const		
{
	...
	FAssertMsg(i > -1, "Index out of bounds");
	return m_piUnitGroupRequired ? m_piUnitGroupRequired[i] : 0;
}
...
const TCHAR* CvUnitInfo::getEarlyArtDefineTag(int i, UnitArtStyleTypes eStyle) const
{
	...
	return (m_paszEarlyArtDefineTags) ? m_paszEarlyArtDefineTags[i].c_str() : nullptr;
}
...
const TCHAR* CvUnitInfo::getLateArtDefineTag(int i, UnitArtStyleTypes eStyle) const
{
	...
	return (m_paszLateArtDefineTags) ? m_paszLateArtDefineTags[i].c_str() : nullptr;
}
...
const TCHAR* CvUnitInfo::getMiddleArtDefineTag(int i, UnitArtStyleTypes eStyle) const
{
	...
	return (m_paszMiddleArtDefineTags) ? m_paszMiddleArtDefineTags[i].c_str() : nullptr;
}
...
const TCHAR* CvUnitInfo::getUnitNames(int i) const
{
	...
	FAssertMsg(i > -1, "Index out of bounds");
	return (m_paszUnitNames) ? m_paszUnitNames[i].c_str() : nullptr;
}

const TCHAR* CvUnitInfo::getFormationType() const
{
	return m_szFormationType.c_str();
}

const TCHAR* CvUnitInfo::getButton() const
{
	return m_szArtDefineButton.c_str();
}
...
bool CvUnitInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iUnitClassType = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "Special");
	m_iSpecialUnitType = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "Capture");
	m_iUnitCaptureClassType = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "Combat");
	m_iUnitCombatType = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "Domain");
	m_iDomainType = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "DefaultUnitAI");
	m_iDefaultUnitAIType = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "Invisible");
	m_iInvisibleType = pXML->FindInInfoClass(szTextVal.c_str());
	...
	{
		int iInvisibleType = pXML->FindInInfoClass(tokens[i].c_str());
	...
	{
		m_piTerrainPassableTech[i] = pszTemp[i].IsEmpty() ? NO_TECH : pXML->FindInInfoClass(pszTemp[i].c_str());
	}
	...
	{
		m_piFeaturePassableTech[i] = pszTemp[i].IsEmpty() ? NO_TECH : pXML->FindInInfoClass(pszTemp[i].c_str());
	}
	SAFE_DELETE_ARRAY(pszTemp);
	...
	m_iHolyCity = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "ReligionType");
	m_iReligionType = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "StateReligion");
	m_iStateReligion = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "PrereqReligion");
	m_iPrereqReligion = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "PrereqCorporation");
	m_iPrereqCorporation = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "PrereqBuilding");
	m_iPrereqBuilding = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "PrereqTech");
	m_iPrereqAndTech = pXML->FindInInfoClass(szTextVal.c_str());
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					...
					{
						m_piPrereqAndTechs[j] = pXML->FindInInfoClass(szTextVal.c_str());
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
	...
	m_iPrereqAndBonus = pXML->FindInInfoClass(szTextVal.c_str());
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					...
					for (j=0;j<iNumSibs;j++)
					{
						m_piPrereqOrBonuses[j] = pXML->FindInInfoClass(szTextVal.c_str());
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
	...
	m_iSpecialCargo = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "DomainCargo");
	m_iDomainCargo = pXML->FindInInfoClass(szTextVal.c_str());
	...
	if ( gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(),"UnitMeshGroups") )
	{
		...
		if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "UnitMeshGroup"))
		{
			for ( k = 0; k < iIndexVal; k++ )
			{
				...
				setEarlyArtDefineTag(k, szTextVal.c_str());
				pXML->GetChildXmlValByName(szTextVal, "LateArtDefineTag");
				setLateArtDefineTag(k, szTextVal.c_str());
				pXML->GetChildXmlValByName(szTextVal, "MiddleArtDefineTag");
				setMiddleArtDefineTag(k, szTextVal.c_str());
	...
	m_iLeaderPromotion = pXML->FindInInfoClass(szTextVal.c_str());
...
{
	return static_cast<int>(m_vctUnitEntries.size());
}
...
{
	return static_cast<int>(m_vctGreatUnitEntries.size());
}
...
{
	return static_cast<int>(m_vctSiegeUnitEntries.size());
}
...
bool CvUnitFormationInfo::read(CvXMLLoadUtility* pXML)
{
	...
	if ( gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "EventMaskList" ))
	{
		if ( gDLL->getXMLIFace()->SetToChild( pXML->GetXML() ) )
		{
			...
			do 
			{
				iIndex = pXML->FindInInfoClass(szTextVal.c_str());
				if ( iIndex != -1 )
...
m_bValid(false),
m_bCityLoad(false),
m_pbCarrierUnitAITypes(nullptr),
m_piProductionTraits(nullptr)
{
}
...
m_piYieldModifier(nullptr),
m_piCapitalYieldModifier(nullptr),
m_piTradeYieldModifier(nullptr),
m_piCommerceModifier(nullptr),
m_piCapitalCommerceModifier(nullptr),
m_piSpecialistExtraCommerce(nullptr),
m_paiBuildingHappinessChanges(nullptr),
m_paiBuildingHealthChanges(nullptr),
m_paiFeatureHappinessChanges(nullptr),
m_pabHurry(nullptr),
m_pabSpecialBuildingNotRequired(nullptr),
m_pabSpecialistValid(nullptr),
m_ppiImprovementYieldChanges(nullptr)
{
}
...
CvCivicInfo::~CvCivicInfo()
{
	...
	if (m_ppiImprovementYieldChanges != nullptr)
	{
...
const wchar_t* CvCivicInfo::getWeLoveTheKing()		
{
	return m_szWeLoveTheKingKey.c_str();
}
...
bool CvCivicInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iCivicOptionType = pXML->FindInInfoClass(szTextVal.c_str());
	...
	m_iUpkeep = pXML->FindInInfoClass(szTextVal.c_str());
	...
	setWeLoveTheKingKey(szTextVal.c_str());
...
{
	return static_cast<int>(m_pResponses.size());
}
...
void CvDiplomacyInfo::write(FDataStreamBase* stream)
{
	...
	int iNumResponses = static_cast<int>(m_pResponses.size());
...
bool CvUnitClassInfo::readPass3()
{
	...
	m_iDefaultUnitIndex = GC.getInfoTypeForString(m_aszExtraXMLforPass3[0].c_str());
...
m_piPrereqOrBonuses(nullptr),
m_piProductionTraits(nullptr),
m_piHappinessTraits(nullptr),
m_piSeaPlotYieldChange(nullptr),
m_piRiverPlotYieldChange(nullptr),
m_piGlobalSeaPlotYieldChange(nullptr),
m_piYieldChange(nullptr),
m_piYieldModifier(nullptr),
m_piPowerYieldModifier(nullptr),
m_piAreaYieldModifier(nullptr),
m_piGlobalYieldModifier(nullptr),
m_piCommerceChange(nullptr),
m_piObsoleteSafeCommerceChange(nullptr),
m_piCommerceChangeDoubleTime(nullptr),
m_piCommerceModifier(nullptr),
m_piGlobalCommerceModifier(nullptr),
m_piSpecialistExtraCommerce(nullptr),
m_piStateReligionCommerce(nullptr),
m_piCommerceHappiness(nullptr),
m_piReligionChange(nullptr),
m_piSpecialistCount(nullptr),
m_piFreeSpecialistCount(nullptr),
m_piBonusHealthChanges(nullptr),
m_piBonusHappinessChanges(nullptr),
m_piBonusProductionModifier(nullptr),
m_piUnitCombatFreeExperience(nullptr),
m_piDomainFreeExperience(nullptr),
m_piDomainProductionModifier(nullptr),
m_piBuildingHappinessChanges(nullptr),
m_piPrereqNumOfBuildingClass(nullptr),
m_piFlavorValue(nullptr),
m_piImprovementFreeSpecialist(nullptr),
m_pbCommerceFlexible(nullptr),
m_pbCommerceChangeOriginalOwner(nullptr),
m_pbBuildingClassNeededInCity(nullptr),
m_ppaiSpecialistYieldChange(nullptr),
m_ppaiBonusYieldModifier(nullptr)
{
}
...
CvBuildingInfo::~CvBuildingInfo()
{
	...
	if (m_ppaiSpecialistYieldChange != nullptr)
	{
	...
	if (m_ppaiBonusYieldModifier != nullptr)
	{
...
{
	return m_szConstructSound.c_str();
}
...
{
	return m_szMovieDefineTag.c_str();
}
...
#ifdef _WIN32
	if (nullptr != pcTag && 0 != _tcscmp(pcTag, "NONE"))
#else
	if (nullptr != pcTag && 0 != strcmp(pcTag, "NONE"))
#endif
	{
...
bool CvBuildingInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iBuildingClassType = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "SpecialBuildingType");
	m_iSpecialBuildingType = pXML->FindInInfoClass(szTextVal.c_str());
	...
	setMovieDefineTag(szTextVal.c_str());
	...
	m_iFoundsCorporation = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "GlobalReligionCommerce");
	m_iGlobalReligionCommerce = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "GlobalCorporationCommerce");
	m_iGlobalCorporationCommerce = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "VictoryPrereq");
	m_iVictoryPrereq = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "FreeStartEra");
	m_iFreeStartEra = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "MaxStartEra");
	m_iMaxStartEra = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "ObsoleteTech");
	m_iObsoleteTech = pXML->FindInInfoClass(szTextVal.c_str());
	...
	m_iNoBonus = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "PowerBonus");
	m_iPowerBonus = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "FreeBonus");
	m_iFreeBonus = pXML->FindInInfoClass(szTextVal.c_str());
	pXML->GetChildXmlValByName(&m_iNumFreeBonuses, "iNumFreeBonuses");

	pXML->GetChildXmlValByName(szTextVal, "FreeBuilding");
	m_iFreeBuildingClass = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "FreePromotion");
	m_iFreePromotion = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "CivicOption");
	m_iCivicOption = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "GreatPeopleUnitClass");
	m_iGreatPeopleUnitClass = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "DiploVoteType");
	m_iVoteSourceType = pXML->FindInInfoClass(szTextVal.c_str());
	...
	setConstructSound(szTextVal.c_str());
	...
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(),"SpecialistYieldChanges"))
	{
		...
		if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(),"SpecialistYieldChange"))
		{
			for(j=0;j<iNumChildren;j++)
			{
				...
				k = pXML->FindInInfoClass(szTextVal.c_str());
				if (k > -1)
				{
					// delete the array since it will be reallocated
...
bool CvSpecialBuildingInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iTechPrereqAnyone = pXML->FindInInfoClass(szTextVal.c_str());
...
bool CvBuildingClassInfo::readPass3()
{
	...
	m_iDefaultBuildingIndex = GC.getInfoTypeForString(m_aszExtraXMLforPass3[0].c_str());
	m_aszExtraXMLforPass3.clear();
...
bool CvRiverModelInfo::read(CvXMLLoadUtility* pXML)
{
	...
	{
		setModelFile(szTextVal.c_str());
	}
	if(pXML->GetChildXmlValByName(szTextVal, "BorderFile"))
	{
		setBorderFile(szTextVal.c_str());
	}
...
{
	return m_szLateModelFile.c_str();
}
...
m_bAIPlayable(false),
m_bPlayable(false),
m_piCivilizationBuildings(nullptr),
m_piCivilizationUnits(nullptr),
m_piCivilizationFreeUnitsClass(nullptr),
m_piCivilizationInitialCivics(nullptr),
m_pbLeaders(nullptr),
m_pbCivilizationFreeBuildingClass(nullptr),
m_pbCivilizationFreeTechs(nullptr),
m_pbCivilizationDisableTechs(nullptr),
...
const wchar_t* CvCivilizationInfo::getShortDescription(unsigned int uiForm)
{
	while(m_aszShortDescription.size() <= uiForm)
	{
		m_aszShortDescription.push_back(gDLL->getObjectText(m_szShortDescriptionKey, static_cast<unsigned int>(m_aszShortDescription.size())));
	}

	return m_aszShortDescription[uiForm].c_str();
}

const wchar_t* CvCivilizationInfo::getShortDescriptionKey() const
{
	return m_szShortDescriptionKey.c_str();
}

const wchar_t* CvCivilizationInfo::getAdjective(unsigned int uiForm)		
{
	while(m_aszAdjective.size() <= uiForm)
	{
		m_aszAdjective.push_back(gDLL->getObjectText(m_szAdjectiveKey, static_cast<unsigned int>(m_aszAdjective.size())));
	}

	return m_aszAdjective[uiForm].c_str();
}

const wchar_t* CvCivilizationInfo::getAdjectiveKey() const
{
	return m_szAdjectiveKey.c_str();
}
...
bool CvCivilizationInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iDefaultPlayerColor = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "ArtDefineTag");
	setArtDefineTag(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "ArtStyleType");
	m_iArtStyleType = GC.getTypesEnum(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "UnitArtStyleType");
	m_iUnitArtStyleType = pXML->FindInInfoClass(szTextVal.c_str());
				...
				for (j=0;j<iNumSibs;j++)
				{
					if (pXML->GetChildXmlVal(szClassVal))
					{
						...
						if (-1 < iBuildingClassIndex)
						{
							...
							// call the find in list function to return either -1 if no value is found
							// or the index in the list the match is found at
							m_piCivilizationBuildings[iBuildingClassIndex] = pXML->FindInInfoClass(szTextVal.c_str());
						}
						else
						{
				...
				for (j=0;j<iNumSibs;j++)
				{
					if (pXML->GetChildXmlVal(szClassVal))
					{
						...
						if (-1 < iUnitClassIndex)
						{
							...
							// call the find in list function to return either -1 if no value is found
							// or the index in the list the match is found at
							m_piCivilizationUnits[iUnitClassIndex] = pXML->FindInInfoClass(szTextVal.c_str());
						}
						else
						{
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					...
					for (j=0;j<iNumSibs;j++)
					{
						m_piCivilizationInitialCivics[j] = pXML->FindInInfoClass(szTextVal.c_str());
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
...
bool CvCivilizationInfo::readPass2(CvXMLLoadUtility* pXML)
{
	...
	m_iDerivativeCiv = GC.getInfoTypeForString(szTextVal.c_str());
...
bool CvVictoryInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iCityCulture = pXML->FindInInfoClass(szTextVal.c_str());
...
m_iNumGoodies(0),
m_piGoodies(nullptr),
m_pbFreeTechs(nullptr),
m_pbAIFreeTechs(nullptr)
{
}
...
bool CvHandicapInfo::read(CvXMLLoadUtility* pXML)
{
	...
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "Goodies"))
	{
		...
		if (m_iNumGoodies > 0)
		{
			...
			{
				m_piGoodies[j] = pXML->FindInInfoClass(pszGoodyNames[j].c_str());
			}
		}
		else
		{
...
m_iMissionType(NO_MISSION),
m_bKill(false),
m_paiFeatureTech(nullptr),
m_paiFeatureTime(nullptr),
m_paiFeatureProduction(nullptr),
...
bool CvBuildInfo::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(szTextVal, "ImprovementType");
	m_iImprovement = pXML->FindInInfoClass(szTextVal.c_str());
	pXML->GetChildXmlValByName(szTextVal, "RouteType");
	m_iRoute = pXML->FindInInfoClass(szTextVal.c_str());
	pXML->GetChildXmlValByName(szTextVal, "EntityEvent");
	m_iEntityEvent = pXML->FindInInfoClass(szTextVal.c_str());
...
bool CvGoodyInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iBarbarianUnitClass = pXML->FindInInfoClass(szTextVal.c_str());
...
m_piYieldChange(nullptr),
m_piTechMovementChange(nullptr),
m_piPrereqOrBonuses(nullptr)
{
}
...
bool CvRouteInfo::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(szTextVal, "BonusType");
	m_iPrereqBonus = pXML->FindInInfoClass(szTextVal.c_str());
...
m_piPrereqNatureYield(nullptr),
m_piYieldChange(nullptr),
m_piRiverSideYieldChange(nullptr),
m_piHillsYieldChange(nullptr),
m_piIrrigatedChange(nullptr),
m_pbTerrainMakesValid(nullptr),
m_pbFeatureMakesValid(nullptr),
m_ppiTechYieldChanges(nullptr),
m_ppiRouteYieldChanges(nullptr),
...
CvImprovementInfo::~CvImprovementInfo()
{
	...
	if (m_ppiRouteYieldChanges != nullptr)
	{
...
bool CvImprovementInfo::readPass2(CvXMLLoadUtility* pXML)
{
	...
	m_iImprovementPillage = GC.getInfoTypeForString(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "ImprovementUpgrade");
	m_iImprovementUpgrade = GC.getInfoTypeForString(szTextVal.c_str());
...
m_bNormalize(false),
m_piYieldChange(nullptr),
m_piImprovementChange(nullptr),
m_pbTerrain(nullptr),
m_pbFeature(nullptr),
m_pbFeatureTerrain(nullptr)
{
}
...
bool CvBonusInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iBonusClassType = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName( szTextVal, "ArtDefineTag");
	setArtDefineTag(szTextVal.c_str());

	pXML->GetChildXmlValByName( szTextVal, "TechReveal");
	m_iTechReveal = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName( szTextVal, "TechCityTrade");
	m_iTechCityTrade = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName( szTextVal, "TechObsolete");
	m_iTechObsolete = pXML->FindInInfoClass(szTextVal.c_str());
...
m_piYieldChange(nullptr),
m_piRiverYieldChange(nullptr),
m_piHillsYieldChange(nullptr),
m_pi3DAudioScriptFootstepIndex(nullptr),
m_pbTerrain(nullptr)
{
}
...
{
	return m_szOnUnitChangeTo.c_str();
}
...
const TCHAR* CvYieldInfo::getSymbolPath(int i) const
{
	...
	FAssertMsg(i > -1, "Index out of bounds");
	return m_paszSymbolPath ? m_paszSymbolPath[i].c_str() : nullptr; 
}
...
bool CvYieldInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iColorType = pXML->FindInInfoClass(szTextVal.c_str());
...
bool CvInterfaceModeInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iCursorIndex = pXML->FindInInfoClass( szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "Mission");
	m_iMissionType = pXML->FindInInfoClass(szTextVal.c_str());
...
{
	return static_cast<int>(m_vctEnableDisableCodes.size());
}

int CvAdvisorInfo::getEnableCode(unsigned int uiCode) const
{
	FAssert( uiCode < m_vctEnableDisableCodes.size() );
	return m_vctEnableDisableCodes[uiCode].first;
}

int CvAdvisorInfo::getDisableCode(unsigned int uiCode) const
{
	FAssert( uiCode < m_vctEnableDisableCodes.size() );
...
m_pbTraits(nullptr),
m_piFlavorValue(nullptr),
m_piContactRand(nullptr),
m_piContactDelay(nullptr),
m_piMemoryDecayRand(nullptr),
m_piMemoryAttitudePercent(nullptr),
m_piNoWarAttitudeProb(nullptr),
m_piUnitAIWeightModifier(nullptr),
m_piImprovementWeightModifier(nullptr),
m_piDiploPeaceIntroMusicScriptIds(nullptr),
m_piDiploPeaceMusicScriptIds(nullptr),
m_piDiploWarIntroMusicScriptIds(nullptr),
m_piDiploWarMusicScriptIds(nullptr)
{
}
...
void CvLeaderHeadInfo::setArtDefineTag(std::string_view szVal)
{
	m_szArtDefineTag = szVal;
}
...
bool CvLeaderHeadInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iDemandTributeAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "NoGiveHelpAttitudeThreshold");
	m_iNoGiveHelpAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "TechRefuseAttitudeThreshold");
	m_iTechRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "StrategicBonusRefuseAttitudeThreshold");
	m_iStrategicBonusRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "HappinessBonusRefuseAttitudeThreshold");
	m_iHappinessBonusRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "HealthBonusRefuseAttitudeThreshold");
	m_iHealthBonusRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "MapRefuseAttitudeThreshold");
	m_iMapRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "DeclareWarRefuseAttitudeThreshold");
	m_iDeclareWarRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "DeclareWarThemRefuseAttitudeThreshold");
	m_iDeclareWarThemRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "StopTradingRefuseAttitudeThreshold");
	m_iStopTradingRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "StopTradingThemRefuseAttitudeThreshold");
	m_iStopTradingThemRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "AdoptCivicRefuseAttitudeThreshold");
	m_iAdoptCivicRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "ConvertReligionRefuseAttitudeThreshold");
	m_iConvertReligionRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "OpenBordersRefuseAttitudeThreshold");
	m_iOpenBordersRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "DefensivePactRefuseAttitudeThreshold");
	m_iDefensivePactRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "PermanentAllianceRefuseAttitudeThreshold");
	m_iPermanentAllianceRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "VassalRefuseAttitudeThreshold");
	m_iVassalRefuseAttitudeThreshold = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "FavoriteCivic");
	m_iFavoriteCivic = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "FavoriteReligion");
	m_iFavoriteReligion = pXML->FindInInfoClass(szTextVal.c_str());
...
m_iTechPrereq(NO_TECH),
m_paiProductionToCommerceModifier(nullptr)
{
}
...
m_bForceWar(false),
m_bAssignCity(false),
m_pbForceCivic(nullptr),
m_abVoteSourceTypes(nullptr)
{
}
...
m_piBonusProductionModifier(nullptr),
m_piVictoryThreshold(nullptr),
m_piVictoryMinThreshold(nullptr),
m_piProjectsNeeded(nullptr)
{
}
...
{
	return m_szMovieArtDef.c_str(); 
}

const TCHAR* CvProjectInfo::getCreateSound() const	
{
	return m_szCreateSound.c_str();
}
...
bool CvProjectInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iEveryoneSpecialUnit = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "EveryoneSpecialBuilding");
	m_iEveryoneSpecialBuilding = pXML->FindInInfoClass(szTextVal.c_str());
	...
	setCreateSound(szTextVal.c_str());
...
m_iSpreadFactor(0),
m_iMissionType(NO_MISSION),
m_paiGlobalReligionCommerce(nullptr),
m_paiHolyCityCommerce(nullptr),
m_paiStateReligionCommerce(nullptr)
{
	reset();
}
...
{
	return m_szGenericTechButton.c_str();
}
...
bool CvReligionInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iFreeUnitClass = pXML->FindInInfoClass(szTextVal.c_str());
...
m_paiPrereqBonuses(nullptr),
m_paiHeadquarterCommerce(nullptr),
m_paiCommerceProduced(nullptr),
m_paiYieldProduced(nullptr)
{
	reset();
}
...
bool CvCorporationInfo::read(CvXMLLoadUtility* pXML)
{
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					...
					for (int j=0; j<iNumSibs; ++j)
					{
						m_paiPrereqBonuses[j] = pXML->FindInInfoClass(szTextVal.c_str());
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
	...
	m_iBonusProduced = pXML->FindInInfoClass(szTextVal.c_str());
...
m_paiExtraYieldThreshold(nullptr),
m_paiTradeYieldModifier(nullptr),
m_paiCommerceChange(nullptr),
m_paiCommerceModifier(nullptr),
m_pabFreePromotion(nullptr),
m_pabFreePromotionUnitCombat(nullptr)
{
}
...
{
	return m_szShortDescription.c_str();
}
...
bool CvTraitInfo::read(CvXMLLoadUtility* pXML)
{
	...
	setShortDescription(szTextVal.c_str());
...
bool CvThroneRoomStyleInfo::read(CvXMLLoadUtility* pXML)
{
	...
	pXML->GetChildXmlValByName(szTextVal, "ArtStyleType");
	setArtStyleType(szTextVal.c_str());
	pXML->GetChildXmlValByName(szTextVal, "EraType");
	setEraType(szTextVal.c_str());
	pXML->GetChildXmlValByName(szTextVal, "FileName");
	setFileName(szTextVal.c_str());
...
{
	return m_szTransitionType.c_str();
}
...
bool CvSlideShowInfo::read(CvXMLLoadUtility* pXML)
{
	...
	setTransitionType(szTextVal.c_str());
...
{
	return static_cast<int>(m_aSizes.size());
}
...
{
	return m_szProjectName.c_str();
}

void CvSpaceShipInfo::setProjectName(const TCHAR* szVal)
{
	m_szProjectName = szVal;
	m_eProjectType = (ProjectTypes) GC.getInfoTypeForString(m_szProjectName.c_str(), true);
}
...
bool CvSpaceShipInfo::read(CvXMLLoadUtility* pXML)
{
	...
	setProjectName(szTextVal.c_str());
...
{
	return static_cast<int>(m_vctPathDefinition.size());
}
...
int CvAnimationCategoryInfo::getCategoryDefaultTo( )	
{
	if ( m_kCategory.second < -1 )
	{
		...
		m_kCategory.second = CvXMLLoadUtility::FindInInfoClass( m_szDefaultTo.c_str());
	}
	...
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(),"AnimationPathTypes"))
	{
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (iNumSibs > 0)
			{
				...
				{
					AnimationPathTypes eAnimationPath = (AnimationPathTypes)CvXMLLoadUtility::FindInInfoClass( szTmp.c_str());
					...
					for (i=1;i<iNumSibs;i++)
					{
						...
						AnimationPathTypes eAnimationPath2 = (AnimationPathTypes)CvXMLLoadUtility::FindInInfoClass( szTmp.c_str());
						if (eAnimationPath2 > ANIMATIONPATH_NONE )
							m_vctAnimationPathType.push_back(eAnimationPath2);
	...
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(),"EffectTypes"))
	{
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (iNumSibs > 0)
			{
				...
				if (pXML->GetChildXmlVal(szTmp))
				{
					EffectTypes eEffectType = (EffectTypes)CvXMLLoadUtility::FindInInfoClass( szTmp.c_str());
					...
					for (i=1;i<iNumSibs;i++)
					{
						...
						EffectTypes eEffectType2 = (EffectTypes)CvXMLLoadUtility::FindInInfoClass( szTmp.c_str());
						if ( eEffectType2 > NO_EFFECT )
							m_vctEffectTypes.push_back( eEffectType2 );
					}
					gDLL->getXMLIFace()->SetToParent(pXML->GetXML());
				}
			}
...
{
	return static_cast<int>(m_vctAnimationPathType.size());
}

int CvEntityEventInfo::getEffectTypeCount() const
{
	return static_cast<int>(m_vctEffectTypes.size());
}
...
CvAssetInfoBase::CvAssetInfoBase()
{
}
CvAssetInfoBase::~CvAssetInfoBase()
{
...
{
	return m_szNIF.c_str();
}

const TCHAR* CvArtInfoAsset::getKFM() const
{
	return m_szKFM.c_str();
}
...
bool CvArtInfoAsset::read(CvXMLLoadUtility* pXML)
{
	...
	setNIF(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "KFM");
	setKFM(szTextVal.c_str());
...
CvArtInfoInterface::CvArtInfoInterface()
{
}
CvArtInfoInterface::~CvArtInfoInterface()
{
...
CvArtInfoLeaderhead::CvArtInfoLeaderhead()
{
}
CvArtInfoLeaderhead::~CvArtInfoLeaderhead()
{
...
{
	return m_szBackgroundKFM.c_str();
}
...
bool CvArtInfoLeaderhead::read(CvXMLLoadUtility* pXML)
{
	...
	setNoShaderNIF(szTextVal.c_str());

	if (pXML->GetChildXmlValByName(szTextVal, "BackgroundKFM"))
		setBackgroundKFM(szTextVal.c_str());
	else
		setBackgroundKFM("");
...
{
	return m_szDetailTexture.c_str();
}
...
void BuildSlotList( CvTextureBlendSlotList &list, CvString &numlist)
{
	...
	char seps[]   = " ,\t\n";
	char *token;
	const char *numstring = numlist.c_str();
...
bool CvArtInfoTerrain::read(CvXMLLoadUtility* pXML)
{
	...
	setGridTexture(szTextVal.c_str());
	pXML->GetChildXmlValByName(szTextVal, "Detail");
	setDetailTexture(szTextVal.c_str());
	...
	for(int i =1; i<m_numTextureBlends;i++ )
	{
		...
		//OutputDebugStringA((std::ostringstream() << i << '\n').str().c_str());
...
{
	return static_cast<int>(m_aFeatureVarieties.size());
}
...
int CvArtInfoFeature::getRotatedConnectionMask(int connectionMask, RotationTypes rotation)
{
	if (rotation == ROTATE_NONE)
		; // connectionMask = connectionMask; fortsnek: explicitly assigning value of variable of type 'int' to itself
...
m_piYieldModifiers(nullptr),
m_piCommerceModifiers(nullptr)
{
}
...
{
	return m_szAudioUnitVictoryScript.c_str();
}

const TCHAR* CvEraInfo::getAudioUnitDefeatScript() const
{
	return m_szAudioUnitDefeatScript.c_str();
}
...
bool CvEraInfo::read(CvXMLLoadUtility* pXML)
{
	...
	if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(), "EraInfoSoundtracks"))
	{
		...
		if (m_iNumSoundtracks > 0)
		{
			...
			{
				m_paiSoundtracks[j] = ((!gDLL->getAudioDisabled()) ? gDLL->getAudioTagIndex(pszSoundTrackNames[j].c_str(), AUDIOTAG_2DSCRIPT) : -1);
			}
		}
		else
		{
...
bool CvPlayerColorInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iColorTypePrimary = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "ColorTypeSecondary");
	m_iColorTypeSecondary = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "TextColorType");
	m_iTextColorType = pXML->FindInInfoClass(szTextVal.c_str());
...
bool CvGameText::read(CvXMLLoadUtility* pXML)
{
	...
	for (j = 0; j < iNumLanguages; j++)
	{
		...
		if (j == GAMETEXT.getCurrentLanguage()) // Only add appropriate language Text 
		{
			...
			{
				setGender(wszTextVal.c_str());
			}

			// PLURAL
			if (pXML->GetChildXmlValByName(wszTextVal, "Plural"))
			{
				setPlural(wszTextVal.c_str());
			}
			if (NUM_LANGUAGES > 0)
			{
				break;
...
const CvDiplomacyTextInfo::Response& CvDiplomacyTextInfo::getResponse(int iNum) const
{
	return m_pResponses[iNum];
}
...
const TCHAR* CvDiplomacyTextInfo::getDiplomacyText(int i, int j) const
{
	...
	FAssertMsg(j < getNumDiplomacyText(i), "Index out of bounds");
	FAssertMsg(j > -1, "Index out of bounds");
	return m_pResponses[i].m_paszDiplomacyText[j].c_str();
}

CvDiplomacyTextInfo::Response::Response() :
	m_iNumDiplomacyText(0),
	m_pbCivilizationTypes(nullptr),
	m_pbLeaderHeadTypes(nullptr),
	m_pbAttitudeTypes(nullptr),
...
void CvDiplomacyTextInfo::read(FDataStreamBase* stream)
{
	...
	for (unsigned int uiIndex = 0; (int) uiIndex < m_iNumResponses; uiIndex++)
	{
...
const TCHAR* CvEffectInfo::getPath() const { return m_szPath.c_str(); }
void CvEffectInfo::setPath(const TCHAR* szVal) { m_szPath = szVal; }
float CvEffectInfo::getUpdateRate() const { return m_fUpdateRate; };
void CvEffectInfo::setUpdateRate(float fUpdateRate) { m_fUpdateRate = fUpdateRate; }
bool CvEffectInfo::isProjectile() const { return m_bProjectile; };
float CvEffectInfo::getProjectileSpeed() const { return m_fProjectileSpeed; };
float CvEffectInfo::getProjectileArc() const { return m_fProjectileArc; };
bool CvEffectInfo::isSticky() const { return m_bSticky; };
...
const TCHAR* CvAttachableInfo::getPath() const
{
	return m_szPath.c_str();
}
void CvAttachableInfo::setPath(const TCHAR* szVal)
{
	m_szPath = szVal;
}
...
const TCHAR* CvCameraInfo::getPath() const
{
	return m_szPath.c_str();
}
void CvCameraInfo::setPath(const TCHAR* szVal)
{
	m_szPath = szVal;
}
...
CvQuestInfo::QuestLink::QuestLink() :
...
m_iNumQuestLinks(0),
m_iNumQuestSounds(0),
m_paszQuestMessages(nullptr),
m_pQuestLinks(nullptr),
m_paszQuestSounds(nullptr)
{
...
{
	return m_szQuestObjective.c_str();
}

const TCHAR* CvQuestInfo::getQuestBodyText() const 
{ 
	return m_szQuestBodyText.c_str();
}
...
{ 
	return m_paszQuestMessages ? m_paszQuestMessages[iIndex].c_str() : "";
}
...
{
	return m_pQuestLinks[iIndex].m_szQuestLinkType.c_str();
}

const TCHAR* CvQuestInfo::getQuestLinkName(int iIndex)  const
{ 
	return m_pQuestLinks[iIndex].m_szQuestLinkName.c_str();
}
...
{
	return m_paszQuestSounds ? m_paszQuestSounds[iIndex].c_str() : "";
}

const TCHAR* CvQuestInfo::getQuestScript() const
{
	return m_szQuestScript.c_str();
}
...
bool CvQuestInfo::read(CvXMLLoadUtility* pXML)
{
	...
	setQuestObjective(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "QuestBodyText");
	setQuestBodyText(szTextVal.c_str());
	...
	setQuestScript(szTextVal.c_str());
...
{
	return m_szTutorialMessageText.c_str();
}

const TCHAR* CvTutorialMessage::getImage() const
{
	return m_szTutorialMessageImage.c_str();
}

const TCHAR* CvTutorialMessage::getSound() const
{
	return m_szTutorialMessageSound.c_str();
}
...
{
	return m_paszTutorialScripts[i].c_str();
}
...
{
	return m_szNextTutorialInfoType.c_str();
}
...
bool CvTutorialInfo::read(CvXMLLoadUtility* pXML)
{
	...
	{
		setNextTutorialInfoType(szTextVal.c_str());
	}
...
int CvEventTriggerInfo::getNumTexts() const
{
	...
	return static_cast<int>(m_aszText.size());
}
...
{
	return static_cast<int>(m_aszWorldNews.size());
}
...
void CvEventTriggerInfo::write(FDataStreamBase* stream)
{
	...
	stream->Write((int)m_aszText.size());
	...
	stream->Write((int)m_aszWorldNews.size());
...
bool CvEventTriggerInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iMinDifficulty = pXML->FindInInfoClass(szTextVal.c_str());
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j = 0; j < iNumSibs; j++)
					{
						m_aiUnitsRequired.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j = 0; j < iNumSibs; j++)
					{
						m_aiBuildingsRequired.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j = 0; j < iNumSibs; j++)
					{
						m_aiPrereqOrTechs.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j = 0; j < iNumSibs; j++)
					{
						m_aiPrereqAndTechs.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
	...
	m_iOtherPlayerHasTech = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "Civic");
	m_iCivic = pXML->FindInInfoClass(szTextVal.c_str());
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j = 0; j < iNumSibs; j++)
					{
						m_aiObsoleteTechs.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j = 0; j < iNumSibs; j++)
					{
						m_aiEvents.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j=0;j<iNumSibs;j++)
					{
						m_aiPrereqEvents.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j = 0; j < iNumSibs; j++)
					{
						m_aiFeaturesRequired.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j = 0; j < iNumSibs; j++)
					{
						m_aiTerrainsRequired.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j = 0; j < iNumSibs; j++)
					{
						m_aiImprovementsRequired.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j = 0; j < iNumSibs; j++)
					{
						m_aiBonusesRequired.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j = 0; j < iNumSibs; j++)
					{
						m_aiRoutesRequired.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j=0; j < iNumSibs; j++)
					{
						m_aiReligionsRequired.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
		...
		if (pXML->SkipToNextVal())
		{
			...
			if (0 < iNumSibs)
			{
				if (pXML->GetChildXmlVal(szTextVal))
				{
					for (int j=0; j < iNumSibs; j++)
					{
						m_aiCorporationsRequired.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
						if (!pXML->GetNextXmlVal(szTextVal))
						{
							break;
						}
				...
				if (gDLL->getXMLIFace()->SetToChild(pXML->GetXML()))
				{
					for (int j = 0; j < iNumSibs; ++j)
					{
						if (pXML->GetChildXmlVal(szTextVal))
						{
							...
							pXML->GetNextXmlVal(szTextVal);
							m_aiTextEra.push_back(pXML->FindInInfoClass(szTextVal.c_str()));
...
CvEventInfo::CvEventInfo() :
	...
	m_iSpaceProductionModifier(0),
	m_iAIValue(0),
	m_piTechFlavorValue(nullptr),
	m_piPlotExtraYields(nullptr),
	m_piFreeSpecialistCount(nullptr),
	m_piAdditionalEventChance(nullptr),
	m_piAdditionalEventTime(nullptr),
	m_piClearEventChance(nullptr),
	m_piUnitCombatPromotions(nullptr),
	m_piUnitClassPromotions(nullptr)
{
}
...
{
	return static_cast<int>(m_aBuildingYieldChanges.size());
}
...
{
	return static_cast<int>(m_aBuildingCommerceChanges.size());
}

int CvEventInfo::getNumBuildingHappyChanges() const
{
	return static_cast<int>(m_aBuildingHappyChanges.size());
}
...
{
	return static_cast<int>(m_aBuildingHealthChanges.size());
}
...
{
	return m_szPythonExpireCheck.c_str();
}
...
void CvEventInfo::write(FDataStreamBase* stream)
{
	...
	stream->Write((int)m_aBuildingHealthChanges.size());
...
bool CvEventInfo::read(CvXMLLoadUtility* pXML)
{
	...
	m_iTech = pXML->FindInInfoClass(szTextVal.c_str());
	...
	pXML->GetChildXmlValByName(szTextVal, "PrereqTech");
	m_iPrereqTech = pXML->FindInInfoClass(szTextVal.c_str());
	pXML->GetChildXmlValByName(szTextVal, "UnitClass");
	m_iUnitClass = pXML->FindInInfoClass(szTextVal.c_str());
	...
	pXML->GetChildXmlValByName(szTextVal, "BuildingClass");
	m_iBuildingClass = pXML->FindInInfoClass(szTextVal.c_str());
	...
	m_iFeature = pXML->FindInInfoClass(szTextVal.c_str());
	...
	pXML->GetChildXmlValByName(szTextVal, "BonusType");
	m_iBonus = pXML->FindInInfoClass(szTextVal.c_str());
	...
	m_iBonusRevealed = pXML->FindInInfoClass(szTextVal.c_str());

	pXML->GetChildXmlValByName(szTextVal, "BonusGift");
	m_iBonusGift = pXML->FindInInfoClass(szTextVal.c_str());
	...
	m_iUnitPromotion = pXML->FindInInfoClass(szTextVal.c_str());
	...
	FAssertMsg(nullptr == m_piUnitCombatPromotions, "Memory leak");
	m_piUnitCombatPromotions = new int[GC.getNumUnitCombatInfos()];
	...
	{
		m_piUnitCombatPromotions[i] = pXML->FindInInfoClass(pszPromotions[i].c_str());
	}
	SAFE_DELETE_ARRAY(pszPromotions);

	FAssertMsg(nullptr == m_piUnitClassPromotions, "Memory leak");
	m_piUnitClassPromotions = new int[GC.getNumUnitClassInfos()];
	...
	{
		m_piUnitClassPromotions[i] = pXML->FindInInfoClass(pszPromotions[i].c_str());
	}
	SAFE_DELETE_ARRAY(pszPromotions);
			...
			if (0 < iNumSibs)
			{
				if (gDLL->getXMLIFace()->SetToChild(pXML->GetXML()))
				{
					for (int j = 0; j < iNumSibs; ++j)
					{
						...
						{
							BuildingYieldChange kChange;
							kChange.eBuildingClass = (BuildingClassTypes)pXML->FindInInfoClass(szTextVal.c_str());
							pXML->GetNextXmlVal(szTextVal);
							kChange.eYield = (YieldTypes)pXML->FindInInfoClass(szTextVal.c_str());
				...
				if (gDLL->getXMLIFace()->SetToChild(pXML->GetXML()))
				{
					for (int j = 0; j < iNumSibs; ++j)
					{
						if (pXML->GetChildXmlVal(szTextVal))
						{
							...
							kChange.eBuildingClass = (BuildingClassTypes)pXML->FindInInfoClass(szTextVal.c_str());
							pXML->GetNextXmlVal(szTextVal);
							kChange.eCommerce = (CommerceTypes)pXML->FindInInfoClass(szTextVal.c_str());
							pXML->GetNextXmlVal(&kChange.iChange);
							m_aBuildingCommerceChanges.push_back(kChange);
			...
			if (0 < iNumSibs)
			{
				if (gDLL->getXMLIFace()->SetToChild(pXML->GetXML()))
				{
					for (int j = 0; j < iNumSibs; ++j)
					{
						if (pXML->GetChildXmlVal(szTextVal))
						{
							BuildingClassTypes eBuildingClass = (BuildingClassTypes)pXML->FindInInfoClass(szTextVal.c_str());
							int iChange;
						...
						if (iIndex > -1)
						{
							...
							if (gDLL->getXMLIFace()->SetToChildByTagName(pXML->GetXML(),"UnitMeshGroup"))
							{
								for (j = 0; j < iMesh; j++)
								{
									...
									pXML->GetChildXmlValByName(szTextVal, "EarlyArtDefineTag");
									setEarlyArtDefineTag(j, iIndex, szTextVal.c_str());
									pXML->GetChildXmlValByName(szTextVal, "LateArtDefineTag");
									setLateArtDefineTag(j, iIndex, szTextVal.c_str());
									pXML->GetChildXmlValByName(szTextVal, "MiddleArtDefineTag");
									setMiddleArtDefineTag(j, iIndex, szTextVal.c_str());
...
CvVoteSourceInfo::CvVoteSourceInfo() :
	...
	m_iCivic(NO_CIVIC),
	m_aiReligionYields(nullptr),
	m_aiReligionCommerces(nullptr)
{
