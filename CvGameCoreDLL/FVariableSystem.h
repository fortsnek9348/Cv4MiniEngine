...
class FVariable
{
	public:
		...
		union
		{
			...
			unsigned int		m_uiValue;		//!< Four byte integer (unsigned) data.
...
class FVariableSystem
{
	public:
		...
		bool GetValue( const char * szVariable, unsigned int & uiValue ) const;
		...
		void SetValue( const char * szVariable, unsigned int uiValue );
