...
public:
	...
	// fortsnek: Used by Cv4MiniEngine. You're supposed to call CvMap::updateFog, but that would needless update every plot on the map.
	DllExportForInterface void updateFog();
	...
	void nukeExplosion(int iRange, CvUnit* pNukeUnit = nullptr);																							// Exposed to Python
	...
	// fortsnek: Need for Cv4MiniEngine if not using CvSymbol.
	DllExportForInterface bool isVisibleWorked() const;
	...
	DllExport CvUnit* getBestDefender(PlayerTypes eOwner, PlayerTypes eAttackingPlayer = NO_PLAYER, const CvUnit* pAttacker = nullptr, bool bTestAtWar = false, bool bTestPotentialEnemy = false, bool bTestCanMove = false) const;		// Exposed to Python
	...
	int plotCount(ConstPlotUnitFunc funcA, int iData1A = -1, int iData2A = -1, PlayerTypes eOwner = NO_PLAYER, TeamTypes eTeam = NO_TEAM, ConstPlotUnitFunc funcB = nullptr, int iData1B = -1, int iData2B = -1) const;
	...
	DllExportForInterface PlotTypes getPlotType() const;																																			// Exposed to Python
	...
	// fortsnek: Needed for Cv4MiniEngine to avoid having to mess with symbol machinery. Just call this instead for each displayed plot.
	DllExportForInterface int calculateYield(YieldTypes eIndex, bool bDisplay = false) const;												// Exposed to Python
...
//#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
//	int getCitySiteFoundValue(PlayerTypes eIndex) const;
//	int grabLazyFoundValue(PlayerTypes eIndex);
//#else
	int getCitySiteFoundValue(PlayerTypes eIndex);
//#endif
	...
	DllExport CvPlotBuilder* getPlotBuilder();
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	// Fed up of seeing isCoastalLand on the profiler.
	bool m_bIsCoastalLand : 1;
#endif
