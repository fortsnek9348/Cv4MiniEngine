...
CvGame::CvGame()
{
	...
	m_paiUnitCreatedCount = nullptr;
	m_paiUnitClassCreatedCount = nullptr;
	m_paiBuildingClassCreatedCount = nullptr;
	m_paiProjectCreatedCount = nullptr;
	m_paiForceCivicCount = nullptr;
	m_paiVoteOutcome = nullptr;
	m_paiReligionGameTurnFounded = nullptr;
	m_paiCorporationGameTurnFounded = nullptr;
	m_aiSecretaryGeneralTimer = nullptr;
	m_aiVoteTimer = nullptr;
	m_aiDiploVote = nullptr;

	m_pabSpecialUnitValid = nullptr;
	m_pabSpecialBuildingValid = nullptr;
	m_abReligionSlotTaken = nullptr;

	m_paHolyCity = nullptr;
	m_paHeadquarters = nullptr;

	m_pReplayInfo = nullptr;

	m_aiShrineBuilding = nullptr;
	m_aiShrineReligion = nullptr;
...
void CvGame::init(HandicapTypes eHandicap)
{
	...
	if (isMPOption(MPOPTION_SHUFFLE_TEAMS))
	{
		...
		{
			int j = (getSorenRand().get(static_cast<unsigned short>(iNumPlayers - i), nullptr) + i);
	...
	if (isOption(GAMEOPTION_LOCK_MODS))
	{
		...
		else
		{
			...
			{
				szRandomPassword[i] = static_cast<char>(getSorenRandNum(128, nullptr));
			}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	// This is called for new games. Create the context now.
	gGlobals.enhancedDLLGameContext = std::make_unique<GiganticMapsOptimisationsLib::GameContext>();
#endif
...
void CvGame::reset(HandicapTypes eHandicap, bool bConstructorCall)
{
	...
	if (!bConstructorCall)
	{
		FAssertMsg(m_paiUnitCreatedCount==nullptr, "about to leak memory, CvGame::m_paiUnitCreatedCount");
		...
		FAssertMsg(m_paiUnitClassCreatedCount==nullptr, "about to leak memory, CvGame::m_paiUnitClassCreatedCount");
		...
		FAssertMsg(m_paiBuildingClassCreatedCount==nullptr, "about to leak memory, CvGame::m_paiBuildingClassCreatedCount");
		...
		FAssertMsg(m_paiProjectCreatedCount==nullptr, "about to leak memory, CvGame::m_paiProjectCreatedCount");
		...
		FAssertMsg(m_paiForceCivicCount==nullptr, "about to leak memory, CvGame::m_paiForceCivicCount");
		...
		FAssertMsg(m_paiVoteOutcome==nullptr, "about to leak memory, CvGame::m_paiVoteOutcome");
		...
		FAssertMsg(m_aiDiploVote==nullptr, "about to leak memory, CvGame::m_aiDiploVote");
		...
		FAssertMsg(m_pabSpecialUnitValid==nullptr, "about to leak memory, CvGame::m_pabSpecialUnitValid");
		...
		FAssertMsg(m_pabSpecialBuildingValid==nullptr, "about to leak memory, CvGame::m_pabSpecialBuildingValid");
		...
		FAssertMsg(m_paiReligionGameTurnFounded==nullptr, "about to leak memory, CvGame::m_paiReligionGameTurnFounded");
		m_paiReligionGameTurnFounded = new int[GC.getNumReligionInfos()];
		FAssertMsg(m_abReligionSlotTaken==nullptr, "about to leak memory, CvGame::m_abReligionSlotTaken");
		m_abReligionSlotTaken = new bool[GC.getNumReligionInfos()];
		FAssertMsg(m_paHolyCity==nullptr, "about to leak memory, CvGame::m_paHolyCity");
		...
		FAssertMsg(m_paiCorporationGameTurnFounded==nullptr, "about to leak memory, CvGame::m_paiCorporationGameTurnFounded");
		...
		FAssertMsg(m_aiShrineBuilding==nullptr, "about to leak memory, CvGame::m_aiShrineBuilding");
		FAssertMsg(m_aiShrineReligion==nullptr, "about to leak memory, CvGame::m_aiShrineReligion");
		...
		FAssertMsg(m_aiSecretaryGeneralTimer==nullptr, "about to leak memory, CvGame::m_aiSecretaryGeneralTimer");
		FAssertMsg(m_aiVoteTimer==nullptr, "about to leak memory, CvGame::m_aiVoteTimer");
...
void CvGame::assignStartingPlots()
{
	...
	//fortsnek::appendMapLogStartingPlots("Initial starting plots");
	...
	{ 
		...
		{
			// Python override
			//fortsnek::appendMapLogStartingPlots("After python assignStartingPlots");
			return;
		}
	}
	...
	else
	{
		...
		//fortsnek::appendMapLogStartingPlots("assignStartingPlots - A");
	}
	...
	//fortsnek::appendMapLogStartingPlots("assignStartingPlots - B");

	//fortsnek::appendMapLogFoundValues("Found values for assignStartingPlots");
}
		...
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			...
			if (pStartingPlot != nullptr)
			{
				if (!pStartingPlot->isFreshWater())
				{
					...
					if (normalizeFindLakePlot((PlayerTypes)iI) != nullptr)
					{
...
void CvGame::normalizeRemovePeaks()
{
	...
	for (iI = 0; iI < MAX_CIV_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			...
			if (pStartingPlot != nullptr)
			{
				//printf("%i: %i, %i\n", iI, pStartingPlot->getX(), pStartingPlot->getY());
...
void CvGame::normalizeAddExtras()
{
	...
	[[maybe_unused]] int iTotalValue = 0;
	[[maybe_unused]] int iPlayerCount = 0;
	int iBestValue = 0;
		...
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			...
			if (pStartingPlot != nullptr)
			{
				...
				for (iJ = 0; iJ < NUM_CITY_PLOTS; iJ++)
				{
                    ...
                    if ((pLoopPlot != nullptr) && (pLoopPlot != pStartingPlot))
                    {
...
void CvGame::normalizeStartingPlots()
{
	...
	{
		if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeStartingPlotLocations", nullptr)  || gDLL->getPythonIFace()->pythonUsingDefaultImpl())
		{
			normalizeStartingPlotLocations();
			//fortsnek::appendMapLog("After native normalizeStartingPlotLocations");
		}
		//else
		//	fortsnek::appendMapLog("After python normalizeStartingPlotLocations");
	}
	...
	if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeAddRiver", nullptr)  || gDLL->getPythonIFace()->pythonUsingDefaultImpl())
	{
		normalizeAddRiver();
		//fortsnek::appendMapLog("After native normalizeAddRiver");
	}
	//else
	//	fortsnek::appendMapLog("After python normalizeAddRiver");

	if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeRemovePeaks", nullptr)  || gDLL->getPythonIFace()->pythonUsingDefaultImpl())
	{
		normalizeRemovePeaks();
		//fortsnek::appendMapLog("After native normalizeRemovePeaks");
	}
	//else
	//	fortsnek::appendMapLog("After python normalizeRemovePeaks");

	if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeAddLakes", nullptr)  || gDLL->getPythonIFace()->pythonUsingDefaultImpl())
	{
		normalizeAddLakes();
		//fortsnek::appendMapLog("After native normalizeAddLakes");
	}
	//else
	//	fortsnek::appendMapLog("After python normalizeAddLakes");

	if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeRemoveBadFeatures", nullptr)  || gDLL->getPythonIFace()->pythonUsingDefaultImpl())
	{
		normalizeRemoveBadFeatures();
		//fortsnek::appendMapLog("After native normalizeRemoveBadFeatures");
	}
	//else
	//	fortsnek::appendMapLog("After python normalizeRemoveBadFeatures");

	if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeRemoveBadTerrain", nullptr)  || gDLL->getPythonIFace()->pythonUsingDefaultImpl())
	{
		normalizeRemoveBadTerrain();
		//fortsnek::appendMapLog("After native normalizeRemoveBadTerrain");
	}
	//else
	//	fortsnek::appendMapLog("After python normalizeRemoveBadTerrain");

	if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeAddFoodBonuses", nullptr)  || gDLL->getPythonIFace()->pythonUsingDefaultImpl())
	{
		normalizeAddFoodBonuses();
		//fortsnek::appendMapLog("After native normalizeAddFoodBonuses");
	}
	//else
	//	fortsnek::appendMapLog("After python normalizeAddFoodBonuses");

	if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeAddGoodTerrain", nullptr)  || gDLL->getPythonIFace()->pythonUsingDefaultImpl())
	{
		normalizeAddGoodTerrain();
		//fortsnek::appendMapLog("After native normalizeAddGoodTerrain");
	}
	//else
	//	fortsnek::appendMapLog("After python normalizeAddGoodTerrain");

	if (!gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "normalizeAddExtras", nullptr)  || gDLL->getPythonIFace()->pythonUsingDefaultImpl())
	{
		normalizeAddExtras();
		//fortsnek::appendMapLog("After native normalizeAddGoodTerrain");
	}
...
//#else
//	std::for_each_n(std::execution::par, std::views::iota(0).begin(), MAX_PLAYERS, [](int iI) {
//		if (GET_PLAYER((PlayerTypes)iI).isAlive())
//		{
//			GET_PLAYER((PlayerTypes)iI).updatePlotGroups();
//		}
	...
	for (int j = 0; j < GC.getNumVoteInfos(); ++j)
	{
		...
		if (kVote.isVoteSourceType(eVoteSource))
		{
			if (kVote.isSecretaryGeneral())
			{
				...
				(void)kData.kVoteOption.szText.empty(); // BUG?
...
int CvGame::countCivTeamsEverAlive() const
{
	...
	return static_cast<int>(setTeamsEverAlive.size());
}
...
void CvGame::replaceCorporation(CorporationTypes eCorporation1, CorporationTypes eCorporation2)
{
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		{
			int iIter;
			for (CvCity* pCity = kLoopPlayer.firstCity(&iIter); nullptr != pCity; pCity = kLoopPlayer.nextCity(&iIter))
			{
			...
			for (CvUnit* pUnit = kLoopPlayer.firstUnit(&iIter); nullptr != pUnit; pUnit = kLoopPlayer.nextUnit(&iIter))
			{
...
bool CvGame::isVotePassed(VoteTypes eIndex) const
{
	...
	{
		return (ePlayerVote >= 0 && std::to_underlying(ePlayerVote) < MAX_CIV_TEAMS);
	}
	else
	{
	...
	if (!m_pabSpecialBuildingValid[eIndex])
	{
		...
		if (bAnnounce)
		{
			...
			for (int iI = 0; iI < MAX_PLAYERS; iI++)
			{
				if (GET_PLAYER((PlayerTypes)iI).isAlive())
				{
					gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_PROJECT_COMPLETED", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
				}
			}
		}
	}
...
PlayerVoteTypes CvGame::getPlayerVote(PlayerTypes eOwnerIndex, int iVoteId) const
{
	...
	FAssert(std::to_underlying(eOwnerIndex) < MAX_CIV_PLAYERS);
	FAssert(nullptr != getVoteTriggered(iVoteId));
...
void CvGame::doTurn()
{
	...
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::GeneralFixes])
	{
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
			if (GET_PLAYER((PlayerTypes)iI).isAlive())
				GET_PLAYER((PlayerTypes)iI).updateMaintenance();
	}
#endif
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	gGlobals.enhancedDLLGameContext->onGameTurn();
#endif
				...
				if (bChanged)
				{
					...
					if (pCity != nullptr)
					{
						if (pPlot->isVisible(pCity->getTeam(), false))
						{
							...
							gDLL->getInterfaceIFace()->addMessage(pCity->getOwnerINLINE(), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_GLOBALWARMING", MESSAGE_TYPE_INFO, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pPlot->getX_INLINE(), pPlot->getY_INLINE(), true, true);
						}
					}
				}
			}
...
void CvGame::doHolyCity()
{
	...
	lResult = 0;
	gDLL->getPythonIFace()->callFunction(PYGameModule, "doHolyCity", nullptr, &lResult);
	if (lResult == 1)
	{
		return;
	}
...
{
	long lResult = 0;
	gDLL->getPythonIFace()->callFunction(PYGameModule, "doHeadquarters", nullptr, &lResult);
	if (lResult == 1)
	{
		return;
	}
...
void CvGame::createBarbarianCities()
{
	...
	lResult = 0;
	gDLL->getPythonIFace()->callFunction(PYGameModule, "createBarbarianCities", nullptr, &lResult);
	if (lResult == 1)
	{
		return;
	}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::HardCapBarbSpawns])
	{
		// Hard cap barbs.
		if (GET_PLAYER(BARBARIAN_PLAYER).getNumCities() > 4000
			|| GET_PLAYER(BARBARIAN_PLAYER).getNumUnits() + GC.getHandicapInfo(GC.getGameINLINE().getHandicapType()).getBarbarianInitialDefenders() > 4000)
			return;
	}
#endif
	...
	{
#if 0 // Old method, just a trivial parallel loop
		struct Result
		{
			int value = 0;
			int plotI = -1;
			...
			{
				// Want plots in opposite order.
				return std::pair(value, -plotI) < std::pair(b.value, -b.plotI);
			}
		};

		std::atomic<Result> bestResult{};

		const GiganticMapsOptimisationsLib::DeterministicParallelPlotHashRNG rng(getSorenRand(), "Barb City Found");

		// Needs to be done to initialise allocations.
		// Also done in CvPlayerAI::AI_updateFoundValues.
		for (size_t i = 0, n = GC.getNumBonusInfos(); i < n; ++i)
			(void)GET_PLAYER(BARBARIAN_PLAYER).AI_baseBonusVal(BonusTypes(i));

		std::for_each_n(std::execution::par, std::views::iota(0).begin(), GC.getMapINLINE().numPlotsINLINE(), [&bestResult, &rng, iTargetCitiesMultiplier, handicap = getHandicapType()](int iI) {
			...
			if (!(pLoopPlot->isWater()))
			{
				if (!(pLoopPlot->isVisibleToCivTeam()))
				{
					...
					int iUnownedTilesThreshold = GC.getHandicapInfo(handicap).getUnownedTilesPerBarbarianCity();
					...
					if (pLoopPlot->area()->getCitiesPerPlayer(BARBARIAN_PLAYER) < iTargetCities)
					{
						...
						iValue += (100 + rng.get(50, iI, "Barb City Found"));
						iValue /= 100;

						heck::atomicMaxRelaxed(bestResult, Result{ .value = iValue, .plotI = iI });
					}
				}
			}
			});

		if (const auto bestResultLocal = bestResult.load(std::memory_order_relaxed); bestResultLocal.plotI >= 0)
			pBestPlot = GC.getMapINLINE().plotByIndexINLINE(bestResultLocal.plotI);
#else
		
		/// New method. V2 Found Value System

		auto& ctx = *gGlobals.enhancedDLLGameContext;
		ctx.prepareForFoundValueQuerying(BARBARIAN_PLAYER);

		CvMap& map = GC.getMapINLINE();

		// Cache per-area eval first.
		std::vector<bool> areaFilter(map.getIndexAfterLastArea());
		for (const int areaI : heck::range(static_cast<int>(areaFilter.size())))
		{
			if (const CvArea* const area = map.getArea(areaI))
			{
				int iTargetCities = area->getNumUnownedTiles();

				if (area->getNumCities() == area->getCitiesPerPlayer(BARBARIAN_PLAYER))
				{
					iTargetCities *= 3;
				}
				...
				if (area->getNumTiles() < (iUnownedTilesThreshold / 3))
				{
					iTargetCities *= iTargetCitiesMultiplier;
					iTargetCities /= 100;
				}
				...
				areaFilter[areaI] = area->getCitiesPerPlayer(BARBARIAN_PLAYER) < iTargetCities;
			}
		}

		const GiganticMapsOptimisationsLib::DeterministicParallelPlotHashRNG rng(getSorenRand(), "Barb City Found");

		const int minRivalRange = GC.getDefineINT("MIN_BARBARIAN_CITY_STARTING_DISTANCE");

		struct Choice
		{
			int value = 0;
			int plotI = -1;
			int originalFoundValue = 0;

			// For equivalent values, we need the lower plotI choice to be "greater".
		...
		// Now do found value calculation.
		heck::parallelForEachN(map.getGridHeightINLINE(), [&ctx, &bestChoice, &rng, &map, &areaFilter, isTargetCitiesMultiplier = iTargetCitiesMultiplier > 100, minRivalRange, width = map.getGridWidthINLINE()](int y) {
			const std::vector<int> rowFoundValues = ctx.computeFoundValuesRow(BARBARIAN_PLAYER, y, minRivalRange);
			for (const int x : heck::range(width))
			{
				const CvPlot& plot = *map.plotINLINE(x, y);

				if (areaFilter[plot.getArea() & FLTA_INDEX_MASK])
				{
					const int foundValue = rowFoundValues[x];
					int iValue = foundValue;
					if (isTargetCitiesMultiplier)
						iValue *= plot.area()->getNumOwnedTiles();

					const int plotI = y * width + x;

					iValue += 100 + rng.get(50, plotI, "Barb City Found");
					iValue /= 100;

					const Choice thisChoice{ iValue, plotI, foundValue };

					// TODO: isVisibleToCivTeam has loop. Could make it faster by remembering the last civ team.
					if (bestChoice.load(std::memory_order_relaxed) < thisChoice && !plot.isWater() && !plot.isVisibleToCivTeam())
						heck::atomicMaxRelaxed(bestChoice, thisChoice);
				}
			}
		});

		const Choice finalBestChoice = bestChoice.load(std::memory_order_relaxed);
		if (finalBestChoice.value > iBestValue)
		{
			pBestPlot = GC.getMapINLINE().plotByIndexINLINE(finalBestChoice.plotI);
			ctx.notifyFoundValueUsed(BARBARIAN_PLAYER, { pBestPlot->getX_INLINE(), pBestPlot->getY_INLINE() }, minRivalRange, finalBestChoice.originalFoundValue);
		}
#endif
	}
	else
...
void CvGame::createBarbarianUnits()
{
	...
	lResult = 0;
	gDLL->getPythonIFace()->callFunction(PYGameModule, "createBarbarianUnits", nullptr, &lResult);
	if (lResult == 1)
	{
		return;
	}
	...
	else
	{
		...
		if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::HardCapBarbSpawns])
		{
			// Kill animals /first/ to make room for barbs.
			for (pLoopUnit = GET_PLAYER(BARBARIAN_PLAYER).firstUnit(&iLoop); pLoopUnit != nullptr; pLoopUnit = GET_PLAYER(BARBARIAN_PLAYER).nextUnit(&iLoop))
			{
		...
		for(pLoopArea = GC.getMapINLINE().firstArea(&iLoop); pLoopArea != nullptr; pLoopArea = GC.getMapINLINE().nextArea(&iLoop))
		{
			...
			if (iDivisor > 0)
			{
				...
				if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::HardCapBarbSpawns])
				{
					// Hard cap random barb spawns to 4000. But remember that we might get extra barbs from cities and huts.
					iNeededBarbs = std::min(4000 - GET_PLAYER(BARBARIAN_PLAYER).getNumUnits(), iNeededBarbs);
				}
#endif
		...
		// Run the vanilla code here if HardCapBarbSpawns is not enabled.
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::HardCapBarbSpawns])
			;
		else
...
void CvGame::createAnimals()
{
	...
	for(pLoopArea = GC.getMapINLINE().firstArea(&iLoop); pLoopArea != nullptr; pLoopArea = GC.getMapINLINE().nextArea(&iLoop))
	{
		if (!(pLoopArea->isWater()))
		{
			...
			{
				// Hard cap random barb spawns to 4000. But remember that we might get extra barbs from cities and huts.
				iNeededAnimals = std::min(4000 - GET_PLAYER(BARBARIAN_PLAYER).getNumUnits(), iNeededAnimals);
			}
#endif
...
{
	FAssert(eVictory >= 0 && eVictory < GC.getNumVictoryInfos());
	FAssert(eTeam >=0 && std::to_underlying(eTeam) < MAX_CIV_TEAMS);
	...
	if (bValid)
	{
		if (GC.getVictoryInfo(eVictory).isDiploVote())
		{
			...
			for (int iK = 0; iK < GC.getNumVoteInfos(); iK++)
			{
				...
				{
					// fortsnek: Casting enums to int for clang
					if ((int)getVoteOutcome((VoteTypes)iK) == (int)eTeam)
					{
						bFound = true;
						break;
					}
...
void CvGame::testVictory()
{
	...
	{
		int iWinner = getSorenRandNum(static_cast<int>(aaiWinners.size()), "Victory tie breaker");
...
{
	return m_mapRand.get(static_cast<unsigned short>(iNum), pszLog);
}
...
{
	return m_sorenRand.get(static_cast<unsigned short>(iNum), pszLog);
}
...
int CvGame::getReplayMessageTurn(unsigned int i) const
{
...
ReplayMessageTypes CvGame::getReplayMessageType(unsigned int i) const
{
	if (i >= m_listReplayMessages.size())
	{
...
int CvGame::getReplayMessagePlotX(unsigned int i) const
{
	if (i >= m_listReplayMessages.size())
	{
		return (-1);
...
int CvGame::getReplayMessagePlotY(unsigned int i) const
{
	if (i >= m_listReplayMessages.size())
	{
		return (-1);
...
PlayerTypes CvGame::getReplayMessagePlayer(unsigned int i) const
{
	if (i >= m_listReplayMessages.size())
	{
		return (NO_PLAYER);
...
const wchar_t* CvGame::getReplayMessageText(unsigned int i) const
{
	if (i >= m_listReplayMessages.size())
	{
		return (nullptr);
...
ColorTypes CvGame::getReplayMessageColor(unsigned int i) const
{
	if (i >= m_listReplayMessages.size())
	{
		return (NO_COLOR);
...
unsigned int CvGame::getNumReplayMessages() const
{
	return static_cast<unsigned int>(m_listReplayMessages.size());
}
...
void CvGame::onCvAppFinishedLoading()
{
#if ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::GeneralFixes])
	{
		// City maintenance may be out of date if this is a save written by the vanilla BTS DLL. Possibly. Haven't yet checked for a case.
		for (int i = 0; i < MAX_PLAYERS; ++i)
		{
			GET_PLAYER(static_cast<PlayerTypes>(i)).updateMaintenance();
			GET_PLAYER(static_cast<PlayerTypes>(i)).updateCommerce();
		}
	}

	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::BlockwisePlotGroupSystem])
	{
		// Init this before GameContext.
		for (int i = 0; i < MAX_PLAYERS; ++i)
			GET_PLAYER(static_cast<PlayerTypes>(i)).initPlayerTradeNetworkSystem();
	}
...
void CvGame::write(FDataStreamBase* pStream)
{
...

	{
		...
		pStream->Write((int)m_aszDestroyedCities.size());
		for (it = m_aszDestroyedCities.begin(); it != m_aszDestroyedCities.end(); it++)
		{
			pStream->WriteString(*it);
		}

		pStream->Write((int)m_aszGreatPeopleBorn.size());
		for (it = m_aszGreatPeopleBorn.begin(); it != m_aszGreatPeopleBorn.end(); it++)
		{
			pStream->WriteString(*it);
		}
	...
	int iSize = (int)m_listReplayMessages.size();
	pStream->Write(iSize);
	...
	pStream->Write((int)m_aPlotExtraYields.size());
	for (auto it = m_aPlotExtraYields.begin(); it != m_aPlotExtraYields.end(); ++it)
	{
		(*it).write(pStream);
	}

	pStream->Write((int)m_aPlotExtraCosts.size());
	for (auto it = m_aPlotExtraCosts.begin(); it != m_aPlotExtraCosts.end(); ++it)
	{
		(*it).write(pStream);
	}

	pStream->Write((int)m_mapVoteSourceReligions.size());
	for (auto it = m_mapVoteSourceReligions.begin(); it != m_mapVoteSourceReligions.end(); ++it)
	{
		pStream->Write(it->first);
		pStream->Write(it->second);
	}

	pStream->Write((int)m_aeInactiveTriggers.size());
	for (auto it = m_aeInactiveTriggers.begin(); it != m_aeInactiveTriggers.end(); ++it)
	{
		pStream->Write(*it);
	}
...
void CvGame::showEndGameSequence()
{
	...
	// fortsnek: Changed to int for Linux
...
{
	const auto it = m_mapVoteSourceReligions.find(eVoteSource);
	...
	if (bAnnounce)
	{
		if (NO_RELIGION != eReligion)
		{
			...
			for (int iI = 0; iI < MAX_PLAYERS; iI++)
			{
				if (GET_PLAYER((PlayerTypes)iI).isAlive())
				{
					gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, GC.getReligionInfo(eReligion).getSound(), MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
				}
			}
		}
	}
				...
				if (isChooseElection((VoteTypes)iI))
				{
					...
					else if (GC.getVoteInfo(kData.eVote).isAssignCity())
					{
						for (int iPlayer1 = 0; iPlayer1 < MAX_CIV_PLAYERS; ++iPlayer1)
						{
							...
							int iLoop;
							for (CvCity* pLoopCity = kPlayer1.firstCity(&iLoop); nullptr != pLoopCity; pLoopCity = kPlayer1.nextCity(&iLoop))
							{
...
{
	int iLoop;
	for (VoteTriggeredData* pVoteTriggered = m_votesTriggered.beginIter(&iLoop); nullptr != pVoteTriggered; pVoteTriggered = m_votesTriggered.nextIter(&iLoop))
	{
		CvWString szBuffer;
		...
		if (!canDoResolution(eVoteSource, pVoteTriggered->kVoteOption))
		{
			for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; ++iPlayer)
			{
				...
				{
					CvWString szMessage;
					szMessage.Format(L"%ls: %ls", gDLL->getText("TXT_KEY_ELECTION_CANCELLED").GetCString(), GC.getVoteInfo(eVote).getDescription());
					gDLL->getInterfaceIFace()->addMessage((PlayerTypes)iPlayer, false, GC.getEVENT_MESSAGE_TIME(), szMessage, "AS2D_NEW_ERA", MESSAGE_TYPE_INFO, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));					
				}
			}
		}
		else
			...
			for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
			{
				...
				if (kPlayer.isHuman())
				{
					...
					if (bShow && bPassed)
					{
						...
						gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szMessage, "AS2D_NEW_ERA", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
					}
...
{
	long lResult = -1;
	if (gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "isBonusIgnoreLatitude", nullptr, &lResult))
	{
