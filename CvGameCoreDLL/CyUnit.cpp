...
{
	return m_pUnit ? new CyPlot(m_pUnit->getPathEndTurnPlot()) : nullptr;
}
...
{
	return m_pUnit ? new CyCity(m_pUnit->bombardTarget(pPlot->getPlot())) : nullptr;
}
...
{
	return m_pUnit ? m_pUnit->getBuildType() : (int) NO_BUILD;
}
...
{
	return m_pUnit ? new CyUnit(m_pUnit->bestInterceptor(pPlot->getPlot())) : nullptr;
}
...
{
	return m_pUnit ? new CyPlot(m_pUnit->getReconPlot()) : nullptr;
}
...
{
	return m_pUnit ? m_pUnit->getName().c_str() : L"";
}

std::wstring CyUnit::getNameForm(int iForm)
{
	return m_pUnit ? m_pUnit->getName((unsigned int)iForm).c_str() : L"";
}
...
{
	return m_pUnit ? m_pUnit->getNameNoDesc().c_str() : L"";
}
...
{
	return m_pUnit ? m_pUnit->getArtInfo(i, eEra) : nullptr;
}
