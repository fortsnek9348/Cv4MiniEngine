		...
		while (pUnitNode != nullptr)
		{
			...
			if (pLoopUnit->canMove())
			{
				...
				case MISSION_ESPIONAGE:
					...
					pUnitNode = nullptr; // allow one unit at a time to do espionage
					break;
...
bool CvSelectionGroup::canAllMove() const
{
	...
	if (getNumUnits() > 0)
	{
		...
		while (pUnitNode != nullptr)
		{
			...
			FAssertMsg(pLoopUnit != nullptr, "existing node, but nullptr unit");
...
bool CvSelectionGroup::canMoveThrough(CvPlot* pPlot) const
{
	CLLNode<IDInfo>* pUnitNode;
	CvUnit* pLoopUnit;
...
int CvSelectionGroup::countNumUnitAIType(UnitAITypes eUnitAI)
{
	...
	FAssertMsg(headUnitNode() != nullptr, "headUnitNode() is not expected to be equal with nullptr");
	...
	if (getNumUnits() > 0)
	{
		if ((getDomainType() == DOMAIN_AIR) || (stepDistance(getX(), getY(), pDestPlot->getX_INLINE(), pDestPlot->getY_INLINE()) == 1))
		{
			if ((iFlags & MOVE_DIRECT_ATTACK) || (getDomainType() == DOMAIN_AIR) || (iFlags & MOVE_THROUGH_ENEMY)
				|| (
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
				(gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::PathLimit_CvSelectionGroup_groupAttack]
					? generatePath(plot(), pDestPlot, PathingArg(iFlags, 2))
					: generatePath(plot(), pDestPlot, iFlags))
#else
				generatePath(plot(), pDestPlot, iFlags)
#endif
				&& (getPathFirstPlot() == pDestPlot)))
			{
				int iAttackOdds;
...
//                if ((pWorkingCity != nullptr) && (AI_getMissionAIPlot() == pPlot))
//                {
		...
		if (stepDistance(getX(), getY(), pPlot->getX_INLINE(), pPlot->getY_INLINE()) == 1)
		{
			...
			while (pUnitNode1 != nullptr)
			{
				...
				if ((pLoopUnit1->getCargo() > 0) && (pLoopUnit1->domainCargo() == DOMAIN_LAND))
				{
					...
					for (unsigned int i = 0; i < aCargoGroups.size(); ++i)
					{
...
CvPlot* CvSelectionGroup::getPathEndTurnPlot() const
{
	...
	if (nullptr != pNode)
	{
		if ((pNode->m_pParent == nullptr) || (pNode->m_iData2 == 1))
		{
			return GC.getMapINLINE().plotSorenINLINE(pNode->m_iX, pNode->m_iY);
		}
...
bool CvSelectionGroup::generatePath( const CvPlot* pFromPlot, const CvPlot* pToPlot, PathingArg pathingArg, bool bReuse, int* piPathTurns) const
{
	...
	bSuccess = gDLL->getFAStarIFace()->GeneratePath(&GC.getPathFinder(), pFromPlot->getX_INLINE(), pFromPlot->getY_INLINE(), pToPlot->getX_INLINE(), pToPlot->getY_INLINE(), false, pathingArg, bReuse);
#else
	bSuccess = gGlobals.enhancedDLLGameContext->generatePath(*this, *pFromPlot, *pToPlot, pathingArg.iFlags, pathingArg.iMaxTurns, bReuse);
#endif
...
void CvSelectionGroup::resetPath()
{
	...
	{
		// The context always resets the original FAStar pathfinder when it emplaces the resulting path into it.
		// So to actually reset the internal pathing state, we have to do it explicitly.
		gGlobals.enhancedDLLGameContext->resetPathfinder();
	}
#endif
}
...
CvSelectionGroup* CvSelectionGroup::splitGroup(int iSplitSize, CvUnit* pNewHeadUnit, CvSelectionGroup** ppOtherGroup)
{
	...
	FAssertMsg(pSplitGroup != nullptr, "join resulted in nullptr group");
	...
	if (pRemainderHeadUnit != nullptr)
	{
		...
		FAssertMsg(pRemainderGroup != nullptr, "join resulted in nullptr group");
	}
			...
			if (pLoopUnit != nullptr)
			{
				...
				if (eLoopUnitAI == eTargetUnitAI)
				{
					...
					else
					{
						...
						// (if pRemainderGroup nullptr, it gets its own group)
...
//------------------------------------------------------------------------------------------------
int CvSelectionGroup::getUnitIndex(const CvUnit* pUnit, int maxIndex /* = -1 */) const
{
	CLLNode<IDInfo>* pUnitNode;
	CvUnit* pLoopUnit;
	int iIndex;
