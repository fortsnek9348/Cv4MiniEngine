...
void CvMapGenerator::FreeInstance() { SAFE_DELETE(m_pInst); }
...
{
	FAssertMsg(pFreshWaterPlot != nullptr, "nullptr plot parameter");
...
void CvMapGenerator::addFeatures()
{
	...
	if (gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "addFeatures", nullptr))
	{
		if (!gDLL->getPythonIFace()->pythonUsingDefaultImpl())
		{
			return;
...
void CvMapGenerator::addBonuses()
{
	...
	if (gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "addBonuses", nullptr))
	{
		if (!gDLL->getPythonIFace()->pythonUsingDefaultImpl())
		{
	...
	for (int iI = 0; iI < GC.getNumImprovementInfos(); iI++)
	{
		if (GC.getImprovementInfo((ImprovementTypes)iI).isGoody() && GC.getImprovementInfo((ImprovementTypes)iI).getTilesPerGoody() > 0)
		{
			for (int iJ = 0; iJ < iNumPlots; iJ++)
			{
				...
				FAssertMsg(pPlot, "pPlot is expected not to be nullptr");
				if (!(pPlot->isWater()))
				{
					CvArea *pArea = GC.getMapINLINE().getArea(pPlot->getArea());
					FAssertMsg(pArea, "pArea is expected not to be nullptr");
...
void CvMapGenerator::generateRandomMap()
{
	...
	if (gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "generateRandomMap"))
	{
		if (!gDLL->getPythonIFace()->pythonUsingDefaultImpl())
		{
			//fortsnek::appendMapLog("generateRandomMap python override");
			return; // Python override
		}
	}
	...
	sprintf(buf, "Generating Random Map %ls, %ls...", gDLL->getMapScriptName().GetCString(), GC.getWorldInfo(GC.getMapINLINE().getWorldSize()).getDescription());
	gDLL->NiTextOut(buf);

	generatePlotTypes();
	//fortsnek::appendMapLog("After generatePlotTypes");
	generateTerrain();
	//fortsnek::appendMapLog("After generateTerrain");
}
...
void CvMapGenerator::generatePlotTypes()
{
	...
	if (gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "generatePlotTypes", nullptr, &plotTypesOut) && !gDLL->getPythonIFace()->pythonUsingDefaultImpl())
	{
		// Python override
...
void CvMapGenerator::generateTerrain()
{
	...
	if (gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "generateTerrainTypes", nullptr, &terrainMapOut) && !gDLL->getPythonIFace()->pythonUsingDefaultImpl())
	{
		 // Python override
		int iNumPlots = GC.getMapINLINE().numPlotsINLINE();
