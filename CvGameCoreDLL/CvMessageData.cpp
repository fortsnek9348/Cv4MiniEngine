...
{		
	sprintf(szAddendum, "Do Mission, who is %d, unit ID is %d, mission is %ls", m_ePlayer, m_iUnitID, GC.getMissionInfo(m_eMission).getDescription());
}
...
void CvNetDoCommand::Execute()
{
	if (m_ePlayer != NO_PLAYER)
	{
		...
		if (pUnit != nullptr)
		{
			...
			{
				int iLoop;
				for (CvUnit* pLoopUnit = GET_PLAYER(m_ePlayer).firstUnit(&iLoop); pLoopUnit != nullptr; pLoopUnit = GET_PLAYER(m_ePlayer).nextUnit(&iLoop))
				{
...
CvNetChangeVassal::CvNetChangeVassal() : CvMessageData(GAMEMESSAGE_CHANGE_VASSAL), m_bVassal(false), m_bCapitulated(false), m_ePlayer(NO_PLAYER), m_eMasterTeam(NO_TEAM)
{
}

CvNetChangeVassal::CvNetChangeVassal(PlayerTypes ePlayer, TeamTypes eMasterTeam, bool bVassal, bool bCapitulated) : CvMessageData(GAMEMESSAGE_CHANGE_VASSAL), m_bVassal(bVassal), m_bCapitulated(bCapitulated), m_ePlayer(ePlayer), m_eMasterTeam(eMasterTeam)
{
}
...
CvNetChangeWar::CvNetChangeWar() : CvMessageData(GAMEMESSAGE_CHANGE_WAR), m_bWar(false), m_ePlayer(NO_PLAYER), m_eRivalTeam(NO_TEAM)
{
}

CvNetChangeWar::CvNetChangeWar(PlayerTypes ePlayer, TeamTypes eRivalTeam, bool bWar) : CvMessageData(GAMEMESSAGE_CHANGE_WAR), m_bWar(bWar), m_ePlayer(ePlayer), m_eRivalTeam(eRivalTeam)
{
}
