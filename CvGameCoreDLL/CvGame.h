...
public:
	...
	DllExport bool canHandleAction(int iAction, CvPlot* pPlot = nullptr, bool bTestVisible = false, bool bUseCache = false) const;
	...
	// fortsnek: Exported to enumerate calculateSyncChecksum.
	DllExportForInterface void changeTurnSlice(int iChange);
	...
	// fortsnek: Need this to know when to exit the update loop when AI autoplay is active.
	DllExportForInterface int getAIAutoPlay();																				// Exposed to Python
	...
	int getReplayMessageTurn(unsigned int i) const;
	ReplayMessageTypes getReplayMessageType(unsigned int i) const;
	int getReplayMessagePlotX(unsigned int i) const;
	int getReplayMessagePlotY(unsigned int i) const;
	PlayerTypes getReplayMessagePlayer(unsigned int i) const;
	const wchar_t* getReplayMessageText(unsigned int i) const;
	unsigned int getNumReplayMessages() const;
	ColorTypes getReplayMessageColor(unsigned int i) const;

	DllExport virtual void read(FDataStreamBase* pStream);
	DllExport void onCvAppFinishedLoading(); // fortsnek: New function. Engine will call this to let us init the enhanced DLL GameContext.
