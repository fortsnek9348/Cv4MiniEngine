...
CvReplayInfo::~CvReplayInfo()
{
	[[maybe_unused]] ReplayMessageList::const_iterator it;
	for (unsigned int i = 0; i < m_listReplayMessages.size(); i++)
	{
		SAFE_DELETE(m_listReplayMessages[i]);
	}
	// fortsnek: Address sanitiser fix, use array delete
...
void CvReplayInfo::createInfo(PlayerTypes ePlayer)
{
	...
	for (unsigned int i = 0; i < game.getNumReplayMessages(); i++)
	{
...
{
	for (unsigned int i = 0; i < m_listGameOptions.size(); i++)
	{
...
{
	for (unsigned int i = 0; i < m_listVictoryTypes.size(); i++)
	{
...
int CvReplayInfo::getReplayMessageTurn(unsigned int i) const
{
...
ReplayMessageTypes CvReplayInfo::getReplayMessageType(unsigned int i) const
{
	if (i >= m_listReplayMessages.size())
	{
...
int CvReplayInfo::getReplayMessagePlotX(unsigned int i) const
{
	if (i >= m_listReplayMessages.size())
	{
		return (-1);
...
int CvReplayInfo::getReplayMessagePlotY(unsigned int i) const
{
	if (i >= m_listReplayMessages.size())
	{
		return (-1);
...
PlayerTypes CvReplayInfo::getReplayMessagePlayer(unsigned int i) const
{
	if (i >= m_listReplayMessages.size())
	{
		return (NO_PLAYER);
...
const wchar_t* CvReplayInfo::getReplayMessageText(unsigned int i) const
{
	if (i >= m_listReplayMessages.size())
	{
		return (nullptr);
...
ColorTypes CvReplayInfo::getReplayMessageColor(unsigned int i) const
{
	if (i >= m_listReplayMessages.size())
	{
		return (NO_COLOR);
...
unsigned int CvReplayInfo::getNumReplayMessages() const
{
...
void CvReplayInfo::write(FDataStreamBase& stream)
{
	...
	for (unsigned int i = 0; i < m_listReplayMessages.size(); i++)
	{
		if (nullptr != m_listReplayMessages[i])
		{
	...
	for (unsigned int i = 0; i < m_listPlayerScoreHistory.size(); i++)
	{
		...
		for (unsigned int j = 0; j < info.m_listScore.size(); j++)
		{
