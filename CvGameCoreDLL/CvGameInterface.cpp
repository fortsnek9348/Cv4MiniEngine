	...
	else if (pHeadSelectedUnit != nullptr)
	{
		...
		else if(pHeadSelectedUnit->airRange() > 0) //other ranged units
		{
			...
			for (iDX = -(iRange); iDX <= iRange; iDX++)
			{
				for (iDY = -(iRange); iDY <= iRange; iDY++)
				{
					...
					if (pTargetPlot != nullptr && pTargetPlot->isVisible(pHeadSelectedUnit->getTeam(), false))
					{
		...
		if (pHeadSelectedUnit->isBlockading())
		{
			...
			for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; ++iPlayer)
			{
				...
				{
					int iLoop;
					for (CvUnit* pLoopUnit = kPlayer.firstUnit(&iLoop); nullptr != pLoopUnit; pLoopUnit = kPlayer.nextUnit(&iLoop))
					{
						if (pLoopUnit->isBlockading())
						{
							for (int i = -iBlockadeRange; i <= iBlockadeRange; ++i)
							{
								for (int j = -iBlockadeRange; j <= iBlockadeRange; ++j)
								{
									...
									if (nullptr != pLoopPlot && pLoopPlot->isRevealed(getActiveTeam(), false))
									{
...
void CvGame::cycleSelectionGroups(bool bClear, bool bForward, bool bWorkers) const
{
	...
	else
	{
		...
		pNextSelectionGroup = GC.getMapINLINE().findSelectionGroup(((pPlot != nullptr) ? pPlot->getX() : 0), ((pPlot != nullptr) ? pPlot->getY() : 0), getActivePlayer(), true, bWorkers);
	}
...
{
	if (pSelectCity == nullptr || !pSelectCity->canBeSelected())
	{
		return false;
	}
	...
	{
		int iLoop;
		for (CvCity* pLoopCity = GET_PLAYER(pSelectCity->getOwnerINLINE()).firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = GET_PLAYER(pSelectCity->getOwnerINLINE()).nextCity(&iLoop))
		{
...
void CvGame::startFlyoutMenu(const CvPlot* pPlot, std::vector<CvFlyoutMenuData>& aFlyoutItems) const
{
	...
	if (pCity != nullptr)
	{
		if (pCity->getOwnerINLINE() == getActivePlayer())
		{
			...
			aFlyoutItems.push_back(CvFlyoutMenuData(NO_FLYOUT, -1, -1, -1, szBuffer.c_str()));
			for (int iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
			{
				...
				if (eLoopUnit != NO_UNIT)
				{
					...
					{
						szBuffer.Format(L"%ls (%d)", GC.getUnitInfo(eLoopUnit).getDescription(), pCity->getProductionTurnsLeft(eLoopUnit, 0));
						aFlyoutItems.push_back(CvFlyoutMenuData(FLYOUT_TRAIN, eLoopUnit, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer.c_str()));
					}
				}
			}
			...
			for (int iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
			{
				...
				if (eLoopBuilding != NO_BUILDING)
				{
					...
					{
						szBuffer.Format(L"%ls (%d)", GC.getBuildingInfo(eLoopBuilding).getDescription(), pCity->getProductionTurnsLeft(eLoopBuilding, 0));
						aFlyoutItems.push_back(CvFlyoutMenuData(FLYOUT_CONSTRUCT, eLoopBuilding, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer.c_str()));
					}
				}
			}
			...
			for (int iI = 0; iI < GC.getNumProjectInfos(); iI++)
			{
				...
				{
					szBuffer.Format(L"%ls (%d)", GC.getProjectInfo(eProject).getDescription(), pCity->getProductionTurnsLeft(eProject, 0));
					aFlyoutItems.push_back(CvFlyoutMenuData(FLYOUT_CREATE, eProject, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer.c_str()));
				}
			}
			...
			for (int iI = 0; iI < GC.getNumProcessInfos(); iI++)
			{
				if (pCity->canMaintain((ProcessTypes)iI))
				{
					...
					aFlyoutItems.push_back(CvFlyoutMenuData(FLYOUT_MAINTAIN, iI, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer.c_str()));
				}
			}
			...
			for (iI = 0; iI < GC.getNumHurryInfos(); iI++)
			{
				if (pCity->canHurry((HurryTypes)iI))
				{
					...
					aFlyoutItems.push_back(CvFlyoutMenuData(FLYOUT_HURRY, iI, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer.c_str()));
				}
			}
			...
			if (pCity->canConscript())
			{
				...
				if (eConscriptUnit != NO_UNIT)
				{
					...
					aFlyoutItems.push_back(CvFlyoutMenuData(FLYOUT_CONSCRIPT, iI, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer.c_str()));
				}
			}
		}
	}
	...
	if (pHeadSelectedUnit != nullptr && !pHeadSelectedUnit->atPlot(pPlot))
	{
		...
		if ((pHeadSelectedUnit->getDomainType() == DOMAIN_AIR) || gDLL->getFAStarIFace()->GeneratePath(&GC.getInterfacePathFinder(), pHeadSelectedUnit->getX(), pHeadSelectedUnit->getY(), pPlot->getX(), pPlot->getY(), false, MOVE_DECLARE_WAR, true))
		{
			...
			aFlyoutItems.push_back(CvFlyoutMenuData(FLYOUT_MOVE_TO, 0, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer.c_str()));
		}
	}
	...
	if (bUnits)
	{
		...
		aFlyoutItems.push_back(CvFlyoutMenuData(FLYOUT_SELECT_ALL, 0, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer.c_str()));

		if (bWakeUnit)
		{
			szBuffer = gDLL->getText("TXT_KEY_FLYOUT_MENU_WAKE_ALL");
			aFlyoutItems.push_back(CvFlyoutMenuData(FLYOUT_WAKE_ALL, 0, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer.c_str()));
		}

		if (bFortifyUnit)
		{
			szBuffer = gDLL->getText("TXT_KEY_FLYOUT_MENU_FORTIFY_ALL");
			aFlyoutItems.push_back(CvFlyoutMenuData(FLYOUR_FORTIFY_ALL, 0, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer.c_str()));
		}
		else if (bSleepUnit)
		{
			szBuffer = gDLL->getText("TXT_KEY_FLYOUT_MENU_SLEEP_ALL");
			aFlyoutItems.push_back(CvFlyoutMenuData(FLYOUR_SLEEP_ALL, 0, pPlot->getX_INLINE(), pPlot->getY_INLINE(), szBuffer.c_str()));
		}
...
int CvGame::getNextSoundtrack(EraTypes eLastEra, [[maybe_unused]] int iLastSoundtrack) const
{
	...
	{
		return kCurrentEra.getSoundtracks(GC.getASyncRand().get(static_cast<unsigned short>(kCurrentEra.getNumSoundtracks()), "Pick Song ASYNC"));
	}
}
...
void CvGame::initSelection() const
{
	...
	int iLoop;
	for (CvUnit* pLoopUnit = GET_PLAYER(getActivePlayer()).firstUnit(&iLoop); pLoopUnit != nullptr; pLoopUnit = GET_PLAYER(getActivePlayer()).nextUnit(&iLoop))
	{
		if (pLoopUnit->getGroup()->readyToSelect())
		{
...
{
	if (pPlot == nullptr || !pPlot->isRevealed(getActiveTeam(), false))
	{
		return false;
	}
...
{
	if (gDLL->getInterfaceIFace()->isCitySelection() ||
		gDLL->getInterfaceIFace()->getHeadSelectedCity() != nullptr ||
		gDLL->isDiplomacy() ||
...
bool CvGame::shouldDisplayUnitModel() const
{
	...
	if (gDLL->getInterfaceIFace()->getHeadSelectedUnit() != nullptr || gDLL->getInterfaceIFace()->isCityScreenUp())
	{
		return true;
	}
...
void CvGame::handleCityScreenPlotPicked(CvCity* pCity, CvPlot* pPlot, bool bAlt, bool bShift, bool bCtrl) const
{
	...
	if (pCity != nullptr && pPlot != nullptr)
	{
...
void CvGame::handleCityScreenPlotDoublePicked(CvCity* pCity, CvPlot* pPlot, [[maybe_unused]] bool bAlt, [[maybe_unused]] bool bShift, [[maybe_unused]] bool bCtrl) const
{
	if (pCity != nullptr)
	{
		if (pCity->plot() == pPlot)
...
void CvGame::handleCityPlotRightPicked(CvCity* pCity, CvPlot* pPlot, [[maybe_unused]] bool bAlt, bool bShift, [[maybe_unused]] bool bCtrl) const
{
	if (pPlot != nullptr)
	{
		if ((pCity != nullptr) && gDLL->getInterfaceIFace()->isCitySelected(pCity))
		{
...
void CvGame::handleMiddleMouse(bool bCtrl, bool bAlt, [[maybe_unused]] bool bShift)
{
	if (gDLL->getInterfaceIFace()->isCitySelection())
	{
		gDLL->getInterfaceIFace()->clearSelectedCities();
