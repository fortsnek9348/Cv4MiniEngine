...
struct CvMapInitData
{
	...
	CvMapInitData(int iGridW=0, int iGridH=0, int iTopLatitude=90, int iBottomLatitude=-90, bool bWrapX=false, bool bWrapY=false) :
		m_iGridW(iGridW), m_iGridH(iGridH),m_iTopLatitude(iTopLatitude),m_iBottomLatitude(iBottomLatitude), m_bWrapX(bWrapX), m_bWrapY(bWrapY)
	{ }
};
...
public:
	...
	DllExport CvCity* findCity(int iX, int iY, PlayerTypes eOwner = NO_PLAYER, TeamTypes eTeam = NO_TEAM, bool bSameArea = true, bool bCoastalOnly = false, TeamTypes eTeamAtWarWith = NO_TEAM, DirectionTypes eDirection = NO_DIRECTION, CvCity* pSkipCity = nullptr);	// Exposed to Python
	...
	inline int plotNumINLINE(const CvPlot* plot) const
	{
		return plotNumINLINE(plot->getX_INLINE(), plot->getY_INLINE());
	}
#endif
#endif
...
#ifdef _USRDLL
	HECK_FORCEINLINE CvPlot* plotINLINE(int iX, int iY) const
	{
	...
	HECK_FORCEINLINE CvPlot* plotSorenINLINE(int iX, int iY) const
	{
		if ((iX == INVALID_PLOT_COORD) || (iY == INVALID_PLOT_COORD))
		{
			return nullptr;
	...
	// fortsnek: const
	DllExport int getNumLandAreas() const;
