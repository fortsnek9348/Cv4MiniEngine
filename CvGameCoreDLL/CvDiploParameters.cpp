...
void addVar(std::vector<FVariable>& argsList, const CvWString& arg)
{
	//if (arg)
	{
		FVariable var;
		var.m_eType = FVARTYPE_WSTRING;
		var.m_wszValue = new wchar_t[arg.size()+1];
		wcscpy(var.m_wszValue, arg.c_str());
...
{ \
	...
	setDiploComment(eCommentType, argsList.size() ? &argsList : nullptr); \
}
...
void CvDiploParameters::write(FDataStreamBase& stream) const
{
	...
	int i, iSize = static_cast<int>(m_diploCommentArgs.size());
	stream.Write(iSize);
	for(i=0;i<iSize;i++)
