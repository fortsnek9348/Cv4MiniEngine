...
// for logging
//
void CvXMLLoadUtility::logMsg(const char* format, ... )
{
	static char buf[kBufSize];
	// fortsnek: What the HECK
...
void CvXMLLoadUtility::ResetGlobalEffectInfo()
{
	...
	LoadGlobalClassInfo(GC.getEffectInfo(), "CIV4EffectInfos", "Misc", "Civ4EffectInfos/EffectInfos/EffectInfo", false, nullptr);
}
...
bool CvXMLLoadUtility::LoadCivXml(FXml* pFXml, const TCHAR* szFilename)
{
	...
	logMsg("Loading XML file %s", szPath.c_str());

	if (!gDLL->getXMLIFace()->LoadXml(pFXml, szPath.c_str()))
	{
		logMsg("Load XML file %s FAILED", szPath.c_str());
		return false;
	}

	logMsg("Load XML file %s SUCCEEDED", szPath.c_str());
...
CvWString CvXMLLoadUtility::CreateKeyStringFromKBCode(const TCHAR* pszHotKey)
{
	...
	// fortsnek: Let's make this static...
	static const CvKeyBoardMapping asCvKeyBoardMapping[iNumKeyBoardMappings] =
	{
