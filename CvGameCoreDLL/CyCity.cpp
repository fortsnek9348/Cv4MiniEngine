...
{
	return m_pCity ? new CyPlot(m_pCity->getCityIndexPlot(iIndex)) : nullptr;
}
...
{
	return m_pCity ? m_pCity->getProductionUnitAI() : NO_UNITAI;
}
...
{
	return m_pCity ? m_pCity->getName().c_str() : L"";
}

std::wstring CyCity::getNameForm(int iForm)
{
	return m_pCity ? m_pCity->getName((unsigned int)iForm).c_str() : L"";
}
...
{
	if (m_pCity)
		m_pCity->setName(szNewValue.c_str(), bFound);
}
...
{
	return m_pCity ? new CyCity(m_pCity->getTradeCity(iIndex)) : nullptr;
}
...
void CyCity::setWallOverridePoints(const pybind11::tuple& kPoints)
{
	if (!m_pCity)
		return;
...
pybind11::tuple CyCity::getWallOverridePoints() const
{
	pybind11::tuple tup{};
	if (m_pCity)
	{
		...
		for(i=0;i<pointsVec.size();i++)
			tup += pybind11::make_tuple(pointsVec[i].first, pointsVec[i].second);
	}
