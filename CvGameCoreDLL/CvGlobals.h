...
#include "GiganticMapsOptimisationsLib/EnhancedDLLConfig.h"
...
public:
	...
	// singleton accessor
	DllExport static CvGlobals& getInstance();
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	std::unique_ptr<GiganticMapsOptimisationsLib::GameContext> enhancedDLLGameContext;
	// fortsnek: We'll stick the enhanced DLL config here rather than access from CvDLLUtilityIFaceBase so that users don't need extra indirections.
	EnhancedDLLConfig enhancedDLLConfig{};
#endif
	
	// fortsnek: Called by the engine, when the DLL may or may not have enhanced features.
	DllExportForInterface void setEnhancedDLLConfig(EnhancedDLLConfig);

	// fortsnek: We'll call this when exiting the game interface.
	DllExportForInterface void clearGiganticMapsOptimisationsLibContext();
	...
	int getInfoTypeForString(const CvString& szType, bool hideAssert = false) const;
	...
	DllExportForInterface CvUnitInfo& getUnitInfo(UnitTypes eUnitNum);
	...
	DllExportForInterface CvYieldInfo& getYieldInfo(YieldTypes eYieldNum);

	std::vector<CvCommerceInfo*>& getCommerceInfo();
	DllExportForInterface CvCommerceInfo& getCommerceInfo(CommerceTypes eCommerceNum);
	...
	DllExportForInterface int getNumProcessInfos();
	std::vector<CvProcessInfo*>& getProcessInfo();
	DllExportForInterface CvProcessInfo& getProcessInfo(ProcessTypes e);

	int getNumVoteInfos();
	std::vector<CvVoteInfo*>& getVoteInfo();
	CvVoteInfo& getVoteInfo(VoteTypes e);

	DllExportForInterface int getNumProjectInfos();
	std::vector<CvProjectInfo*>& getProjectInfo();
	DllExportForInterface CvProjectInfo& getProjectInfo(ProjectTypes e);

	DllExportForInterface int getNumBuildingClassInfos();
	...
	DllExportForInterface CvBuildingInfo& getBuildingInfo(BuildingTypes eBuildingNum);
	...
	DllExportForInterface int getNumUnitClassInfos();
	std::vector<CvUnitClassInfo*>& getUnitClassInfo();
	DllExportForInterface CvUnitClassInfo& getUnitClassInfo(UnitClassTypes eUnitClassNum);
	...
	DllExportForInterface int getNumReligionInfos();
	std::vector<CvReligionInfo*>& getReligionInfo();
	DllExportForInterface CvReligionInfo& getReligionInfo(ReligionTypes eReligionNum);

	DllExportForInterface int getNumCorporationInfos();
	std::vector<CvCorporationInfo*>& getCorporationInfo();
	DllExportForInterface CvCorporationInfo& getCorporationInfo(CorporationTypes eCorporationNum);
	...
	DllExportForInterface CvEmphasizeInfo& getEmphasizeInfo(EmphasizeTypes eEmphasizeNum);
	...
	DllExportForInterface int getMOVE_DENOMINATOR();
...
// fortsnek: DllExporting this anyway. Why not.
DllExportForInterface extern CvGlobals gGlobals;	// for debugging
