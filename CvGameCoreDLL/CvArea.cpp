...
int CvArea::countHasCorporation(CorporationTypes eCorporation, PlayerTypes eOwner) const
{
	...
	for (int iI = 0; iI < MAX_PLAYERS; ++iI)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			...
			{
				int iLoop;
				for (CvCity* pLoopCity = GET_PLAYER((PlayerTypes)iI).firstCity(&iLoop); nullptr != pLoopCity; pLoopCity = GET_PLAYER((PlayerTypes)iI).nextCity(&iLoop))
				{
					if (pLoopCity->area()->getID() == getID())
					{
...
int CvArea::getUnitsPerPlayer(PlayerTypes eIndex) const												
{
	...
	FAssertMsg(std::to_underlying(eIndex) < MAX_PLAYERS, "eIndex is expected to be < MAX_PLAYERS");
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	gGlobals.enhancedDLLGameContext->onPlayerAreaCityCountChanged(*this, eIndex, m_aiCitiesPerPlayer[eIndex] - iChange, m_aiCitiesPerPlayer[eIndex]);
	gGlobals.enhancedDLLGameContext->onGlobalAreaCityCountChanged(*this, m_iNumCities - iChange, m_iNumCities);
#endif
}
...
//#else
//void CvArea::setBestFoundValueIsPositive(PlayerTypes eIndex, bool isPositive)
//{
//	FAssertMsg(eIndex >= 0, "eIndex is expected to be >= 0");
//	FAssertMsg(std::to_underlying(eIndex) < MAX_PLAYERS, "eIndex is expected to be < MAX_PLAYERS");
//	// We re-use this variable so that the value loaded in from a save would also work.
//	m_aiBestFoundValue[eIndex] = isPositive ? 1 : 0;
//}
//#endif
//bool CvArea::getBestFoundValueIsPositive(PlayerTypes eIndex) const
//{
//	FAssertMsg(eIndex >= 0, "eIndex is expected to be >= 0");
//	FAssertMsg(std::to_underlying(eIndex) < MAX_PLAYERS, "eIndex is expected to be < MAX_PLAYERS");
//	return m_aiBestFoundValue[eIndex] > 0;
//}
...
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be >= 0");
	FAssertMsg(std::to_underlying(eIndex) < MAX_TEAMS, "eIndex is expected to be < MAX_TEAMS");
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (iChange)
		gGlobals.enhancedDLLGameContext->onTeamChange(GiganticMapsOptimisationsLib::EGameTeamChangeEvent::AreaBorderObstacle, eIndex, eIndex);
#endif
}
...
int CvArea::getYieldRateModifier(PlayerTypes eIndex1, YieldTypes eIndex2) const
{
	...
	FAssertMsg(std::to_underlying(eIndex1) < MAX_PLAYERS, "eIndex1 is expected to be < MAX_PLAYERS");
...
void CvArea::changeYieldRateModifier(PlayerTypes eIndex1, YieldTypes eIndex2, int iChange)
{
	...
	if (iChange != 0)
	{
		...
		if (eIndex2 == YIELD_COMMERCE
			...
			|| (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::GeneralFixes] ? eIndex2 == YIELD_PRODUCTION : false)
#endif
			)
		{
