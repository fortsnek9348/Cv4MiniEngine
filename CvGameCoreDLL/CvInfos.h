...
public:
	...
	std::wstring pyGetDescriptionForm(unsigned int uiForm) { return getDescription(uiForm); }	// Exposed to Python
	...
	DllExport const wchar_t* getDescription(unsigned int uiForm = 0) const;
...
public:
	...
	DllExport void setHotKeyDescription(const wchar_t* szHotKeyDescKey, const wchar_t* szHotKeyAltDescKey, const wchar_t* szHotKeyString);
...
public:
	...
	DllExport const wchar_t* getCivilopedia() const;
	DllExport const wchar_t* getHelp() const;
	DllExport const wchar_t* getStrategy() const;
...
public:
	...
	DllExportForInterface int getCombat() const;				// Exposed to Python
...
public:
	...
	const wchar_t* getWeLoveTheKing();
...
public:
	...
	DllExportForInterface int getBuildingClassType() const;				// Exposed to Python
...
public:
	...
	std::wstring pyGetShortDescription(unsigned int uiForm) { return getShortDescription(uiForm); }				// Exposed to Python
	DllExport const wchar_t* getShortDescription(unsigned int uiForm = 0);
	DllExport const wchar_t* getShortDescriptionKey() const;				// Exposed to Python
	std::wstring pyGetShortDescriptionKey() { return getShortDescriptionKey(); }				// Exposed to Python
	
	std::wstring pyGetAdjective(unsigned int uiForm) { return getAdjective(uiForm);  }	// Exposed to Python
	DllExport const wchar_t* getAdjective(unsigned int uiForm = 0);				
	DllExport const wchar_t* getAdjectiveKey() const;				// Exposed to Python
...
public:
	...
	DllExportForInterface int getNumTurnIncrements() const;				// Exposed to Python

	DllExportForInterface GameTurnInfo& getGameTurnInfo(int iIndex) const;				// Exposed to Python
...
public:
	...
	DllExportForInterface int getChar() const;								// Exposed to Python
...
public:
	...
	int getEnableCode(unsigned int uiCode) const;
	int getDisableCode(unsigned int uiCode) const;
...
public:
	...
	const TCHAR* getArtDefineTag() const;				// Exposed to Python
	void setArtDefineTag(std::string_view szVal);
...
public:
	...
	DllExportForInterface int getProductionToCommerceModifier(int i) const;				// Exposed to Python
...
public:
	...
	// fortsnek: Used for city billboards.
	DllExportForInterface int getChar() const;								// Exposed to Python
	void setChar(int i);			
	// fortsnek: Used for city billboards.
	DllExportForInterface int getHolyCityChar() const;				// Exposed to Python
...
public:
	...
	DllExportForInterface int getChar() const;								// Exposed to Python
	void setChar(int i);			
	DllExportForInterface int getHeadquarterChar() const;				// Exposed to Python
...
public:
	...
	public:
		...
		const CvString getFeatureDummyNodeName(const CvString &tagName) const
		{
			...
			{
				if(m_aFeatureDummyNodes[i].getTagName().CompareNoCase(tagName.c_str()) == 0)
		...
		const CvString getFeatureDummyTag(const CvString &nodeName) const
		{
			for(int i=0;i<(int)m_aFeatureDummyNodes.size();i++)
			{
				if(m_aFeatureDummyNodes[i].getNodeName().CompareNoCase(nodeName.c_str()) == 0)
...
public:
	...
	const wchar_t* getGender() const { return m_szGender.c_str(); }

	void setPlural(const wchar_t* szPlural) { m_szPlural = szPlural; }
	const wchar_t* getPlural() const { return m_szPlural.c_str(); }
	...
	// fortsnek: Exporting this so that the engine doesn't need to instantiate this class.
	DllExport static int NUM_LANGUAGES;
...
public:
	...
	DllExport virtual ~CvDiplomacyTextInfo();

	// note - Response member vars allocated by CvXmlLoadUtility  
	DllExport void init(int iNum);			
	DllExport void uninit();			

	DllExport const Response& getResponse(int iNum) const;	// Exposed to Python
	DllExport int getNumResponses() const;															// Exposed to Python
...
public:
	...
	DllExport void setPath(const TCHAR* szVal);
	DllExport float getUpdateRate() const;
	DllExport void setUpdateRate(float fUpdateRate);
	DllExport bool isProjectile() const;
	DllExport float getProjectileSpeed() const;
	DllExport float getProjectileArc() const;
	DllExport bool isSticky() const;
	bool read(CvXMLLoadUtility* pXML);
...
private:
	CvString m_szPath;
	[[maybe_unused]] float m_fUpdateRate;
};
