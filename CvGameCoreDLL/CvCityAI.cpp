...
void CvCityAI::AI_reset()
{
	...
	FAssertMsg(m_pbEmphasize == nullptr, "m_pbEmphasize not nullptr!!!");
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (!gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::CvCity_doTurnParallel]) // Else, we'll do this later, in a parallel loop.
#endif
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
// Be very very careful what you put inside this thing!
void CvCityAI::AI_doTurnParallel()
{
	AI_updateRouteToCity();
}
#endif
...
int CvCityAI::AI_specialistValue(SpecialistTypes eSpecialist, bool bAvoidGrowth, bool bRemove)
{
	...
	for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		aiYields[iI] = static_cast<short>(GET_PLAYER(getOwnerINLINE()).specialistYield(eSpecialist, ((YieldTypes)iI)));
	}
	...
	for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		aiCommerceYields[iI] = static_cast<short>(GET_PLAYER(getOwnerINLINE()).specialistCommerce(eSpecialist, ((CommerceTypes)iI)));
	}
			...
			for (iJ = 0; iJ < GC.getNumReligionInfos(); iJ++)
            {
                	...
                	&& ((iCurrentEra < iTotalEras / 2) || GC.getGameINLINE().countReligionLevels(eReligion) >= 10))
                {
					...
					if (eGreatPeopleUnit != NO_UNIT)
					{
						...
						for (int iK = 0; iK < shrineBuildingCount; iK++)
						{
							int eBuilding = (int) GC.getGameINLINE().getShrineBuilding(iK, eReligion);
...
void CvCityAI::AI_chooseProduction()
{
	...
	[[maybe_unused]] bool bHasMetHuman;
	bool bLandWar;
	bool bAssault;
	bool bDefenseWar;
	bool bPrimaryArea;
	...
	[[maybe_unused]] bool bBigCultureCity = false;
    ...
    int iNeededSeaWorkers = (pWaterArea == nullptr) ? 0 : AI_neededSeaWorkers();
    ...
    int iNumWaterAreaCitySites = (pWaterArea == nullptr) ? 0 : kPlayer.AI_getNumAdjacentAreaCitySites(pWaterArea->getID(), getArea(), iWaterAreaBestFoundValue);
	...
	if (isBarbarian())
	{
		...
		if ((pWaterArea != nullptr) && (iWaterPercent > 30))
		{
    ...
    if (bMaybeWaterArea)
	{
		if (kPlayer.AI_getNumTrainAIUnits(UNITAI_ATTACK_SEA) + kPlayer.AI_getNumTrainAIUnits(UNITAI_PIRATE_SEA) + kPlayer.AI_getNumTrainAIUnits(UNITAI_RESERVE_SEA) < 3)
		{
			if ((bMaybeWaterArea && kPlayer.AI_getWaterDanger(plot(), 4) > 0)
				|| (pWaterArea != nullptr && bPrimaryArea && kPlayer.AI_countNumAreaHostileUnits(pWaterArea, true, false, false, false) > 0))
			{
				if (AI_chooseUnit(UNITAI_ATTACK_SEA))
				{
					return;
    ...
    {
    	if (AI_chooseBuilding(BUILDINGFOCUS_EXPERIENCE, (std::to_underlying(kPlayer.getCurrentEra()) > 1) ? 0 : 7, 33))
		{
			return;
		}
    }
    ...
    if (iUnitCostPercentage < (iMaxUnitSpending + 10))
	{
		...
		if (nullptr != pWaterArea && (bAssault))
		{
	...
	if (!bAlwaysPeace && !(bLandWar || bAssault) && (kPlayer.AI_isDoStrategy(AI_STRATEGY_OWABWNW) || (GC.getGame().getSorenRandNum(12, "AI consider Nuke") == 0)))
	{
		...
		{
			if ((pWaterArea != nullptr) && (GC.getGame().getSorenRandNum(4, "AI train nuke carrier")))
			{
	...
	//Arr.
	if ((pWaterArea != nullptr) && !bLandWar && !bAssault)
	{
	...
	if (!bLandWar)
	{
		if ((pWaterArea != nullptr) && (iWaterPercent > 40))
		{
...
//	}
	...
	if (plot()->plotCheck(PUF_isUnitAIType, UNITAI_CITY_COUNTER, -1, getOwnerINLINE()) == nullptr)
	{
		if (AI_chooseUnit(UNITAI_CITY_COUNTER))
		{
			return;
...
//#else
	//if (!bFinancialTrouble && ((bPrimaryArea) ? GET_PLAYER(getOwnerINLINE()).isBestFoundValuePositive() : area()->getBestFoundValue(getOwnerINLINE()) > 0))
//#endif
	{
...
//#else
				//if (bPrimaryArea && (GET_PLAYER(getOwnerINLINE()).isBestFoundValuePositive()) && (pWaterArea->getNumTiles() > 300))
//#endif
				{
	...
	for (iI = 0; iI < NUM_UNITAI_TYPES; iI++)
	{
		if (aiUnitAIVal[iI] > 0)
		{
			if (bAsync)
			{
				aiUnitAIVal[iI] += GC.getASyncRand().get(static_cast<unsigned short>(iMilitaryWeight), "AI Best UnitAI ASYNC");
			}
			else
			{
				aiUnitAIVal[iI] += GC.getGameINLINE().getSorenRandNum(static_cast<unsigned short>(iMilitaryWeight), "AI Best UnitAI");
			}
...
BuildingTypes CvCityAI::AI_bestBuildingThreshold(int iFocusFlags, int iMaxTurns, int iMinThreshold, bool bAsync, AdvisorTypes eIgnoreAdvisor)
{
	...
	{
		int iBestTurnsLeft = iMaxTurns > 0 ? iMaxTurns : INT_MAX;
							...
							if (iValue > 0)
							{
								...
								if (isWorldWonderClass((BuildingClassTypes)iI))
								{
									if (iProductionRank <= std::min(3, ((GET_PLAYER(getOwnerINLINE()).getNumCities() + 2) / 3)))
									{
										if (bAsync)
										{
											iTempValue = GC.getASyncRand().get(static_cast<unsigned short>(GC.getLeaderHeadInfo(getPersonalityType()).getWonderConstructRand()), "Wonder Construction Rand ASYNC");
										}
										else
										{
											iTempValue = GC.getGameINLINE().getSorenRandNum(static_cast<unsigned short>(GC.getLeaderHeadInfo(getPersonalityType()).getWonderConstructRand()), "Wonder Construction Rand");
										}
...
int CvCityAI::AI_buildingValueThreshold(BuildingTypes eBuilding, int iFocusFlags, int iThreshold)
{
	...
	for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		aiYieldRank[iI] = INT_MAX;
	}

	int aiCommerceRank[NUM_COMMERCE_TYPES];
	for (iI = 0; iI < NUM_COMMERCE_TYPES; iI++)
	{
		aiCommerceRank[iI] = INT_MAX;
	}
		...
		if ((iFocusFlags == 0) || (iValue > 0) || (iPass == 0))
		{
			...
			else
			{
				...
				if (iFocusFlags & BUILDINGFOCUS_GOLD)
				{
					...
					if (INT_MAX == aiCommerceRank[COMMERCE_GOLD])
					{
			...
			else
			{
				...
				if (iFocusFlags & BUILDINGFOCUS_RESEARCH)
				{
					...
					if (iTempValue != 0)
					{
						if (INT_MAX == aiCommerceRank[COMMERCE_RESEARCH])
						{
				...
				if (iFocusFlags & BUILDINGFOCUS_CULTURE)
				{
					...
					if (iTempValue != 0)
					{
						if (INT_MAX == aiCommerceRank[COMMERCE_CULTURE])
						{
				...
				if (iFocusFlags & BUILDINGFOCUS_ESPIONAGE || (GC.getGameINLINE().isOption(GAMEOPTION_NO_ESPIONAGE) && (iFocusFlags & BUILDINGFOCUS_CULTURE)))
				{
					...
					if (iTempValue != 0)
					{
						if (INT_MAX == aiCommerceRank[COMMERCE_ESPIONAGE])
						{
...
//	int iErasUntilObsolete = INT_MAX;
...
// fortsnek: Make this const by moving the update to AI_doturn. I assume that won't change AI much.
//#ifndef ENABLE_GAMECOREDLL_ENHANCEMENTS_CVCITY_NONTRIVIAL_CONST
...
//#else
//int CvCityAI::AI_neededFloatingDefenders() const
//{
//	return m_iNeededFloatingDefenders;
//}
//#endif
...
int CvCityAI::AI_neededAirDefenders()
{
	...
	for (int iDX = -(iRange); iDX <= iRange; iDX++)
	{
		for (int iDY = -(iRange); iDY <= iRange; iDY++)
		{
			...
			if ((pLoopPlot != nullptr) && pLoopPlot->isOwned() && (pLoopPlot->getTeam() != getTeam()))
			{
				iOtherTeam++;
...
void CvCityAI::AI_updateRouteToCity()
{
	...
	// NOTE: This function is called in parallel if ENABLE_GAMECOREDLL_ENHANCEMENTS!!
...
bool CvCityAI::AI_isEmphasize(EmphasizeTypes eIndex)
{
	...
	FAssertMsg(m_pbEmphasize != nullptr, "m_pbEmphasize is not expected to be equal with nullptr");
...
void CvCityAI::AI_updateBestBuild()
{
	...
	[[maybe_unused]] int iWorkedFood = 0;
	int iWorkableFood = 0;
			...
			if (iI != CITY_HOME_PLOT)
			{
				...
				if (nullptr != pLoopPlot && pLoopPlot->getWorkingCity() == this)
				{
					if (m_aeBestBuild[iI] != NO_BUILD)
					{
						for (iJ = 0; iJ < NUM_YIELD_TYPES; iJ++)
						{
							aiYields[iJ] = static_cast<short>(pLoopPlot->getYieldWithBuild(m_aeBestBuild[iI], (YieldTypes)iJ, true));
						}
						
						iValue = AI_yieldValue(aiYields, nullptr, false, false, false, false, true, true);
						aiValues[iI] = iValue;
					...
					if (!pLoopPlot->isBeingWorked())
					{
						for (iJ = 0; iJ < NUM_YIELD_TYPES; iJ++)
						{
							aiYields[iJ] = static_cast<short>(pLoopPlot->getYield((YieldTypes)iJ));
						}
...
void CvCityAI::AI_doHurry(bool bForce)
{
	...
	for (iI = 0; iI < GC.getNumHurryInfos(); iI++)
	{
		if (canHurry((HurryTypes)iI))
		{
			...
			{
				if (plot()->plotCheck(PUF_isUnitAIType, UNITAI_SETTLE, -1, getOwnerINLINE()) != nullptr)
				{
...
//#else
//						if (area()->getBestFoundValueIsPositive(getOwnerINLINE()))
//#endif
						{
							iMinTurns = std::min(iMinTurns, 5);
							bEssential = true;
...
{
	if (plot()->plotCheck(PUF_isUnitAIType, UNITAI_CITY_SPECIAL, -1, getOwnerINLINE()) == nullptr)
	{
		if (AI_chooseUnit(UNITAI_CITY_SPECIAL))
		{
			return true;
							...
							if (eLoopUnit != NO_UNIT)
							{
								...
								if (kUnitInfo.getCorporationSpreads(eCorporation) > 0)
								{
									if (canTrain(eLoopUnit))
									{
										...
										int iTotalCount = 0;
										int iPlotCount = 0;
										for (CvUnit* pLoopUnit = kPlayer.firstUnit(&iLoop); pLoopUnit != nullptr; pLoopUnit = kPlayer.nextUnit(&iLoop))
										{
...
bool CvCityAI::AI_addBestCitizen(bool bWorkers, bool bSpecialists, int* piBestPlot, SpecialistTypes* peBestSpecialist)
{
	...
	if (bWorkers)
	{
		for (int iI = 0; iI < NUM_CITY_PLOTS; iI++)
		{
			if (iI != CITY_HOME_PLOT)
			{
				...
				//	CvString::format("%s: (%i, %i): %i, isWorkingPlot %i", __FUNCTION__, getX_INLINE(), getY_INLINE(), iI, isWorkingPlot(iI)).c_str(), true);
				if (!isWorkingPlot(iI))
				{
					...
					{
						//GC.getDLLIFace()->logMsg("GameStateLog.txt",
						//	CvString::format("%s: (%i, %i): %i, canWork %i", __FUNCTION__, getX_INLINE(), getY_INLINE(), iI, canWork(pLoopPlot)).c_str(), true);
						if (canWork(pLoopPlot))
						{
							int iValue = AI_plotValue(pLoopPlot, bAvoidGrowth, /*bRemove*/ false, /*bIgnoreFood*/ false, bIgnoreGrowth);
							//GC.getDLLIFace()->logMsg("GameStateLog.txt",
							//	CvString::format("%s: (%i, %i): %i, value %i", __FUNCTION__, getX_INLINE(), getY_INLINE(), iI, iValue).c_str(), true);
	...
	if (eBestSpecialist != NO_SPECIALIST)
	{
		...
		if (piBestPlot != nullptr)
		{
			FAssert(peBestSpecialist != nullptr);
			...
			if (!bAvoidGrowth)
			{
				...
				if (bWorkerOptimization || (!bIgnoreGrowth))// && !bEmphasizeFood))
				{
					...
					if (iHappinessLevel < 10)
					{
						...
						{
							int iApproxTurnsToGrow = (iNewFoodPerTurn > 0) ? ((iFoodToGrow - iFoodLevel) / iNewFoodPerTurn) : INT_MAX;
					...
					// fortsnek: warning C4456: declaration of 'iPopToGrow' hides previous local declaration.
					//           I've renamed, but it looks like the iPopToGrow above is used below after this scope.
					int iPopToGrow2 = std::max(0, iHappinessLevel);
					...
					if (!bEmphasizeFood)
					{
						iPopToGrow2 = std::min(iPopToGrow2, iGoodTiles + ((bRemove) ? 1 : 0));
					}

					// if we have growth pontential, fill food bar to 85%
					bool bFillingBar = false;
					if (iPopToGrow2 == 0 && iHappinessLevel >= 0 && iGoodTiles >= 0 && iHealthLevel >= 0)
					{
						if (!bBarFull)
						{
							if (AI_specialYieldMultiplier(YIELD_PRODUCTION) < 50)
					...
					if (getPopulation() < 3)
					{
						iPopToGrow2 = std::max(iPopToGrow2, 3 - getPopulation());
						iPopToGrow2 += 2;
					}

					// if we want to grow
					if (iPopToGrow2 > 0 || bFillingBar)
					{
						...
						if (iPopToGrow2 < 1 || bFillingBar)
							iFactorPopToGrow = 20 - (10 * (iFoodLevel + iFoodPerTurn + aiYields[YIELD_FOOD])) / iFoodToGrow;
						else if (iPopToGrow2 < 7)
							iFactorPopToGrow = 17 + 3 * iPopToGrow2;
						else
							iFactorPopToGrow = 41;
						...
						int iHighGrowthThreshold = 2 + std::max(std::max(0, 5 - getPopulation()), (iPopToGrow2 + 1) / 2);
						if (bEmphasizeFood)
						{
...
int CvCityAI::AI_plotValue(CvPlot* pPlot, bool bAvoidGrowth, bool bRemove, bool bIgnoreFood, bool bIgnoreGrowth, bool bIgnoreStarvation)
{
	...
	for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
	{
		aiYields[iI] = static_cast<short>(pPlot->getYield((YieldTypes)iI));
	}
	...
	int iYieldValue = (AI_yieldValue(aiYields, nullptr, bAvoidGrowth, bRemove, bIgnoreFood, bIgnoreGrowth, bIgnoreStarvation) * 100);
	...
	if (eFinalImprovement != NO_IMPROVEMENT)
	{
		for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
		{
			...
			aiYields[iI] += static_cast<short>(iYieldDiff);
		}
		int iFinalYieldValue = (AI_yieldValue(aiYields, nullptr, bAvoidGrowth, bRemove, bIgnoreFood, bIgnoreGrowth, bIgnoreStarvation) * 100);
...
// Improved worker AI provided by Blake - thank you!
void CvCityAI::AI_bestPlotBuild(CvPlot* pPlot, int* piBestValue, BuildTypes* peBestBuild, int iFoodPriority, int iProductionPriority, int iCommercePriority, bool bChop, int iHappyAdjust, [[maybe_unused]] int iHealthAdjust, int iFoodChange)
{
	PROFILE_FUNC();
	...
	if (!bHasBonusImprovement)
	{
        ...
        if ((pPlot->isIrrigated()) || (pPlot->isFreshWater() && pPlot->canHavePotentialIrrigation()))
        {
            for (iI = 0; iI < NUM_DIRECTION_TYPES; iI++)
            {
                ...
                if ((pAdjacentPlot != nullptr) && (pAdjacentPlot->getOwner() == getOwner()) && (pAdjacentPlot->isCityRadius()))
                {
                    if (!pAdjacentPlot->isFreshWater())
                    {
                        ...
                        {
                            CvPlot* eBestIrrigationPlot = nullptr;
                            ...
                            for (iJ = 0; iJ < NUM_DIRECTION_TYPES; iJ++)
                            {
                                ...
                                if ((pAdjacentPlot2 != nullptr) && (pAdjacentPlot2->getOwner() == getOwner()))
                                {
				...
				if (iValue >= 0)
				{
					...
					if (iValue > 0)
					{
						...
						else if (aiFinalYields[YIELD_FOOD] < GC.getFOOD_CONSUMPTION_PER_POPULATION())
						{
							...
							{
								if (iFoodPriority < 100 || std::to_underlying(GET_PLAYER(getOwnerINLINE()).getCurrentEra()) < 2)
								{
...
int CvCityAI::AI_countGoodTiles(bool bHealthy, bool bUnworkedOnly, int iThreshold, [[maybe_unused]] bool bWorkerOptimization)
{
    CvPlot* pLoopPlot;
    int iI;
    int iCount;
...
int CvCityAI::AI_getCityImportance([[maybe_unused]] bool bEconomy, bool bMilitary)
{
    int iValue = 0;
    if (GET_PLAYER(getOwnerINLINE()).AI_isDoStrategy(AI_STRATEGY_CULTURE2))
    {
        ...
        if (pLoopPlot != nullptr)
        {
            if (iImportance > 0)
            {
                if (pLoopPlot->getOwnerINLINE() == getOwnerINLINE())
                {
                    ...
                    if ((pWorkingCity != this) && (pWorkingCity != nullptr))
                    {
...
int CvCityAI::AI_buildingSpecialYieldChangeValue(BuildingTypes eBuilding, YieldTypes eYield)
{
    ...
    if (iYieldChange > 0)
    {
		[[maybe_unused]] int iWaterCount = 0;
        for (iI = 0; iI < NUM_CITY_PLOTS; iI++)
        {
            if (iI != CITY_HOME_PLOT)
            {
...
int CvCityAI::AI_countNumBonuses(BonusTypes eBonus, bool bIncludeOurs, bool bIncludeNeutral, int iOtherCultureThreshold, [[maybe_unused]] bool bLand, bool bWater)
{
    CvPlot* pLoopPlot;
...
int CvCityAI::AI_cityThreat([[maybe_unused]] bool bDangerPercent)
{
	PROFILE_FUNC();
	int iValue = 0;
...
void CvCityAI::AI_updateWorkersNeededHere()
{
	...
	int iWorstWorkedPlotValue = INT_MAX;
	int iBestUnworkedPlotValue = 0;
		...
		if (nullptr != pLoopPlot && pLoopPlot->getWorkingCity() == this)
		{
			if (pLoopPlot->getArea() == getArea())
			{
				...
				if (iI != CITY_HOME_PLOT)
				{
					...
					if (pLoopPlot->isBeingWorked())
					{
						int iPlotValue = AI_yieldValue(aiYields, nullptr, false, false, false, false, true, true);
	...
	if (iWorstWorkedPlotValue != INT_MAX)
	{
		...
		for (int iI = 0; iI < NUM_CITY_PLOTS; iI++)
		{
			if (iI != CITY_HOME_PLOT)
			{
				...
				if (nullptr != pLoopPlot && pLoopPlot->getWorkingCity() == this && pLoopPlot->getArea() == getArea())
				{
					if (AI_getBestBuild(iI) != NO_BUILD)
					{
						for (int iJ = 0; iJ < NUM_YIELD_TYPES; iJ++)
