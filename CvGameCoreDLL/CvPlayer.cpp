...
#ifdef _WIN32
// fortsnek: For CvPlayer total time played
...
CvPlayer::CvPlayer()
{
	...
	m_paiBonusExport = nullptr;
	m_paiBonusImport = nullptr;
	m_paiImprovementCount = nullptr;
	m_paiFreeBuildingCount = nullptr;
	m_paiExtraBuildingHappiness = nullptr;
	m_paiExtraBuildingHealth = nullptr;
	m_paiFeatureHappiness = nullptr;
	m_paiUnitClassCount = nullptr;
	m_paiUnitClassMaking = nullptr;
	m_paiBuildingClassCount = nullptr;
	m_paiBuildingClassMaking = nullptr;
	m_paiHurryCount = nullptr;
	m_paiSpecialBuildingNotRequiredCount = nullptr;
	m_paiHasCivicOptionCount = nullptr;
	m_paiNoCivicUpkeepCount = nullptr;
	m_paiHasReligionCount = nullptr;
	m_paiHasCorporationCount = nullptr;
	m_paiUpkeepCount = nullptr;
	m_paiSpecialistValidCount = nullptr;

	m_pabResearchingTech = nullptr;
	m_pabLoyalMember = nullptr;

	m_paeCivics = nullptr;

	m_ppaaiSpecialistExtraYield = nullptr;
	m_ppaaiImprovementYieldChange = nullptr;
...
void CvPlayer::uninit()
{
	...
	if (m_ppaaiSpecialistExtraYield != nullptr)
	{
	...
	if (m_ppaaiImprovementYieldChange != nullptr)
	{
...
void CvPlayer::reset(PlayerTypes eID, bool bConstructorCall)
{
	...
	if (!bConstructorCall)
	{
		...
		FAssertMsg(m_paiBonusExport==nullptr, "about to leak memory, CvPlayer::m_paiBonusExport");
		m_paiBonusExport = new int [GC.getNumBonusInfos()];
		FAssertMsg(m_paiBonusImport==nullptr, "about to leak memory, CvPlayer::m_paiBonusImport");
		...
		FAssertMsg(m_paiImprovementCount==nullptr, "about to leak memory, CvPlayer::m_paiImprovementCount");
		...
		FAssertMsg(m_paiFreeBuildingCount==nullptr, "about to leak memory, CvPlayer::m_paiFreeBuildingCount");
		m_paiFreeBuildingCount = new int [GC.getNumBuildingInfos()];
		FAssertMsg(m_paiExtraBuildingHappiness==nullptr, "about to leak memory, CvPlayer::m_paiExtraBuildingHappiness");
		m_paiExtraBuildingHappiness = new int [GC.getNumBuildingInfos()];
		FAssertMsg(m_paiExtraBuildingHealth==nullptr, "about to leak memory, CvPlayer::m_paiExtraBuildingHealth");
		...
		FAssertMsg(m_paiFeatureHappiness==nullptr, "about to leak memory, CvPlayer::m_paiFeatureHappiness");
		...
		FAssertMsg(m_paiUnitClassCount==nullptr, "about to leak memory, CvPlayer::m_paiUnitClassCount");
		m_paiUnitClassCount = new int [GC.getNumUnitClassInfos()];
		FAssertMsg(m_paiUnitClassMaking==nullptr, "about to leak memory, CvPlayer::m_paiUnitClassMaking");
		...
		FAssertMsg(m_paiBuildingClassCount==nullptr, "about to leak memory, CvPlayer::m_paiBuildingClassCount");
		m_paiBuildingClassCount = new int [GC.getNumBuildingClassInfos()];
		FAssertMsg(m_paiBuildingClassMaking==nullptr, "about to leak memory, CvPlayer::m_paiBuildingClassMaking");
		...
		FAssertMsg(m_paiHurryCount==nullptr, "about to leak memory, CvPlayer::m_paiHurryCount");
		...
		FAssertMsg(m_paiSpecialBuildingNotRequiredCount==nullptr, "about to leak memory, CvPlayer::m_paiSpecialBuildingNotRequiredCount");
		...
		FAssertMsg(m_paiHasCivicOptionCount==nullptr, "about to leak memory, CvPlayer::m_paiHasCivicOptionCount");
		m_paiHasCivicOptionCount = new int[GC.getNumCivicOptionInfos()];
		FAssertMsg(m_paiNoCivicUpkeepCount==nullptr, "about to leak memory, CvPlayer::m_paiNoCivicUpkeepCount");
		m_paiNoCivicUpkeepCount = new int[GC.getNumCivicOptionInfos()];
		FAssertMsg(m_paeCivics==nullptr, "about to leak memory, CvPlayer::m_paeCivics");
		...
		FAssertMsg(m_paiHasReligionCount==nullptr, "about to leak memory, CvPlayer::m_paiHasReligionCount");
		...
		FAssertMsg(m_paiHasCorporationCount==nullptr, "about to leak memory, CvPlayer::m_paiHasReligionCount");
		...
		FAssertMsg(m_pabResearchingTech==nullptr, "about to leak memory, CvPlayer::m_pabResearchingTech");
		...
		FAssertMsg(m_pabLoyalMember==nullptr, "about to leak memory, CvPlayer::m_pabLoyalMember");
		...
		FAssertMsg(m_paiUpkeepCount==nullptr, "about to leak memory, CvPlayer::m_paiUpkeepCount");
		...
		FAssertMsg(m_paiSpecialistValidCount==nullptr, "about to leak memory, CvPlayer::m_paiSpecialistValidCount");
		...
		FAssertMsg(0 < GC.getNumSpecialistInfos(), "GC.getNumSpecialistInfos() is not greater than zero but it is used to allocate memory in CvPlayer::reset");
		FAssertMsg(m_ppaaiSpecialistExtraYield==nullptr, "about to leak memory, CvPlayer::m_ppaaiSpecialistExtraYield");
		...
		FAssertMsg(m_ppaaiImprovementYieldChange==nullptr, "about to leak memory, CvPlayer::m_ppaaiImprovementYieldChange");
	...
	for (iI = 0; iI < GC.getNumUnitClassInfos(); iI++)
	{
		...
		if (eLoopUnit != NO_UNIT)
		{
			if (canTrain(eLoopUnit))
			{
				...
				if (bValid)
				{
					iValue = AI_unitValue(eLoopUnit, eUnitAI, nullptr);
...
void CvPlayer::addFreeUnit(UnitTypes eUnit, UnitAITypes eUnitAI)
{
	...
	if (pStartingPlot != nullptr)
	{
		...
		{
			long lResult=0;
			gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "startHumansOnSameTile", nullptr, &lResult);
			if (lResult == 0)
			{
				if (!(GC.getUnitInfo(eUnit).isFound()))
				{
...
{
	//gDLL->logMemState(("maxStepDistance = " + std::to_string(GC.getMapINLINE().maxStepDistance())).c_str());
	//gDLL->logMemState(("getLandPlots = " + std::to_string(GC.getMapINLINE().getLandPlots())).c_str());
	//gDLL->logMemState(("getNumAreas = " + std::to_string(GC.getMapINLINE().getNumAreas())).c_str());
	...
	//const CvWorldInfo& worldInfo = GC.getWorldInfo(GC.getMapINLINE().getWorldSize());
	//int numPlots = GC.getMapINLINE().getLandPlots();
	//const int maxExpectedPlots = worldInfo.getGridWidth() * worldInfo.getGridHeight() *
	//	GC.getLandscapeInfo(GC.getActiveLandscapeID()).getPlotsPerCellX() *
	//	GC.getLandscapeInfo(GC.getActiveLandscapeID()).getPlotsPerCellY();
	//if (numPlots > maxExpectedPlots && numPlots > 128 * 80 * 2)
	//{
	//	static bool shown = false;
	//	if (!std::exchange(shown, true))
	//		gDLL->logMemState("WARNING: Map is much larger than a huge map. Artificially limiting startingPlotRange.");
	//	numPlots = maxExpectedPlots;
	//}
	//iRange *= (numPlots / (GC.getWorldInfo(GC.getMapINLINE().getWorldSize()).getTargetNumCities() * GC.getGameINLINE().countCivPlayersAlive()));

	// fortsnek: The stuff above is cubic in map width, and the stuff below is constant.
	//           Divide by map multiplier twice to get a linear scaling of the above.
	iRange /= NUM_CITY_PLOTS * GC.getInitCore().getWorldSizeMultiplier() * GC.getInitCore().getWorldSizeMultiplier();
...
bool CvPlayer::startingPlotWithinRange([[maybe_unused]] CvPlot* pPlot, [[maybe_unused]] PlayerTypes ePlayer, [[maybe_unused]] int iRange, [[maybe_unused]] int iPass) const
{
	PROFILE_FUNC();
...
int CvPlayer::findStartingArea() const
{
	...
	if (gDLL->getPythonIFace()->callFunction(gDLL->getPythonIFace()->getMapScriptModule(), "findStartingArea", argsList.makeFunctionArgs(), &result))
	{
		...
		{
			if (result == -1 || GC.getMapINLINE().getArea(result) != nullptr)
			{
				return result;
			}
			else
...
CvPlotGroup* CvPlayer::initPlotGroup(CvPlot* pPlot)
{
	...
	FAssertMsg(pPlotGroup != nullptr, "PlotGroup is not assigned a valid value");
...
void CvPlayer::acquireCity(CvCity* pOldCity, bool bConquest, bool bTrade, bool bUpdatePlotGroups)
{
	...
	if (bConquest)
	{
		...
		szName.Format(L"%ls (%ls)", pOldCity->getName().GetCString(), GET_PLAYER(pOldCity->getOwnerINLINE()).getName());
...
//	{
//		// BUG FIX: Eh... seems a bit risky to not update the plot groups, doesn't it?
//		//          Fixes capture of a barb city to have consistent logging between both plot group systems.
...
//	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::GeneralFixes])
//	{
//		// BUG FIX: Fixes capture of a barb city to have consistent logging between both plot group systems.
...
CvWString CvPlayer::getNewCityName() const
{
	...
	for (pNode = headCityNameNode(); (pNode != nullptr); pNode = nextCityNameNode(pNode))
	{
...
void CvPlayer::disbandUnit([[maybe_unused]] bool bAnnounce)
{
	CvUnit* pLoopUnit;
	CvUnit* pBestUnit;
...
const wchar_t* CvPlayer::getName(unsigned int uiForm) const
{
	...
	{
		return GC.getInitCore().getLeaderName(getID(), uiForm).c_str();
	}
}
...
const wchar_t* CvPlayer::getNameKey() const
{
	...
	{
		return GC.getInitCore().getLeaderNameKey(getID()).c_str();
	}
}


const wchar_t* CvPlayer::getCivilizationDescription(unsigned int uiForm) const
{
	...
	{
		return GC.getInitCore().getCivDescription(getID(), uiForm).c_str();
	}
}
...
const wchar_t* CvPlayer::getCivilizationDescriptionKey() const
{
	...
	{
		return GC.getInitCore().getCivDescriptionKey(getID()).c_str();
	}
}


const wchar_t* CvPlayer::getCivilizationShortDescription(unsigned int uiForm) const
{
	...
	{
		return GC.getInitCore().getCivShortDesc(getID(), uiForm).c_str();
	}
}
...
const wchar_t* CvPlayer::getCivilizationShortDescriptionKey() const
{
	...
	{
		return GC.getInitCore().getCivShortDescKey(getID()).c_str();
	}
}


const wchar_t* CvPlayer::getCivilizationAdjective(unsigned int uiForm) const
{
	...
	{
		return GC.getInitCore().getCivAdjective(getID(), uiForm).c_str();
	}
}
...
const wchar_t* CvPlayer::getCivilizationAdjectiveKey() const
{
	...
	{
		return GC.getInitCore().getCivAdjectiveKey(getID()).c_str();
	}
}
...
const wchar_t* CvPlayer::getStateReligionName(unsigned int uiForm) const
{
...
void CvPlayer::doTurn()
{
	...
	FORTSNEK_DIVERGENCE_CHECKPOINT("before");
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::CvCity_doTurnParallel])
	{
		// BY THE POWER OF MULTI CORE PROCESSING
		heck::parallelForEach(cities, [](CvCity* city) {
			city->doTurnParallel();
			});
	}
#endif
	...
	// fortsnek: Uncomment to test event UI
	//if (getID() == 0)
	//	trigger(*initTriggeredData((EventTriggerTypes)GC.getInfoTypeForString("EVENTTRIGGER_CLASSIC_LITERATURE")));
	doEvents();
...
#if ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (m_playerTradeNetworkSystem)
		m_playerTradeNetworkSystem->onTurn();
#endif

	CvEventReporter::getInstance().endPlayerTurn( GC.getGameINLINE().getGameTurn(),  getID());

	FORTSNEK_DIVERGENCE_CHECKPOINT("after");
}
	...
	for (int iPass = 0; iPass < 4; iPass++)
	{
		for(pLoopSelectionGroup = firstSelectionGroup(&iLoop); pLoopSelectionGroup != nullptr; pLoopSelectionGroup = nextSelectionGroup(&iLoop))
		{
			...
			case DOMAIN_IMMOBILE:
				...
				break;
			case NO_DOMAIN:
				FAssertMsg(nullptr == pLoopSelectionGroup->getHeadUnit(), "Unit with no Domain");
			default:
				if (iPass == 3)
				{
					pLoopSelectionGroup->doTurn();
...
void CvPlayer::updatePlotGroups()
{
	...
	// This at least stops updatePlotGroup spam.
	++heck::gPlotGroupUpdateNesting;
#endif

#if ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (!m_playerTradeNetworkSystem && gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::BlockwisePlotGroupSystem])
		initPlayerTradeNetworkSystem();
	
	if (m_playerTradeNetworkSystem)
		m_playerTradeNetworkSystem->onUpdateAllPlotGroups();
	else
#endif
	{
		...
		for (pLoopPlotGroup = firstPlotGroup(&iLoop); pLoopPlotGroup != nullptr; pLoopPlotGroup = nextPlotGroup(&iLoop))
		{
...
#if ENABLE_GAMECOREDLL_ENHANCEMENTS
void CvPlayer::initPlayerTradeNetworkSystem()
{
	delete m_playerTradeNetworkSystem;
	m_playerTradeNetworkSystem = new GiganticMapsOptimisationsLib::PlayerTradeNetworkSystem(getID());
}
GiganticMapsOptimisationsLib::PlayerTradeNetworkSystem* CvPlayer::getPlayerTradeNetworkSystem()
{
	return m_playerTradeNetworkSystem;
}
#endif
...
void CvPlayer::updateTradeRoutes()
{
	...
	FORTSNEK_DIVERGENCE_CHECKPOINT("");
...
{
	int iLoop;
	for (CvUnit* pLoopUnit = firstUnit(&iLoop); nullptr != pLoopUnit; pLoopUnit = nextUnit(&iLoop))
	{
...
//#else
/*bool CvPlayer::isBestFoundValuePositive() const
{
	int iLoop{};
	for (CvArea* pLoopArea = GC.getMapINLINE().firstArea(&iLoop); pLoopArea; pLoopArea = GC.getMapINLINE().nextArea(&iLoop))
		if (pLoopArea->getBestFoundValue(getID()) > 0)
			return true;
	return false;
}*/
//#endif
...
int CvPlayer::countOwnedBonuses(BonusTypes eBonus) const
{
    ...
    bool bAdvancedStart = (getAdvancedStartPoints() >= 0) && (std::to_underlying(getCurrentEra()) < 3);
...
int CvPlayer::countUnimprovedBonuses(CvArea* pArea, CvPlot* pFromPlot) const
{
	...
	const PlayerTypes playerId = getID();
	const TeamTypes teamId = getTeam();

#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::Parallel_CvPlayer_countUnimprovedBonuses])
	{
		const int areaId = pArea->getID();
		std::atomic_int iCount{};
		std::mutex mutex;
		heck::parallelForEachN(GC.getMapINLINE().numPlotsINLINE(), [&mutex, &iCount, areaId, pFromPlot, playerId = getID(), teamId = getTeam(), this](int iI) {
			const CvPlot* const pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);

			if (pLoopPlot->getArea() == areaId)
			{
				if (pLoopPlot->getOwnerINLINE() == playerId)
				{
					if (!(pLoopPlot->isCity()))
					{
						...
						if (eNonObsoleteBonus != NO_BONUS)
						{
							...
							{
								bool isCounted = pFromPlot == nullptr;
								if (!isCounted)
								{
									const std::lock_guard lock(mutex);
									isCounted |= gDLL->getFAStarIFace()->GeneratePath(&GC.getBorderFinder(), pFromPlot->getX_INLINE(), pFromPlot->getY_INLINE(), pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE(), false, getID(), true);
								}

								if (isCounted)
								{
									for (int iJ = 0, numBuilds = GC.getNumBuildInfos(); iJ < numBuilds; iJ++)
									{
		...
		return iCount.load(std::memory_order_relaxed);
	}
	else
#endif
	{
				...
				if (pLoopPlot->getOwnerINLINE() == playerId)
				{
					if (!(pLoopPlot->isCity()))
					{
						...
						if (eNonObsoleteBonus != NO_BONUS)
						{
							...
							if ((eImprovement == NO_IMPROVEMENT) || !(GC.getImprovementInfo(eImprovement).isImprovementBonusTrade(eNonObsoleteBonus)))
							{
								if ((pFromPlot == nullptr) || (gDLL->getFAStarIFace()->GeneratePath(&GC.getBorderFinder(), pFromPlot->getX_INLINE(), pFromPlot->getY_INLINE(), pLoopPlot->getX_INLINE(), pLoopPlot->getY_INLINE(), false, getID(), true)))
								{
...
#else
							if (pLoopCity->plot()->isSamePlotGroup(getID(), pCapitalCity->plot()))
#endif
							{
								iCount++;
							}
...
bool CvPlayer::canTradeItem(PlayerTypes eWhoTo, TradeData item, bool bTestDenial) const
{
	...
	case TRADE_CITIES:
		{
			...
			if (nullptr != pCityTraded && pCityTraded->getLiberationPlayer(false) == eWhoTo)
			{
				return true;
			}
	...
	case TRADE_PEACE_TREATY:
		...
		// fortsnek: silence warnings
	case TRADE_ITEM_NONE:
	case NUM_TRADEABLE_HEADINGS:
		break;
	}
...
int CvPlayer::getNumAvailableBonuses(BonusTypes eBonus) const
{
	...
	{
		if (const CvCity* const city = getCapitalCity())
		{
			const CvPlot& plot = *city->plot();
			//return plot.getPlotGroupConnectedBonus(plot.getOwnerINLINE(), eBonus);
			return GET_PLAYER(plot.getOwnerINLINE()).getPlayerTradeNetworkSystem()->getPlotGroupNumBonuses(plot, eBonus);
		}
	...
	pPlotGroup = ((getCapitalCity() != nullptr) ? getCapitalCity()->plot()->getOwnerPlotGroup() : nullptr);

	if (pPlotGroup != nullptr)
	{
...
{
	int iLoop;
	for (CvCity* pLoopCity = firstCity(&iLoop); nullptr != pLoopCity; pLoopCity = nextCity(&iLoop))
	{
...
bool CvPlayer::canReceiveGoody(CvPlot* pPlot, GoodyTypes eGoody, CvUnit* pUnit) const
{
	...
	{
		if ((pUnit == nullptr) || !(pUnit->canAcquirePromotionAny()) || (GC.getGameINLINE().getElapsedGameTurns() < 10))
		{
			return false;
		}
	}
	...
	{
		if ((pUnit == nullptr) || (pUnit->getDamage() < ((pUnit->maxHitPoints() * GC.getGoodyInfo(eGoody).getDamagePrereq()) / 100)))
		{
			return false;
		}
	}
...
void CvPlayer::receiveGoody(CvPlot* pPlot, GoodyTypes eGoody, CvUnit* pUnit)
{
	...
	// fortsnek: Fix UNSEQUENCED(!!). Proper order (from Firaxis BTS DLL):
	// Rand = -1247776661 on 199 (Goodies)
	// Rand = 1431480264 on 199 (Goody Gold 2)
	// Rand = 268472417 on 199 (Goody Gold 1)
	// Rand = 1212233606 on 199 (AI Goody)
	const int goodyGold2 = GC.getGameINLINE().getSorenRandNum(GC.getGoodyInfo(eGoody).getGoldRand2(), "Goody Gold 2");
	const int goodyGold1 = GC.getGameINLINE().getSorenRandNum(GC.getGoodyInfo(eGoody).getGoldRand1(), "Goody Gold 1");
	iGold = GC.getGoodyInfo(eGoody).getGold() + goodyGold1 + goodyGold2;
...
bool CvPlayer::canMaintain(ProcessTypes eProcess, [[maybe_unused]] bool bContinue) const
{
...
#else
	for (int iI = 0, n = GC.getNumBuildInfos(); iI < n; iI++)
#endif
	{
		...
		if (eRoute != NO_ROUTE)
		{
			if ((pPlot != nullptr) ? ((pPlot->getRouteType() == eRoute) || canBuild(pPlot, ((BuildTypes)iI))) : GET_TEAM(getTeam()).isHasTech((TechTypes)(GC.getBuildInfo((BuildTypes)iI).getTechPrereq())))
			{
...
int CvPlayer::calculateTotalImports(YieldTypes eYield) const
{
	...
	for (iPlayerLoop = 0; iPlayerLoop < MAX_CIV_PLAYERS; iPlayerLoop++)
	{
		...
		{
			for (pLoopCity = GET_PLAYER((PlayerTypes) iPlayerLoop).firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = GET_PLAYER((PlayerTypes) iPlayerLoop).nextCity(&iLoop))
			{
				for (iTradeLoop = 0; iTradeLoop < pLoopCity->getTradeRoutes(); iTradeLoop++)
				{
					pTradeCity = pLoopCity->getTradeCity(iTradeLoop);
...
int CvPlayer::getRealPopulation() const
{
	...
	return static_cast<int>(iTotalPopulation);
}
				...
				if (GET_PLAYER((PlayerTypes)iI).isAlive())
				{
					if (GET_TEAM(getTeam()).isHasMet(GET_PLAYER((PlayerTypes)iI).getTeam()))
					{
						if (isGoldenAge())
						{
							...
							gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), (((PlayerTypes)iI) == getID()), GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_GOLDAGESTART", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
						}
						else
						{
							szBuffer = gDLL->getText("TXT_KEY_MISC_PLAYER_GOLDEN_AGE_ENDED", getNameKey());
							gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_GOLDAGEEND", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
						}
					}
				}
			}
...
void CvPlayer::changeAnarchyTurns(int iChange)
{
	...
	if (iChange != 0)
	{
		...
		if (bOldAnarchy != isAnarchy())
		{
			...
			if (isAnarchy())
			{
				gDLL->getInterfaceIFace()->addMessage(getID(), true, GC.getEVENT_MESSAGE_TIME(), gDLL->getText("TXT_KEY_MISC_REVOLUTION_HAS_BEGUN").GetCString(), "AS2D_REVOLTSTART", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
			}
			else
			{
				gDLL->getInterfaceIFace()->addMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), gDLL->getText("TXT_KEY_MISC_REVOLUTION_OVER").GetCString(), "AS2D_REVOLTEND", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
			}
...
void CvPlayer::changeCityDefenseModifier(int iChange)
{
	...
	{
		int it{};
		for (CvCity* city = firstCity(&it); city; city = nextCity(&it))
			gGlobals.enhancedDLLGameContext->onPlotChange(GiganticMapsOptimisationsLib::EGamePlotChangeEvent::CityPlayerAddedDefence, *city->plot(), 0, 1);
	}
#endif
}
...
#endif
			bUpdatePlotGroups = ((pOldCapitalCity == nullptr) || (pNewCapitalCity == nullptr) || (pOldCapitalCity->plot()->getOwnerPlotGroup() != pNewCapitalCity->plot()->getOwnerPlotGroup()));

		//std::clog << "PlotGroup check log: " << __FUNCTION__ << ' ' << (int)getID() << std::endl;
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		gGlobals.enhancedDLLGameContext->onCapitalChanged(
			getID(),
			pOldCapitalCity ? heck::ivec2(pOldCapitalCity->getX_INLINE(), pOldCapitalCity->getY_INLINE()) : heck::ivec2(),
			pNewCapitalCity ? heck::ivec2(pNewCapitalCity->getX_INLINE(), pNewCapitalCity->getY_INLINE()) : heck::ivec2()
		);
#endif
	}
}
...
void CvPlayer::sendReminder()
{
		...
		!gDLL->GetPitbossSmtpHost().empty() )
	{
		...
		// fortsnek: let's init this
		long iResult = 0;
		[[maybe_unused]] bool bOK = gDLL->getPythonIFace()->callFunction(PYPitBossModule, "sendEmail", argsList.makeFunctionArgs(), &iResult);
...
unsigned int CvPlayer::getStartTime() const
{
	return m_uiStartTime;
}


void CvPlayer::setStartTime(unsigned int uiStartTime)
{
	m_uiStartTime = uiStartTime;
}


unsigned int CvPlayer::getTotalTimePlayed() const
{
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
			// Have to do this before setTurnActive.
			if (const auto& ctx = gGlobals.enhancedDLLGameContext)
				ctx->onPlayerAliveChanged(getID(), bNewValue);
#endif
			...
			if (GC.getGameINLINE().getElapsedGameTurns() > 0)
			{
				if (!isBarbarian())
				{
					...
					for (iI = 0; iI < MAX_PLAYERS; iI++)
					{
						if (GET_PLAYER((PlayerTypes)iI).isAlive())
						{
							gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_CIVDESTROYED", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
						}
					}
...
void CvPlayer::setStrike(bool bNewValue)
{
	if (isStrike() != bNewValue)
	{
		...
		if (isStrike())
		{
			if (getID() == GC.getGameINLINE().getActivePlayer())
			{
				gDLL->getInterfaceIFace()->addMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), gDLL->getText("TXT_KEY_MISC_UNITS_ON_STRIKE").GetCString(), "AS2D_STRIKE", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
...
void CvPlayer::setCurrentEra(EraTypes eNewValue)
{
	...
	if (getCurrentEra() != eNewValue)
	{
		...
		if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::GeneralFixes])
		{
			// CvCity maintenance depends on GET_PLAYER(getOwnerINLINE()).calculateInflationRate(), which depends on game turn and elapsed game turns.
			// Do this to keep cities up to date.
			updateMaintenance();
		}
#endif
	}
...
#if ENABLE_GAMECOREDLL_ENHANCEMENTS
			// BUG FIX: This invalidates commerce too if getProductionToCommerceModifier != 0.
			|| (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::GeneralFixes] ? eIndex == YIELD_PRODUCTION : false)
#endif
			)
		{
			updateCommerce();
...
void CvPlayer::changeCommerceRate(CommerceTypes eIndex, int iChange)
{
	...
	if (iChange != 0)
	{
		...
		FORTSNEK_DIVERGENCE_CHECKPOINT((int)getID() << ' ' << (int)eIndex << ' ' << iChange << ' ' << m_aiCommerceRate[eIndex]);
...
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(std::to_underlying(eIndex) < MAX_PLAYERS, "eIndex is expected to be within maximum bounds (invalid Index)");
...
void CvPlayer::changeGoldPerTurnByPlayer(PlayerTypes eIndex, int iChange)
{
	...
	if (iChange != 0)
	{
		FORTSNEK_DIVERGENCE_CHECKPOINT(iChange);
...
int CvPlayer::getUpkeepCount(UpkeepTypes eIndex) const
{
	...
	FAssertMsg(m_paiUpkeepCount != nullptr, "m_paiUpkeepCount is not expected to be equal with nullptr");
...
int CvPlayer::getSpecialistValidCount(SpecialistTypes eIndex) const
{
	...
	FAssertMsg(m_paiSpecialistValidCount != nullptr, "m_paiSpecialistValidCount is not expected to be equal with nullptr");
...
{
	FAssertMsg(eVoteSource >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(std::to_underlying(eVoteSource) < MAX_PLAYERS, "eIndex is expected to be within maximum bounds (invalid Index)");
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::Fast_CvPlayer_updateGroupCycle])
	{
		// If AI, group cycle order hopefully doesn't matter (practically). As long as the unit is in the cycle, that's fine (AI code does loop through this list).
		// And leave human group order untouched.
		if (!isHuman() && m_groupCycleSet.contains(pUnit->getGroupID()))
			return;
	}
#endif
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::Fast_CvPlayer_updateGroupCycle])
		m_groupCycleSet.insert(pUnit->getGroupID());
#endif
}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
bool CvPlayer::isUnitInGroupCycle(const CvUnit* unit) const
{
	return m_groupCycleSet.contains(unit->getGroupID());
}
#endif
...
{
	[[maybe_unused]] bool bRemoved = m_selectionGroups.removeAt(iID);
...
int CvPlayer::getEspionageMissionBaseCost(EspionageMissionTypes eMission, PlayerTypes eTargetPlayer, const CvPlot* pPlot, int iExtraData, [[maybe_unused]] const CvUnit* pSpyUnit) const
{
	CvEspionageMissionInfo& kMission = GC.getEspionageMissionInfo(eMission);
	...
	if (nullptr == pCity && kMission.isTargetsCity())
	{
		return -1;
	}
	...
	else if (kMission.getDestroyBuildingCostFactor() > 0)
	{
		...
		if (NO_BUILDING == eBuilding)
		{
			...
			{
				if (nullptr != pCity && pCity->getNumRealBuilding((BuildingTypes)iBuilding) > 0)
				{
		...
		if (NO_BUILDING != eBuilding)
		{
			if (nullptr != pCity && pCity->getNumRealBuilding(eBuilding) > 0)
			{
	...
	else if (kMission.getCityInsertCultureCostFactor() > 0)
	{
		...
		if (nullptr != pPlot && pPlot->getCulture(getID()) > 0)
		{
	...
	else if (kMission.getCityPoisonWaterCounter() > 0)
	{
		...
		if (nullptr != pCity && pCity->getEspionageHealthCounter() <= 0)
		{
	...
	else if (kMission.getCityUnhappinessCounter() > 0)
	{
		...
		if (nullptr != pCity && pCity->getEspionageHappinessCounter() <= 0)
		{
			iMissionCost = iBaseMissionCost;
		}
	}
	...
	else if (kMission.getCityRevoltCounter() > 0)
	{
		...
		if (nullptr != pCity && pCity->getOccupationTimer() == 0)
		{
			iMissionCost = iBaseMissionCost;
		}
	}
...
int CvPlayer::getEspionageMissionCostModifier(EspionageMissionTypes eMission, PlayerTypes eTargetPlayer, const CvPlot* pPlot, [[maybe_unused]] int iExtraData, const CvUnit* pSpyUnit) const
{
	CvEspionageMissionInfo& kMission = GC.getEspionageMissionInfo(eMission);
	int iModifier = 100;
	...
	if (pCity != nullptr && kMission.isTargetsCity())
	{
		// City Population
...
bool CvPlayer::doEspionageMission(EspionageMissionTypes eMission, PlayerTypes eTargetPlayer, CvPlot* pPlot, int iExtraData, CvUnit* pSpyUnit)
{
	...
	else if (getID() == GC.getGameINLINE().getActivePlayer())
	{
		...
		{
			pInfo->setText(gDLL->getText("TXT_KEY_ESPIONAGE_TOO_EXPENSIVE", iMissionCost, iHave).c_str());
		}
		else
		{
			pInfo->setText(gDLL->getText("TXT_KEY_ESPIONAGE_CANNOT_DO_MISSION").c_str());
		}
...
{
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(std::to_underlying(eIndex) < MAX_TEAMS, "eIndex is expected to be within maximum bounds (invalid Index)");
...
void CvPlayer::doAdvancedStartAction(AdvancedStartActionTypes eAction, int iX, int iY, int iData, bool bAdd)
{
	...
	case ADVANCEDSTARTACTION_EXIT:
		...
		{
			int iLoop;
			for (CvCity* pCity = firstCity(&iLoop); nullptr != pCity; pCity = nextCity(&iLoop))
			{
				pCity->chooseProduction();
			}
...
int CvPlayer::getAdvancedStartUnitCost(UnitTypes eUnit, bool bAdd, CvPlot* pPlot) const
{
	...
	else
	{
		...
		if (0 == GC.getDefineINT("ADVANCED_START_ALLOW_UNITS_OUTSIDE_CITIES"))
		{
			...
			if (nullptr == pCity || pCity->getOwnerINLINE() != getID())
			{
				return -1;
			}
...
{
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	// AI doesn't read warnings.
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::Skip_AI_CvPlayer_doWarnings] && !isHuman())
		return;
#endif
...
void CvPlayer::read(FDataStreamBase* pStream)
{
	...
	{
		m_groupCycleSet.clear();
		for (auto* node = headGroupCycleNode(); node; node = nextGroupCycleNode(node))
			m_groupCycleSet.insert(node->m_data);
	}
#endif
...
void CvPlayer::write(FDataStreamBase* pStream)
{
	...
	{
		CLLNode<CvWString>* pNode;
		unsigned int iSize = m_cityNames.getLength();
		pStream->Write(iSize);
	...
	{
		if (m_playerTradeNetworkSystem)
			m_playerTradeNetworkSystem->writeAsStreamableFFreeListTrashArray(*pStream);
		else
		{
			m_plotGroups.removeAll();
	...
	{
		int iSize = static_cast<int>(m_listGameMessages.size());
		pStream->Write(iSize);
...

	{
		...
		int iSize = static_cast<int>(m_listPopups.size() + currentPopups.size());
		pStream->Write(iSize);
		CvPopupQueue::iterator it;
	...
	{
		int iSize = static_cast<int>(m_listDiplomacy.size());
		pStream->Write(iSize);
		CvDiploQueue::iterator it;
		for (it = m_listDiplomacy.begin(); it != m_listDiplomacy.end(); ++it)
		{
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_mapScoreHistory.size());
		pStream->Write(iSize);
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_mapEconomyHistory.size());
		pStream->Write(iSize);
		CvTurnScoreMap::iterator it;
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_mapIndustryHistory.size());
		pStream->Write(iSize);
		CvTurnScoreMap::iterator it;
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_mapAgricultureHistory.size());
		pStream->Write(iSize);
		CvTurnScoreMap::iterator it;
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_mapPowerHistory.size());
		pStream->Write(iSize);
		CvTurnScoreMap::iterator it;
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_mapCultureHistory.size());
		pStream->Write(iSize);
		CvTurnScoreMap::iterator it;
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_mapEspionageHistory.size());
		pStream->Write(iSize);
		CvTurnScoreMap::iterator it;
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_mapEventsOccured.size());
		pStream->Write(iSize);
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_mapEventCountdown.size());
		pStream->Write(iSize);
		CvEventMap::iterator it;
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_aFreeUnitCombatPromotions.size());
		pStream->Write(iSize);
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_aFreeUnitClassPromotions.size());
		pStream->Write(iSize);
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_aVote.size());
		pStream->Write(iSize);
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_aUnitExtraCosts.size());
		pStream->Write(iSize);
	...
	{
		unsigned int iSize = static_cast<unsigned int>(m_triggersFired.size());
		pStream->Write(iSize);
...
void CvPlayer::createGreatPeople(UnitTypes eGreatPersonUnit, bool bIncrementThreshold, bool bIncrementExperience, int iX, int iY)
{
	...
	if (pPlot)
	{
		...
		{
			CvWString szCity;
			szCity.Format(L"%ls (%ls)", pCity->getName().GetCString(), GET_PLAYER(pCity->getOwnerINLINE()).getName());
...
void CvPlayer::setTriggerFired(const EventTriggeredData& kTriggeredData, bool bOthers, bool bAnnounce)
{
	...
	if (bAnnounce)
	{
				...
				if (kLoopPlayer.isAlive())
				{
					if (GET_TEAM(kLoopPlayer.getTeam()).isHasMet(getTeam()) && (NO_PLAYER == kTriggeredData.m_eOtherPlayer || GET_TEAM(GET_PLAYER(kTriggeredData.m_eOtherPlayer).getTeam()).isHasMet(getTeam())))
					{
						...
						if (bShowPlot)
						{
							...
							{
								if (nullptr == pPlot || !pPlot->isRevealed(kLoopPlayer.getTeam(), false))
								{
									bShowPlot = false;
								}
							}
						...
						if (bShowPlot)
						{
							gDLL->getInterfaceIFace()->addMessage((PlayerTypes)iPlayer, false, GC.getEVENT_MESSAGE_TIME(), kTriggeredData.m_szGlobalText, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), kTriggeredData.m_iPlotX, kTriggeredData.m_iPlotY, true, true);
						}
						else
						{
		...
		{
			if (kTrigger.isShowPlot() && nullptr != pPlot && pPlot->isRevealed(getTeam(), false))
			{
				gDLL->getInterfaceIFace()->addMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), kTriggeredData.m_szText, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), kTriggeredData.m_iPlotX, kTriggeredData.m_iPlotY, true, true);
			}
			else
			{
				gDLL->getInterfaceIFace()->addMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), kTriggeredData.m_szText, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
			}
		}
	}
}
...
EventTriggeredData* CvPlayer::initTriggeredData(EventTriggerTypes eEventTrigger, bool bFire, int iCityId, int iPlotX, int iPlotY, PlayerTypes eOtherPlayer, int iOtherPlayerCityId, ReligionTypes eReligion, CorporationTypes eCorporation, int iUnitId, BuildingTypes eBuilding)
{
	...
	if (nullptr == pPlot)
	{
		if (apPlots.size() > 0)
		{
			int iChosen = GC.getGameINLINE().getSorenRandNum(static_cast<int>(apPlots.size()), "Event pick plot");
	...
	{
		if (nullptr != pCity && NO_BUILDING == eBuilding)
		{
			...
			{
				int iChosen = GC.getGameINLINE().getSorenRandNum(static_cast<int>(aeBuildings.size()), "Event pick building");
	...
	if (nullptr == pUnit && kTrigger.getNumUnits() > 0)
	{
		return nullptr;
	}

	if (nullptr == pPlot && nullptr != pUnit)
	{
		pPlot = pUnit->plot();
	}

	if (nullptr == pPlot && bPickPlot)
	{
		return nullptr;
	}
	...
	if (kTrigger.getNumUnitsGlobal() > 0)
	{
		...
		for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; ++iPlayer)
		{
			...
			if (kLoopPlayer.isAlive())
			{
				...
				{
					if (INT_MIN != pLoopUnit->getTriggerValue(eEventTrigger, pPlot, true))
					{
						++iNumUnits;
					}
				}
	...
	if (kTrigger.isPickPlayer())
	{
		...
		if (NO_PLAYER == eOtherPlayer)
		{
			...
			if (aePlayers.size() > 0)
			{
				int iChosen = GC.getGameINLINE().getSorenRandNum(static_cast<int>(aePlayers.size()), "Event pick player");
	...
	if (nullptr != pTriggerData)
	{
		pTriggerData->m_eTrigger = eEventTrigger;
		pTriggerData->m_ePlayer = getID();
		pTriggerData->m_iTurn = GC.getGameINLINE().getGameTurn();
		pTriggerData->m_iCityId = (nullptr != pCity) ? pCity->getID() : -1;
		pTriggerData->m_iPlotX = (nullptr != pPlot) ? pPlot->getX_INLINE() : INVALID_PLOT_COORD;
		pTriggerData->m_iPlotY = (nullptr != pPlot) ? pPlot->getY_INLINE() : INVALID_PLOT_COORD;
		pTriggerData->m_eOtherPlayer = eOtherPlayer;
		pTriggerData->m_iOtherPlayerCityId = (nullptr != pOtherPlayerCity) ? pOtherPlayerCity->getID() : -1;
		pTriggerData->m_eReligion = eReligion;
		pTriggerData->m_eCorporation = eCorporation;
		pTriggerData->m_iUnitId = (nullptr != pUnit) ? pUnit->getID() : -1;
	...
	if (aszTexts.size() > 0)
	{
		int iText = GC.getGameINLINE().getSorenRandNum(static_cast<int>(aszTexts.size()), "Event Text choice");
		...
		pTriggerData->m_szText = gDLL->getText(aszTexts[iText].GetCString(), 
			...
			nullptr != pCity ? pCity->getNameKey() : L"", 
			nullptr != pUnit ? pUnit->getNameKey() : L"", 
			...
			nullptr != pOtherPlayerCity ? pOtherPlayerCity->getNameKey() : L"",
			nullptr != pPlot && NO_TERRAIN != pPlot->getTerrainType() ? GC.getTerrainInfo(pPlot->getTerrainType()).getTextKeyWide() : L"",
			nullptr != pPlot && NO_IMPROVEMENT != pPlot->getImprovementType() ? GC.getImprovementInfo(pPlot->getImprovementType()).getTextKeyWide() : L"",
			nullptr != pPlot && NO_BONUS != pPlot->getBonusType() ? GC.getBonusInfo(pPlot->getBonusType()).getTextKeyWide() : L"",
			nullptr != pPlot && NO_ROUTE != pPlot->getRouteType() ? GC.getRouteInfo(pPlot->getRouteType()).getTextKeyWide() : L"",
...
bool CvPlayer::canDoEvent(EventTypes eEvent, const EventTriggeredData& kTriggeredData) const
{
	...
	if (kEvent.isCityEffect())
	{
		...
		if (nullptr == pCity || !pCity->canApplyEvent(eEvent, kTriggeredData))
		{
			return false;
		}
	}
	...
	if (kEvent.getConvertOtherCities() > 0)
	{
		...
		if (NO_RELIGION != kTriggeredData.m_eReligion)
		{
			...
			{
				int iLoop;
				for (CvCity* pLoopCity = GET_PLAYER(kTriggeredData.m_eOtherPlayer).firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = GET_PLAYER(kTriggeredData.m_eOtherPlayer).nextCity(&iLoop))
				{
					if (!pLoopCity->isHasReligion(kTriggeredData.m_eReligion))
					{
						if (-1 == kEvent.getMaxNumReligions() || pLoopCity->getReligionCount() <= kEvent.getMaxNumReligions())
...
void CvPlayer::applyEvent(EventTypes eEvent, int iEventTriggeredId, bool bUpdateTrigger)
{
		...
		if (eBestTech != NO_TECH)
		{
			...
			if (iBeakers > 0)
			{
				for (int iI = 0; iI < MAX_CIV_PLAYERS; iI++)
				{
					if (GET_PLAYER((PlayerTypes)iI).isAlive())
					{
						// fortsnek: comparison of different enumeration types. Preserving bug.
						if ((int)GET_PLAYER((PlayerTypes)iI).getTeam() == (int)getID())
						{
							CvWString szBuffer = gDLL->getText("TXT_KEY_MISC_PROGRESS_TOWARDS_TECH", iBeakers, GC.getTechInfo(eBestTech).getTextKeyWide());

							gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, nullptr, MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_TECH_TEXT"));
						}
					}
				}
			}
		...
		if (kEvent.getMaxPillage() > 0)
		{
			...
			for (int i = 0; i < iNumPillage; ++i)
			{
				...
				for (int j = 0; j < GC.getMapINLINE().numPlotsINLINE(); ++j)
				{
					...
					if (nullptr != pPlot && pPlot->getOwnerINLINE() == getID() && pPlot->isCity())
					{
	...
	{
		int iChosen = GC.getGameINLINE().getSorenRandNum(static_cast<int>(apSpreadReligionCities.size()), "Even Spread Religion (own)");
	...
	if (kEvent.getConvertOtherCities() > 0)
	{
		if (NO_RELIGION != pTriggeredData->m_eReligion)
		{
			if (NO_PLAYER != pTriggeredData->m_eOtherPlayer)
			{
				std::vector<CvCity*> apCities;
				int iLoop;
				for (CvCity* pLoopCity = GET_PLAYER(pTriggeredData->m_eOtherPlayer).firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = GET_PLAYER(pTriggeredData->m_eOtherPlayer).nextCity(&iLoop))
				{
					if (!pLoopCity->isHasReligion(pTriggeredData->m_eReligion))
					{
						if (-1 == kEvent.getMaxNumReligions() || pLoopCity->getReligionCount() <= kEvent.getMaxNumReligions())
	...
	{
		int iChosen = GC.getGameINLINE().getSorenRandNum(static_cast<int>(apSpreadReligionCities.size()), "Even Spread Religion (other)");
		...
		for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; ++iPlayer)
		{
			...
			if (kLoopPlayer.isAlive())
			{
				if (GET_TEAM(kLoopPlayer.getTeam()).isHasMet(getTeam()) && (NO_PLAYER == pTriggeredData->m_eOtherPlayer || GET_TEAM(GET_PLAYER(pTriggeredData->m_eOtherPlayer).getTeam()).isHasMet(getTeam())))
				{
					...
					if (bShowPlot)
					{
						gDLL->getInterfaceIFace()->addMessage((PlayerTypes)iPlayer, false, GC.getEVENT_MESSAGE_TIME(), szGlobalText, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), pTriggeredData->m_iPlotX, pTriggeredData->m_iPlotY, true, true);
					}
					else
					{
	...
	if (!CvWString(kEvent.getLocalInfoTextKey()).empty())
	{
		...
		szLocalText = gDLL->getText(kEvent.getLocalInfoTextKey(),
			...
			{
				gDLL->getInterfaceIFace()->addMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), szLocalText, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), pTriggeredData->m_iPlotX, pTriggeredData->m_iPlotY, true, true);
			}
			else
			{
				gDLL->getInterfaceIFace()->addMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), szLocalText, "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"));
			}
...
void CvPlayer::expireEvent(EventTypes eEvent, const EventTriggeredData& kTriggeredData, bool bFail)
{
	...
	if (GC.getEventInfo(eEvent).isQuest())
	{
		...
		if (bFail)
		{
			gDLL->getInterfaceIFace()->addMessage(getID(), false, GC.getEVENT_MESSAGE_TIME(), gDLL->getText(GC.getEventInfo(eEvent).getQuestFailTextKey()), "AS2D_CIVIC_ADOPT", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_RED"));
		}
	}
}
...
bool CvPlayer::checkExpireEvent(EventTypes eEvent, const EventTriggeredData& kTriggeredData) const
{
	...
	{
		if (nullptr == kPlayer.getCity(kTriggeredData.m_iCityId))
		{
			return true;
		}
	}
	...
	{
		if (nullptr == kPlayer.getUnit(kTriggeredData.m_iUnitId))
		{
			return true;
		}
	}
	...
	if (NO_PLAYER != kTriggeredData.m_eOtherPlayer)
	{
		...
		{
			if (nullptr == GET_PLAYER(kTriggeredData.m_eOtherPlayer).getCity(kTriggeredData.m_iOtherPlayerCityId))
			{
				return true;
			}
		}
...
CvCity* CvPlayer::pickTriggerCity(EventTriggerTypes eTrigger) const
{
	...
	for (CvCity* pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
	{
		...
		if (iValue >= iBestValue && iValue != INT_MIN)
		{
			if (iValue > iBestValue)
			{
				apCities.clear();
	...
	if (apCities.size() > 0)
	{
		int iChosen = GC.getGameINLINE().getSorenRandNum(static_cast<int>(apCities.size()), "Event pick city");
...
CvUnit* CvPlayer::pickTriggerUnit(EventTriggerTypes eTrigger, CvPlot* pPlot, bool bPickPlot) const
{
	...
	if (apUnits.size() > 0)
	{
		int iChosen = GC.getGameINLINE().getSorenRandNum(static_cast<int>(apUnits.size()), "Event pick unit");
...
int CvPlayer::getEventTriggerWeight(EventTriggerTypes eTrigger) const
{
	...
	if (kTrigger.getNumPrereqEvents() > 0)
	{
		...
		{
			if (nullptr == getEventOccured((EventTypes)kTrigger.getPrereqEvent(iI)))
			{
				bFoundValid = false;
				break;
			}
	...
	if (kTrigger.getMinOurLandmass() > 0 || kTrigger.getMaxOurLandmass() != -1)
	{
		...
		int iLoop;
		for (CvArea* pArea = GC.getMapINLINE().firstArea(&iLoop); nullptr != pArea; pArea = GC.getMapINLINE().nextArea(&iLoop))
		{
			if (!pArea->isWater())
			{
	...
	if (kTrigger.isProbabilityUnitMultiply() && kTrigger.getNumUnits() > 0)
	{
		...
		for (CvUnit* pLoopUnit = firstUnit(&iLoop); pLoopUnit != nullptr; pLoopUnit = nextUnit(&iLoop))
		{
			if (INT_MIN != pLoopUnit->getTriggerValue(eTrigger, nullptr, true))
			{
				++iNumUnits;
			}
		}
...
bool CvPlayer::canSplitEmpire() const
{
	...
	for (CvArea* pLoopArea = GC.getMapINLINE().firstArea(&iLoopArea); pLoopArea != nullptr; pLoopArea = GC.getMapINLINE().nextArea(&iLoopArea))
	{
...
bool CvPlayer::canSplitArea(int iAreaId) const
{
	...
	if (nullptr == pArea || pArea == pCapital->area())
	{
		return false;
	}
...
bool CvPlayer::splitEmpire(int iAreaId)
{
	...
	for (int iPlot = 0; iPlot < GC.getMapINLINE().numPlotsINLINE(); ++iPlot)
	{
		...
		if (!bTranferPlot)
		{
			...
			if (nullptr != pWorkingCity && pWorkingCity->getOwnerINLINE() == getID() && pWorkingCity->area() == pArea)
			{
				bTranferPlot = true;
			}
		}
	...
	for (unsigned int i = 0; i < aCultures.size(); ++i)
	{
...
bool CvPlayer::isValidTriggerReligion(const CvEventTriggerInfo& kTrigger, CvCity* pCity, ReligionTypes eReligion) const
{
	...
	else
	{
		...
		if (kTrigger.isHolyCity())
		{
			...
			if (nullptr == pHolyCity || pHolyCity->getOwnerINLINE() != getID())
			{
				return false;
			}
		}
...
bool CvPlayer::isValidTriggerCorporation(const CvEventTriggerInfo& kTrigger, CvCity* pCity, CorporationTypes eCorporation) const
{
	...
	else
	{
		...
		{
			CvCity* pHeadquarters = GC.getGameINLINE().getHeadquarters(eCorporation);
			if (nullptr == pHeadquarters || pHeadquarters->getOwnerINLINE() != getID())
			{
				return false;
			}
		}
...
void CvPlayer::launch(VictoryTypes eVictory)
{
	...
	for(int i = 0; i < MAX_PLAYERS; ++i)
	{
		if (GET_PLAYER((PlayerTypes)i).isAlive())
		{
			int plotX = -1;
			int plotY = -1;
			if((capital != nullptr) && capital->isRevealed(GET_PLAYER((PlayerTypes) i).getTeam(), false))
			{
				plotX = capital->getX();
				plotY = capital->getY();
			}
...
bool CvPlayer::hasShrine(ReligionTypes eReligion)
{
	...
	if (eReligion != NO_RELIGION)
	{
		...
		if (pHolyCity != nullptr && pHolyCity->getOwnerINLINE() == getID())
...
void CvPlayer::invalidateYieldRankCache([[maybe_unused]] YieldTypes eYield)
{
	int iLoop;
	CvCity* pLoopCity;
	for (pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
...
void CvPlayer::invalidateCommerceRankCache([[maybe_unused]] CommerceTypes eCommerce)
{
	int iLoop;
	CvCity* pLoopCity;
	for (pLoopCity = firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = nextCity(&iLoop))
...
bool CvPlayer::canDefyResolution(VoteSourceTypes eVoteSource, const VoteSelectionSubData& kData) const
{
	...
	if (GC.getVoteInfo(kData.eVote).isOpenBorders())
	{
		for (int iTeam = 0; iTeam < MAX_CIV_TEAMS; ++iTeam)
		{
			...
			// fortsnek: comparison of different enumeration types.
			if ((int)(PlayerTypes)iTeam != (int)getTeam())
			{
...
void CvPlayer::setDefiedResolution(VoteSourceTypes eVoteSource, [[maybe_unused]] const VoteSelectionSubData& kData)
{
...
void CvPlayer::setEndorsedResolution(VoteSourceTypes eVoteSource, [[maybe_unused]] const VoteSelectionSubData& kData)
{
...
bool CvPlayer::canSpyDestroyUnit([[maybe_unused]] PlayerTypes eTarget, CvUnit& kUnit) const
{
...
bool CvPlayer::canSpyBribeUnit(PlayerTypes eTarget, CvUnit& kUnit) const
{
	...
	while (pUnitNode != nullptr)
	{
		...
		if (nullptr != pLoopUnit && pLoopUnit != &kUnit)
		{
...
bool CvPlayer::canSpyDestroyBuilding([[maybe_unused]] PlayerTypes eTarget, BuildingTypes eBuilding) const
{
	CvBuildingInfo& kBuilding = GC.getBuildingInfo(eBuilding);
...
bool CvPlayer::canSpiesEnterBorders(PlayerTypes ePlayer) const
{
	for (int iMission = 0; iMission < GC.getNumEspionageMissionInfos(); ++iMission)
	{
		...
		{
			if (GET_PLAYER(ePlayer).canDoEspionageMission((EspionageMissionTypes)iMission, getID(), nullptr, -1, nullptr))
			{
				return false;
			}
		}
...
void CvPlayer::buildTradeTable(PlayerTypes eOtherPlayer, CLinkList<TradeData>& ourList) const
{
	...
	{
		[[maybe_unused]] bool bFoundItemUs = false;
...
bool CvPlayer::getHeadingTradeString([[maybe_unused]] PlayerTypes eOtherPlayer, TradeableItems eItem, CvWString& szString, CvString& szIcon) const
{
	szIcon.clear();
...
bool CvPlayer::getItemTradeString(PlayerTypes eOtherPlayer, bool bOffer, bool bShowingCurrent, const TradeData& zTradeData, CvWString& szString, CvString& szIcon) const
{
	...
	case TRADE_RESOURCES:
		...
		{
			szString.Format( L"%ls (%d)", GC.getBonusInfo((BonusTypes)zTradeData.m_iData).getDescription(), getNumTradeableBonuses((BonusTypes)zTradeData.m_iData));
		}
	...
	case TRADE_CITIES:
		{
			...
			if (nullptr != pCity)
			{
				...
				{
					szString.Format(L"%ls (%ls)", pCity->getName().GetCString(), gDLL->getText("TXT_KEY_LIBERATE_CITY").GetCString());
				}
				else
				{
...
void CvPlayer::updateTradeList(PlayerTypes eOtherPlayer, CLinkList<TradeData>& ourInventory, const CLinkList<TradeData>& ourOffer, const CLinkList<TradeData>& theirOffer) const
{
	...
	for (CLLNode<TradeData>* pNode = ourInventory.head(); pNode != nullptr; pNode = ourInventory.next(pNode))
	{
		...
		{
		case TRADE_PEACE_TREATY:
			for (CLLNode<TradeData>* pOfferNode = ourOffer.head(); pOfferNode != nullptr; pOfferNode = ourOffer.next(pOfferNode))
			{
...
void CvPlayer::getTradeLayerColors(std::vector<NiColorA>& aColors, std::vector<CvPlotIndicatorData>& aIndicators) const
{
	...
	for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); ++iI)
	{
		CvPlot* pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);
#if ENABLE_GAMECOREDLL_ENHANCEMENTS
		if (pLoopPlot->isRevealed(getTeam(), true) && pLoopPlot->getTeam() == getTeam())
			if (const std::optional<int> cpgI = pLoopPlot->tryGetPlotGroupId(getID()))
				mapPlotGroups[*cpgI].push_back(iI);
#else
		CvPlotGroup* pPlotGroup = pLoopPlot->getPlotGroup(getID());
		if (pPlotGroup != nullptr && pLoopPlot->isRevealed(getTeam(), true) && pLoopPlot->getTeam() == getTeam())
		{
...
void CvPlayer::getResourceLayerColors(GlobeLayerResourceOptionTypes eOption, std::vector<NiColorA>& aColors, std::vector<CvPlotIndicatorData>& aIndicators) const
{
	...
	[[maybe_unused]] PlayerColorTypes ePlayerColor = getPlayerColor();
	...
	for (int iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
	{
		...
		if (pLoopPlot->isRevealed(getTeam(), true))
		{
			...
			if (eCurType != NO_BONUS)
			{
				...
				case NUM_RESOURCE_OPTION_TYPES:
					break;
				}
...
void CvPlayer::getCultureLayerColors(std::vector<NiColorA>& aColors, std::vector<CvPlotIndicatorData>& aIndicators) const
{
	...
	{
		CvPlot* pLoopPlot = GC.getMapINLINE().plotByIndexINLINE(iI);
		[[maybe_unused]] PlayerTypes eOwner = pLoopPlot->getRevealedOwner(getTeam(), true);

		// how many people own this plot?
		std::vector < std::pair<int,int> > plot_owners;
		[[maybe_unused]] int iNumNonzeroOwners = 0;
		for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; iPlayer++)
		{
			if (GET_PLAYER((PlayerTypes)iPlayer).isAlive())
			{
...
void CvPlayer::cheat([[maybe_unused]] bool bCtrl, [[maybe_unused]] bool bAlt, [[maybe_unused]] bool bShift)
{
	if (gDLL->getChtLvl() > 0)
	{
