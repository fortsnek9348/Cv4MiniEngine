...
public:
	...
	// fortsnek: Cache objects sometimes depend on multiple XML files, so there needs to be a way to specify all such files up-front.
	virtual void cacheSpecifyExtraXmlDependencies(CvCacheObject* pCache, std::span<const char* const> szSourceFileNames) = 0;

	virtual bool cacheRead(CvCacheObject* pCache, const TCHAR* szSourceFileName=nullptr) = 0;
	...
	using TextArg = std::variant<std::wstring_view, std::string_view, int>;

	virtual CvWString getTextGeneric(const CvWString& szIDTag, std::span<const TextArg> args) = 0;

	CvWString getText(const CvWString& szIDTag, const auto&... args)
	{
		return getTextGeneric(szIDTag, std::array<TextArg, sizeof...(args)>{ args... });
	}

	virtual CvWString getObjectText(const CvWString& szIDTag, unsigned int uiForm, bool bNoSubs = false) = 0;
	...
	virtual unsigned int getNumForms(CvWString szIDTag) = 0;

	virtual WorldSizeTypes getWorldSize() = 0;
	virtual unsigned int getFrameCounter() const = 0;
