...
bool CvXMLLoadUtility::ReadGlobalDefines(const TCHAR* szXMLFileName)
{
...
bool CvXMLLoadUtility::SetGlobalDefines()
{
	...
	// fortsnek: New implementation.

	// fortsnek: Cache only these fixed files.
	...
	gDLL->cacheSpecifyExtraXmlDependencies(cache, kSources);

	if (gDLL->cacheRead(cache, nullptr))
		logMsg("Read GobalDefines from cache");
	else
	{
		for (const char* const source : kSources)
			if (!ReadGlobalDefines(source))
				return false;

		if (!gDLL->cacheWrite(cache))
			gDLL->MessageBox("Failed writing to global defines cache.", "XML Caching Error");
		else
...
bool CvXMLLoadUtility::SetGlobalTypes()
{
	...
	if (bLoaded)
	{
		SetGlobalStringArray(&GC.getAnimationOperatorTypes(), std::string("Civ4Types/AnimationOperatorTypes/AnimationOperatorType").data(), &GC.getNumAnimationOperatorTypes());
		int iEnumVal = NUM_FUNC_TYPES;
		SetGlobalStringArray(&GC.getFunctionTypes(),  std::string("Civ4Types/FunctionTypes/FunctionType").data(), &iEnumVal, true);
		SetGlobalStringArray(&GC.getFlavorTypes(),  std::string("Civ4Types/FlavorTypes/FlavorType").data(), &GC.getNumFlavorTypes());
		SetGlobalStringArray(&GC.getArtStyleTypes(),  std::string("Civ4Types/ArtStyleTypes/ArtStyleType").data(), &GC.getNumArtStyleTypes());
		SetGlobalStringArray(&GC.getCitySizeTypes(),  std::string("Civ4Types/CitySizeTypes/CitySizeType").data(), &GC.getNumCitySizeTypes());
		iEnumVal = NUM_CONTACT_TYPES;
		SetGlobalStringArray(&GC.getContactTypes(),  std::string("Civ4Types/ContactTypes/ContactType").data(), &iEnumVal, true);
		iEnumVal = NUM_DIPLOMACYPOWER_TYPES;
		SetGlobalStringArray(&GC.getDiplomacyPowerTypes(),  std::string("Civ4Types/DiplomacyPowerTypes/DiplomacyPowerType").data(), &iEnumVal, true);
		iEnumVal = NUM_AUTOMATE_TYPES;
		SetGlobalStringArray(&GC.getAutomateTypes(),  std::string("Civ4Types/AutomateTypes/AutomateType").data(), &iEnumVal, true);
		iEnumVal = NUM_DIRECTION_TYPES;
		SetGlobalStringArray(&GC.getDirectionTypes(),  std::string("Civ4Types/DirectionTypes/DirectionType").data(), &iEnumVal, true);
		SetGlobalStringArray(&GC.getFootstepAudioTypes(),  std::string("Civ4Types/FootstepAudioTypes/FootstepAudioType").data(), &GC.getNumFootstepAudioTypes());
...
//
//------------------------------------------------------------------------------------------------------
void CvXMLLoadUtility::SetDiplomacyCommentTypes([[maybe_unused]] CvString** ppszString, [[maybe_unused]] int* iNumVals)
{
...
bool CvXMLLoadUtility::LoadGlobalText()
{
	...
	if (!gDLL->cacheRead(cache))
	{
		...
		gDLL->enumerateFiles(aszFiles, "xml/text/*.xml");
		...
		for(std::vector<CvString>::iterator it = aszFiles.begin(); it != aszFiles.end(); ++it)
		{
			bLoaded = LoadCivXml(m_pFXml, it->c_str()); // Load the XML
			if (!bLoaded)
			{
				char	szMessage[1024];
...
bool CvXMLLoadUtility::LoadPreMenuGlobals()
{
	...
	for (int i = 0; i < GC.getNumCursorInfos(); ++i)
	{
		...
		if (GC.getDefinesVarSystem()->GetValue(szType.c_str(), iVal))
		{
			char szMessage[1024];
			sprintf(szMessage, "cursor type already set? \n Current XML file is: %s", GC.getCurrentXMLFile().GetCString());
			gDLL->MessageBox(szMessage, "XML Error");
		}
		GC.getDefinesVarSystem()->SetValue(szType.c_str(), i);
	}
...
{
	PROFILE_FUNC();
	logMsg("SetGlobalStringArray %s", szTagName);
	...
	if (gDLL->getXMLIFace()->LocateNode(m_pFXml,szTagName))
	{
		...
		*ppszString = new CvString[*iNumVals];
		...
		for (i=0;i<*iNumVals;i++)
		{
			...
			GC.setTypesEnum(pszString[i].c_str(), i);
...
void CvXMLLoadUtility::SetGlobalActionInfo()
{
	...
	for (i=0;i<iNumActionInfos;i++)
	{
		...
		else if ((ActionSubTypes)piActionInfoTypeList[piOrderedIndex[i]] == ACTIONSUBTYPE_PROMOTION)
		{
			...
			GC.getPromotionInfo((PromotionTypes)piIndexList[piOrderedIndex[i]]).setHotKeyDescription(GC.getPromotionInfo((PromotionTypes)piIndexList[piOrderedIndex[i]]).getTextKeyWide(), GC.getCommandInfo((CommandTypes)(GC.getPromotionInfo((PromotionTypes)piIndexList[piOrderedIndex[i]]).getCommandType())).getTextKeyWide(), CreateHotKeyFromDescription(GC.getPromotionInfo((PromotionTypes)piIndexList[piOrderedIndex[i]]).getHotKey(), GC.getPromotionInfo((PromotionTypes)piIndexList[piOrderedIndex[i]]).isShiftDown(), GC.getPromotionInfo((PromotionTypes)piIndexList[piOrderedIndex[i]]).isAltDown(), GC.getPromotionInfo((PromotionTypes)piIndexList[piOrderedIndex[i]]).isCtrlDown()).c_str());
		}
		...
		else if ((ActionSubTypes)piActionInfoTypeList[piOrderedIndex[i]] == ACTIONSUBTYPE_SPECIALIST)
		{
			...
			GC.getSpecialistInfo((SpecialistTypes)piIndexList[piOrderedIndex[i]]).setHotKeyDescription(GC.getSpecialistInfo((SpecialistTypes)piIndexList[piOrderedIndex[i]]).getTextKeyWide(), GC.getMissionInfo((MissionTypes)(GC.getSpecialistInfo((SpecialistTypes)piIndexList[piOrderedIndex[i]]).getMissionType())).getTextKeyWide(), CreateHotKeyFromDescription(GC.getSpecialistInfo((SpecialistTypes)piIndexList[piOrderedIndex[i]]).getHotKey(), GC.getSpecialistInfo((SpecialistTypes)piIndexList[piOrderedIndex[i]]).isShiftDown(), GC.getSpecialistInfo((SpecialistTypes)piIndexList[piOrderedIndex[i]]).isAltDown(), GC.getSpecialistInfo((SpecialistTypes)piIndexList[piOrderedIndex[i]]).isCtrlDown()).c_str());
		}
		else if ((ActionSubTypes)piActionInfoTypeList[piOrderedIndex[i]] == ACTIONSUBTYPE_BUILDING)
		{
			...
			GC.getBuildingInfo((BuildingTypes)piIndexList[piOrderedIndex[i]]).setHotKeyDescription(GC.getBuildingInfo((BuildingTypes)piIndexList[piOrderedIndex[i]]).getTextKeyWide(), GC.getMissionInfo((MissionTypes)(GC.getBuildingInfo((BuildingTypes)piIndexList[piOrderedIndex[i]]).getMissionType())).getTextKeyWide(), CreateHotKeyFromDescription(GC.getBuildingInfo((BuildingTypes)piIndexList[piOrderedIndex[i]]).getHotKey(), GC.getBuildingInfo((BuildingTypes)piIndexList[piOrderedIndex[i]]).isShiftDown(), GC.getBuildingInfo((BuildingTypes)piIndexList[piOrderedIndex[i]]).isAltDown(), GC.getBuildingInfo((BuildingTypes)piIndexList[piOrderedIndex[i]]).isCtrlDown()).c_str());
		}
...
{
	PROFILE_FUNC();
	logMsg( "SetGlobalAnimationPathInfo %s", szTagName );
...
{
	PROFILE_FUNC();
	logMsg("SetGlobalUnitScales %s", szTagName);
...
{
	PROFILE_FUNC();
	logMsg("SetGameText %s", szTagName);
	...
	if (gDLL->getXMLIFace()->LocateNode(m_pFXml, szTagName))
	{
		...
		do
		{
			SkipToNextVal();	// skip to the next non-comment node
				...
				FAssert(nullptr != pClassInfo);
				if (nullptr == pClassInfo)
				{
					break;
				}
...
void CvXMLLoadUtility::SetDiplomacyInfo(std::vector<CvDiplomacyInfo*>& DiploInfos, const char* szTagName)
{
	...
	if (gDLL->getXMLIFace()->LocateNode(m_pFXml, szTagName))
	{
		...
		do
		{
			...
			int iIndex = GC.getInfoTypeForString(szType.c_str(), true);
...
void CvXMLLoadUtility::LoadGlobalClassInfo(std::vector<T*>& aInfos, const char* szFileRoot, const char* szFileDirectory, const char* szXmlPath, bool bTwoPass, CvCacheObject* (CvDLLUtilityIFaceBase::*pArgFunction) (const TCHAR*))
{
	...
	if (nullptr != pArgFunction)
	{
		pCache = (gDLL->*pArgFunction)(CvString::format("%s.dat", szFileRoot).c_str());	// cache file name

		if (gDLL->cacheRead(pCache, CvString::format("xml/%s/%s.xml", szFileDirectory, szFileRoot).c_str()))
		{
	...
	if (!bLoaded)
	{
		bLoaded = LoadCivXml(m_pFXml, CvString::format("xml/%s/%s.xml", szFileDirectory, szFileRoot).c_str());
		...
		else
		{
			...
			{
				std::vector<CvString> aszFiles;
				gDLL->enumerateFiles(aszFiles, CvString::format("modules/*_%s.xml", szFileRoot).c_str());  // search for the modular files

				for (std::vector<CvString>::iterator it = aszFiles.begin(); it != aszFiles.end(); ++it)
				{
					bLoaded = LoadCivXml(m_pFXml, it->c_str());
			...
			if (nullptr != pArgFunction && bWriteCache)
			{
				// write info to cache
...
void CvXMLLoadUtility::SetFeatureStruct(int** ppiFeatureTech, int** ppiFeatureTime, int** ppiFeatureProduction, bool** ppbFeatureRemove)
{
	...
	int* paiFeatureTech = nullptr;
	int* paiFeatureTime = nullptr;
	int* paiFeatureProduction = nullptr;
...
void CvXMLLoadUtility::SetVariableListTagPair(int **ppiList, const TCHAR* szRootTagName,
	[[maybe_unused]] int iInfoBaseSize, int iInfoBaseLength, int iDefaultListVal)
{
	int i;
	int iIndexVal;
	int iNumSibs;
...
void CvXMLLoadUtility::SetVariableListTagPair(bool **ppbList, const TCHAR* szRootTagName,
	[[maybe_unused]] int iInfoBaseSize, int iInfoBaseLength, bool bDefaultListVal)
{
	int i;
	int iIndexVal;
	int iNumSibs;
...
void CvXMLLoadUtility::SetVariableListTagPair(float **ppfList, const TCHAR* szRootTagName,
	[[maybe_unused]] int iInfoBaseSize, int iInfoBaseLength, float fDefaultListVal)
{
	int i;
	int iIndexVal;
	int iNumSibs;
...
void CvXMLLoadUtility::SetVariableListTagPair(CvString **ppszList, const TCHAR* szRootTagName,
	[[maybe_unused]] int iInfoBaseSize, int iInfoBaseLength, CvString szDefaultListVal)
{
	int i;
	int iIndexVal;
	int iNumSibs;
...
//------------------------------------------------------------------------------------------------------
void CvXMLLoadUtility::SetVariableListTagPair(int **ppiList, const TCHAR* szRootTagName,
	[[maybe_unused]] CvString* m_paszTagList, int iTagListLength, int iDefaultListVal)
{
	int i;
	int iIndexVal;
	int iNumSibs;
						...
						if (GetChildXmlVal(szTextVal))
						{
							...
							if (iIndexVal != -1)
							{
								...
								if ( szTemp.GetLength() > 0 )
									piList[iIndexVal] = gDLL->getAudioTagIndex(szTemp.c_str());
								else
									piList[iIndexVal] = -1;
							}
...
//------------------------------------------------------------------------------------------------------
void CvXMLLoadUtility::SetVariableListTagPair(bool **ppbList, const TCHAR* szRootTagName,
	[[maybe_unused]] CvString* m_paszTagList, int iTagListLength, bool bDefaultListVal)
{
	int i;
	int iIndexVal;
	int iNumSibs;
...
//------------------------------------------------------------------------------------------------------
void CvXMLLoadUtility::SetVariableListTagPair(CvString **ppszList, const TCHAR* szRootTagName,
	[[maybe_unused]] CvString* m_paszTagList, int iTagListLength, CvString szDefaultListVal)
{
	int i;
	int iIndexVal;
	int iNumSibs;
