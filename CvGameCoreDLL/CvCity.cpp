...
CvCity::CvCity()
{
	...
	m_paiNoBonus = nullptr;
	m_paiFreeBonus = nullptr;
	m_paiNumBonuses = nullptr;
	m_paiNumCorpProducedBonuses = nullptr;
	m_paiProjectProduction = nullptr;
	m_paiBuildingProduction = nullptr;
	m_paiBuildingProductionTime = nullptr;
	m_paiBuildingOriginalOwner = nullptr;
	m_paiBuildingOriginalTime = nullptr;
	m_paiUnitProduction = nullptr;
	m_paiUnitProductionTime = nullptr;
	m_paiGreatPeopleUnitRate = nullptr;
	m_paiGreatPeopleUnitProgress = nullptr;
	m_paiSpecialistCount = nullptr;
	m_paiMaxSpecialistCount = nullptr;
	m_paiForceSpecialistCount = nullptr;
	m_paiFreeSpecialistCount = nullptr;
	m_paiImprovementFreeSpecialists = nullptr;
	m_paiReligionInfluence = nullptr;
	m_paiStateReligionHappiness = nullptr;
	m_paiUnitCombatFreeExperience = nullptr;
	m_paiFreePromotionCount = nullptr;
	m_paiNumRealBuilding = nullptr;
	m_paiNumFreeBuilding = nullptr;

	m_pabWorkingPlot = nullptr;
	m_pabHasReligion = nullptr;
	m_pabHasCorporation = nullptr;
...
void CvCity::init(int iID, PlayerTypes eOwner, int iX, int iY, bool bBumpUnits, bool bUpdatePlotGroups)
{
	...
	setName(GET_PLAYER(getOwnerINLINE()).getNewCityName().c_str());
	...
	{
		// BUG FIX: Keep plot groups up to date.
	...
	if (pPlot->getCulture(getOwnerINLINE()) < GC.getDefineINT("FREE_CITY_CULTURE"))
	{
		...
		{
			// BUG FIX: Keep plot groups up to date.
			pPlot->setCulture(getOwnerINLINE(), GC.getDefineINT("FREE_CITY_CULTURE"), bBumpUnits, bUpdatePlotGroups);
		}
		else
#endif
	...
	{
		// BUG FIX: Keep plot groups up to date.
		pPlot->setOwner(getOwnerINLINE(), bBumpUnits, bUpdatePlotGroups);
	}
	else
#endif
	...
	for (iI = 0; iI < NUM_DIRECTION_TYPES; iI++)
	{
		...
		if (pAdjacentPlot != nullptr)
		{
			if (pAdjacentPlot->getCulture(getOwnerINLINE()) < GC.getDefineINT("FREE_CITY_ADJACENT_CULTURE"))
			{
				...
				{
					// BUG FIX: Keep plot groups up to date.
					pAdjacentPlot->setCulture(getOwnerINLINE(), GC.getDefineINT("FREE_CITY_ADJACENT_CULTURE"), bBumpUnits, bUpdatePlotGroups);
				}
				else
#endif
			...
			{
				// BUG FIX: Keep plot groups up to date.
				pAdjacentPlot->updateCulture(bBumpUnits, bUpdatePlotGroups);
			}
			else
#endif
	...
	for (iI = 0; iI < MAX_TEAMS; iI++)
	{
		if (GET_TEAM(getTeam()).isVassal((TeamTypes)iI))
		{
			...
			{
				// BUG FIX: Keep plot groups up to date.
				pPlot->changeAdjacentSight((TeamTypes)iI, GC.getDefineINT("PLOT_VISIBILITY_RANGE"), true, nullptr, bUpdatePlotGroups);
			}
			else
#endif
				pPlot->changeAdjacentSight((TeamTypes)iI, GC.getDefineINT("PLOT_VISIBILITY_RANGE"), true, nullptr, false);
		}
	}
	...
	{
		// BUG FIX: Keep plot groups up to date.
		pPlot->updateCityRoute(bUpdatePlotGroups);
	}
	else
#endif
	...
	if (gGlobals.enhancedDLLConfig[EEnhancedDLLConfigOption::GeneralFixes])
	{
		// BUG FIX: City maintenance depends on the number of cities vassals have. Invalidate ourselves and master.
		//          Same fix exists in CvCity::kill.
		for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; iPlayer++)
		{
			CvPlayer& kLoopPlayer = GET_PLAYER((PlayerTypes)iPlayer);
			if (kLoopPlayer.getTeam() != getTeam() && GET_TEAM(getTeam()).isVassal(kLoopPlayer.getTeam()))
			{
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	gGlobals.enhancedDLLGameContext->onPlotChange(GiganticMapsOptimisationsLib::EGamePlotChangeEvent::CityAfterCreated, *plot(), eOwner, eOwner);
#endif
}
...
void CvCity::kill(bool bUpdatePlotGroups)
{
	...
	for (iI = 0; iI < GC.getNumReligionInfos(); iI++)
	{
		...
		{
			GC.getGameINLINE().setHolyCity(((ReligionTypes)iI), nullptr, false);
		}
	}
	...
	for (iI = 0; iI < GC.getNumCorporationInfos(); iI++)
	{
		...
		{
			GC.getGameINLINE().setHeadquarters(((CorporationTypes)iI), nullptr, false);
		}
	}
	...
	for (iI = 0; iI < MAX_TEAMS; iI++)
	{
		...
		{
			pPlot->changeAdjacentSight((TeamTypes)iI, GC.getDefineINT("PLOT_VISIBILITY_RANGE"), false, nullptr, false);
		}
	}
	...
	{
		// BUG FIX: City maintenance depends on the number of cities vassals have. Invalidate ourselves and master.
		//          Same fix exists in CvCity::init.
		const TeamTypes ownerTeam = GET_PLAYER(eOwner).getTeam();
		for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; iPlayer++)
		{
			CvPlayer& kLoopPlayer = GET_PLAYER((PlayerTypes)iPlayer);
			if (kLoopPlayer.getTeam() != ownerTeam && GET_TEAM(ownerTeam).isVassal(kLoopPlayer.getTeam()))
			{
				kLoopPlayer.updateMaintenance();
			}
		}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	gGlobals.enhancedDLLGameContext->onPlotChange(GiganticMapsOptimisationsLib::EGamePlotChangeEvent::CityAfterDestroyed, *pPlot, eOwner, eOwner);
#endif
}
...
#if ENABLE_GAMECOREDLL_ENHANCEMENTS
	/*{
		const int oldValue = getMaintenanceTimes100();
		updateMaintenance();
		const int newValue = getMaintenanceTimes100();
...
#if ENABLE_GAMECOREDLL_ENHANCEMENTS
	//const int oldValue = getCommerceRateTimes100(COMMERCE_RESEARCH);
	//updateCommerce();
	//const int newValue = getCommerceRateTimes100(COMMERCE_RESEARCH);
	//if (oldValue != newValue)
	//	std::abort();
#endif
...
bool CvCity::canBeSelected() const
{
	...
	for (int iLoop = 0; iLoop < GC.getNumEspionageMissionInfos(); iLoop++)
	{
		...
		{
			// Is Mission good?
			if (GET_PLAYER(GC.getGameINLINE().getActivePlayer()).canDoEspionageMission((EspionageMissionTypes)iLoop, getOwnerINLINE(), plot(), -1, nullptr))
			{
				return true;
			}
		}
		...
		if (!(pPlot->plotCheck(PUF_canDefend, -1, -1, NO_PLAYER, getTeam())))
		{
			for (int iI = 0; iI < NUM_DIRECTION_TYPES; iI++)
			{
				...
				if (pLoopPlot != nullptr)
				{
					...
					{
						if (pLoopPlot->plotCheck(PUF_canSiege, getOwnerINLINE()) != nullptr)
						{
							return false;
						}
					}
...
int CvCity::getProductionDifference([[maybe_unused]] int iProductionNeeded, [[maybe_unused]] int iProduction, int iProductionModifier, bool bFoodProduction, bool bOverflow) const
{
	if (isDisorder())
	{
		return 0;
...
CvUnit* CvCity::initConscriptedUnit()
{
	...
	FAssertMsg(pUnit != nullptr, "pUnit expected to be assigned (not nullptr)");
...
void CvCity::setPopulation(int iNewValue)
{
	...
	if (iOldPopulation != iNewValue)
	{
		...
		FORTSNEK_DIVERGENCE_CHECKPOINT((int)getOwnerINLINE() << ' ' << getID() << ' ' << iNewValue);
	}
}
...
{
	return (((int)(pow((float)getPopulation(), 2.8f))) * 1000);
}
...
void CvCity::updateMaintenance()
{
	...
	// fortsnek: Use this when city maintenance missed an update.
	//if ((int)getOwnerINLINE() == 1 && getID() == 16385)
	//{
	//	std::clog << __FUNCTION__ << ": " << iOldMaintenance << ' ' << iNewMaintenance
	...
	//		<< ' ' << getPopulation()
	//		<< ' ' << GET_PLAYER(getOwnerINLINE()).getCorporationMaintenanceModifier()
	//		<< ' ' << GET_PLAYER(getOwnerINLINE()).calculateInflationRate()
	//		;
	//	for (int corpI = 0; corpI < GC.getNumCorporationInfos(); ++corpI)
	//		if (isActiveCorporation((CorporationTypes)corpI))
	//			for (int iCommerce = 0; iCommerce < NUM_COMMERCE_TYPES; ++iCommerce)
	//				std::clog << ' ' << GC.getCorporationInfo((CorporationTypes)corpI).getHeadquarterCommerce(iCommerce);
	//
	...
	//			std::clog << " Vassal#=" << kLoopPlayer.getNumCities();
	//		}
	//	}
	//
	//	std::clog << " Owner#=" << GET_PLAYER(getOwnerINLINE()).getNumCities();
	//
	//	//for (int i = 0; i < GC.getNumBonusInfos(); ++i)
	//	//	std::clog << ' ' << getNumBonuses((BonusTypes)i);
	//	std::clog << std::endl;
	//}
...
int CvCity::calculateDistanceMaintenanceTimes100() const
{
	...
	iBestCapitalMaintenance = INT_MAX;
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		gGlobals.enhancedDLLGameContext->onPlotChange(GiganticMapsOptimisationsLib::EGamePlotChangeEvent::CityBuildingDefence, *plot(), m_iBuildingDefense - iChange, m_iBuildingDefense);
#endif
	}
}
...
void CvCity::changeDefenseDamage(int iChange)
{
	...
	{
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		const int oldValue = m_iDefenseDamage;
#endif
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		gGlobals.enhancedDLLGameContext->onPlotChange(GiganticMapsOptimisationsLib::EGamePlotChangeEvent::CityDefenceDamage, *plot(), oldValue, m_iDefenseDamage);
#endif
	}
}
...
{
	if (nullptr != pUnit && !pUnit->isEnemy(getTeam()))
	{
		return false;
	}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
			gGlobals.enhancedDLLGameContext->onPlotChange(GiganticMapsOptimisationsLib::EGamePlotChangeEvent::CityIsOccupation, *plot(), bOldOccupation, isOccupation());
#endif
		}
...
void CvCity::setCultureLevel(CultureLevelTypes eNewValue, bool bUpdatePlotGroups)
{
	...
	if (eOldValue != eNewValue)
	{
		...
		if (GC.getGameINLINE().isFinalInitialized())
		{
			if ((getCultureLevel() > eOldValue) && (std::to_underlying(getCultureLevel()) > 1))
			{
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		gGlobals.enhancedDLLGameContext->onPlotChange(GiganticMapsOptimisationsLib::EGamePlotChangeEvent::CityCultureLevel, *plot(), eOldValue, eNewValue);
#endif
	}
}
	...
	if (getNumBuilding(eBuilding) > 0)
	{
		...
		if (!(kBuilding.isCommerceChangeOriginalOwner(eIndex)) || (getBuildingOriginalOwner(eBuilding) == getOwnerINLINE()))
		{
			...
			if ((GC.getBuildingInfo(eBuilding).getCommerceChangeDoubleTime(eIndex) != 0) &&
				(getBuildingOriginalTime(eBuilding) != INT_MIN) &&
...
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(std::to_underlying(eIndex) < MAX_PLAYERS, "eIndex expected to be < MAX_PLAYERS");
...
bool CvCity::isRevealed(TeamTypes eIndex, bool bDebug) const
{
	...
	{
		FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
		FAssertMsg(std::to_underlying(eIndex) < MAX_TEAMS, "eIndex expected to be < MAX_TEAMS");
			...
			if (isRevealed((TeamTypes)iTeam, false))
			{
				for (int iPlayer = 0; iPlayer < MAX_CIV_PLAYERS; ++iPlayer)
				{
					...
					if (kPlayer.isAlive() && kPlayer.getTeam() == iTeam)
					{
						...
						{
							if (kPlayer.canDoEspionageMission(*it, getOwnerINLINE(), plot(), -1, nullptr))
							{
								bVisibility = true;
								break;
							}
...
const CvWString CvCity::getName(unsigned int uiForm) const
{
...
void CvCity::doFoundMessage()
{
	...
	gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), false, -1, szBuffer, ARTFILEMGR.getInterfaceArtInfo("WORLDBUILDER_CITY_EDIT")->getPath(), MESSAGE_TYPE_MAJOR_EVENT, nullptr, NO_COLOR, getX_INLINE(), getY_INLINE());
...
#if ENABLE_GAMECOREDLL_ENHANCEMENTS
void CvCity::setNumBonuses(BonusTypes eIndex, int iNewValue)
{
	FAssertMsg(eIndex >= 0, "eIndex expected to be >= 0");
	FAssertMsg(eIndex < GC.getNumBonusInfos(), "eIndex expected to be < GC.getNumBonusInfos()");

	if (iNewValue != m_paiNumBonuses[eIndex])
	{
		bool bOldHasBonus = hasBonus(eIndex);
...
void CvCity::setWorkingPlot(int iIndex, bool bNewValue)
{
	...
	{
		//GC.getDLLIFace()->logMsg("GameStateLog.txt",
		//	CvString::format("%s: (%i, %i): %i, %i", __FUNCTION__, getX_INLINE(), getY_INLINE(), iIndex, bNewValue).c_str(), true);
		//
		//if (std::tuple(getX_INLINE(), getY_INLINE(), iIndex, bNewValue) ==
		//	std::tuple(88, 29, 8, true))
		//	_CrtDbgBreak();
				...
				if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
				{
					...
					if (GC.getBuildingInfo(eIndex).getFreeTechs() > 0)
					{
						if (!isHuman())
						{
							...
							{
								for (int iLoop2 = 0; iLoop2 < iChangeNumRealBuilding; iLoop2++)
								{
					...
					if (getOwnerINLINE() == iI || GET_PLAYER((PlayerTypes)iI).hasHeadquarters(eIndex))
					{
						...
						if (getOwnerINLINE() == iI)
						{
							...
							for (int iJ = 0; iJ < GC.getDefineINT("NUM_CORPORATION_PREREQ_BONUSES"); ++iJ)
							{
								...
								{
									CvWString szTemp;
									szTemp.Format(L"%ls", GC.getBonusInfo((BonusTypes)iBonus).getDescription());
									setListHelp(szBonusList, L"", szTemp.c_str(), L", ", bFirst);
									bFirst = false;
								}
							}
...
#else
							if (pLoopCity->plot()->isSamePlotGroup(getOwnerINLINE(), plot())
#endif
								|| GC.getDefineINT("IGNORE_PLOT_GROUP_FOR_TRADE_ROUTES"))
							{
...
void CvCity::pushOrder(OrderTypes eOrder, int iData1, int iData2, bool bSave, bool bPop, bool bAppend, bool bForce)
{
  OrderData order;
	bool bValid;
	[[maybe_unused]] bool bBuildingUnit = false;
	[[maybe_unused]] bool bBuildingBuilding = false;
...
#else
			swprintf(szBuffer, std::size(szBuffer) - 1, gDLL->getText(((isLimitedUnitClass((UnitClassTypes)(GC.getUnitInfo(eTrainUnit).getUnitClassType()))) ? "TXT_KEY_MISC_TRAINED_UNIT_IN_LIMITED" : "TXT_KEY_MISC_TRAINED_UNIT_IN"), GC.getUnitInfo(eTrainUnit).getTextKeyWide(), getNameKey()).GetCString());
#endif
...
#else
			swprintf(szBuffer, std::size(szBuffer) - 1, gDLL->getText(((isLimitedWonderClass((BuildingClassTypes)(GC.getBuildingInfo(eConstructBuilding).getBuildingClassType()))) ? "TXT_KEY_MISC_CONSTRUCTED_BUILD_IN_LIMITED" : "TXT_KEY_MISC_CONSTRUCTED_BUILD_IN"), GC.getBuildingInfo(eConstructBuilding).getTextKeyWide(), getNameKey()).GetCString());
#endif
...
#else
			swprintf(szBuffer, std::size(szBuffer) - 1, gDLL->getText(((isLimitedProject(eCreateProject)) ? "TXT_KEY_MISC_CREATED_PROJECT_IN_LIMITED" : "TXT_KEY_MISC_CREATED_PROJECT_IN"), GC.getProjectInfo(eCreateProject).getTextKeyWide(), getNameKey()).GetCString());
#endif
...
#else
			swprintf(szTempBuffer, std::size(szTempBuffer) - 1, gDLL->getText(((isProductionLimited()) ? "TXT_KEY_MISC_WORK_HAS_BEGUN_LIMITED" : "TXT_KEY_MISC_WORK_HAS_BEGUN"), getProductionNameKey()).GetCString());
#endif
			...
			if (!isHasReligion((ReligionTypes)iI))
			{
				if ((iI == GET_PLAYER(getOwnerINLINE()).getStateReligion()) || !(GET_PLAYER(getOwnerINLINE()).isNoNonStateReligionSpread()))
				{
					...
					for (iJ = 0; iJ < MAX_PLAYERS; iJ++)
					{
						...
						{
							for (pLoopCity = GET_PLAYER((PlayerTypes)iJ).firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = GET_PLAYER((PlayerTypes)iJ).nextCity(&iLoop))
							{
...
void CvCity::write(FDataStreamBase* pStream)
{
	...
	pStream->Write((int)m_aBuildingHealthChange.size());
...
void CvCity::getVisibleBuildings(std::list<BuildingTypes>& kChosenVisible, int& iChosenNumGenerics)
{
...
#else
	if (strcasecmp(GC.getDefineSTRING("GAME_CITY_SIZE_METHOD"), "METHOD_EXPONENTIAL") == 0)
#endif
	{
	...
	{
		iNumUniques = static_cast<int>(kVisible.size());
	}
...
void CvCity::getVisibleEffects(ZoomLevelTypes eCurZoom, std::vector<const TCHAR*>& kEffectNames) 
{
	...
	if ((getTeam() == GC.getGameINLINE().getActiveTeam()) || GC.getGameINLINE().isDebugMode()) 
	{
		...
		if (isWeLoveTheKingDay())
		{
			...
			// fortsnek: Don't use `rand`. Instead, just give a fake plot index to the RNG, and rely on the RNG to generate something random.
			//           It's only visual seffects, so RNG quality isn't so important. Just make it not dependent on std lib RNG choice.
			CvRandom kRand;
			kRand.init(getX_INLINE() + getY_INLINE() * 1861);
			...
			{
				kEffectNames.push_back(szFireworkEffects[kRand.get(static_cast<unsigned short>(iNumEffects))]);
			}
		}
	}
}
...
int CvCity::getTriggerValue(EventTriggerTypes eTrigger) const
{
	...
	{
		// fortsnek: Lets init this.
		long lResult = -1;
		// fortsnek: Watchout for event functions that lack a return statement, such as `canTriggerImmigrantCity`.Because the Fireaxis DLL does not initialise this variable, who knows what you'll get!
	...
	if (kEvent.getMinPillage() > 0)
	{
		...
		for (int i = 0; i < NUM_CITY_PLOTS; ++i)
		{
			if (CITY_HOME_PLOT != i)
			{
				...
				if (nullptr != pPlot && pPlot->getOwnerINLINE() == getOwnerINLINE())
				{
...
{
	// fortsnek: invalid comparison of different enumeration types
	FAssertMsg((int)eCommerce >= (int)NO_YIELD && (int)eCommerce < (int)NUM_YIELD_TYPES, "invalidateCommerceRankCache passed bogus commerce index");
	...
	for (int iJ = 0; iJ < NUM_CITY_PLOTS; ++iJ)
	{
		if (iJ != CITY_HOME_PLOT)
		{
			if (!isWorkingPlot(iJ))
			{
				...
				if (nullptr != pPlot && canWork(pPlot))
				{
...
void CvCity::cheat(bool bCtrl, [[maybe_unused]] bool bAlt, bool bShift)
{
	if (gDLL->getChtLvl() > 0)
	{
		if (bCtrl)
