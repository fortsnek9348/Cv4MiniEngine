...
CvTeam::CvTeam()
{
	...
	m_abCanLaunch = nullptr;

	m_paiRouteChange = nullptr;
	m_paiProjectCount = nullptr;
	m_paiProjectDefaultArtTypes = nullptr;
	m_pavProjectArtTypes = nullptr;
	m_paiProjectMaking = nullptr;
	m_paiUnitClassCount = nullptr;
	m_paiBuildingClassCount = nullptr;
	m_paiObsoleteBuildingCount = nullptr;
	m_paiResearchProgress = nullptr;
	m_paiTechCount = nullptr;
	m_paiTerrainTradeCount = nullptr;
	m_aiVictoryCountdown = nullptr;
	m_aiForceTeamVoteEligibilityCount = nullptr;

	m_pabHasTech = nullptr;
	m_pabNoTradeTech = nullptr;
...
void CvTeam::reset(TeamTypes eID, bool bConstructorCall)
{
	...
	if (!bConstructorCall)
	{
		FAssertMsg(m_abCanLaunch==nullptr, "about to leak memory, CvTeam::m_abCanLaunch");
		...
		FAssertMsg(m_paiRouteChange==nullptr, "about to leak memory, CvTeam::m_paiRouteChange");
		...
		FAssertMsg(m_paiProjectCount==nullptr, "about to leak memory, CvPlayer::m_paiProjectCount");
		m_paiProjectCount = new int [GC.getNumProjectInfos()];
		FAssertMsg(m_paiProjectDefaultArtTypes==nullptr, "about to leak memory, CvPlayer::m_paiProjectDefaultArtTypes");
		m_paiProjectDefaultArtTypes = new int [GC.getNumProjectInfos()];
		FAssertMsg(m_pavProjectArtTypes==nullptr, "about to leak memory, CvPlayer::m_pavProjectArtTypes");
		m_pavProjectArtTypes = new std::vector<int> [GC.getNumProjectInfos()];
		FAssertMsg(m_paiProjectMaking==nullptr, "about to leak memory, CvPlayer::m_paiProjectMaking");
		...
		FAssertMsg(m_paiUnitClassCount==nullptr, "about to leak memory, CvTeam::m_paiUnitClassCount");
		...
		FAssertMsg(m_paiBuildingClassCount==nullptr, "about to leak memory, CvTeam::m_paiBuildingClassCount");
		...
		FAssertMsg(m_paiObsoleteBuildingCount==nullptr, "about to leak memory, CvTeam::m_paiObsoleteBuildingCount");
		...
		FAssertMsg(m_paiResearchProgress==nullptr, "about to leak memory, CvPlayer::m_paiResearchProgress");
		m_paiResearchProgress = new int [GC.getNumTechInfos()];
		FAssertMsg(m_paiTechCount==nullptr, "about to leak memory, CvPlayer::m_paiTechCount");
		...
		FAssertMsg(m_paiTerrainTradeCount==nullptr, "about to leak memory, CvTeam::m_paiTerrainTradeCount");
		...
		FAssertMsg(m_aiVictoryCountdown==nullptr, "about to leak memory, CvTeam::m_aiVictoryCountdown");
		...
		FAssertMsg(m_pabHasTech==nullptr, "about to leak memory, CvTeam::m_pabHasTech");
		m_pabHasTech = new bool[GC.getNumTechInfos()];
		FAssertMsg(m_pabNoTradeTech==nullptr, "about to leak memory, CvTeam::m_pabNoTradeTech");
		...
		FAssertMsg(m_ppaaiImprovementYieldChange==nullptr, "about to leak memory, CvTeam::m_ppaaiImprovementYieldChange");
		...
		FAssertMsg(m_aiForceTeamVoteEligibilityCount==nullptr, "about to leak memory, CvTeam::m_aiForceTeamVoteEligibilityCount");
		...
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if ((GET_PLAYER((PlayerTypes)iI).getTeam() != getID()) && (GET_PLAYER((PlayerTypes)iI).getTeam() != eTeam))
			{
				if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(getID()) && GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(eTeam))
				{
					...
					gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_THEIRALLIANCE", MESSAGE_TYPE_MINOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
				}
			}
		}
	}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		// We do it here instead of inside setAtWar as the war state is consistent.
		if (const auto& ctx = gGlobals.enhancedDLLGameContext) // Not initialised yet during new game init
			ctx->onTeamChange(GiganticMapsOptimisationsLib::EGameTeamChangeEvent::War, getID(), eTeam);
#endif
				...
				for (iI = 0; iI < MAX_PLAYERS; iI++)
				{
					if (GET_PLAYER((PlayerTypes)iI).isAlive())
					{
						if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
						{
							...
							gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_DECLAREWAR", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
						}
						...
						else if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(getID()) && GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(eTeam))
						{
							...
							gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_THEIRDECLAREWAR", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_WARNING_TEXT"));
						}
					}
				}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		// We do it here instead of inside setAtWar as the war state is consistent.
		gGlobals.enhancedDLLGameContext->onTeamChange(GiganticMapsOptimisationsLib::EGameTeamChangeEvent::War, getID(), eTeam);
#endif
		...
		for (iI = 0; iI < MAX_PLAYERS; iI++)
		{
			if (GET_PLAYER((PlayerTypes)iI).isAlive())
			{
				if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
				{
					...
					gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_MAKEPEACE", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
				}
				else if (GET_PLAYER((PlayerTypes)iI).getTeam() == eTeam)
				{
				...
				else if (GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(getID()) && GET_TEAM(GET_PLAYER((PlayerTypes)iI).getTeam()).isHasMet(eTeam))
				{
					...
					gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_THEIRMAKEPEACE", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
				}
			}
		}
...
void CvTeam::changeAliveCount(int iChange)
{
	...
	if (const auto& ctx = gGlobals.enhancedDLLGameContext)
		ctx->onTeamChange(GiganticMapsOptimisationsLib::EGameTeamChangeEvent::Alive, getID(), 0);
#endif
}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		gGlobals.enhancedDLLGameContext->onTeamChange(GiganticMapsOptimisationsLib::EGameTeamChangeEvent::BridgeBuilding, getID(), getID());
#endif
	}
}
...
int CvTeam::getStolenVisibilityTimer(TeamTypes eIndex) const
{
	...
	FAssertMsg(std::to_underlying(eIndex) < MAX_TEAMS, "iIndex is expected to be within maximum bounds (invalid Index)");
...
{
	FAssert(eIndex >= 0 && std::to_underlying(eIndex) < MAX_TEAMS);
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		if (const auto& ctx = gGlobals.enhancedDLLGameContext)
			ctx->onTeamChange(GiganticMapsOptimisationsLib::EGameTeamChangeEvent::HasMet, getID(), eIndex);
#endif
	}
}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	gGlobals.enhancedDLLGameContext->onTeamChange(GiganticMapsOptimisationsLib::EGameTeamChangeEvent::PermanentWarPeace, getID(), eIndex);
#endif
}
...
void CvTeam::setOpenBorders(TeamTypes eIndex, bool bNewValue)
{
	...
	if (isOpenBorders(eIndex) != bNewValue)
	{
		...
		{
			// Affects CvPlot::isCity, which affects plot groups.
			for (iI = 0; iI < MAX_PLAYERS; iI++)
				if (GET_PLAYER((PlayerTypes)iI).isAlive())
					if (const auto playerTeamI = GET_PLAYER((PlayerTypes)iI).getTeam(); playerTeamI == getID() || playerTeamI == eIndex)
						GET_PLAYER((PlayerTypes)iI).updatePlotGroups();
		}
#endif
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		gGlobals.enhancedDLLGameContext->onTeamChange(GiganticMapsOptimisationsLib::EGameTeamChangeEvent::OpenBorders, getID(), eIndex);
#endif
	}
}
		...
		if (bNewValue && !GET_TEAM(eIndex).isDefensivePact(getID()))
		{
			...
			for (int iI = 0; iI < MAX_PLAYERS; iI++)
			{
				...
				if (kPlayer.isAlive())
				{
					...
					{
						gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_WELOVEKING", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
					}
				}
			}
		}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	gGlobals.enhancedDLLGameContext->onTeamChange(GiganticMapsOptimisationsLib::EGameTeamChangeEvent::ForcePeace, getID(), eIndex);
#endif
}
...
void CvTeam::setVassal(TeamTypes eIndex, bool bNewValue, bool bCapitulated)
{
	...
	if (isVassal(eIndex) != bNewValue)
	{
		...
		for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
		{
			...
			{
				int iLoop;
				for (CvUnit* pLoopUnit = kLoopPlayer.firstUnit(&iLoop); nullptr != pLoopUnit; pLoopUnit = kLoopPlayer.nextUnit(&iLoop))
				{
		...
		if (isVassal(eIndex))
		{
			...
			for (pLoopDeal = GC.getGameINLINE().firstDeal(&iLoop); pLoopDeal != nullptr; pLoopDeal = GC.getGameINLINE().nextDeal(&iLoop))
			{
				if ((GET_PLAYER(pLoopDeal->getFirstPlayer()).getTeam() == getID()) || (GET_PLAYER(pLoopDeal->getSecondPlayer()).getTeam() == getID()))
				{
					...
					if (bValid && pLoopDeal->getSecondTrades() != nullptr)
					{
						for (pNode = pLoopDeal->getSecondTrades()->head(); pNode; pNode = pLoopDeal->getSecondTrades()->next(pNode))
						{
							if ((pNode->m_data.m_eItemType == TRADE_DEFENSIVE_PACT) ||
			...
			if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()))
			{
				...
				for (int iI = 0; iI < MAX_PLAYERS; iI++)
				{
					...
					if (kPlayer.isAlive())
					{
						if (isHasMet(kPlayer.getTeam()) && GET_TEAM(eIndex).isHasMet(kPlayer.getTeam()))
						{
							gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szReplayMessage, "AS2D_WELOVEKING", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
						}
					}
				}
			}
			...
			if (GC.getGameINLINE().isFinalInitialized() && !(gDLL->GetWorldBuilderMode()) && isAlive() && GET_TEAM(eIndex).isAlive())
			{
				...
				for (int iI = 0; iI < MAX_PLAYERS; iI++)
				{
					...
					if (kPlayer.isAlive())
					{
						...
						if (!szBuffer.empty())
						{
							gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_REVOLTSTART", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
						}
					}
				}				
			}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		gGlobals.enhancedDLLGameContext->onTeamChange(GiganticMapsOptimisationsLib::EGameTeamChangeEvent::Vassal, getID(), eIndex);
#endif
	}
}
...
bool CvTeam::isHasTech(TechTypes eIndex) const
{
	...
	FAssertMsg(eIndex >= 0, "eIndex is expected to be non-negative (invalid Index)");
	FAssertMsg(eIndex < GC.getNumTechInfos(), "eIndex is expected to be within maximum bounds (invalid Index)");
	FAssertMsg(m_pabHasTech != nullptr, "m_pabHasTech is not expected to be equal with nullptr");
	...
	for (int iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
			{
				...
				gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, (bSound ? GC.getEVENT_MESSAGE_TIME() : -1), szBuffer, (bSound ? GC.getTechInfo(eIndex).getSoundMP() : nullptr), MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_TECH_TEXT"));
			}
		}
	}
}
					...
					if (GC.getTechInfo(eIndex).getFirstFreeTechs() > 0)
					{
						...
						for (iI = 0; iI < MAX_PLAYERS; iI++)
						{
							if (GET_PLAYER((PlayerTypes)iI).isAlive())
							{
								...
								gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_FIRSTTOTECH", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
							}
						}
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
		if (const auto& ctx = gGlobals.enhancedDLLGameContext) // Not initialised yet during new game init
			gGlobals.enhancedDLLGameContext->onTeamChange(GiganticMapsOptimisationsLib::EGameTeamChangeEvent::TeamTech, getID(), eIndex);
#endif
	}
}
		...
		if (GC.getDefineINT("CIRCUMNAVIGATE_FREE_MOVES") != 0)
		{
			...
			for (int iI = 0; iI < MAX_PLAYERS; iI++)
			{
				if (GET_PLAYER((PlayerTypes)iI).isAlive())
				{
					...
					gDLL->getInterfaceIFace()->addMessage(((PlayerTypes)iI), false, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_GLOBECIRCUMNAVIGATED", MESSAGE_TYPE_MAJOR_EVENT, nullptr, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
				}
			}
...
void CvTeam::verifySpyUnitsValidPlot()
{
	...
	for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
	{
		...
		{
			int iLoop;
			for (CvUnit* pUnit = kPlayer.firstUnit(&iLoop); pUnit != nullptr; pUnit = kPlayer.nextUnit(&iLoop))
			{
...
#ifdef ENABLE_GAMECOREDLL_ENHANCEMENTS
	// Okay, so this actually happened. Event revealed oil.
	gGlobals.enhancedDLLGameContext->onTeamChange(GiganticMapsOptimisationsLib::EGameTeamChangeEvent::ForceRevealBonusChanged, getID(), eBonus);
#endif
}
...
void CvTeam::write(FDataStreamBase* pStream)
{
	...
	pStream->Write((int)m_aeRevealedBonuses.size());
...
bool CvTeam::hasShrine(ReligionTypes eReligion)
{
	...
	if (eReligion != NO_RELIGION)
	{
		...
		if (pHolyCity != nullptr && GET_PLAYER(pHolyCity->getOwnerINLINE()).getTeam() == getID())
