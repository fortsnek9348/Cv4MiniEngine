...
// fortsnek: Moving this to its own file for modules.
#include "XYCoords.h"
...
struct /*DllExport*/ IDInfo
{
	...
	// fortsnek: Using this in the engine.
//#ifdef _USRDLL
...
struct /*DllExport*/ FOWVis
{
	...
	XYCoords* pOffsets;  // array of "Offset" points

	// python friendly accessors
	XYCoords getOffsets(int i) const { return pOffsets[i]; }
};
...
public:
	...
	CvBattleDefinition& operator=(const CvBattleDefinition&) = delete; // fortsnek: Explicitly delete to silence warning
...
public:
	...
	CvAirMissionDefinition& operator=(const CvAirMissionDefinition&) = delete; // fortsnek: Explicitly delete to silence warning
...
{
	CvFlyoutMenuData(FlyoutTypes eType, int iId, int iX, int iY, const wchar_t* strTitle);
...
struct /*DllExport*/ CvWBData
{
	...
	CvWBData(int iId, std::wstring_view strHelp, std::string_view strButton) : m_iId(iId), m_strHelp(strHelp), m_strButton(strButton) { }
	DllExport ~CvWBData() = default;
	int m_iId;
	CvWString m_strHelp;
...
{
	int iFlags = 0;
	int iMaxTurns = std::numeric_limits<int>::max();

	PathingArg(int iFlags) : iFlags(iFlags) {}
	explicit PathingArg(int iFlags, int iMaxTurns) : iFlags(iFlags), iMaxTurns(iMaxTurns) {}
};
#else
using PathingArg = int;
#endif