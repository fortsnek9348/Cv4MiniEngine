...
[[maybe_unused]] static char* szErrorMsg;
...
void CvGameTextMgr::setInterfaceTime(CvWString& szString, PlayerTypes ePlayer)
{
	...
	{
		szString.Format(L"%lc(%d) ", gDLL->getSymbolID(GOLDEN_AGE_CHAR), GET_PLAYER(ePlayer).getGoldenAgeTurns());
	}
	else
	{
		szString.clear();
...
void CvGameTextMgr::setGoldStr(CvWString& szString, PlayerTypes ePlayer)
{
	...
	{
		szString.Format(L"%lc: " SETCOLR L"%d" ENDCOLR, GC.getCommerceInfo(COMMERCE_GOLD).getChar(), TEXT_COLOR("COLOR_NEGATIVE_TEXT"), GET_PLAYER(ePlayer).getGold());
	}
	else
	{
		szString.Format(L"%lc: %d", GC.getCommerceInfo(COMMERCE_GOLD).getChar(), GET_PLAYER(ePlayer).getGold());
	}
...
void CvGameTextMgr::setUnitHelp(CvWStringBuffer &szString, const CvUnit* pUnit, bool bOneLine, bool bShort)
{
	...
	szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR("COLOR_UNIT_TEXT"), pUnit->getName().GetCString());
	...
	if (pUnit->getDomainType() == DOMAIN_AIR)
	{
		if (pUnit->airBaseCombatStr() > 0)
		{
			if (pUnit->isFighting())
			{
				szTempBuffer.Format(L"?/%d%lc, ", pUnit->airBaseCombatStr(), gDLL->getSymbolID(STRENGTH_CHAR));
			}
			else if (pUnit->isHurt())
			{
				szTempBuffer.Format(L"%.1f/%d%lc, ", (((float)(pUnit->airBaseCombatStr() * pUnit->currHitPoints())) / ((float)(pUnit->maxHitPoints()))), pUnit->airBaseCombatStr(), gDLL->getSymbolID(STRENGTH_CHAR));
			}
			else
			{
				szTempBuffer.Format(L"%d%lc, ", pUnit->airBaseCombatStr(), gDLL->getSymbolID(STRENGTH_CHAR));
	...
	else
	{
		if (pUnit->canFight())
		{
			if (pUnit->isFighting())
			{
				szTempBuffer.Format(L"?/%d%lc, ", pUnit->baseCombatStr(), gDLL->getSymbolID(STRENGTH_CHAR));
			}
			else if (pUnit->isHurt())
			{
				szTempBuffer.Format(L"%.1f/%d%lc, ", (((float)(pUnit->baseCombatStr() * pUnit->currHitPoints())) / ((float)(pUnit->maxHitPoints()))), pUnit->baseCombatStr(), gDLL->getSymbolID(STRENGTH_CHAR));
			}
			else
			{
				szTempBuffer.Format(L"%d%lc, ", pUnit->baseCombatStr(), gDLL->getSymbolID(STRENGTH_CHAR));
	...
	{
		szTempBuffer.Format(L"%d%lc", pUnit->baseMoves(), gDLL->getSymbolID(MOVES_CHAR));
	}
	else
	{
		szTempBuffer.Format(L"%d/%d%lc", iCurrMoves, pUnit->baseMoves(), gDLL->getSymbolID(MOVES_CHAR));
	}
	szString.append(szTempBuffer);
	...
	{
		szString.append(L", ");
		szTempBuffer.Format(L"%ls (%d)", GC.getBuildInfo(eBuild).getDescription(), pUnit->plot()->getBuildTurnsLeft(eBuild, 0, 0));
		szString.append(szTempBuffer);
	}
	...
	/*if (!bOneLine)
	{
		...
		{
			szTempBuffer.Format(L" (%ls)", GC.getUnitCombatInfo(pUnit->getUnitCombatType()).getDescription());
			szString += szTempBuffer;
		}
	}*/
	...
	if (GC.getGameINLINE().isDebugMode() && !bAlt && !bShift)
	{
		...
		szTempBuffer.Format(L" (%ls)", GC.getUnitAIInfo(pUnit->AI_getUnitAIType()).getDescription());
		szString.append(szTempBuffer);
	}
	...
	{
		szString.append(L", ");
		szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, GET_PLAYER(pUnit->getOwnerINLINE()).getPlayerTextColorR(), GET_PLAYER(pUnit->getOwnerINLINE()).getPlayerTextColorG(), GET_PLAYER(pUnit->getOwnerINLINE()).getPlayerTextColorB(), GET_PLAYER(pUnit->getOwnerINLINE()).getPlayerTextColorA(), GET_PLAYER(pUnit->getOwnerINLINE()).getName());
		szString.append(szTempBuffer);
	}
	...
	for (iI = 0; iI < GC.getNumPromotionInfos(); ++iI)
	{
		...
		{
			szTempBuffer.Format(L"<img=%hs size=16></img>", GC.getPromotionInfo((PromotionTypes)iI).getButton());
			szString.append(szTempBuffer);
		}
	}
	...
	if (!bOneLine)
	{
		...
		for (iI = 0; iI < GC.getNumUnitClassInfos(); ++iI)
		{
			if (pUnit->getUnitInfo().getTargetUnitClass(iI))
			{
				...
				szTempBuffer += CvWString::format(L"<link=literal>%ls</link>", GC.getUnitClassInfo((UnitClassTypes)iI).getDescription());
			}
		}
		...
		for (iI = 0; iI < GC.getNumUnitCombatInfos(); ++iI)
		{
			if (pUnit->getUnitInfo().getTargetUnitCombat(iI))
			{
				...
				szTempBuffer += CvWString::format(L"<link=literal>%ls</link>", GC.getUnitCombatInfo((UnitCombatTypes)iI).getDescription());
			}
		}
        ...
        {
            szTempBuffer.Format(L"\nUnitAI Type = %ls.", GC.getUnitAIInfo(pUnit->AI_getUnitAIType()).getDescription());
            szString.append(szTempBuffer);
...
void CvGameTextMgr::setPlotListHelp(CvWStringBuffer &szString, CvPlot* pPlot, [[maybe_unused]] bool bOneLine, [[maybe_unused]] bool bShort)
{
	PROFILE_FUNC();
		...
		if (pPlot->isVisible(GC.getGameINLINE().getActiveTeam(), GC.getGameINLINE().isDebugMode()))
		{
			...
			while(pUnitNode != nullptr)
			{
				...
				if (pHeadUnit && pHeadUnit->isGroupHead() && !pHeadUnit->isCargo() && !pHeadUnit->isInvisible(GC.getGameINLINE().getActiveTeam(), GC.getGameINLINE().isDebugMode()))
				{
					...
					szString.append(CvWString::format(SETCOLR L"%ls" ENDCOLR, 255,190,0,255, pHeadUnit->getName().GetCString()));
					szString.append(CvWString::format(L" (%d)", shortenID(pHeadUnit->getID())));
					getUnitAIString(szTempString, pHeadUnit->AI_getUnitAIType());
					szString.append(CvWString::format(SETCOLR L" %ls " ENDCOLR, GET_PLAYER(pHeadUnit->getOwnerINLINE()).getPlayerTextColorR(), GET_PLAYER(pHeadUnit->getOwnerINLINE()).getPlayerTextColorG(), GET_PLAYER(pHeadUnit->getOwnerINLINE()).getPlayerTextColorB(), GET_PLAYER(pHeadUnit->getOwnerINLINE()).getPlayerTextColorA(), szTempString.GetCString()));
					...
					for (int iPromotionIndex = 0; iPromotionIndex < numPromotionInfos; iPromotionIndex++)
					{
						...
						{
							szString.append(CvWString::format(L"<img=%hs size=16></img>", GC.getPromotionInfo(ePromotion).getButton()));
						}
					}

					// group
					CvSelectionGroup* pHeadGroup = pHeadUnit->getGroup();
					FAssertMsg(pHeadGroup != nullptr, "unit has nullptr group");
					if (pHeadGroup->getNumUnits() > 1)
					{
						...
						CLLNode<IDInfo>* pUnitNode2 = pHeadGroup->headUnitNode();
						while (pUnitNode2 != nullptr)
						{
							CvUnit* pLoopUnit = ::getUnit(pUnitNode2->m_data);
							pUnitNode2 = pHeadGroup->nextUnitNode(pUnitNode2);
					...
					if (eMissionAI != NO_MISSIONAI)
					{
						...
						szString.append(CvWString::format(SETCOLR L"\n%ls" ENDCOLR, TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), szTempString.GetCString()));
					}
					...
					if (pMissionUnit != nullptr && (eMissionAI != NO_MISSIONAI || eMissionType != NO_MISSION))
					{
						// mission unit
						szString.append(L"\n to ");
						szString.append(CvWString::format(SETCOLR L"%ls" ENDCOLR, GET_PLAYER(pMissionUnit->getOwnerINLINE()).getPlayerTextColorR(), GET_PLAYER(pMissionUnit->getOwnerINLINE()).getPlayerTextColorG(), GET_PLAYER(pMissionUnit->getOwnerINLINE()).getPlayerTextColorB(), GET_PLAYER(pMissionUnit->getOwnerINLINE()).getPlayerTextColorA(), pMissionUnit->getName().GetCString()));
						...
						if (pMissionPlot != nullptr)
						{
							...
							if (pCity != nullptr)
							{
								...
								if (!pMissionPlot->isCity())
								{
									...
									szString.append(CvWString::format(L"%ls of ", szTempString.GetCString()));
								}

								szString.append(CvWString::format(SETCOLR L"%ls" ENDCOLR L")", GET_PLAYER(pCity->getOwnerINLINE()).getPlayerTextColorR(), GET_PLAYER(pCity->getOwnerINLINE()).getPlayerTextColorG(), GET_PLAYER(pCity->getOwnerINLINE()).getPlayerTextColorB(), GET_PLAYER(pCity->getOwnerINLINE()).getPlayerTextColorA(), pCity->getName().GetCString()));
							}
							else
							{
								if (pMissionPlot != pPlot)
								{
									...
									getDirectionTypeString(szTempString, eDirection);
									szString.append(CvWString::format(L" (%ls)", szTempString.GetCString()));
								}

								PlayerTypes eMissionPlotOwner = pMissionPlot->getOwnerINLINE();
								if (eMissionPlotOwner != NO_PLAYER)
								{
									szString.append(CvWString::format(L", " SETCOLR L"%ls" ENDCOLR, GET_PLAYER(eMissionPlotOwner).getPlayerTextColorR(), GET_PLAYER(eMissionPlotOwner).getPlayerTextColorG(), GET_PLAYER(eMissionPlotOwner).getPlayerTextColorB(), GET_PLAYER(eMissionPlotOwner).getPlayerTextColorA(), GET_PLAYER(eMissionPlotOwner).getName()));
								}
							}
						}
					}
					...
					while(pUnitNode3 != nullptr)
					{
						...
						if (pUnit && (pUnit != pHeadUnit) && (pUnit->getGroupID() == pHeadUnit->getGroupID()) && !pUnit->isInvisible(GC.getGameINLINE().getActiveTeam(), GC.getGameINLINE().isDebugMode()))
						{
							...
							// name and unitai
							szString.append(CvWString::format(SETCOLR L"\n-%ls" ENDCOLR, TEXT_COLOR("COLOR_UNIT_TEXT"), pUnit->getName().GetCString()));
							szString.append(CvWString::format(L" (%d)", shortenID(pUnit->getID())));
							getUnitAIString(szTempString, pUnit->AI_getUnitAIType());
							szString.append(CvWString::format(SETCOLR L" %ls " ENDCOLR, GET_PLAYER(pUnit->getOwnerINLINE()).getPlayerTextColorR(), GET_PLAYER(pUnit->getOwnerINLINE()).getPlayerTextColorG(), GET_PLAYER(pUnit->getOwnerINLINE()).getPlayerTextColorB(), GET_PLAYER(pUnit->getOwnerINLINE()).getPlayerTextColorA(), szTempString.GetCString()));
							...
							for (unsigned int i = 0; i < aLoopCargoUnits.size(); ++i)
							{
	...
	CvUnit* pLoopUnit;
	static const unsigned int iMaxNumUnits = 15;
	...
	if (static_cast<unsigned int>(iNumVisibleUnits) > iMaxNumUnits)
	{
	...
	if (iNumVisibleUnits > 0)
	{
		...
		{
			setUnitHelp(szString, pPlot->getCenterUnit(), static_cast<unsigned int>(iNumVisibleUnits) > iMaxNumUnits, true);
		}

		unsigned int iNumShown = std::min<unsigned int>(iMaxNumUnits, iNumVisibleUnits);
		for (unsigned int iI = 0; iI < iNumShown && iI < plotUnits.size(); ++iI)
		{
			pLoopUnit = plotUnits[iI];
			...
			for (int iI = 0; iI < GC.getNumUnitInfos(); ++iI)
			{
				for (int iJ = 0; iJ < MAX_PLAYERS; iJ++)
				{
					...
					if (aiUnitNumbers[iIndex] > 0)
					{
						...
						szString.append(CvWString::format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR("COLOR_UNIT_TEXT"), GC.getUnitInfo((UnitTypes)iI).getDescription()));
						...
						if (aiUnitMaxStrength[iIndex] > 0)
						{
							...
							szString.append(CvWString::format(L"%lc", gDLL->getSymbolID(STRENGTH_CHAR)));
						}
						...
						for (int iK = 0; iK < numPromotionInfos; iK++)
						{
							...
							{
								szString.append(CvWString::format(L"%d<img=%hs size=16></img>", m_apbPromotion[iIndex][iK], GC.getPromotionInfo((PromotionTypes)iK).getButton()));
							}
						}
						...
						{
							szString.append(L", ");
							szString.append(CvWString::format(SETCOLR L"%ls" ENDCOLR, GET_PLAYER((PlayerTypes)iJ).getPlayerTextColorR(), GET_PLAYER((PlayerTypes)iJ).getPlayerTextColorG(), GET_PLAYER((PlayerTypes)iJ).getPlayerTextColorB(), GET_PLAYER((PlayerTypes)iJ).getPlayerTextColorA(), GET_PLAYER((PlayerTypes)iJ).getName()));
						}
					}
				}
			}
...
bool CvGameTextMgr::setCombatPlotHelp(CvWStringBuffer &szString, CvPlot* pPlot)
{
	...
	if (pAttacker != nullptr)
	{
		...
		if (pDefender != nullptr && pDefender != pAttacker && pDefender->canDefend(pPlot) && pAttacker->canAttack(*pDefender))
		{
			...
			szOffenseOdds.Format(L"%.2f", ((pAttacker->getDomainType() == DOMAIN_AIR) ? pAttacker->airCurrCombatStrFloat(pDefender) : pAttacker->currCombatStrFloat(nullptr, nullptr)));
			...
			{
				iModifier = pPlot->defenseModifier(pDefender->getTeam(), (pAttacker != nullptr) ? pAttacker->ignoreBuildingDefense() : true);
...
void createTestFontString(CvWStringBuffer& szString)
{
	...
	for (iI=0;iI<NUM_YIELD_TYPES;++iI)
		szString.append(CvWString::format(L"%lc", GC.getYieldInfo((YieldTypes) iI).getChar()));

	szString.append(L"\n");
	for (iI=0;iI<NUM_COMMERCE_TYPES;++iI)
		szString.append(CvWString::format(L"%lc", GC.getCommerceInfo((CommerceTypes) iI).getChar()));
	szString.append(L"\n");
	for (iI = 0; iI < GC.getNumReligionInfos(); ++iI)
	{
		szString.append(CvWString::format(L"%lc", GC.getReligionInfo((ReligionTypes) iI).getChar()));
		szString.append(CvWString::format(L"%lc", GC.getReligionInfo((ReligionTypes) iI).getHolyCityChar()));
	}
	for (iI = 0; iI < GC.getNumCorporationInfos(); ++iI)
	{
		szString.append(CvWString::format(L"%lc", GC.getCorporationInfo((CorporationTypes) iI).getChar()));
		szString.append(CvWString::format(L"%lc", GC.getCorporationInfo((CorporationTypes) iI).getHeadquarterChar()));
	}
	szString.append(L"\n");
	for (iI = 0; iI < GC.getNumBonusInfos(); ++iI)
		szString.append(CvWString::format(L"%lc", GC.getBonusInfo((BonusTypes) iI).getChar()));
	for (iI=0; iI<MAX_NUM_SYMBOLS; ++iI)
		szString.append(CvWString::format(L"%lc", gDLL->getSymbolID(iI)));
}
...
void CvGameTextMgr::setPlotHelp(CvWStringBuffer& szString, CvPlot* pPlot)
{
	...
	{
		if (bShift && pPlot->headUnitNode() != nullptr)
		{
			return;
		}
...
//			}
			...
			for (int iI = 0; iI < MAX_PLAYERS; ++iI)
			{
				...
				if ((eLoopPlayer != ePlayer) && kLoopPlayer.isAlive())
				{
					...
					if (iCloseness != 0)
					{
						...
						szString.append(CvWString::format(L"\n%ls(7) : %d", kLoopPlayer.getName(), iCloseness));
			...
			int iNumOtherCitySites = (pPlot->waterArea() == nullptr) ? 0 : kPlayer.AI_getNumAdjacentAreaCitySites(pPlot->waterArea()->getID(), pPlot->getArea(), iOtherSiteBestValue);
		...
		else if (pPlot->getOwner() != NO_PLAYER)
		{
			...
			{
				szString.append(CvWString::format(L"\n %ls = %d", GC.getCivicInfo((CivicTypes)iI).getDescription(), GET_PLAYER(pPlot->getOwner()).AI_civicValue((CivicTypes)iI)));			
			}
		}
		return;	
	}
	else if (bShift && !bAlt && (gDLL->getChtLvl() > 0))
	{
		...
		if (pPlot->getPlotGroup(GC.getGameINLINE().getActivePlayer()) != nullptr)
		{
...
#else
		szTempBuffer.Format(L"\n(%d, %d) group: %d", pPlot->getX_INLINE(), pPlot->getY_INLINE(), pPlot->tryGetPlotGroupId(GC.getGameINLINE().getActivePlayer()).value_or(-1));
#endif
		szString.append(szTempBuffer);
		...
		szTempBuffer.Format(L"\nWERiverFlow: %lc", tempChar);
		szString.append(szTempBuffer);

		if(pPlot->getRouteType() != NO_ROUTE)
		{
			szTempBuffer.Format(L"\nRoute: %ls", GC.getRouteInfo(pPlot->getRouteType()).getDescription());
			szString.append(szTempBuffer);
		}
		...
		for (int iI = 0; iI < MAX_PLAYERS; ++iI)
		{
			if (GET_PLAYER((PlayerTypes)iI).isAlive())
			{
				...
				{
					szTempBuffer.Format(L"\n%ls Culture: %d", GET_PLAYER((PlayerTypes)iI).getName(), pPlot->getCulture((PlayerTypes)iI));
					szString.append(szTempBuffer);
				}
			}
		}
		...
		if (nullptr != pWorkingCity)
		{
            ...
            {
                szTempBuffer.Format(L"\nBest Build: %ls (%d)", GC.getBuildInfo(eBestBuild).getDescription(), iBuildValue);
                szString.append(szTempBuffer);
            }
		}
		...
		if (pPlot->getPlotCity() != nullptr)
		{
			...
			for (int iI = 0; iI < GC.getNumUnitClassInfos(); ++iI)
			{
				...
				{
					szString.append(CvWString::format(L"\n%ls = %d", GC.getUnitClassInfo((UnitClassTypes)iI).getDescription(), kPlayer.AI_getUnitClassWeight((UnitClassTypes)iI)));
				}
			}
			...
			for (int iI = 0; iI < GC.getNumUnitCombatInfos(); ++iI)
			{
				...
				{
					szString.append(CvWString::format(L"\n%ls = % d", GC.getUnitCombatInfo((UnitCombatTypes)iI).getDescription(), kPlayer.AI_getUnitCombatWeight((UnitCombatTypes)iI)));
				}
			}			
		}
	}
	else if (!bShift && bAlt && (gDLL->getChtLvl() > 0))
	{
	    ...
	    if (pPlot->isOwned())
	    {
            ...
            for (int iI = 0; iI < GC.getNumReligionInfos(); ++iI)
            {
                ...
                {
                    szTempBuffer.Format(L"\nNeeded %lc missionaries = %d", GC.getReligionInfo((ReligionTypes)iI).getChar(), iNeededMissionaries);
                    szString.append(szTempBuffer);
                }
            }
...
//#else
//				int iBestAreaFoundValue = pPlot->area()->getBestFoundValueIsPositive(ePlayer) ? 999'999 : 0;
//#endif
					...
					|| ((pPlot->getOwner() == iI) && (iBestAreaFoundValue > 0)))
				{
					...
					szString.append(CvWString::format(L"%ls: %d", kPlayer.getName(), iActualFoundValue));
...
//#else
						//int iBestFoundValue = kPlayer.isBestFoundValuePositive() ? 999'999 : 0;
//#endif
	...
	else if (bShift && bAlt && (gDLL->getChtLvl() > 0))
	{
		...
		if (pCity != nullptr)
		{
			...
			if (pPlot->getPlotCity() != nullptr)
			{
				...
				}
				{
					...
					for (int iI = 0; iI < NUM_COMMERCE_TYPES; ++iI)
					{
						...
						szTempBuffer.Format(L", %d%lc", iCommerce, GC.getCommerceInfo((CommerceTypes) iI).getChar());
						szString.append(szTempBuffer);
					}
			...
			else
			{
				...
				if (bWorkingPlot)
					szTempBuffer.Format( SETCOLR L"%ls is working" ENDCOLR, TEXT_COLOR("COLOR_ALT_HIGHLIGHT_TEXT"), pCity->getName().GetCString());
				else
					szTempBuffer.Format( SETCOLR L"%ls not working" ENDCOLR, TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), pCity->getName().GetCString());
				szString.append(szTempBuffer);
		...
		if (eBonus != NO_BONUS)
		{
			szString.append(CvWString::format(L"\n%ls values:", GC.getBonusInfo(eBonus).getDescription()));
			...
			for (int iPlayerIndex = 0; iPlayerIndex < MAX_PLAYERS; iPlayerIndex++)
			{
				...
				{
					szString.append(CvWString::format(L"\n %ls: %d", kLoopPlayer.getName(), kLoopPlayer.AI_bonusVal(eBonus)));
				}
			}
		}
	}
	else
	{
		...
		if (eRevealOwner != NO_PLAYER)
		{
			if (pPlot->isActiveVisible(true))
			{
				...
				for (int iPlayer = 0; iPlayer < MAX_PLAYERS; ++iPlayer)
				{
					if (iPlayer != eRevealOwner)
					{
						...
						{
							szTempBuffer.Format(L"%d%% " SETCOLR L"%ls" ENDCOLR, pPlot->calculateCulturePercent((PlayerTypes)iPlayer), kPlayer.getPlayerTextColorR(), kPlayer.getPlayerTextColorG(), kPlayer.getPlayerTextColorB(), kPlayer.getPlayerTextColorA(), kPlayer.getCivilizationAdjective());
							szString.append(szTempBuffer);
							szString.append(NEWLINE);
						}
					}
			...
			{
				szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, GET_PLAYER(eRevealOwner).getPlayerTextColorR(), GET_PLAYER(eRevealOwner).getPlayerTextColorG(), GET_PLAYER(eRevealOwner).getPlayerTextColorB(), GET_PLAYER(eRevealOwner).getPlayerTextColorA(), GET_PLAYER(eRevealOwner).getCivilizationDescription());
				szString.append(szTempBuffer);
				szString.append(NEWLINE);
			}
		}
		...
		if (pPlot->getTerrainType() != NO_TERRAIN)
		{
			...
			else
			{
				...
				if (pPlot->getFeatureType() != NO_FEATURE)
				{
					szTempBuffer.Format(L"%ls/", GC.getFeatureInfo(pPlot->getFeatureType()).getDescription());
					szString.append(szTempBuffer);
				}
		...
		if (pPlot->hasYield())
		{
			for (int iI = 0; iI < NUM_YIELD_TYPES; ++iI)
			{
				...
				if (iYield != 0)
				{
					szTempBuffer.Format(L", %d%lc", iYield, GC.getYieldInfo((YieldTypes) iI).getChar());
					szString.append(szTempBuffer);
				}
			}
		}
		...
		if (eBonus != NO_BONUS)
		{
			szTempBuffer.Format(L"%lc " SETCOLR L"%ls" ENDCOLR, GC.getBonusInfo(eBonus).getChar(), TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getBonusInfo(eBonus).getDescription());
			szString.append(NEWLINE);
			szString.append(szTempBuffer);

			if (GC.getBonusInfo(eBonus).getHealth() != 0)
			{
				szTempBuffer.Format(L", +%d%lc", abs(GC.getBonusInfo(eBonus).getHealth()), ((GC.getBonusInfo(eBonus).getHealth() > 0) ? gDLL->getSymbolID(HEALTHY_CHAR): gDLL->getSymbolID(UNHEALTHY_CHAR)));
				szString.append(szTempBuffer);
			}

			if (GC.getBonusInfo(eBonus).getHappiness() != 0)
			{
				szTempBuffer.Format(L", +%d%lc", abs(GC.getBonusInfo(eBonus).getHappiness()), ((GC.getBonusInfo(eBonus).getHappiness() > 0) ? gDLL->getSymbolID(HAPPY_CHAR): gDLL->getSymbolID(UNHAPPY_CHAR)));
				szString.append(szTempBuffer);
			}
								...
								if (pPlot->canHaveImprovement(((ImprovementTypes)(GC.getBuildInfo((BuildTypes) iI).getImprovement())), GC.getGameINLINE().getActiveTeam(), true))
								{
									...
									for (int k = 0; k < NUM_YIELD_TYPES; k++)
									{
										...
										if (iYieldChange != 0)
										{
											if (iYieldChange > 0)
											{
												szTempBuffer.Format(L"+%d%lc", iYieldChange, GC.getYieldInfo((YieldTypes)k).getChar());
											}
											else
											{
												szTempBuffer.Format(L"%d%lc", iYieldChange, GC.getYieldInfo((YieldTypes)k).getChar());
											}
											setListHelp(szString, L"\n", szTempBuffer.c_str(), L", ", bFirst);
											bFirst = false;
										}
									}
...
void CvGameTextMgr::setCityBarHelp(CvWStringBuffer &szString, CvCity* pCity)
{
	...
	if (pCity->getProductionNeeded() != INT_MAX)
	{
	...
	for (iI = 0; iI < NUM_COMMERCE_TYPES; ++iI)
	{
		...
		if (iRate != 0)
		{
			szTempBuffer.Format(L"%d.%02d %lc", iRate/100, iRate%100, GC.getCommerceInfo((CommerceTypes)iI).getChar());
			setListHelp(szString, NEWLINE, szTempBuffer.c_str(), L", ", bFirst);
			bFirst = false;
		}
	}
	...
	if (iRate != 0)
	{
		szTempBuffer.Format(L"%d%lc", iRate, gDLL->getSymbolID(GREAT_PEOPLE_CHAR));
		setListHelp(szString, NEWLINE, szTempBuffer.c_str(), L", ", bFirst);
		bFirst = false;
	}
	...
	szString.append(CvWString::format(L" -%d.%02d %lc", iMaintenance/100, iMaintenance%100, GC.getCommerceInfo(COMMERCE_GOLD).getChar()));
...
void CvGameTextMgr::parseTraits(CvWStringBuffer &szHelpString, TraitTypes eTrait, CivilizationTypes eCivilization, bool bDawnOfMan)
{
	...
	if (bDawnOfMan)
	{
		szTempBuffer.Format(L"%ls", szText.GetCString());
	}
	else
	{
		szTempBuffer.Format(NEWLINE SETCOLR L"%ls" ENDCOLR, TEXT_COLOR("COLOR_ALT_HIGHLIGHT_TEXT"), szText.GetCString());
	}
	...
	if (!bDawnOfMan)
	{
		...
		for (iI = 0; iI < GC.getNumPromotionInfos(); ++iI)
		{
			if (GC.getTraitInfo(eTrait).isFreePromotion(iI))
			{
				...
				szTempBuffer += CvWString::format(L"<link=literal>%ls</link>", GC.getPromotionInfo((PromotionTypes) iI).getDescription());
		...
		if (bFoundPromotion)
		{
			...
			for (iJ = 0; iJ < GC.getNumUnitCombatInfos(); iJ++)
			{
				...
				{
					szTempBuffer.Format(L"\n        %lc<link=literal>%ls</link>", gDLL->getSymbolID(BULLET_CHAR), GC.getUnitCombatInfo((UnitCombatTypes)iJ).getDescription());
					szHelpString.append(szTempBuffer);
				}
			}
		}
		...
		for (iI = 0; iI < GC.getNumUnitClassInfos();++iI)
		{
			...
			if (eLoopUnit != NO_UNIT && !isWorldUnitClass((UnitClassTypes)iI))
			{
				if (GC.getUnitInfo(eLoopUnit).getProductionTraits(eTrait) != 0)
				{
					...
					CvWString szUnit;
					szUnit.Format(L"<link=literal>%ls</link>", GC.getUnitInfo(eLoopUnit).getDescription());
					setListHelp(szHelpString, szText.GetCString(), szUnit.c_str(), L", ", (GC.getUnitInfo(eLoopUnit).getProductionTraits(eTrait) != iLast));
		...
		for (iI = 0; iI < GC.getNumBuildingClassInfos(); ++iI)
		{
			...
			if (eLoopBuilding != NO_BUILDING && !isWorldWonderClass((BuildingClassTypes)iI))
			{
				if (GC.getBuildingInfo(eLoopBuilding).getProductionTraits(eTrait) != 0)
				{
					...
					szBuilding.Format(L"<link=literal>%ls</link>", GC.getBuildingInfo(eLoopBuilding).getDescription());
		...
		for (iI = 0; iI < GC.getNumBuildingClassInfos(); ++iI)
		{
			...
			if (eLoopBuilding != NO_BUILDING && !isWorldWonderClass((BuildingClassTypes)iI))
			{
				...
				if (iHappiness != 0)
				{
					...
					CvWString szBuilding;
					szBuilding.Format(L"<link=literal>%ls</link>", GC.getBuildingInfo(eLoopBuilding).getDescription());
					setListHelp(szHelpString, szText.GetCString(), szBuilding.c_str(), L", ", (iHappiness != iLast));
					iLast = iHappiness;
				}
			}
		}
...
void CvGameTextMgr::parseLeaderTraits(CvWStringBuffer &szHelpString, LeaderHeadTypes eLeader, CivilizationTypes eCivilization, bool bDawnOfMan, bool bCivilopediaText)
{
	...
	if (eLeader != NO_LEADER)
	{
		...
		{
			szTempBuffer.Format( SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getLeaderHeadInfo(eLeader).getDescription());
			szHelpString.append(szTempBuffer);
		}
	...
	{
		//	Random leader
		szTempBuffer.Format( SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), gDLL->getText("TXT_KEY_TRAIT_PLAYER_UNKNOWN").c_str());
		szHelpString.append(szTempBuffer);
	}
...
void CvGameTextMgr::parseLeaderShortTraits(CvWStringBuffer &szHelpString, LeaderHeadTypes eLeader)
{
	...
	{
		//	Random leader
		szHelpString.append(CvWString("?""?""?/?""?""?")); // fortsnek: Avoid trigraph
	}
...
void CvGameTextMgr::parseCivInfos(CvWStringBuffer &szInfoText, CivilizationTypes eCivilization, bool bDawnOfMan, bool bLinks)
{
	...
	if (eCivilization != NO_CIVILIZATION)
	{
		...
		{
			// Civ Name
			szBuffer.Format(SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getCivilizationInfo(eCivilization).getDescription());
			szInfoText.append(szBuffer);

			// Free Techs
			szBuffer.Format(NEWLINE SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_ALT_HIGHLIGHT_TEXT"), gDLL->getText("TXT_KEY_FREE_TECHS").GetCString());
			szInfoText.append(szBuffer);
			...
			for (int iI = 0; iI < GC.getNumTechInfos(); ++iI)
			{
				if (GC.getCivilizationInfo(eCivilization).isCivilizationFreeTechs(iI))
				{
					bFound = true;
					// Add Tech
					szText.Format((bLinks ? L"<link=literal>%ls</link>" : L"%ls"), GC.getTechInfo((TechTypes)iI).getDescription());
					szBuffer.Format(L"%ls  %lc%ls", NEWLINE, gDLL->getSymbolID(BULLET_CHAR), szText.GetCString());
					szInfoText.append(szBuffer);
				}
			}
			...
			if (!bFound)
			{
				szBuffer.Format(L"%ls  %ls", NEWLINE, gDLL->getText("TXT_KEY_FREE_TECHS_NO").GetCString());
				szInfoText.append(szBuffer);
			}
		}
		...
		if (bDawnOfMan)
		{
			szTempString.Format(L"%ls: ", szText.GetCString());
		}
		else
		{
			szTempString.Format(NEWLINE SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_ALT_HIGHLIGHT_TEXT"), szText.GetCString());
		}
			...
			if ((eDefaultUnit != NO_UNIT) && (eUniqueUnit != NO_UNIT))
			{
				if (eDefaultUnit != eUniqueUnit)
				{
					...
					if (bDawnOfMan)
					{
						...
						szBuffer.Format((bLinks ? L"<link=literal>%ls</link> - (<link=literal>%ls</link>)" : L"%ls - (%ls)"),
							GC.getUnitInfo(eDefaultUnit).getDescription(),
							GC.getUnitInfo(eUniqueUnit).getDescription());
					}
					else
					{
						szBuffer.Format(L"\n  %lc%ls - (%ls)", gDLL->getSymbolID(BULLET_CHAR),
							GC.getUnitInfo(eDefaultUnit).getDescription(),
							GC.getUnitInfo(eUniqueUnit).getDescription());
					}
					szInfoText.append(szBuffer);
		...
		if (!bFound)
		{
			...
			{
				szTempString.Format(L"%ls  %ls", NEWLINE, szText.GetCString());
			}
			szInfoText.append(szTempString);
			bFound = true;
		}
...
void CvGameTextMgr::parseSpecialistHelp(CvWStringBuffer &szHelpString, SpecialistTypes eSpecialist, CvCity* pCity, bool bCivilopediaText)
{
	...
	if (eSpecialist != NO_SPECIALIST)
	{
		...
		for (iI = 0; iI < NUM_YIELD_TYPES; ++iI)
		{
			...
			{
				aiYields[iI] = GET_PLAYER((pCity != nullptr) ? pCity->getOwnerINLINE() : GC.getGameINLINE().getActivePlayer()).specialistYield(eSpecialist, ((YieldTypes)iI));
			}
		}
		...
		for (iI = 0; iI < NUM_COMMERCE_TYPES; ++iI)
		{
			...
			{
				aiCommerces[iI] = GET_PLAYER((pCity != nullptr) ? pCity->getOwnerINLINE() : GC.getGameINLINE().getActivePlayer()).specialistCommerce(((SpecialistTypes)eSpecialist), ((CommerceTypes)iI));
			}
		}
...
void CvGameTextMgr::parseFreeSpecialistHelp(CvWStringBuffer &szHelpString, const CvCity& kCity)
{
	...
	for (int iLoopSpecialist = 0; iLoopSpecialist < GC.getNumSpecialistInfos(); iLoopSpecialist++)
	{
		...
		if (iNumSpecialists > 0)
		{
			...
			szHelpString.append(NEWLINE);
			szHelpString.append(CvWString::format(L"%ls (%d): ", GC.getSpecialistInfo(eSpecialist).getDescription(), iNumSpecialists));
...
void CvGameTextMgr::parseCivicInfo(CvWStringBuffer &szHelpText, CivicTypes eCivic, bool bCivilopediaText, bool bPlayerContext, bool bSkipName)
{
	...
	for (iI = 0; iI < GC.getNumSpecialistInfos(); ++iI)
	{
		...
		{
			szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_CIVIC_UNLIMTED").c_str());
			CvWString szSpecialist;
			szSpecialist.Format(L"<link=literal>%ls</link>", GC.getSpecialistInfo((SpecialistTypes)iI).getDescription());
			setListHelp(szHelpText, szFirstBuffer.c_str(), szSpecialist.c_str(), L", ", bFirst);
			bFirst = false;
		}
	}
	...
	if (GC.getCivicInfo(eCivic).getImprovementUpgradeRateModifier() != 0)
	{
		...
		for (iI = 0; iI < GC.getNumImprovementInfos(); ++iI)
		{
			...
			{
				szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_CIVIC_IMPROVEMENT_UPGRADE", GC.getCivicInfo(eCivic).getImprovementUpgradeRateModifier()).c_str());
				CvWString szImprovement;
				szImprovement.Format(L"<link=literal>%ls</link>", GC.getImprovementInfo((ImprovementTypes)iI).getDescription());
				setListHelp(szHelpText, szFirstBuffer.c_str(), szImprovement.c_str(), L", ", bFirst);
				bFirst = false;
			}
		}
	}
	...
	for (iI = 0; iI < NUM_YIELD_TYPES; ++iI)
	{
		...
		for (iJ = 0; iJ < GC.getNumImprovementInfos(); iJ++)
		{
			...
			{
				szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_CIVIC_IMPROVEMENT_YIELD_CHANGE", GC.getCivicInfo(eCivic).getImprovementYieldChanges(iJ, iI), GC.getYieldInfo((YieldTypes)iI).getChar()).c_str());
				CvWString szImprovement;
				szImprovement.Format(L"<link=literal>%ls</link>", GC.getImprovementInfo((ImprovementTypes)iJ).getDescription());
				setListHelp(szHelpText, szFirstBuffer.c_str(), szImprovement.c_str(), L", ", (GC.getCivicInfo(eCivic).getImprovementYieldChanges(iJ, iI) != iLast));
	...
	for (iI = 0; iI < GC.getNumFeatureInfos(); ++iI)
	{
		...
		{
			szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_CIVIC_FEATURE_HAPPINESS", GC.getCivicInfo(eCivic).getFeatureHappinessChanges(iI), ((GC.getCivicInfo(eCivic).getFeatureHappinessChanges(iI) > 0) ? gDLL->getSymbolID(HAPPY_CHAR) : gDLL->getSymbolID(UNHAPPY_CHAR))).c_str());
			CvWString szFeature;
			szFeature.Format(L"<link=literal>%ls</link>", GC.getFeatureInfo((FeatureTypes)iI).getDescription());
			setListHelp(szHelpText, szFirstBuffer.c_str(), szFeature.c_str(), L", ", (GC.getCivicInfo(eCivic).getFeatureHappinessChanges(iI) != iLast));
	...
	for (iI = 0; iI < GC.getNumHurryInfos(); ++iI)
	{
		...
		{
			szHelpText.append(CvWString::format(L"%ls%lc%ls", NEWLINE, gDLL->getSymbolID(BULLET_CHAR), GC.getHurryInfo((HurryTypes)iI).getDescription()));
		}
	}
	...
	{
		szHelpText.append(CvWString::format(L"%ls%ls", NEWLINE, GC.getCivicInfo(eCivic).getHelp()).c_str());
	}
}
...
void CvGameTextMgr::setTechHelp(CvWStringBuffer &szBuffer, TechTypes eTech, bool bCivilopediaText, bool bPlayerContext, bool bStrategyText, bool bTreeInfo, TechTypes eFromTech)
{
    ...
    if (bAlt && (gDLL->getChtLvl() > 0))
    {
		...
		for (int iI = 0; iI < MAX_PLAYERS; ++iI)
		{
			...
			if (playerI->isAlive())
			{
				szTempBuffer.Format(L"%ls: ", playerI->getName());
	...
	{
		szTempBuffer.Format( SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_TECH_TEXT"), GC.getTechInfo(eTech).getDescription());
		szBuffer.append(szTempBuffer);
	}
			...
			if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).isProductionMaxedUnitClass((UnitClassTypes)iI)))
			{
				...
				if (eLoopUnit != NO_UNIT)
				{
					if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).canTrain(eLoopUnit)))
					{
						...
						{
							szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_TECH_CAN_TRAIN").c_str());
							szTempBuffer.Format( SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_UNIT_TEXT"), GC.getUnitInfo(eLoopUnit).getDescription());
							setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", bFirst);
							bFirst = false;
						}
						else
						{
			...
			if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).isProductionMaxedBuildingClass((BuildingClassTypes)iI)))
			{
				...
				if (eLoopBuilding != NO_BUILDING)
				{
					if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).canConstruct(eLoopBuilding, false, true)))
					{
						...
						{
							szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_TECH_CAN_CONSTRUCT").c_str());
							szTempBuffer.Format( SETCOLR L"<link=literal>%ls</link>" ENDCOLR , TEXT_COLOR("COLOR_BUILDING_TEXT"), GC.getBuildingInfo(eLoopBuilding).getDescription());
							setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", bFirst);
							bFirst = false;
						}
						else
		...
		for (int iI = 0; iI < GC.getNumProjectInfos(); ++iI)
		{
			if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).isProductionMaxedProject((ProjectTypes)iI)))
			{
				if (!bPlayerContext || !(GET_PLAYER(GC.getGameINLINE().getActivePlayer()).canCreate(((ProjectTypes)iI), false, true)))
				{
					...
					{
						szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_TECH_CAN_CREATE").c_str());
						szTempBuffer.Format( SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_PROJECT_TEXT"), GC.getProjectInfo((ProjectTypes)iI).getDescription());
						setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", bFirst);
						bFirst = false;
					}
				}
	...
	{
		szBuffer.append(CvWString::format(L"%ls%ls", NEWLINE, GC.getTechInfo(eTech).getHelp()).c_str());
	}
	...
	if (!bCivilopediaText)
	{
		if (GC.getGameINLINE().getActivePlayer() == NO_PLAYER)
		{
			szTempBuffer.Format(L"\n%d%lc", GC.getTechInfo(eTech).getResearchCost(), GC.getCommerceInfo(COMMERCE_RESEARCH).getChar());
			szBuffer.append(szTempBuffer);
		}
		else if (GET_TEAM(GC.getGameINLINE().getActiveTeam()).isHasTech(eTech))
		{
			szTempBuffer.Format(L"\n%d%lc", GET_TEAM(GC.getGameINLINE().getActiveTeam()).getResearchCost(eTech), GC.getCommerceInfo(COMMERCE_RESEARCH).getChar());
			szBuffer.append(szTempBuffer);
		}
		else
		{
			...
			szTempBuffer.Format(L" (%d/%d %lc)", GET_TEAM(GC.getGameINLINE().getActiveTeam()).getResearchProgress(eTech), GET_TEAM(GC.getGameINLINE().getActiveTeam()).getResearchCost(eTech), GC.getCommerceInfo(COMMERCE_RESEARCH).getChar());
			szBuffer.append(szTempBuffer);
		}
	}
...
void CvGameTextMgr::setBasicUnitHelp(CvWStringBuffer &szBuffer, UnitTypes eUnit, bool bCivilopediaText)
{
	...
	if (!bCivilopediaText)
	{
		...
		if (GC.getUnitInfo(eUnit).getDomainType() == DOMAIN_AIR)
		{
			...
			{
				szTempBuffer.Format(L"%d%lc, ", GC.getUnitInfo(eUnit).getAirCombat(), gDLL->getSymbolID(STRENGTH_CHAR));
				szBuffer.append(szTempBuffer);
			}
		}
		else
		{
			...
			{
				szTempBuffer.Format(L"%d%lc, ", GC.getUnitInfo(eUnit).getCombat(), gDLL->getSymbolID(STRENGTH_CHAR));
				szBuffer.append(szTempBuffer);
			}
		}

		szTempBuffer.Format(L"%d%lc", GC.getUnitInfo(eUnit).getMoves(), gDLL->getSymbolID(MOVES_CHAR));
		szBuffer.append(szTempBuffer);
	...
	{
		szBuffer.append(CvWString::format(L"%ls%lc%ls", NEWLINE, gDLL->getSymbolID(BULLET_CHAR), gDLL->getText("TXT_KEY_PROMOTION_WHEN_LEADING").GetCString()));
	...
	for (iI = 0; iI < GC.getNumReligionInfos(); ++iI)
	{
		...
		{
			szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_UNIT_CAN_SPREAD").c_str());
			CvWString szReligion;
			szReligion.Format(L"<link=literal>%ls</link>", GC.getReligionInfo((ReligionTypes) iI).getDescription());
			setListHelp(szBuffer, szTempBuffer.c_str(), szReligion.c_str(), L", ", bFirst);
			bFirst = false;
		}
	}
	...
	for (iI = 0; iI < GC.getNumCorporationInfos(); ++iI)
	{
		...
		{
			szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_UNIT_CAN_EXPAND").c_str());
			CvWString szCorporation;
			szCorporation.Format(L"<link=literal>%ls</link>", GC.getCorporationInfo((CorporationTypes) iI).getDescription());
			setListHelp(szBuffer, szTempBuffer.c_str(), szCorporation.c_str(), L", ", bFirst);
			bFirst = false;
		}
	}
	...
	for (iI = 0; iI < GC.getNumSpecialistInfos(); ++iI)
	{
		...
		{
			szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_UNIT_CAN_JOIN").c_str());
			CvWString szSpecialistLink = CvWString::format(L"<link=literal>%ls</link>", GC.getSpecialistInfo((SpecialistTypes) iI).getDescription());
			setListHelp(szBuffer, szTempBuffer.c_str(), szSpecialistLink.GetCString(), L", ", bFirst);
			bFirst = false;
		}
	}
	...
	for (iI = 0; iI < GC.getNumBuildingInfos(); ++iI)
	{
		...
		{
			szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_UNIT_CAN_CONSTRUCT").c_str());
			CvWString szBuildingLink = CvWString::format(L"<link=literal>%ls</link>", GC.getBuildingInfo((BuildingTypes) iI).getDescription());
			setListHelp(szBuffer, szTempBuffer.c_str(), szBuildingLink.GetCString(), L", ", bFirst);
			bFirst = false;
		}
	}
	...
	szTempBuffer.Format(L"%ls%ls ", NEWLINE, gDLL->getText("TXT_KEY_UNIT_CANNOT_ENTER").GetCString());
	...
	for (iI = 0; iI < GC.getNumTerrainInfos(); ++iI)
	{
		if (GC.getUnitInfo(eUnit).getTerrainImpassable(iI))
		{
			...
			if (NO_TECH == eTech)
			{
				szTerrain.Format(L"<link=literal>%ls</link>", GC.getTerrainInfo((TerrainTypes)iI).getDescription());
			}
			else
			{
			...
			setListHelp(szBuffer, szTempBuffer.c_str(), szTerrain.c_str(), L", ", bFirst);
			bFirst = false;
		}
	}
	...
	for (iI = 0; iI < GC.getNumFeatureInfos(); ++iI)
	{
		if (GC.getUnitInfo(eUnit).getFeatureImpassable(iI))
		{
			...
			setListHelp(szBuffer, szTempBuffer.c_str(), szFeature.c_str(), L", ", bFirst);
			bFirst = false;
		}
	}
	...
	else
	{
		...
		for (iI = 0; iI < GC.getNumBuildInfos(); ++iI)
		{
			if (GC.getUnitInfo(eUnit).getBuilds(iI))
			{
				szTempBuffer.Format(L"%ls%ls ", NEWLINE, gDLL->getText("TXT_KEY_UNIT_CAN").c_str());
	...
	for (iI = 0; iI < GC.getNumPromotionInfos(); ++iI)
	{
		...
		{
			szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_UNIT_STARTS_WITH").c_str());
			setListHelp(szBuffer, szTempBuffer.c_str(), CvWString::format(L"<link=literal>%ls</link>", GC.getPromotionInfo((PromotionTypes) iI).getDescription()).c_str(), L", ", bFirst);
			bFirst = false;
		}
	}
...
void CvGameTextMgr::setUnitHelp(CvWStringBuffer &szBuffer, UnitTypes eUnit, bool bCivilopediaText, bool bStrategyText, bool bTechChooserText, CvCity* pCity)
{
	...
	if (!bCivilopediaText)
	{
		szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_UNIT_TEXT"), GC.getUnitInfo(eUnit).getDescription());
		szBuffer.append(szTempBuffer);

		if (GC.getUnitInfo(eUnit).getUnitCombatType() != NO_UNITCOMBAT)
		{
			szTempBuffer.Format(L" (%ls)", GC.getUnitCombatInfo((UnitCombatTypes) GC.getUnitInfo(eUnit).getUnitCombatType()).getDescription());
			szBuffer.append(szTempBuffer);
		}
	}
	...
	if ((pCity == nullptr) || !(pCity->canTrain(eUnit)))
	{
		...
		{
			if ((pCity == nullptr) || !(pCity->isHolyCity((ReligionTypes)(GC.getUnitInfo(eUnit).getHolyCity()))))
			{
				szBuffer.append(NEWLINE);
		...
		{
			if ((pCity == nullptr) || !(GC.getGameINLINE().isSpecialUnitValid((SpecialUnitTypes)(GC.getUnitInfo(eUnit).getSpecialUnitType()))))
			{
				for (iI = 0; iI < GC.getNumProjectInfos(); ++iI)
				{
					if (GC.getProjectInfo((ProjectTypes)iI).getEveryoneSpecialUnit() == GC.getUnitInfo(eUnit).getSpecialUnitType())
					{
						szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_REQUIRES").c_str());
						CvWString szProject;
						szProject.Format(L"<link=literal>%ls</link>", GC.getProjectInfo((ProjectTypes)iI).getDescription());
						setListHelp(szBuffer, szTempBuffer.c_str(), szProject.c_str(), gDLL->getText("TXT_KEY_OR").c_str(), bFirst);
						bFirst = false;
					}
				}
			}
		...
		if (GC.getUnitInfo(eUnit).getNukeRange() != -1)
		{
			if (nullptr == pCity || !GC.getGameINLINE().isNukesValid())
			{
				for (iI = 0; iI < GC.getNumProjectInfos(); ++iI)
				{
				...
				for (iI = 0; iI < GC.getNumBuildingInfos(); ++iI)
				{
					if (GC.getBuildingInfo((BuildingTypes)iI).isAllowsNukes())
					{
						szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_REQUIRES").c_str());
						CvWString szBuilding;
						szBuilding.Format(L"<link=literal>%ls</link>", GC.getBuildingInfo((BuildingTypes)iI).getDescription());
						setListHelp(szBuffer, szTempBuffer.c_str(), szBuilding.c_str(), gDLL->getText("TXT_KEY_OR").c_str(), bFirst);
						bFirst = false;
					}
				}
			}
		...
		if (!bCivilopediaText)
		{
			...
			{
				if ((pCity == nullptr) || (pCity->getNumBuilding((BuildingTypes)(GC.getUnitInfo(eUnit).getPrereqBuilding())) <= 0))
				{
					szBuffer.append(NEWLINE);
			...
			for (iI = 0; iI < GC.getNUM_UNIT_AND_TECH_PREREQS(); ++iI)
			{
				if (GC.getUnitInfo(eUnit).getPrereqAndTechs(iI) != NO_TECH)
				{
					...
					{
						szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_REQUIRES").c_str());
						setListHelp(szBuffer, szTempBuffer.c_str(), GC.getTechInfo(((TechTypes)(GC.getUnitInfo(eUnit).getPrereqAndTechs(iI)))).getDescription(), gDLL->getText("TXT_KEY_AND").c_str(), bFirst);
						bFirst = false;
					}
				}
			}
			...
			{
				if ((pCity == nullptr) || !(pCity->hasBonus((BonusTypes)GC.getUnitInfo(eUnit).getPrereqAndBonus())))
				{
					szBuffer.append(NEWLINE);
			...
			for (iI = 0; iI < GC.getNUM_UNIT_PREREQ_OR_BONUSES(); ++iI)
			{
				...
				{
					if ((pCity == nullptr) || !(pCity->hasBonus((BonusTypes)GC.getUnitInfo(eUnit).getPrereqOrBonuses(iI))))
					{
						szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_REQUIRES").c_str());
						setListHelp(szBuffer, szTempBuffer.c_str(), GC.getBonusInfo((BonusTypes) GC.getUnitInfo(eUnit).getPrereqOrBonuses(iI)).getDescription(), gDLL->getText("TXT_KEY_OR").c_str(), bFirst);
						bFirst = false;
					}
				}
			}
	...
	if (!bCivilopediaText && GC.getGameINLINE().getActivePlayer() != NO_PLAYER)
	{
		if (pCity == nullptr)
		{
			szTempBuffer.Format(L"%ls%d%lc", NEWLINE, GET_PLAYER(ePlayer).getProductionNeeded(eUnit), GC.getYieldInfo(YIELD_PRODUCTION).getChar());
			szBuffer.append(szTempBuffer);
		}
		else
		{
			...
			if (iProduction > 0)
			{
				szTempBuffer.Format(L" - %d/%d%lc", iProduction, pCity->getProductionNeeded(eUnit), GC.getYieldInfo(YIELD_PRODUCTION).getChar());
				szBuffer.append(szTempBuffer);
			}
			else
			{
				szTempBuffer.Format(L" - %d%lc", pCity->getProductionNeeded(eUnit), GC.getYieldInfo(YIELD_PRODUCTION).getChar());
				szBuffer.append(szTempBuffer);
			}
		}
	}
	...
	for (iI = 0; iI < GC.getNumBonusInfos(); ++iI)
	{
		if (GC.getUnitInfo(eUnit).getBonusProductionModifier(iI) != 0)
		{
			...
			{
				szTempBuffer.Format(L"%ls%lc", NEWLINE, gDLL->getSymbolID(BULLET_CHAR));
				szBuffer.append(szTempBuffer);
			}
		...
		if ((gDLL->getChtLvl() > 0) && gDLL->ctrlKey())
		{
			...
			for (int iUnitAI = 0; iUnitAI < NUM_UNITAI_TYPES; iUnitAI++)
			{
				...
				if (iTempValue != 0)
				{
					...
					szBuffer.append(CvWString::format(L"(%ls : %d) ", szTempString.GetCString(), iTempValue));
				}
			}
		}
	}
...
void CvGameTextMgr::setBuildingHelp(CvWStringBuffer &szBuffer, BuildingTypes eBuilding, bool bCivilopediaText, bool bStrategyText, bool bTechChooserText, CvCity* pCity)
{
	...
	if (!bCivilopediaText)
	{
		szTempBuffer.Format( SETCOLR L"<link=literal>%ls</link>" ENDCOLR , TEXT_COLOR("COLOR_BUILDING_TEXT"), kBuilding.getDescription());
		szBuffer.append(szTempBuffer);
		...
		if (iHappiness != 0)
		{
			szTempBuffer.Format(L", +%d%lc", abs(iHappiness), ((iHappiness > 0) ? gDLL->getSymbolID(HAPPY_CHAR) : gDLL->getSymbolID(UNHAPPY_CHAR)));
			szBuffer.append(szTempBuffer);
		}
		...
		if (iHealth != 0)
		{
			szTempBuffer.Format(L", +%d%lc", abs(iHealth), ((iHealth > 0) ? gDLL->getSymbolID(HEALTHY_CHAR): gDLL->getSymbolID(UNHEALTHY_CHAR)));
			szBuffer.append(szTempBuffer);
		}
		...
		for (int iI = 0; iI < NUM_COMMERCE_TYPES; ++iI)
		{
			if ((nullptr != pCity) && (pCity->getNumBuilding(eBuilding) > 0))
			{
		...
		{
			szTempBuffer.Format(L", %ls%d%lc", ((kBuilding.getGreatPeopleRateChange() > 0) ? L"+" : L""), kBuilding.getGreatPeopleRateChange(), gDLL->getSymbolID(GREAT_PEOPLE_CHAR));
			szBuffer.append(szTempBuffer);
			...
			if (kBuilding.getGreatPeopleUnitClass() != NO_UNITCLASS)
			{
				...
				{
					szTempBuffer.Format(L" (%ls)", GC.getUnitInfo(eGreatPeopleUnit).getDescription());
					szBuffer.append(szTempBuffer);
				}
			}
		}
	...
	else
	{
		...
		{
			if (pCity == nullptr || ePlayer == NO_PLAYER)
			{
				szBuffer.append(NEWLINE);
	...
	if (kBuilding.getFreeBonus() != NO_BONUS)
	{
		...
		{
			szTempBuffer.Format(L", +%d%lc", abs(GC.getBonusInfo((BonusTypes) kBuilding.getFreeBonus()).getHealth()), ((GC.getBonusInfo((BonusTypes)(kBuilding.getFreeBonus())).getHealth() > 0) ? gDLL->getSymbolID(HEALTHY_CHAR): gDLL->getSymbolID(UNHEALTHY_CHAR)));
			szBuffer.append(szTempBuffer);
		}

		if (GC.getBonusInfo((BonusTypes)(kBuilding.getFreeBonus())).getHappiness() != 0)
		{
			szTempBuffer.Format(L", +%d%lc", abs(GC.getBonusInfo((BonusTypes) kBuilding.getFreeBonus()).getHappiness()), ((GC.getBonusInfo((BonusTypes)(kBuilding.getFreeBonus())).getHappiness() > 0) ? gDLL->getSymbolID(HAPPY_CHAR) : gDLL->getSymbolID(UNHAPPY_CHAR)));
			szBuffer.append(szTempBuffer);
		}
	}
	...
	if (kBuilding.isPower())
	{
		...
		{
			szTempBuffer.Format(L" (+%d%lc)", abs(GC.getDefineINT("DIRTY_POWER_HEALTH_CHANGE")), ((GC.getDefineINT("DIRTY_POWER_HEALTH_CHANGE") > 0) ? gDLL->getSymbolID(HEALTHY_CHAR): gDLL->getSymbolID(UNHEALTHY_CHAR)));
			szBuffer.append(szTempBuffer);
		}
	}
	...
	for (int iI = 0; iI < GC.getNumReligionInfos(); ++iI)
	{
		...
		{
			szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_BUILDING_SPREADS_RELIGION", GC.getReligionInfo((ReligionTypes) iI).getChar()).c_str());
			szBuffer.append(szTempBuffer);
		}
	}
	...
	for (int iI = 0; iI < GC.getNumImprovementInfos(); ++iI)
	{
		...
		{
			szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_BUILDING_IMPROVEMENT_FREE_SPECIALISTS", kBuilding.getImprovementFreeSpecialist(iI)).GetCString() );
			szTempBuffer.Format(L"<link=literal>%ls</link>", GC.getImprovementInfo((ImprovementTypes)iI).getDescription());
			setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", (kBuilding.getImprovementFreeSpecialist(iI) != iLast));
	...
	for (int iI = 0; iI < GC.getNumBonusInfos(); ++iI)
	{
		...
		{
			szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_BUILDING_HEALTH_HAPPINESS_CHANGE", abs(kBuilding.getBonusHealthChanges(iI)), ((kBuilding.getBonusHealthChanges(iI) > 0) ? gDLL->getSymbolID(HEALTHY_CHAR): gDLL->getSymbolID(UNHEALTHY_CHAR))).c_str());
			szTempBuffer.Format(L"<link=literal>%ls</link>", GC.getBonusInfo((BonusTypes)iI).getDescription());
			setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", (kBuilding.getBonusHealthChanges(iI) != iLast));
	...
	for (int iI = 0; iI < GC.getNumCivicInfos(); ++iI)
	{
		...
		if (0 != iChange)
		{
			szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_BUILDING_CIVIC_HEALTH_HAPPINESS_CHANGE", abs(iChange), ((iChange > 0) ? gDLL->getSymbolID(HEALTHY_CHAR): gDLL->getSymbolID(UNHEALTHY_CHAR))).c_str());
			szTempBuffer.Format(L"<link=literal>%ls</link>", GC.getCivicInfo((CivicTypes)iI).getDescription());
			setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", (iChange != iLast));
			iLast = iChange;
		}
	}
	...
	for (int iI = 0; iI < GC.getNumBonusInfos(); ++iI)
	{
		...
		{
			szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_BUILDING_HEALTH_HAPPINESS_CHANGE", abs(kBuilding.getBonusHappinessChanges(iI)), ((kBuilding.getBonusHappinessChanges(iI) > 0) ? gDLL->getSymbolID(HAPPY_CHAR) : gDLL->getSymbolID(UNHAPPY_CHAR))).c_str());
			szTempBuffer.Format(L"<link=literal>%ls</link>", GC.getBonusInfo((BonusTypes)iI).getDescription());
			setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", (kBuilding.getBonusHappinessChanges(iI) != iLast));
	...
	for (int iI = 0; iI < GC.getNumCivicInfos(); ++iI)
	{
		...
		if (0 != iChange)
		{
			szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_BUILDING_CIVIC_HEALTH_HAPPINESS_CHANGE", abs(iChange), ((iChange > 0) ? gDLL->getSymbolID(HAPPY_CHAR): gDLL->getSymbolID(UNHAPPY_CHAR))).c_str());
			szTempBuffer.Format(L"<link=literal>%ls</link>", GC.getCivicInfo((CivicTypes)iI).getDescription());
			setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", (iChange != iLast));
			iLast = iChange;
		}
	...
	for (int iI = 0; iI < GC.getNumUnitInfos(); ++iI)
	{
		...
		{
			szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_BUILDING_REQUIRED_TO_TRAIN").c_str());
			szTempBuffer.Format( SETCOLR L"<link=literal>%ls</link>" ENDCOLR , TEXT_COLOR("COLOR_UNIT_TEXT"), GC.getUnitInfo((UnitTypes)iI).getDescription());
			setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", bFirst);
			bFirst = false;
		}
	}
	...
	for (int iI = 0; iI < GC.getNumUnitInfos(); ++iI)
	{
		...
		{
			szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_UNIT_REQUIRED_TO_BUILD").c_str());
			szTempBuffer.Format( SETCOLR L"<link=literal>%ls</link>" ENDCOLR , TEXT_COLOR("COLOR_UNIT_TEXT"), GC.getUnitInfo((UnitTypes)iI).getDescription());
			setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", bFirst);
			bFirst = false;
		}
	...
	for (int iI = 0; iI < GC.getNumBuildingClassInfos(); ++iI)
	{
		...
		{
			szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_BUILDING_HAPPINESS_CHANGE", kBuilding.getBuildingHappinessChanges(iI),
			...
			if (NO_PLAYER != ePlayer)
			{
				...
				{
					szBuilding.Format(L"<link=literal>%ls</link>", GC.getBuildingClassInfo((BuildingClassTypes)iI).getDescription());
				}
			}
			else
			{
			...
			setListHelp(szBuffer, szTempBuffer.c_str(), szBuilding.c_str(), L", ", (kBuilding.getBuildingHappinessChanges(iI) != iLast));
	...
	for (int iI = 0; iI < GC.getNumBuildingClassInfos(); ++iI)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			if (GC.getBuildingInfo(eLoopBuilding).isBuildingClassNeededInCity(kBuilding.getBuildingClassType()))
			{
				...
				{
					szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_BUILDING_REQUIRED_TO_BUILD").c_str());
					szTempBuffer.Format(SETCOLR L"<link=literal>%ls</link>" ENDCOLR, TEXT_COLOR("COLOR_BUILDING_TEXT"), GC.getBuildingInfo(eLoopBuilding).getDescription());
					setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", bFirst);
					bFirst = false;
				}
	...
	if ((pCity == nullptr) || pCity->getNumRealBuilding(eBuilding) < GC.getCITY_MAX_NUM_BUILDINGS())
	{
		if (!bCivilopediaText)
		{
			if (pCity == nullptr)
			{
				...
				{
					szTempBuffer.Format(L"\n%d%lc", (ePlayer != NO_PLAYER ? GET_PLAYER(ePlayer).getProductionNeeded(eBuilding) : kBuilding.getProductionCost()), GC.getYieldInfo(YIELD_PRODUCTION).getChar());
					szBuffer.append(szTempBuffer);
				}
			}
			else
			{
				...
				if (iProduction > 0)
				{
					szTempBuffer.Format(L" - %d/%d%lc", iProduction, iProductionNeeded, GC.getYieldInfo(YIELD_PRODUCTION).getChar());
					szBuffer.append(szTempBuffer);
				}
				else
				{
					szTempBuffer.Format(L" - %d%lc", iProductionNeeded, GC.getYieldInfo(YIELD_PRODUCTION).getChar());
					szBuffer.append(szTempBuffer);
				}
			}
		}
		...
		for (int iI = 0; iI < GC.getNumBonusInfos(); ++iI)
		{
			if (kBuilding.getBonusProductionModifier(iI) != 0)
			{
				...
				{
					szTempBuffer.Format(L"\n%lc", gDLL->getSymbolID(BULLET_CHAR));
					szBuffer.append(szTempBuffer);
				}
...
void CvGameTextMgr::buildBuildingRequiresString(CvWStringBuffer& szBuffer, BuildingTypes eBuilding, bool bCivilopediaText, bool bTechChooserText, const CvCity* pCity)
{
	...
	if (nullptr == pCity || !pCity->canConstruct(eBuilding))
	{
		if (kBuilding.getHolyCity() != NO_RELIGION)
		{
			if (pCity == nullptr || !pCity->isHolyCity((ReligionTypes)(kBuilding.getHolyCity())))
			{
				szBuffer.append(NEWLINE);
		...
		if (kBuilding.getSpecialBuildingType() != NO_SPECIALBUILDING)
		{
			if ((pCity == nullptr) || !(GC.getGameINLINE().isSpecialBuildingValid((SpecialBuildingTypes)(kBuilding.getSpecialBuildingType()))))
			{
				for (int iI = 0; iI < GC.getNumProjectInfos(); ++iI)
				{
		...
		for (int iI = 0; iI < GC.getNumBuildingClassInfos(); ++iI)
		{
			...
			{
				eLoopBuilding = (BuildingTypes)GC.getBuildingClassInfo((BuildingClassTypes)iI).getDefaultBuildingIndex();
				szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_BUILDING_REQUIRES_NUM_SPECIAL_BUILDINGS_NO_CITY", GC.getBuildingInfo(eLoopBuilding).getTextKeyWide(), kBuilding.getPrereqNumOfBuildingClass((BuildingClassTypes)iI)).c_str());
			...
			else if (ePlayer != NO_PLAYER && GET_PLAYER(ePlayer).getBuildingClassPrereqBuilding(eBuilding, ((BuildingClassTypes)iI)) > 0)
			{
				if ((pCity == nullptr) || (GET_PLAYER(ePlayer).getBuildingClassCount((BuildingClassTypes)iI) < GET_PLAYER(ePlayer).getBuildingClassPrereqBuilding(eBuilding, ((BuildingClassTypes)iI))))
				{
					...
					if (eLoopBuilding != NO_BUILDING)
					{
						if (pCity != nullptr)
						{
							szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_BUILDING_REQUIRES_NUM_SPECIAL_BUILDINGS", GC.getBuildingInfo(eLoopBuilding).getTextKeyWide(), GET_PLAYER(ePlayer).getBuildingClassCount((BuildingClassTypes)iI), GET_PLAYER(ePlayer).getBuildingClassPrereqBuilding(eBuilding, ((BuildingClassTypes)iI))).c_str());
						}
						else
						{
							szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_BUILDING_REQUIRES_NUM_SPECIAL_BUILDINGS_NO_CITY", GC.getBuildingInfo(eLoopBuilding).getTextKeyWide(), GET_PLAYER(ePlayer).getBuildingClassPrereqBuilding(eBuilding, ((BuildingClassTypes)iI))).c_str());
						}
			...
			else if (kBuilding.isBuildingClassNeededInCity(iI))
			{
				...
				if (eLoopBuilding != NO_BUILDING)
				{
					if ((pCity == nullptr) || (pCity->getNumBuilding(eLoopBuilding) <= 0))
					{
						szBuffer.append(NEWLINE);
		...
		{
			if (nullptr == pCity || NO_PLAYER == ePlayer || NO_RELIGION == GET_PLAYER(ePlayer).getStateReligion() || !pCity->isHasReligion(GET_PLAYER(ePlayer).getStateReligion()))
			{
				szBuffer.append(NEWLINE);
		...
		{
			if (nullptr == pCity || pCity->plot()->getLatitude() < kBuilding.getMinLatitude())
			{
				szBuffer.append(NEWLINE);
		...
		{
			if (nullptr == pCity || pCity->plot()->getLatitude() > kBuilding.getMaxLatitude())
			{
				szBuffer.append(NEWLINE);
		...
		else
		{
			...
			for (int iI = 0; iI < GC.getNUM_BUILDING_AND_TECH_PREREQS(); ++iI)
			{
				if (kBuilding.getPrereqAndTechs(iI) != NO_TECH)
				{
					...
					{
						szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_REQUIRES").c_str());
						setListHelp(szBuffer, szTempBuffer.c_str(), GC.getTechInfo(((TechTypes)(kBuilding.getPrereqAndTechs(iI)))).getDescription(), gDLL->getText("TXT_KEY_AND").c_str(), bFirst);
						bFirst = false;
					}
				}
			}
			...
			{
				if ((pCity == nullptr) || !(pCity->hasBonus((BonusTypes)kBuilding.getPrereqAndBonus())))
				{
					szBuffer.append(NEWLINE);
			...
			for (int iI = 0; iI < GC.getNUM_BUILDING_PREREQ_OR_BONUSES(); ++iI)
			{
				...
				{
					if ((pCity == nullptr) || !(pCity->hasBonus((BonusTypes)kBuilding.getPrereqOrBonuses(iI))))
					{
						szTempBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_REQUIRES").c_str());
						setListHelp(szBonusList, szTempBuffer.c_str(), GC.getBonusInfo((BonusTypes)kBuilding.getPrereqOrBonuses(iI)).getDescription(), gDLL->getText("TXT_KEY_OR").c_str(), bFirst);
						bFirst = false;
					}
			...
			if (NO_CORPORATION != kBuilding.getFoundsCorporation())
			{
				...
				for (int iI = 0; iI < GC.getNUM_CORPORATION_PREREQ_BONUSES(); ++iI)
				{
					...
					if (NO_BONUS != eBonus)
					{
						if ((pCity == nullptr) || !(pCity->hasBonus(eBonus)))
						{
							szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_REQUIRES").c_str());
							szTempBuffer.Format(L"<link=literal>%ls</link>", GC.getBonusInfo(eBonus).getDescription());
							setListHelp(szBonusList, szFirstBuffer.c_str(), szTempBuffer.c_str(), gDLL->getText("TXT_KEY_OR").c_str(), bFirst);
							bFirst = false;
						}
						else if (nullptr != pCity)
						{
...
void CvGameTextMgr::setProjectHelp(CvWStringBuffer &szBuffer, ProjectTypes eProject, bool bCivilopediaText, CvCity* pCity)
{
	...
	if (!bCivilopediaText)
	{
		szTempBuffer.Format( SETCOLR L"<link=literal>%ls</link>" ENDCOLR , TEXT_COLOR("COLOR_PROJECT_TEXT"), kProject.getDescription());
		szBuffer.append(szTempBuffer);
	...
	for (iI = 0; iI < GC.getNumProjectInfos(); ++iI)
	{
		...
		{
			szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_PROJECT_REQUIRED_TO_CREATE_ANYONE").c_str());
			szTempBuffer.Format(SETCOLR L"<link=literal>%ls</link>" ENDCOLR, TEXT_COLOR("COLOR_PROJECT_TEXT"), GC.getProjectInfo((ProjectTypes)iI).getDescription());
			setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", bFirst);
			bFirst = false;
		}
	}
	...
	for (iI = 0; iI < GC.getNumProjectInfos(); ++iI)
	{
		...
		{
			if ((pCity == nullptr) || pCity->canCreate(((ProjectTypes)iI), false, true))
			{
				szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_PROJECT_REQUIRED_TO_CREATE").c_str());
				szTempBuffer.Format(SETCOLR L"<link=literal>%ls</link>" ENDCOLR, TEXT_COLOR("COLOR_PROJECT_TEXT"), GC.getProjectInfo((ProjectTypes)iI).getDescription());
				setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", bFirst);
				bFirst = false;
			}
	...
	if ((pCity == nullptr) || !(pCity->canCreate(eProject)))
	{
		if (pCity != nullptr)
		{
			if (GC.getGameINLINE().isNoNukes())
		...
		{
			if ((pCity == nullptr) || (GC.getGameINLINE().getProjectCreatedCount((ProjectTypes)(kProject.getAnyoneProjectPrereq())) == 0))
			{
				szBuffer.append(NEWLINE);
		...
		for (iI = 0; iI < GC.getNumProjectInfos(); ++iI)
		{
			...
			{
				if ((pCity == nullptr) || (GET_TEAM(GET_PLAYER(ePlayer).getTeam()).getProjectCount((ProjectTypes)iI) < kProject.getProjectsNeeded(iI)))
				{
					if (pCity != nullptr)
					{
						szBuffer.append(NEWLINE);
	...
	if (!bCivilopediaText)
	{
		if (pCity == nullptr)
		{
			if (ePlayer != NO_PLAYER)
			{
				szTempBuffer.Format(L"\n%d%lc", GET_PLAYER(ePlayer).getProductionNeeded(eProject), GC.getYieldInfo(YIELD_PRODUCTION).getChar());
			}
			else
			{
				szTempBuffer.Format(L"\n%d%lc", kProject.getProductionCost(), GC.getYieldInfo(YIELD_PRODUCTION).getChar());
			}
			szBuffer.append(szTempBuffer);
		}
		else
		{
			...
			if (iProduction > 0)
			{
				szTempBuffer.Format(L" - %d/%d%lc", iProduction, pCity->getProductionNeeded(eProject), GC.getYieldInfo(YIELD_PRODUCTION).getChar());
			}
			else
			{
				szTempBuffer.Format(L" - %d%lc", pCity->getProductionNeeded(eProject), GC.getYieldInfo(YIELD_PRODUCTION).getChar());
			}
			szBuffer.append(szTempBuffer);
		}
	}
	...
	for (iI = 0; iI < NUM_YIELD_TYPES; ++iI)
	{
		if (piYieldChange[iI] != 0)
		{
			if (!bStarted)
			{
				...
				szTempBuffer += CvWString::format(L"%ls%ls%ls%d%ls%lc",
					szStart.GetCString(),
					szSpace.GetCString(),
			...
			{
				szTempBuffer.Format(L", %ls%d%ls%lc",
					piYieldChange[iI] > 0 ? L"+" : L"",
					piYieldChange[iI],
					bPercent ? L"%" : L"",
					GC.getYieldInfo((YieldTypes)iI).getChar());
	...
	for (iI = 0; iI < NUM_COMMERCE_TYPES; ++iI)
	{
		if (piCommerceChange[iI] != 0)
		{
			if (!bStarted)
			{
				...
				szTempBuffer += CvWString::format(L"%ls%ls%ls%d%ls%lc", szStart.GetCString(), szSpace.GetCString(), ((piCommerceChange[iI] > 0) ? L"+" : L""), piCommerceChange[iI], ((bPercent) ? L"%" : L""), GC.getCommerceInfo((CommerceTypes) iI).getChar());
			}
			else
			{
				szTempBuffer.Format(L", %ls%d%ls%lc", ((piCommerceChange[iI] > 0) ? L"+" : L""), piCommerceChange[iI], ((bPercent) ? L"%" : L""), GC.getCommerceInfo((CommerceTypes) iI).getChar());
			}
			szBuffer.append(szTempBuffer);
...
void CvGameTextMgr::setBonusHelp(CvWStringBuffer &szBuffer, BonusTypes eBonus, bool bCivilopediaText)
{
	...
	if (!bCivilopediaText)
	{		
		szBuffer.append(CvWString::format( SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getBonusInfo(eBonus).getDescription()));
			...
			for (int iCorp = 0; iCorp < GC.getNumCorporationInfos(); ++iCorp)
			{
				...
				if (kActivePlayer.isActiveCorporation((CorporationTypes)iCorp))
				{
					for (int i = 0; i < GC.getNUM_CORPORATION_PREREQ_BONUSES(); ++i)
					{
						...
						{
							int iLoop;
							for (CvCity* pCity = kActivePlayer.firstCity(&iLoop); nullptr != pCity; pCity = kActivePlayer.nextCity(&iLoop))
							{
				...
				if (bFound)
				{
					szBuffer.append(static_cast<wchar_t>(GC.getCorporationInfo((CorporationTypes)iCorp).getChar()));
				}
			}
		}
...
void CvGameTextMgr::setReligionHelp(CvWStringBuffer &szBuffer, ReligionTypes eReligion, bool bCivilopedia)
{
	...
	if (!bCivilopedia)
	{
		szBuffer.append(CvWString::format(SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), religion.getDescription()));
	}
...
void CvGameTextMgr::setReligionHelpCity(CvWStringBuffer &szBuffer, ReligionTypes eReligion, CvCity *pCity, bool bCityScreen, bool bForceReligion, bool bForceState, bool bNoStateReligion)
{
	...
	if (bCityScreen)
	{
		szBuffer.append(CvWString::format(SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getReligionInfo(eReligion).getDescription()));
		szBuffer.append(NEWLINE);
	...
	if (eStateReligion == eReligion || eStateReligion == NO_RELIGION || bForceState)
	{
		for (i = 0; i < NUM_COMMERCE_TYPES; i++)
		{
			...
			if (iCommerce != 0)
			{
				...
				szTempBuffer.Format(L"%ls%d%lc", iCommerce > 0 ? L"+" : L"", iCommerce, GC.getCommerceInfo((CommerceTypes)i).getChar());
				szBuffer.append(szTempBuffer);
				bHandled = true;
			}
		}
	...
	if (eStateReligion == eReligion || bForceState)
	{
		...
		if (iHappiness != 0)
		{
			...
			szTempBuffer.Format(L"%d%lc", iHappiness, ((iHappiness > 0) ? gDLL->getSymbolID(HAPPY_CHAR) : gDLL->getSymbolID(UNHAPPY_CHAR)));
			szBuffer.append(szTempBuffer);
			bHandled = true;
		}
...
void CvGameTextMgr::setCorporationHelp(CvWStringBuffer &szBuffer, CorporationTypes eCorporation, bool bCivilopedia)
{
	...
	if (!bCivilopedia)
	{
		szBuffer.append(CvWString::format(SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), kCorporation.getDescription()));
	}
	...
	for (int iI = 0; iI < NUM_YIELD_TYPES; ++iI)
	{
		...
		if (iYieldProduced != 0)
		{
			...
			{
				szTempBuffer += CvWString::format(L"%ls%d%lc",
			...
			{
				szTempBuffer += CvWString::format(L"%ls%.2f%lc",
					iYieldProduced > 0 ? L"+" : L"",
	...
	for (int i = 0; i < GC.getNUM_CORPORATION_PREREQ_BONUSES(); ++i)
	{
		if (NO_BONUS != kCorporation.getPrereqBonus(i))
		{
			...
			szBuffer.append(CvWString::format(L"%lc", GC.getBonusInfo((BonusTypes)kCorporation.getPrereqBonus(i)).getChar()));
		}
	}
	...
	for (int iCorporation = 0; iCorporation < GC.getNumCorporationInfos(); ++iCorporation)
	{
		if (iCorporation != eCorporation)
		{
			...
			if (bCompeting)
			{
				CvWString szTemp = CvWString::format(L"<link=literal>%ls</link>", kLoopCorporation.getDescription());
...
void CvGameTextMgr::setCorporationHelpCity(CvWStringBuffer &szBuffer, CorporationTypes eCorporation, CvCity *pCity, bool bCityScreen, bool bForceCorporation)
{
	...
	for (int i = 0; i < NUM_YIELD_TYPES; ++i)
	{
		...
		if (iYield != 0)
		{
			...
			CvWString szTempBuffer;
			szTempBuffer.Format(L"%ls%d%lc", iYield > 0 ? L"+" : L"", (iYield + 99) / 100, GC.getYieldInfo((YieldTypes)i).getChar());
			szBuffer.append(szTempBuffer);
			bHandled = true;
		}
	}
	...
	for (int i = 0; i < NUM_COMMERCE_TYPES; ++i)
	{
		...
		if (iCommerce != 0)
		{
			...
			CvWString szTempBuffer;
			szTempBuffer.Format(L"%ls%d%lc", iCommerce > 0 ? L"+" : L"", (iCommerce + 99) / 100, GC.getCommerceInfo((CommerceTypes)i).getChar());
			szBuffer.append(szTempBuffer);
			bHandled = true;
		}
	}
	...
	if (0 != iMaintenance)
	{
		...
		CvWString szTempBuffer;
		szTempBuffer.Format(L"%d%lc", -iMaintenance / 100, GC.getCommerceInfo(COMMERCE_GOLD).getChar());
		szBuffer.append(szTempBuffer);
		bHandled = true;
	}
	...
	else
	{
		if (kCorporation.getBonusProduced() != NO_BONUS)
		{
			...
			{		
				...
				szBuffer.append(CvWString::format(L"%lc", GC.getBonusInfo((BonusTypes)kCorporation.getBonusProduced()).getChar()));
			}
		}
	}
}

void CvGameTextMgr::buildObsoleteString(CvWStringBuffer &szBuffer, int iItem, bool bList, [[maybe_unused]] bool bPlayerContext)
{
	CvWString szTempBuffer;
...
void CvGameTextMgr::buildObsoleteBonusString(CvWStringBuffer &szBuffer, int iItem, bool bList, [[maybe_unused]] bool bPlayerContext)
{
	CvWString szTempBuffer;
...
void CvGameTextMgr::buildObsoleteSpecialString(CvWStringBuffer &szBuffer, int iItem, bool bList, [[maybe_unused]] bool bPlayerContext)
{
	CvWString szTempBuffer;
...
void CvGameTextMgr::buildFeatureProductionString(CvWStringBuffer &szBuffer, TechTypes eTech, bool bList, [[maybe_unused]] bool bPlayerContext)
{
...
void CvGameTextMgr::buildWorkerRateString(CvWStringBuffer &szBuffer, TechTypes eTech, bool bList, [[maybe_unused]] bool bPlayerContext)
{
...
void CvGameTextMgr::buildTradeRouteString(CvWStringBuffer &szBuffer, TechTypes eTech, bool bList, [[maybe_unused]] bool bPlayerContext)
{
...
void CvGameTextMgr::buildHealthRateString(CvWStringBuffer &szBuffer, TechTypes eTech, bool bList, [[maybe_unused]] bool bPlayerContext)
{
...
void CvGameTextMgr::buildHappinessRateString(CvWStringBuffer &szBuffer, TechTypes eTech, bool bList, [[maybe_unused]] bool bPlayerContext)
{
...
void CvGameTextMgr::buildImprovementString(CvWStringBuffer &szBuffer, TechTypes eTech, int iImprovement, bool bList, [[maybe_unused]] bool bPlayerContext)
{
	bool bTechFound;
	int iJ;
...
void CvGameTextMgr::buildDomainExtraMovesString(CvWStringBuffer &szBuffer, TechTypes eTech, int iDomainType, bool bList, [[maybe_unused]] bool bPlayerContext)
{
...
void CvGameTextMgr::buildSpecialBuildingString(CvWStringBuffer &szBuffer, TechTypes eTech, int iBuildingType, bool bList, [[maybe_unused]] bool bPlayerContext)
{
...
void CvGameTextMgr::buildYieldChangeString(CvWStringBuffer &szBuffer, TechTypes eTech, int iYieldType, bool bList, [[maybe_unused]] bool bPlayerContext)
{
	CvWString szTempBuffer;
	if (bList)
	{
		szTempBuffer.Format(L"<link=literal>%ls</link>", GC.getImprovementInfo((ImprovementTypes)iYieldType).getDescription());
	}
	else
	{
		szTempBuffer.Format(L"%lc<link=literal>%ls</link>", gDLL->getSymbolID(BULLET_CHAR), GC.getImprovementInfo((ImprovementTypes)iYieldType).getDescription());
	}
...
bool CvGameTextMgr::buildBonusRevealString(CvWStringBuffer &szBuffer, TechTypes eTech, int iBonusType, bool bFirst, bool bList, [[maybe_unused]] bool bPlayerContext)
{
	CvWString szTempBuffer;
	...
	if (GC.getBonusInfo((BonusTypes) iBonusType).getTechReveal() == eTech)
	{
		...
		szTempBuffer.Format( SETCOLR L"<link=literal>%ls</link>" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getBonusInfo((BonusTypes) iBonusType).getDescription());
...
bool CvGameTextMgr::buildCivicRevealString(CvWStringBuffer &szBuffer, TechTypes eTech, int iCivicType, bool bFirst, bool bList, [[maybe_unused]] bool bPlayerContext)
{
	CvWString szTempBuffer;
	...
	if (GC.getCivicInfo((CivicTypes) iCivicType).getTechPrereq() == eTech)
	{
		...
		szTempBuffer.Format( SETCOLR L"<link=literal>%ls</link>" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getCivicInfo((CivicTypes) iCivicType).getDescription());
...
bool CvGameTextMgr::buildProcessInfoString(CvWStringBuffer &szBuffer, TechTypes eTech, int iProcessType, bool bFirst, bool bList, [[maybe_unused]] bool bPlayerContext)
{
	CvWString szTempBuffer;
	...
	if (GC.getProcessInfo((ProcessTypes) iProcessType).getTechPrereq() == eTech)
	{
		...
		szTempBuffer.Format( SETCOLR L"<link=literal>%ls</link>" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getProcessInfo((ProcessTypes) iProcessType).getDescription());
...
bool CvGameTextMgr::buildFoundReligionString(CvWStringBuffer &szBuffer, TechTypes eTech, int iReligionType, bool bFirst, bool bList, bool bPlayerContext)
{
	...
	if (GC.getReligionInfo((ReligionTypes) iReligionType).getTechPrereq() == eTech)
	{
		if (!bPlayerContext || (GC.getGameINLINE().countKnownTechNumTeams(eTech) == 0))
		{
			...
			{
				szTempBuffer.Format( SETCOLR L"<link=literal>%ls</link>" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getReligionInfo((ReligionTypes) iReligionType).getDescription());
			}
...
bool CvGameTextMgr::buildFoundCorporationString(CvWStringBuffer &szBuffer, TechTypes eTech, int iCorporationType, bool bFirst, bool bList, bool bPlayerContext)
{
	...
	if (GC.getCorporationInfo((CorporationTypes) iCorporationType).getTechPrereq() == eTech)
	{
		if (!bPlayerContext || (GC.getGameINLINE().countKnownTechNumTeams(eTech) == 0))
		{
			...
			szTempBuffer.Format( SETCOLR L"<link=literal>%ls</link>" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getCorporationInfo((CorporationTypes) iCorporationType).getDescription());
...
bool CvGameTextMgr::buildPromotionString(CvWStringBuffer &szBuffer, TechTypes eTech, int iPromotionType, bool bFirst, bool bList, [[maybe_unused]] bool bPlayerContext)
{
	CvWString szTempBuffer;
	...
	if (GC.getPromotionInfo((PromotionTypes) iPromotionType).getTechPrereq() == eTech)
	{
		...
		szTempBuffer.Format( SETCOLR L"<link=literal>%ls</link>" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getPromotionInfo((PromotionTypes) iPromotionType).getDescription());
		setListHelp(szBuffer, gDLL->getText("TXT_KEY_MISC_ENABLES").c_str(), szTempBuffer, L", ", bFirst);
		bFirst = false;
	}
	return bFirst;
...
void CvGameTextMgr::buildSingleLineTechTreeString(CvWStringBuffer &szBuffer, TechTypes eTech, bool bPlayerContext)
{
	...
	for (int iI = 0; iI < GC.getNumTechInfos(); ++iI)
	{
		...
		if (!bTechAlreadyAccessible)
		{
			...
			if (bTechFound)
			{
				szTempBuffer.Format( SETCOLR L"<link=literal>%ls</link>" ENDCOLR , TEXT_COLOR("COLOR_TECH_TEXT"), GC.getTechInfo((TechTypes) iI).getDescription());
	...
	for (int iJ = 0; iJ < GC.getNUM_OR_TECH_PREREQS(); iJ++)
	{
		...
		if (eTestTech >= 0)
		{
			...
			if (!bTechAlreadyResearched)
			{
				...
				{
					szTempBuffer.Format( SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_TECH_TEXT"), GC.getTechInfo(eTestTech).getDescription());
...
void CvGameTextMgr::setPromotionHelp(CvWStringBuffer &szBuffer, PromotionTypes ePromotion, bool bCivilopediaText)
{
	if (!bCivilopediaText)
	{
		...
		szTempBuffer.Format( SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), promo.getDescription());
		szBuffer.append(szTempBuffer);
	}
...
void CvGameTextMgr::setImprovementHelp(CvWStringBuffer &szBuffer, ImprovementTypes eImprovement, bool bCivilopediaText)
{
	...
	if (!bCivilopediaText)
	{
		szTempBuffer.Format( SETCOLR L"%ls" ENDCOLR, TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), info.getDescription());
		szBuffer.append(szTempBuffer);
		...
		for (int iYield = 0; iYield < NUM_YIELD_TYPES; iYield++)
		{
			for (int iCivic = 0; iCivic < GC.getNumCivicInfos(); iCivic++)
			{
				...
				if (0 != iChange)
				{
					szTempBuffer.Format( SETCOLR L"%ls" ENDCOLR , TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getCivicInfo((CivicTypes)iCivic).getDescription());
					szBuffer.append(NEWLINE);
	...
	for (int iBonus = 0; iBonus < GC.getNumBonusInfos(); iBonus++)
	{
		...
		if (iRand > 0)
		{
			szFirstBuffer.Format(L"%ls%ls", NEWLINE, gDLL->getText("TXT_KEY_IMPROVEMENT_CHANCE_DISCOVER").c_str());
			szTempBuffer.Format(L"%lc", GC.getBonusInfo((BonusTypes) iBonus).getChar());
			setListHelp(szBuffer, szFirstBuffer.c_str(), szTempBuffer.c_str(), L", ", iRand != iLast);
			iLast = iRand;
		}
	}
...
void CvGameTextMgr::getDealString(CvWStringBuffer& szBuffer, PlayerTypes ePlayer1, PlayerTypes ePlayer2, const CLinkList<TradeData>* pListPlayer1, const CLinkList<TradeData>* pListPlayer2, PlayerTypes ePlayerPerspective)
{
	...
	if (nullptr != pListPlayer1 && pListPlayer1->getLength() > 0)
	{
	...
	if (nullptr != pListPlayer2 && pListPlayer2->getLength() > 0)
	{
		CLLNode<TradeData>* pTradeNode;
		bool bFirst = true;
...
void CvGameTextMgr::getAttitudeString(CvWStringBuffer& szBuffer, PlayerTypes ePlayer, PlayerTypes eTargetPlayer)
{
	...
	for (iPass = 0; iPass < 2; iPass++)
	{
		...
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_LAND_TARGET", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_WAR", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_PEACE", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_SAME_RELIGION", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_DIFFERENT_RELIGION", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_BONUS_TRADE", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_OPEN_BORDERS", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_DEFENSIVE_PACT", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_RIVAL_DEFENSIVE_PACT", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_RIVAL_VASSAL", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_SHARE_WAR", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_FAVORITE_CIVIC", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_TRADE", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_RIVAL_TRADE", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_FREEDOM", iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
		{
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText(((iAttitudeChange > 0) ? "TXT_KEY_MISC_ATTITUDE_EXTRA_GOOD" : "TXT_KEY_MISC_ATTITUDE_EXTRA_BAD"), iAttitudeChange).GetCString());
			szBuffer.append(NEWLINE);
			szBuffer.append(szTempBuffer);
		}
		...
		for (iI = 0; iI < NUM_MEMORY_TYPES; ++iI)
		{
			...
			if ((iPass == 0) ? (iAttitudeChange > 0) : (iAttitudeChange < 0))
			{
				szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR, TEXT_COLOR((iAttitudeChange > 0) ? "COLOR_POSITIVE_TEXT" : "COLOR_NEGATIVE_TEXT"), gDLL->getText("TXT_KEY_MISC_ATTITUDE_MEMORY", iAttitudeChange, GC.getMemoryInfo((MemoryTypes)iI).getDescription()).GetCString());
				szBuffer.append(NEWLINE);
				szBuffer.append(szTempBuffer);
			}
		}
...
void CvGameTextMgr::getTradeString(CvWStringBuffer& szBuffer, const TradeData& tradeData, PlayerTypes ePlayer1, [[maybe_unused]] PlayerTypes ePlayer2)
{
	...
	case TRADE_TECHNOLOGIES:
		szBuffer.assign(CvWString::format(L"%ls", GC.getTechInfo((TechTypes)tradeData.m_iData).getDescription()));
		break;
	case TRADE_RESOURCES:
		szBuffer.assign(CvWString::format(L"%ls", GC.getBonusInfo((BonusTypes)tradeData.m_iData).getDescription()));
		break;
	case TRADE_CITIES:
		szBuffer.assign(CvWString::format(L"%ls", GET_PLAYER(ePlayer1).getCity(tradeData.m_iData)->getName().GetCString()));
		break;
	case TRADE_PEACE:
	case TRADE_WAR:
	case TRADE_EMBARGO:
		szBuffer.assign(CvWString::format(L"%ls", GET_TEAM((TeamTypes)tradeData.m_iData).getName().GetCString()));
		break;
	case TRADE_CIVIC:
		szBuffer.assign(CvWString::format(L"%ls", GC.getCivicInfo((CivicTypes)tradeData.m_iData).getDescription()));
		break;
	case TRADE_RELIGION:
		szBuffer.assign(CvWString::format(L"%ls", GC.getReligionInfo((ReligionTypes)tradeData.m_iData).getDescription()));
		break;
	default:
		FAssert(false);
		break;
...
void CvGameTextMgr::buildFinanceCityMaintString(CvWStringBuffer& szBuffer, PlayerTypes ePlayer)
{
	...
	CvPlayer& player = GET_PLAYER(ePlayer);
	for (CvCity* pLoopCity = player.firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = player.nextCity(&iLoop))
	{
...
void CvGameTextMgr::buildFinanceCivicUpkeepString(CvWStringBuffer& szBuffer, PlayerTypes ePlayer)
{
	...
	for (int iI = 0; iI < GC.getNumCivicOptionInfos(); ++iI)
	{
		...
		{
			CvWString szTemp;
			szTemp.Format(L"%d%lc: %ls", player.getSingleCivicUpkeep(eCivic), GC.getCommerceInfo(COMMERCE_GOLD).getChar(),  GC.getCivicInfo(eCivic).getDescription());
...
void CvGameTextMgr::buildFinanceForeignIncomeString(CvWStringBuffer& szBuffer, PlayerTypes ePlayer)
{
	...
	for (int iI = 0; iI < MAX_PLAYERS; ++iI)
	{
		...
		{
			CvWString szTemp;
			szTemp.Format(L"%d%lc: %ls", player.getGoldPerTurnByPlayer((PlayerTypes)iI), GC.getCommerceInfo(COMMERCE_GOLD).getChar(), otherPlayer.getCivilizationShortDescription());
...
void CvGameTextMgr::parseLeaderHeadHelp(CvWStringBuffer &szBuffer, PlayerTypes eThisPlayer, PlayerTypes eOtherPlayer)
{
	...
	szBuffer.append(CvWString::format(L"%ls", GET_PLAYER(eThisPlayer).getName()));
...
void CvGameTextMgr::getActiveDealsString(CvWStringBuffer &szBuffer, PlayerTypes eThisPlayer, PlayerTypes eOtherPlayer)
{
	...
	while (nullptr != pDeal)
	{
		...
		{
			szBuffer.append(NEWLINE);
			szBuffer.append(CvWString::format(L"%lc", gDLL->getSymbolID(BULLET_CHAR)));
...
void CvGameTextMgr::buildHintsList(CvWStringBuffer& szBuffer)
{
	...
	{
		szBuffer.append(CvWString::format(L"%lc%ls", gDLL->getSymbolID(BULLET_CHAR), GC.getHints(i).getText()));
		szBuffer.append(NEWLINE);
		szBuffer.append(NEWLINE);
	}
}
	...
	for (int i = 0; i < GC.getNumBuildingInfos(); i++)
	{
		...
		for (int j = 0; j < MAX_PLAYERS; j++)
		{
			if (GET_PLAYER((PlayerTypes)j).isAlive())
			{
				...
				{
					int iLoop;
					for (CvCity* pLoopCity = GET_PLAYER((PlayerTypes)j).firstCity(&iLoop); pLoopCity != nullptr; pLoopCity = GET_PLAYER((PlayerTypes)j).nextCity(&iLoop))
					{
						if (pLoopCity->getNumBuilding((BuildingTypes)i) > 0 && !GET_TEAM(pLoopCity->getTeam()).isObsoleteBuilding((BuildingTypes)i))
						{
							for (int iLoop2 = 0; iLoop2 < pLoopCity->getNumBuilding((BuildingTypes)i); iLoop2++)
							{
...
void CvGameTextMgr::setYieldHelp(CvWStringBuffer &szBuffer, CvCity& city, YieldTypes eYieldType)
{
	...
	[[maybe_unused]] int iBaseModifier = 100;
...
void CvGameTextMgr::parseGreatPeopleHelp(CvWStringBuffer &szBuffer, CvCity& city)
{
	...
	if (iTotalGreatPeopleUnitProgress > 0)
	{
		...
		{
			szBuffer.append(CvWString::format(L"%ls%ls - %d%%", NEWLINE, GC.getUnitInfo(aUnitProgress[iI].first).getDescription(), aUnitProgress[iI].second));
		}
	}
...
void CvGameTextMgr::buildCityBillboardIconString( CvWStringBuffer& szBuffer, CvCity* pCity)
{
	...
	{
		szBuffer.append(CvWString::format(L"%lc", gDLL->getSymbolID(SILVER_STAR_CHAR)));
	}
	...
	if (pCity->canBeSelected())
	{
		...
		{
			szBuffer.append(CvWString::format(L"%lc", gDLL->getSymbolID(UNHAPPY_CHAR)));
		}

		if (pCity->healthRate() < 0)
		{
			szBuffer.append(CvWString::format(L"%lc", gDLL->getSymbolID(UNHEALTHY_CHAR)));
		}
		...
		if (gDLL->getGraphicOption(GRAPHICOPTION_CITY_DETAIL))
		{
			if (GET_PLAYER(pCity->getOwnerINLINE()).getNumCities() > 2)
			{
				...
				{
					szBuffer.append(CvWString::format(L"%lc", GC.getYieldInfo(YIELD_PRODUCTION).getChar()));
				}
				if (pCity->findCommerceRateRank(COMMERCE_GOLD) == 1)
				{
					szBuffer.append(CvWString::format(L"%lc", GC.getCommerceInfo(COMMERCE_GOLD).getChar()));
				}
				if (pCity->findCommerceRateRank(COMMERCE_RESEARCH) == 1)
				{
					szBuffer.append(CvWString::format(L"%lc", GC.getCommerceInfo(COMMERCE_RESEARCH).getChar()));
				}
			}
		}
		...
		if (pCity->isConnectedToCapital())
		{
			...
			{
				szBuffer.append(CvWString::format(L"%lc", gDLL->getSymbolID(TRADE_CHAR)));
			}
		}
	}
	...
	for (int iI = 0; iI < GC.getNumReligionInfos(); ++iI)
	{
		if (pCity->isHasReligion((ReligionTypes)iI))
		{
			...
			{
				szBuffer.append(CvWString::format(L"%lc", GC.getReligionInfo((ReligionTypes) iI).getHolyCityChar()));
			}
			else
			{
				szBuffer.append(CvWString::format(L"%lc", GC.getReligionInfo((ReligionTypes) iI).getChar()));
			}
		}
	}
	...
	for (int iI = 0; iI < GC.getNumCorporationInfos(); ++iI)
	{
		if (pCity->isHeadquarters((CorporationTypes)iI))
		{
			...
			{
				szBuffer.append(CvWString::format(L"%lc", GC.getCorporationInfo((CorporationTypes) iI).getHeadquarterChar()));
			}
		}
		else
		{
			...
			{
				szBuffer.append(CvWString::format(L"%lc", GC.getCorporationInfo((CorporationTypes) iI).getChar()));
			}
		}
	}
	...
	if (pCity->getTeam() == GC.getGameINLINE().getActiveTeam())
	{
		if (pCity->isPower())
		{
			szBuffer.append(CvWString::format(L"%lc", gDLL->getSymbolID(POWER_CHAR)));
		}
	}
	...
	{
		szBuffer.append(CvWString::format(L" (%lc:%d)", gDLL->getSymbolID(OCCUPATION_CHAR), pCity->getOccupationTimer()));
	}
	...
	//if (pCity->getTeam() != GC.getGameINLINE().getActiveTeam())
	{
		if (pCity->isVisible(GC.getGameINLINE().getActiveTeam(), true))
		{
			...
			if (iDefenseModifier != 0)
			{
				// fortsnek: Had to change the string args to wide string literals to get formatting to work. Is this needed anywhere else?
				szBuffer.append(CvWString::format(L" %lc:%ls%d%%", gDLL->getSymbolID(DEFENSE_CHAR), ((iDefenseModifier > 0) ? L"+" : L""), iDefenseModifier));
			}
		}
	}
}
...
void CvGameTextMgr::getCityBillboardFoodbarColors([[maybe_unused]] CvCity* pCity, std::vector<NiColorA>& aColors)
{
...
void CvGameTextMgr::getCityBillboardProductionbarColors([[maybe_unused]] CvCity* pCity, std::vector<NiColorA>& aColors)
{
	aColors.resize(NUM_INFOBAR_TYPES);
...
void CvGameTextMgr::setEventHelp(CvWStringBuffer& szBuffer, EventTypes eEvent, int iEventTriggeredId, PlayerTypes ePlayer)
{
	...
	if (nullptr != pCity && kEvent.isCityEffect())
	{
		szCity = pCity->getNameKey();
	}
	else if (nullptr != pOtherPlayerCity && kEvent.isOtherPlayerCityEffect())
	{
	...
	{
		if (nullptr != pPlot && NO_FEATURE != pPlot->getFeatureType())
		{
			szBuffer.append(NEWLINE);
	...
	{
		if (nullptr != pPlot && NO_IMPROVEMENT != pPlot->getImprovementType())
		{
			szBuffer.append(NEWLINE);
	...
	{
		if (nullptr != pPlot && NO_BONUS != pPlot->getBonusType())
		{
			szBuffer.append(NEWLINE);
	...
	{
		if (nullptr != pPlot && NO_ROUTE != pPlot->getRouteType())
		{
			szBuffer.append(NEWLINE);
...
void CvGameTextMgr::setEspionageCostHelp(CvWStringBuffer &szBuffer, EspionageMissionTypes eMission, PlayerTypes eTargetPlayer, const CvPlot* pPlot, int iExtraData, const CvUnit* pSpyUnit)
{
	...
	if (kMission.getCityInsertCultureCostFactor() > 0)
	{
		if (nullptr != pPlot)
		{
			...
			if (nullptr != pCity && pPlot->getCulture(GC.getGameINLINE().getActivePlayer()) > 0)
			{
	...
	if (kPlayer.getEspionageMissionCost(eMission, eTargetPlayer, pPlot, iExtraData, pSpyUnit) > 0)
	{
		...
		if (pCity != nullptr && GC.getEspionageMissionInfo(eMission).isTargetsCity())
		{
			// City Population
...
{
	CvPlayer& kPlayer = GET_PLAYER(ePlayer);
	szHeader.Format(L"%ls - %ls", CvWString(kPlayer.getName()).GetCString(), CvWString(kPlayer.getCivilizationDescription()).GetCString());
	if (bAttitude)
	{
		szHeader += CvWString::format(L" (%ls)", GC.getAttitudeInfo(kPlayer.AI_getAttitude(eOtherPlayer)).getDescription());
	}
}
...
void CvGameTextMgr::getPlotHelp(CvPlot* pMouseOverPlot, CvCity* pCity, CvPlot* pFlagPlot, bool bAlt, CvWStringBuffer& strHelp)
{
	...
	else 
	{
		...
		{
			CvWString szTempBuffer;
			szTempBuffer.Format(SETCOLR L"%ls" ENDCOLR NEWLINE, TEXT_COLOR("COLOR_HIGHLIGHT_TEXT"), GC.getInterfaceModeInfo(eInterfaceMode).getDescription());
...
void CvGameTextMgr::getTurnTimerText(CvWString& strText)
{
	...
	if (gDLL->getInterfaceIFace()->getShowInterface() == INTERFACE_SHOW || gDLL->getInterfaceIFace()->getShowInterface() == INTERFACE_ADVANCED_START)
	{
		...
		if (GC.getGameINLINE().getGameState() == GAMESTATE_ON)
		{
			...
			else if (iMinVictoryTurns < INT_MAX)
			{
				if (!strText.empty())
				{
					strText += L" -- ";
...
void CvGameTextMgr::getFontSymbols(std::vector< std::vector<wchar_t> >& aacSymbols, std::vector<int>& aiMaxNumRows)
{
	...
	for (int iI = 0; iI < GC.getNumBonusInfos(); iI++)
	{
		aacSymbols[aacSymbols.size() - 1].push_back((wchar_t) GC.getBonusInfo((BonusTypes) iI).getChar());
	}
...
void CvGameTextMgr::getCityDataForAS(std::vector<CvWBData>& mapCityList, std::vector<CvWBData>& mapBuildingList, std::vector<CvWBData>& mapAutomateList)
{
	...
	if (iCost > 0)
	{
		...
		mapCityList.push_back(CvWBData(0, szHelp.c_str(), ARTFILEMGR.getInterfaceArtInfo("INTERFACE_BUTTONS_CITYSELECTION")->getPath()));
	}
	...
	if (iCost > 0)
	{
		...
		mapCityList.push_back(CvWBData(1, szHelp.c_str(), ARTFILEMGR.getInterfaceArtInfo("INTERFACE_ANGRYCITIZEN_TEXTURE")->getPath()));
	}
	...
	if (iCost > 0)
	{
		...
		szHelp += gDLL->getText("TXT_KEY_AS_UNREMOVABLE");
		mapCityList.push_back(CvWBData(2, szHelp.c_str(), ARTFILEMGR.getInterfaceArtInfo("CULTURE_BUTTON")->getPath()));
	}
	...
	mapAutomateList.push_back(CvWBData(0, szHelp.c_str(), ARTFILEMGR.getInterfaceArtInfo("INTERFACE_AUTOMATE")->getPath()));
}
...
void CvGameTextMgr::getVisibilityDataForAS(std::vector<CvWBData>& mapVisibilityList)
{
	...
	if (iCost > 0)
	{
		...
		mapVisibilityList.push_back(CvWBData(0, szHelp.c_str(), ARTFILEMGR.getInterfaceArtInfo("INTERFACE_TECH_LOS")->getPath()));
	}
}

void CvGameTextMgr::getTechDataForAS(std::vector<CvWBData>& mapTechList)
{
	mapTechList.push_back(CvWBData(0, gDLL->getText("TXT_KEY_WB_AS_TECH").c_str(), ARTFILEMGR.getInterfaceArtInfo("INTERFACE_BTN_TECH")->getPath()));
}
...
void CvGameTextMgr::getTerrainDataForWB(std::vector<CvWBData>& mapTerrainData, std::vector<CvWBData>& mapFeatureData, std::vector<CvWBData>& mapPlotData, std::vector<CvWBData>& mapRouteData)
{
	...
	for (int i = 0; i < GC.getNumFeatureInfos(); i++)
	{
		for (int k = 0; k < GC.getFeatureInfo((FeatureTypes)i).getArtInfo()->getNumVarieties(); k++)
		{
			...
			mapFeatureData.push_back(CvWBData(i + GC.getNumFeatureInfos() * k, szBuffer.getCString(), GC.getFeatureInfo((FeatureTypes)i).getArtInfo()->getVariety(k).getVarietyButton().c_str()));
		}
	}

	mapPlotData.push_back(CvWBData(0, gDLL->getText("TXT_KEY_WB_PLOT_TYPE_MOUNTAIN").c_str(), ARTFILEMGR.getInterfaceArtInfo("WORLDBUILDER_PLOT_TYPE_MOUNTAIN")->getPath()));
	mapPlotData.push_back(CvWBData(1, gDLL->getText("TXT_KEY_WB_PLOT_TYPE_HILL").c_str(), ARTFILEMGR.getInterfaceArtInfo("WORLDBUILDER_PLOT_TYPE_HILL")->getPath()));
	mapPlotData.push_back(CvWBData(2, gDLL->getText("TXT_KEY_WB_PLOT_TYPE_PLAINS").c_str(), ARTFILEMGR.getInterfaceArtInfo("WORLDBUILDER_PLOT_TYPE_PLAINS")->getPath()));
	mapPlotData.push_back(CvWBData(3, gDLL->getText("TXT_KEY_WB_PLOT_TYPE_OCEAN").c_str(), ARTFILEMGR.getInterfaceArtInfo("WORLDBUILDER_PLOT_TYPE_OCEAN")->getPath()));
