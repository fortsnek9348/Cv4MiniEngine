...
bool MyCvDLLPython::callFunction(const char* moduleName, const char* fxnName, PyObject* fxnArg, std::vector<int>* result)
{
	...
	//if (mgr.mapSizeOverrideMultiplier != 1 && std::string_view(fxnName) == "getGridSize")
	//{
	//	const int mulW = GC.getLandscapeInfo(GC.getActiveLandscapeID()).getPlotsPerCellX();
	//	const int mulH = GC.getLandscapeInfo(GC.getActiveLandscapeID()).getPlotsPerCellX();
	//	if (mgr.tryCall(moduleName, fxnName, fxnArg, result) && !mgr.usingDefaultImpl)
	//	{
	//		const int oldW = result->at(0);
	//		const int oldH = result->at(1);
	//		result->at(0) *= mgr.mapSizeOverrideMultiplier;
	//		result->at(1) *= mgr.mapSizeOverrideMultiplier;
	//		std::clog << std::format("Overriding map script getGridSize. Original grid size = [{}, {}] plots. Returned grid size = [{}, {}] plots.",
	//			oldW * mulW,
	//			oldH * mulH,
	//			result->at(0) * mulW,
	//			result->at(1) * mulH
	...
	//	{
	//		
	//		const int oldW = gGlobals.getMap().getGridWidth(); // These are already set.
	//		const int oldH = gGlobals.getMap().getGridHeight();
	//		result->at(0) = oldW * mgr.mapSizeOverrideMultiplier;
	//		result->at(1) = oldH * mgr.mapSizeOverrideMultiplier;
	//		std::clog << std::format("Overriding map script getGridSize. Map script is using the default size. Original grid size = [{}, {}] plots. Returned grid size = [{}, {}] plots.",
	//			oldW * mulW,
	//			oldH * mulH,
	//			result->at(0) * mulW,
	//			result->at(1) * mulH
...
void MyCvDLLPython::callFunctionWithRetryPrompt(const char* moduleName, const char* fxnName)
{
	for (;;)
	{
		try
		{
			(void)callFunction(moduleName, fxnName, nullptr);
			break;
		}
