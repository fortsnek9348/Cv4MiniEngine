		...
		case CvPopup::EControlType::Button:
		{
			...
			case WIDGET_MAINTAIN:
			{
				...
				if (info.getProductionToCommerceModifier(COMMERCE_GOLD) != 0)
					prefix = CvTranslator().lookupSymbolChar(static_cast<wchar_t>(gGlobals.getCommerceInfo(COMMERCE_GOLD).getChar()));
				if (info.getProductionToCommerceModifier(COMMERCE_RESEARCH) != 0)
					prefix = CvTranslator().lookupSymbolChar(static_cast<wchar_t>(gGlobals.getCommerceInfo(COMMERCE_RESEARCH).getChar()));
				if (info.getProductionToCommerceModifier(COMMERCE_CULTURE) != 0)
					prefix = CvTranslator().lookupSymbolChar(static_cast<wchar_t>(gGlobals.getCommerceInfo(COMMERCE_CULTURE).getChar()));
				break;
			}
			default:
				break;
