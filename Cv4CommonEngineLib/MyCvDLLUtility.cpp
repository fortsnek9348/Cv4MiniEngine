...
{
	return &MyCvDLLInterfaceIFace::getInstance();
}

CvDLLEngineIFaceBase* MyCvDLLUtility::getEngineIFace()
{
	return &MyCvDLLEngineIFace::getInstance();
}
...
void MyCvDLLUtility::sendPlayerInfo([[maybe_unused]] PlayerTypes eActivePlayer)
{
	// This is called in CvInitCore::closeInactiveSlots and after changing your details in-game. No multiplayer, so we don't need to do anything.
	std::clog << __func__ << '\n';
}
...
{
	AudioXmlDefs& xmlDefs = AudioXmlDefs::getInstance();

	int xmlScriptType{};
	switch (iScriptType)
	{
	case AUDIOTAG_2DSCRIPT: xmlScriptType = xmlDefs.scriptType2D; break;
	case AUDIOTAG_3DSCRIPT: xmlScriptType = xmlDefs.scriptType3D; break;
	case AUDIOTAG_SOUNDSCAPE: xmlScriptType = xmlDefs.scriptTypeSoundscape; break;
	case AUDIOTAG_NONE: xmlScriptType = -1; break; // Accept any, used in XML loading.
	default:
		throw std::runtime_error(std::string("Unexpected script type in '") + __FUNCTION__ + "'.");
	}

	const auto it = xmlDefs.tagIndexLookup.find(szTag);
	if (it != xmlDefs.tagIndexLookup.end())
	{
		if (xmlScriptType < 0 || it->second.type == xmlScriptType)
			return it->second.index;
		else
			throw std::runtime_error(std::string("Wrong script type in '") + __FUNCTION__ + "'.");
	}
	else
		throw std::runtime_error(std::string("Sound script type '") + szTag + "' not found.");
}
...
void MyCvDLLUtility::Do3DSound(int iScriptId, NiPoint3 vPosition)
{
	// Called for Nukes.
	
	//return playScript3DSoundByIndex(iScriptId, {
...
CvCacheObject* MyCvDLLUtility::createGlobalTextCacheObject([[maybe_unused]] const TCHAR* szCacheFileName)
{
	...
	const auto read = [](FDataStreamBase* stream) {
		auto& translator = CvTranslator::getInstance();
		...
		translator.textEntries.clear();
		translator.textEntries.reserve(numEntries);
		for (unsigned int i = 0; i < numEntries; ++i)
		{
			std::wstring k;
			CvTranslator::TextEntryValue v;
			stream->ReadString(k);
			stream->ReadString(v.text);
			stream->ReadString(v.gender);
			stream->ReadString(v.plural);
			translator.textEntries.emplace(std::move(k), std::move(v));
		}
		return true;
	};
	const auto write = [](FDataStreamBase* stream) {
		...
		stream->Write((uint32_t)translator.textEntries.size());
		for (const auto& [k, v] : translator.textEntries)
		{
...
{
	return CvTranslator::getInstance().symbols[iID];
}

void MyCvDLLUtility::setSymbolID(int iID, int iValue)
{
	CvTranslator::getInstance().symbols[iID] = (wchar_t)iValue;
}

CvWString MyCvDLLUtility::getTextGeneric(const CvWString& szIDTag, std::span<const TextArg> args)
{
	return CvTranslator::getInstance().getText(szIDTag, args);
}
...
CvWString MyCvDLLUtility::getObjectText(const CvWString& szIDTag, unsigned int uiForm, bool bNoSubs)
{
	...
	const auto& translator = CvTranslator::getInstance();
	const auto it = translator.textEntries.find(szIDTag);
	if (it == translator.textEntries.end())
		return szIDTag;
	...
	if (bNoSubs)
		...
		return CvTranslator::getInstance().formatTextcode(str, {});
}
...
{
	CvTranslator::TextEntryValue value{ szString, szGender, szPlural };
	...
	CvTranslator::getInstance().textEntries[CvWString(szIDTag)] = std::move(value);
}

unsigned int MyCvDLLUtility::getNumForms(CvWString szIDTag)
{
	return (unsigned int)(std::ranges::count(CvTranslator::getInstance().textEntries.at(szIDTag).text, L':') + 1);
}
